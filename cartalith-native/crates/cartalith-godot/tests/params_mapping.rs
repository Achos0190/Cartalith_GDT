...
/// The v2.57 plate-base blur (`RC_ENGINE_CHANGES.md` §6i) takes the same
/// shape: every save written before the row existed, and every reference-app
/// export, was generated at the old 0.35 blur, so its parameters must
/// regenerate that coastline rather than today's.
#[test]
fn a_save_without_the_v2_57_blur_reloads_without_it() {
    assert!(params::defaults().tect.narrow_plate_base_blur, "the shipped app generates with it on");

    let mut p = params::defaults();
    let n = params::apply_saved_state(&mut p, &serde_json::json!({params::NATIVE_PARAMS_KEY: {"tect.plates": 20.0}}));
    assert_eq!(n, 1);
    assert!(!p.tect.narrow_plate_base_blur, "a pre-existing save must not silently gain the v2.57 blur");

    for on in [true, false] {
        let mut src = params::defaults();
        src.tect.narrow_plate_base_blur = on;
        let state = params::save_state(&src);
        assert_eq!(state[params::NATIVE_PARAMS_KEY]["tect.narrow_plate_base_blur"], serde_json::json!(on));
        let mut back = params::defaults();
        back.tect.narrow_plate_base_blur = !on;
        params::apply_saved_state(&mut back, &state);
        assert_eq!(back.tect.narrow_plate_base_blur, on);
    }
}
...
/// recompute that applies nothing. The two must agree for every row, so a
...
/// is the answer for 61 of the 100 rows (2026-09-24), including every
/// terrain, tectonic and erosion knob.
#[test]
fn generation_time_only_parameters_mark_nothing() {
    for key in [
        "tect.plates", "tect.warp", "tect.narrow_plate_base_blur", "stream.k", "passes.evolve_cycles",
