...
struct CivData {
    ...
    /// state and not `cartalith-civ` state). Seeded to `civ.factions` real
    /// factions plus "Unclaimed" at index 0 on every `compute_civilisation`,
    /// then grown/shrunk by `civ_add_faction`/`civ_remove_faction`.
    ///
    /// **Generation runs at the `civ.factions` parameter** (default 6, the
    /// reference's own `CIV_FACTIONS.length-1`). `assign_factions` is called
    /// inside `generate()`/`civ_populate` with that value, so adding a
    /// faction *by hand* yields a real, paintable, assignable, editable id
    /// that no generated settlement was seeded into -- which is exactly the
    /// reference's own behaviour: its `_civAddFaction` likewise touches
    /// nothing already placed. Raising the *parameter* and re-running
    ...
    /// `civ.villages` is on (empty otherwise, which is the default).
    ///
    ...
    /// `RELIGION_DIFFUSION_SCOPE.md` §3 milestone 1's per-settlement
    /// adherence (`cartalith_civ::belief::SettlementReligionState`), same
    /// order/index as `settlements` above.
    ///
    /// # Empty means "nobody ran it", and that is the whole design
    ///
    /// **Generated state, not saved state**, and not even generated by
    /// `compute_civilisation`: this starts empty and stays empty until
    /// [`WorldGen::civ_belief_run`] is called. Three consequences, all
    /// intended:
    ///
    /// - **World replacement needs no code.** `absorb` builds a fresh
    ///   `CivData`, so a new world starts with no belief layer, exactly as
    ///   `timeline` does. There is no equivalent of `release_world`'s vault
    ///   clear to forget — which is precisely the 2026-09-03 defect that
    ///   clear's own comment records, avoided by not having state that
    ///   outlives its world.
    /// - **`civ_merge` is already correct, unedited.** `Downstream` and
    ///   `Replace` take `fresh`, whose belief is empty — right, because the
    ///   settlement list they re-derive is what this is indexed by. `Routes`
    ///   keeps `old`, whose belief is still valid — right, because `Routes`
    ///   does not touch settlements.
    /// - **Nothing is persisted.** `SAVEFILE_COMPAT.md` gains no document
    ///   slot and no tree array. The cost is real and is stated rather than
    ///   hidden: a diffusion run does not survive a save/load. It is also
    ///   small, because the model is fully deterministic — the same world,
    ///   ways and roster re-run to the same adherence, bit for bit
    ///   (`the_same_world_produces_the_same_adherence_bit_for_bit` in
    ///   `cartalith_civ::belief`).
    ///
    /// Kept parallel to `settlements` by length check rather than by
    /// construction: the Settlement tool appends and deletes, so a run
    /// followed by an edit leaves this stale. Every reader compares
    /// `belief.len()` against `settlements.len()` and treats a mismatch as
    /// "no layer" — [`CivData::belief_current`] is the one place that check
    /// lives.
    belief: Vec<cartalith_civ::belief::SettlementReligionState>,
    /// The roster's religion column as it stood when `belief` was seeded.
    ///
    /// **Found by `_belief_probe.gd`, not by a unit test**, and it is the
    /// `MISTAKES.md` "wire a capability into every consumer path" shape: the
    /// first version re-seeded only when `belief.len()` disagreed with
    /// `settlements.len()`, so setting a faction's religion *after* a run —
    /// which is the only way a religion ever gets set in this app — changed
    /// the seed and nothing else. The probe's own output was
    /// `after setting faction 1 to sun_cult -> any_faith: false`.
    ///
    /// Comparing the whole column rather than hooking `civ_set_faction_field`
    /// is deliberate: `project_bridge`'s restore, `civ_add_faction` and
    /// `civ_remove_faction` all rewrite this roster too, and a hook on one
    /// setter is exactly the single-consumer-path fix that mistake is about.
    belief_seed_key: Vec<String>,
}

/// Which civ stage [`WorldGen::civ_rebuild`] is running — the two axes that
/// separate the three rebuilding `#[func]`s (`LARGE_ITEM_RULINGS.md`'s
/// civ-authoring ruling).
///
/// The axes are *what goes in* (is the settlement list an input, or derived
/// from suitability?) and *what comes out* (is the whole fresh `CivData`
/// taken, or only part of it?). Every combination that is not one of these
/// three is either `generate()` or a no-op, which is why this is an enum of
/// three and not two booleans.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum CivRebuild {
    /// Settlements in, everything downstream out —
    /// [`WorldGen::recompute_civilisation`], `GUI_GAP_REGISTER.md` SG-02.
    Downstream,
    /// Nothing in, everything out — [`WorldGen::civ_populate`], the
    /// reference's Auto-populate.
    Replace,
    /// Settlements in, only the route network out —
    /// [`WorldGen::civ_auto_routes`], the reference's Generate Roads.
    Routes,
}

/// What [`WorldGen::civ_rebuild`] hands [`compute_civilisation`] to hold
/// fixed. See that function's `keep` parameter.
struct KeptCiv {
    settlements: Vec<cartalith_civ::NamedSettlement>,
    next_tid: u64,
    village_tids: std::collections::HashSet<u64>,
...
/// `compute_civilisation` result with the layer it replaces, and report
/// **whether the merged layer is wholly re-derived from the settlement list
/// it now holds.**
///
/// The bool travels *with* the merge rather than beside it, and that is the
/// whole point of this signature. `Routes` takes only the route network and
/// leaves `territory`, `provinces`, `province_list`, `continents`,
/// `trade_balances`, `explanations`, `water_bodies` and `dens` at their
/// pre-run values, so after it the civ layer is current with neither the
/// settlement list nor the terrain the tail just settled. Re-deriving that
/// fact a second time at the call site is what produced the regression this
/// function exists to prevent: the shared tail cleared `civ_dirty` and marked
/// `PipelineStage::Civ` recomputed for every mode, so SG-02's "Recompute now"
/// indicator went quiet after a Generate Roads while the territory and
/// provinces it speaks for were still stale. One source of truth, returned
/// from the merge that knows.
///
/// The `false` half is not "something failed" — `Routes` succeeding is
/// exactly the case that returns it.
fn civ_merge(mode: CivRebuild, mut fresh: CivData, mut old: CivData) -> (CivData, bool) {
    match mode {
        CivRebuild::Routes => {
            // Only the route network is taken; everything else on `fresh`
            // is discarded unread -- the cost this stage pays for being
            // narrower than the button beside it (see `civ_auto_routes`).
            // `next_tid` comes with them because the fresh ways were issued
            // ids out of the same counter.
            old.ways = fresh.ways;
            old.road_edges = fresh.road_edges;
            old.sea_routes = fresh.sea_routes;
            old.next_tid = old.next_tid.max(fresh.next_tid);
            (old, false)
        }
        CivRebuild::Downstream => {
            // Boundary state with no terrain input: moved across rather
            // than rebuilt, which is the whole difference between this and
            // `generate`.
        ...
        CivRebuild::Replace => {
            // `place_extras` and the timeline are keyed by `tid`, and every
            // `tid` here is new, so both stay at the empty values
            // `compute_civilisation` just built. The roster is keyed by
            // faction index and survives -- grown, never shrunk, so
            // `civ.factions` can be raised without leaving a settlement in
            // a faction the roster has no row for. `civ_populate`'s own doc
...
/// The staleness half of [`WorldGen::civ_rebuild`]'s tail: act on
/// [`civ_merge`]'s answer, and decide nothing of its own.
///
/// Free-standing over the two pieces of state it touches because the `#[func]`
/// wrapped around it cannot run under `cargo test` at all (`landmark_dict_tests`
/// states why at length), and the regression this pair replaces lived exactly
/// here — in a tail that cleared both unconditionally. `civ_merge_tests` drives
/// this with each mode's real merge result, so removing the guard fails a test
/// instead of only quietening an indicator.
fn civ_settle_staleness(stages: &mut cartalith_spatial::StageGraph, civ_dirty: &mut bool, rederived: bool) {
    if rederived {
        *civ_dirty = false;
        stages.mark_recomputed(PipelineStage::Civ.id(), "civ_recomputed");
    }
}
...
/// claim `civ_rebuild`'s tail acts on when it clears `civ_dirty` and marks
/// `PipelineStage::Civ` recomputed, and a shared tail that made it for
/// `Routes` too is the regression these exist to catch. A `Downstream`-only
/// test is what let it through, so the generic assertion is written three
/// times rather than once.
///
/// Runs with no Godot runtime, for the reason `civ_timeline_tests` above
/// states: `CivData` carries no `godot` type, and neither does `StageGraph`.
/// The `#[func]` itself cannot be driven from here (every `VarDictionary`
/// call panics without a live engine — see `landmark_dict_tests`), so the
/// tail was split at the last point that needs one: [`civ_merge`] and
/// [`civ_settle_staleness`] are both exercised below with the real values
/// `civ_rebuild` passes them, and what is left inside the `#[func]` is one
/// `recomputed.push` of a `GString`.
#[cfg(test)]
mod civ_merge_tests {
    use super::{
    ...
    use cartalith_civ::{
        ...
        SuitExplanation, TradeBalance, Way, WayType,
    };

    /// Every field whose value `civ_rebuild`'s tail claims to have just
    /// re-derived from the settlement list — `CivData` minus the six pieces
    /// of boundary state (`next_tid`, `timeline`, `year`, `faction_roster`,
    /// `place_extras`, `village_tids`) that are carried across on purpose.
    /// Read as lengths, which is all [`tagged`] varies.
    fn derived_lengths(c: &CivData) -> [usize; 12] {
        [
            c.settlements.len(),
            c.ways.len(),
            c.sea_routes.len(),
    ...
    fn tagged(n: usize) -> CivData {
        CivData {
            ...
            ways: (0..n)
                ...
                .collect(),
            sea_routes: (0..n)
                .map(|_| SeaRoute { pts: Vec::new(), brks: Vec::new(), km: 0.0, name: String::new() })
                .collect(),
            road_edges: (0..n).map(|_| RoadEdge { a: 0, b: 0, path: Vec::new() }).collect(),
            territory: vec![0; n],
            provinces: vec![0; n],
            province_list: (0..n)
                .map(|i| Province {
            ...
            continents: (0..n)
                ...
                .collect(),
            trade_balances: (0..n).map(|_| TradeBalance::default()).collect(),
            explanations: (0..n)
                .map(|_| SettlementExplanation {
                    suit: SuitExplanation {
                        x: 0,
                        y: 0,
                        score: 0.0,
                        z: 0.0,
    ...
    /// The claim itself, driven the whole way through `civ_rebuild`'s tail:
    /// merge, then settle the staleness state exactly as the `#[func]` does,
    /// then check that what the state now says is what actually happened.
    /// Clearing `civ_dirty` (and marking `Civ` recomputed) is honest exactly
    /// when nothing pre-run survived into a derived field.
    ///
    /// Both halves of the fix run here, so reverting either — `civ_merge`
    /// claiming `true` for `Routes`, or `civ_settle_staleness` dropping its
    /// guard — fails this.
    fn assert_claim_matches_reality(mode: CivRebuild) -> (CivData, bool) {
        let (merged, rederived) = civ_merge(mode, tagged(FRESH), tagged(OLD));
        let all_fresh = derived_lengths(&merged) == [FRESH; 12];

        // A world whose terrain was edited under it, then caught up as far as
        // `civ_rebuild` catches it up before this point: its own
        // `mark_and_recompute(Height, [], "recompute_civ")` settles hydrology
        // and climate, leaving `Civ` as the one stale stage. The SG-01 flag is
        // set the way a hand-dropped settlement sets it.
        let mut stages = pipeline_stage_graph(1);
        stages.mark_changed(PipelineStage::Height.id(), 0, "sculpt");
        stages.mark_recomputed(PipelineStage::Hydrology.id(), "flow_recomputed");
        stages.mark_recomputed(PipelineStage::Climate.id(), "climate_recomputed");
        assert!(stages.any_stale(PipelineStage::Civ.id()), "fixture: civ starts stale");
        let mut civ_dirty = true;
        civ_settle_staleness(&mut stages, &mut civ_dirty, rederived);

        let says_current = !civ_dirty;
        assert_eq!(
            says_current, all_fresh,
            "{mode:?} leaves civ_dirty={civ_dirty} with derived fields {:?} \
             (fresh={FRESH}, pre-run={OLD}); saying \"up to date\" over pre-run data is the \
             regression this test exists for",
            derived_lengths(&merged)
        );
        assert_eq!(
            !stages.any_stale(PipelineStage::Civ.id()),
            all_fresh,
            "{mode:?} marked PipelineStage::Civ recomputed without re-deriving it"
        );
        (merged, rederived)
    }

    /// Generate Roads takes the route network and nothing else, so it may
    /// not claim the layer was re-derived.
    #[test]
    fn routes_keeps_derived_state_and_says_so() {
        let (merged, rederived) = assert_claim_matches_reality(CivRebuild::Routes);
        assert!(!rederived, "Routes re-derives nothing but the routes");
        // Taken from the fresh pass.
        assert_eq!((merged.ways.len(), merged.road_edges.len(), merged.sea_routes.len()), (FRESH, FRESH, FRESH));
        // Kept at their pre-run values -- the six the tail used to claim for.
        assert_eq!(merged.territory.len(), OLD);
        assert_eq!(merged.provinces.len(), OLD);
        assert_eq!(merged.province_list.len(), OLD);
        assert_eq!(merged.trade_balances.len(), OLD);
        assert_eq!(merged.explanations.len(), OLD);
        assert_eq!(merged.dens.len(), OLD);
        // ...and the settlement list is untouched by definition.
    ...
    /// it may.
    #[test]
    fn downstream_rederives_every_derived_field_and_carries_boundary_state() {
        ...
        assert!(rederived);
        assert_eq!(derived_lengths(&merged), [FRESH; 12]);
        // Boundary state with no terrain input, moved across rather than rebuilt.
        assert_eq!(merged.year, -1200);
    ...
    /// Auto-populate replaces the settlement list and everything under it,
    /// so it may too — and drops the `tid`-keyed side tables saying why.
    #[test]
    fn replace_rederives_every_derived_field_and_grows_the_kept_roster() {
        let mut fresh = tagged(FRESH);
        fresh.faction_roster = civ_roster_bridge::FactionRoster::seeded(9);
        let mut old = tagged(OLD);
        old.civ_add_year(-1200);
        old.place_extras.0.insert(7, Default::default());
...
impl CivData {
    ...
    /// The roster's religion column, the thing [`Self::belief_seed_key`]
    /// remembers.
    /// Every input [`cartalith_civ::belief::belief_seed`] reads, in one value.
    ///
    /// `belief_seed(faction_of, faction_religion)` takes **two** arguments and
    /// this key must cover both, or a change to the uncovered one leaves a
    /// stale layer that reports itself current.
    ///
    /// *This returned only the roster's religion column until 2026-09-03, and
    /// a verifier measured the gap:* `civ_edit_settlement(i, {"faction": N})`
    /// writes `s.placement.faction` — which **is** `faction_of` — with no
    /// length change and no roster change, so reassigning a settlement to a
    /// faction of a different faith left it showing its old religion
    /// (`reassign … faction 1 -> 2 (edit ok=true) … re-seeded=false`). That is
    /// the identical miss `_belief_probe.gd` had already caught once for the
    /// religion column, surviving the fix for it — which is why the key is now
    ...
    fn belief_key(&self) -> Vec<String> {
        ...
        key.push("\u{1}faction_of".to_string());
        // `faction_of` — the per-settlement assignment.
        key.extend(self.settlements.iter().map(|s| s.placement.faction.to_string()));
        key
    }

    /// The belief layer, but only when it still describes *this* settlement
    /// list and **every input it was seeded from** — see [`Self::belief_key`],
    /// which is derived from `belief_seed`'s signature so a new argument
    /// cannot be forgotten here.
    ///
    /// The single place the staleness checks live, so a settlement dropped
    /// after a run cannot leave one reader emitting another settlement's
    /// adherence while its neighbour omits the key; a religion changed after a
    /// run cannot leave the map showing the faith the world used to have; and
    /// a settlement reassigned to a different faction cannot keep the old
    /// faith. `None` here and an empty `belief` are the same answer to a
    /// caller — "no layer" — deliberately: a stale layer is not partial
    /// information, it is wrong information.
    ///
    /// **Known and deliberately not covered: `culture`.** It feeds `compat` on
    /// every diffusion *step*, not the seed, so a culture edit leaves a layer
    /// that was correctly seeded and diffused under the old culture. Widening
    /// the key to cover it would re-seed — discarding diffused adherence — on
    /// an edit that should arguably only change the next run. That is a
    /// behaviour question, recorded rather than answered.
    fn belief_current(&self) -> Option<&[cartalith_civ::belief::SettlementReligionState]> {
        (!self.belief.is_empty()
            && self.belief.len() == self.settlements.len()
            && self.belief_seed_key == self.belief_key())
        .then_some(self.belief.as_slice())
    }

    /// `RELIGION_DIFFUSION_SCOPE.md` milestone 1: seed if needed, then run
    /// `years` diffusion steps over the road network.
    ///
    /// Split out of the `#[func]` wrapper so it can be tested at all —
    /// `civ_merge_tests` states the reason at length (a `#[func]` cannot run
    /// under `cargo test`, and every `VarDictionary` call panics without a
    /// live engine).
    ///
    /// Re-seeds whenever [`Self::belief_current`] says the layer does not
    /// match the settlement list *or* the roster's religion column — which
    /// covers "never run", "run then a settlement was added or deleted", and
    /// "run then a faction's religion was changed". The last of those is the
    /// only way a religion is ever set in this app, and missing it made the
    /// whole feature inert; see [`CivData::belief_seed_key`].
    ///
    /// A re-seed **discards the previous run**, on purpose: the adherence
    /// that existed was derived from an assignment that no longer holds, and
    /// re-deriving it is free because the model is deterministic. Returns
    /// `(settlements, seeded, any_faith)`.
    ///
    /// # Culture and religion both come from the roster, and neither from this crate
    ///
    /// `cartalith-civ` is stateless (`ARCHITECTURE.md`) and the faction
    /// roster is boundary state living here (`civ_roster_bridge`'s own module
    /// doc says why), so the two per-faction strings the model needs are
    /// resolved on this side and handed in as slices. That is also why the
    /// engine functions take `&[&str]` rather than a `NamedSettlement`: they
    /// cannot see a roster and must not learn to.
    fn civ_belief_run(&mut self, years: u32) -> (usize, bool, bool) {
        use cartalith_civ::belief;
        ...
        if !self.belief.is_empty() && years > 0 {
            let pops: Vec<u32> = self.settlements.iter().map(|s| s.pop).collect();
            let links = belief::belief_links_from_ways(&self.ways);
            let net = belief::BeliefNetwork::build(&pops, &links);
            // A settlement in a faction the roster has no row for keeps the
            // empty key, which `compat` routes to its own stated neutral --
            // the same treatment `belief_seed` gives an unresolvable faction.
            let cultures: Vec<&str> = self
                .settlements
                .iter()
                .map(|s| {
                    self.faction_roster
                        ...
                        .map_or("", |e| e.culture.as_str())
                })
                .collect();
            for _ in 0..years {
                belief::belief_step(&mut self.belief, &net, &cultures, belief::BELIEF_STEP_RATE);
            }
        }
        (self.belief.len(), seeded, belief::belief_any_faith(&self.belief))
    }
...
mod civ_timeline_tests {
    ...
    // ---- RELIGION_DIFFUSION_SCOPE.md milestone 1, boundary half ----
    //
    // The engine half is tested in `cartalith_civ::belief`. What can only be
    // tested here is the wiring: that the roster's two per-faction strings
    // reach the model, that the layer is recognised as stale when the
    // settlement list changes under it, and that "no layer" and "the model
    // ran and found nothing" stay distinguishable. `civ_belief_run` the
    // `#[func]` cannot run under `cargo test` (`civ_merge_tests` states why),
    // so the `CivData` method it wraps is what is driven.

    fn way(a: usize, b: usize, km: f64) -> cartalith_civ::Way {
        cartalith_civ::Way {
            tid: 0,
            pts: Vec::new(),
            brks: Vec::new(),
    ...
    /// A two-settlement world, both in faction 1, joined by one 30 km road.
    fn linked_pair() -> CivData {
        let mut civ = empty_civ(
            vec![mk_settlement(1, 0, "Alpha"), mk_settlement(2, 9, "Beta")],
            vec![0; 4],
        );
        civ.ways = vec![way(0, 1, 30.0)];
        civ
    }

    #[test]
    fn belief_is_absent_until_it_is_run_and_never_defaults() {
        let civ = linked_pair();
        assert!(civ.belief.is_empty(), "compute_civilisation must not build the layer");
        assert!(
            civ.belief_current().is_none(),
            "an empty layer is `no layer`, not `everyone is secular`"
        );
    }
    ...
    fn the_shipped_roster_makes_a_run_report_no_faith_rather_than_an_empty_result() {
        ...
        assert!(
            !any_faith,
            "with every faction's religion at None there is nothing to spread -- and this \
             `false` is the sentence a surface must show instead of a blank breakdown"
        );
        assert!(civ.belief_current().is_some(), "the layer exists; it is just all-secular");
        for s in civ.belief.iter() {
            assert_eq!(
                cartalith_civ::belief::religion_key(s.plurality()),
                Some("none")
            );
        }
    }
    ...
    #[test]
    fn a_hand_set_faction_religion_reaches_the_model_and_spreads_down_a_road() {
        let mut civ = linked_pair();
        civ.settlements[1].placement.faction = 2;
        assert!(civ.faction_roster.set_field(1, "religion", "sun_cult"));
        assert!(civ.faction_roster.set_field(1, "culture", "desert"));
        assert!(civ.faction_roster.set_field(2, "culture", "desert"));

        let (_, _, any_faith) = civ.civ_belief_run(0);
        assert!(any_faith, "faction 1's settlement seeds wholly into the sun cult");
        let sun = cartalith_civ::belief::religion_index("sun_cult").unwrap();
        assert_eq!(civ.belief[0].share[sun], 1.0);
        assert_eq!(civ.belief[1].share[sun], 0.0, "faction 2 starts unaffiliated");

        // Now let it run. The road is the only thing connecting them.
        civ.civ_belief_run(40);
        assert!(
            civ.belief[1].share[sun] > 0.0,
            "the faith must cross the road into the neighbouring faction's town"
        );
        assert!(civ.belief[0].share[sun] > civ.belief[1].share[sun]);

        // And with the road removed it must not, which is what proves the
        // graph is the road network rather than proximity.
        let mut isolated = linked_pair();
        isolated.settlements[1].placement.faction = 2;
        assert!(isolated.faction_roster.set_field(1, "religion", "sun_cult"));
        isolated.ways.clear();
        isolated.civ_belief_run(40);
        assert_eq!(
    ...
    ///
    /// Setting a faction's religion is the **only** way a religion is ever
    /// set in this app, and it always happens after the world exists. A
    /// version of `civ_belief_run` that re-seeded on a settlement-count
    /// change alone made the entire feature inert: the probe's own line was
    /// `after setting faction 1 to sun_cult -> any_faith: false`.
    #[test]
    fn changing_a_factions_religion_after_a_run_re_seeds_the_layer() {
        ...
        assert!(
            civ.belief_current().is_none(),
            "the layer is now stale against the roster it was seeded from, so every reader \
             must see `no layer` rather than the world's previous faith"
        );

        let (_, seeded, any_faith) = civ.civ_belief_run(50);
        assert!(seeded, "a religion change must force a re-seed");
        assert!(any_faith, "and the new religion must actually reach the model");
        let sun = cartalith_civ::belief::religion_index("sun_cult").unwrap();
        assert_eq!(civ.belief[0].share[sun], 1.0);

        // Changing it back invalidates again -- the check is on the column,
        // not on a one-way "has ever been edited" flag.
        assert!(civ.faction_roster.set_field(1, "religion", "none"));
        assert!(civ.belief_current().is_none());
        let (_, _, any_faith) = civ.civ_belief_run(10);
        assert!(!any_faith);

        // A field that is not `religion` must NOT invalidate: re-seeding on
        // every roster touch would silently reset the simulation whenever
        // somebody renamed a faction.
        assert!(civ.faction_roster.set_field(1, "religion", "old_gods"));
        civ.civ_belief_run(5);
        let before = civ.belief.clone();
        assert!(civ.faction_roster.set_field(1, "name", "The Concord"));
        assert!(civ.belief_current().is_some(), "a rename is not a religion change");
        let (_, seeded, _) = civ.civ_belief_run(0);
        assert!(!seeded);
        assert_eq!(civ.belief, before, "a rename must not restart the simulation");
    }

    #[test]
    fn editing_the_settlement_list_invalidates_a_finished_run() {
        let mut civ = linked_pair();
        assert!(civ.faction_roster.set_field(1, "religion", "old_gods"));
        civ.civ_belief_run(10);
        assert!(civ.belief_current().is_some());

        // The Settlement tool appends. The layer is now one row short and
        // must read as absent rather than mis-aligning Gamma's adherence
        // onto Beta.
        civ.settlements.push(mk_settlement(3, 20, "Gamma"));
        assert!(
            civ.belief_current().is_none(),
            "a stale layer is wrong information, not partial information"
        );

        // A re-run re-seeds rather than extending, so nothing carries over
        // from a settlement list that no longer exists.
        let (n, seeded, _) = civ.civ_belief_run(0);
        assert_eq!(n, 3);
        assert!(seeded, "the length mismatch must force a re-seed");
        assert!(civ.belief_current().is_some());
    }

    #[test]
    fn a_second_run_continues_rather_than_restarting() {
        let sun = cartalith_civ::belief::religion_index("sun_cult").unwrap();
        let run_once = |years: u32| -> f64 {
            let mut civ = linked_pair();
            civ.settlements[1].placement.faction = 2;
            civ.faction_roster.set_field(1, "religion", "sun_cult");
            civ.civ_belief_run(years);
            civ.belief[1].share[sun]
        };
        let mut split = linked_pair();
        split.settlements[1].placement.faction = 2;
        split.faction_roster.set_field(1, "religion", "sun_cult");
        split.civ_belief_run(12);
        let (_, seeded_again, _) = split.civ_belief_run(8);
        assert!(!seeded_again, "the second call must not re-seed");
        assert_eq!(
            split.belief[1].share[sun],
            run_once(20),
...
///
/// **No longer the value generation runs at.** Since the civ `PARAMS` group
/// landed, `cartalith_engine::CivParams::factions` is what
/// [`compute_civilisation`] passes to `assign_factions` and seeds the roster
/// from, and `the_civ_group_defaults_are_the_references_own_constants` pins
/// that parameter's default to this same 6 so the two cannot drift apart.
///
/// Its one remaining live use is `project_bridge`'s fallback for a project
/// archive that carries no `factions.json` at all — a free function with no
/// `WorldParams` in reach, restoring a roster rather than generating one.
...
/// [`cartalith_engine::CivParams::recovery_phase`] as the enum
/// `cartalith_civ` actually takes. Clamped, not validated: the parameter
/// table already bounds the row to `0..=4`, and this is the reference's own
/// `Math.max(0,Math.min(4,rp.value|0))` for anything that reaches here by
/// another route.
fn recovery_phase_of(civ: &cartalith_engine::CivParams) -> cartalith_civ::timeline::RecoveryPhase {
...
/// generated `WorldState`, in exactly the dependency order each milestone's
/// own golden test exercises
/// (`cartalith-civ/tests/golden_parity_settlement_placement.rs`'s
/// `compute_placements` helper is the canonical reference for this chain —
...
fn compute_civilisation(
    ...
    // `civBiomeKChk` (`set_biome_k_enabled`, `CivParams::biome_k`). The
    ...
    if !keeping && recovery_phase_of(opts) != cartalith_civ::timeline::RecoveryPhase::Stable {
    ...
    let trade_balances: Vec<cartalith_civ::TradeBalance> = settlements
        ...
        .map(|s| {
            ...
            let ctx_mean = cartalith_civ::civ_place_resource_context(
                &resources, &ws.field, gw, gh, sea_level, s.placement.x as i64,
                s.placement.y as i64, radius, world,
            );
    ...
    CivData {
        ...
        // fresh roster. `civ.factions` is what `assign_factions` was just run
        // with, so the roster and the settlements agree on which ids exist at
        // the moment this struct is built. (`CIV_FACTION_COUNT` until the civ
...
///
/// Elevations go through `sample_bridge::FieldRefs::elevation_m`, which is the
/// Sample dock's own metre conversion (`metersPerUnit()`'s anchoring), rather
...
fn river_dict(f: &sample_bridge::FieldRefs<'_>, r: &cartalith_hydrology::River, index: usize) -> VarDictionary {
    ...
    let mut d = vdict! {
        ...
        "discharge" => r.discharge as f64,
        "mouth_discharge" => r.mouth_discharge as f64,
        // See `get_rivers`' own doc: the same reading as `discharge`, in area
        // units, and rainfall-weighted rather than a true cell count.
        "catchment_km2" => r.discharge as f64 * ck * ck,
        "tributaries" => r.tributaries as i64,
        "head_x" => (head % f.gw) as i64,
        "head_y" => (head / f.gw) as i64,
        "mouth_x" => (mouth % f.gw) as i64,
        "mouth_y" => (mouth / f.gw) as i64,
        "source_m" => source_m,
        "mouth_m" => mouth_m,
        "drop_m" => source_m - mouth_m,
    };
    // Omitted rather than zero-filled when the mouth carries no flow, the same
    // rule `sample_cell()`'s optional keys follow.
    //
    // **In cells, and deliberately not converted to metres here.** `half_w` is
    // the radius of the disc `stamp_river_intensity` inks, and it is a
    // cartographic symbol width, not a hydraulic one: `river_width_scale_k`
    // makes it grow as the map's real extent shrinks, precisely so a river
    // stays visible on a zoomed-in sheet. Multiplied out at an 800 km / 192
    // cell world it reads ~8 000 m, which is true of the *drawn channel* and
    // absurd of a river. Handing back the engine's own unit keeps the caller
    // from mistaking one for the other; `right_dock.gd` prints both and says
    // which is which.
    if let Some(half_w) = r.half_width_cells {
        d.set("width_cells", 2.0 * half_w);
    }
    d
}
...
struct WorldGen {
    ...
    /// The caller's own raster layer stack (`GUI_GAP_REGISTER.md` CA-03/CA-04,
    /// RD-10), or `None` for `render::LayerStack::DEFAULT`.
    ///
    /// A fourth authority, for `appearance_ramp`'s own reason: the stack, the
    /// stops, the tier and the scalar overrides are four separate things a user
    /// edits in four separate places, and folding any of them into another
    /// means one edit silently discards a different one.
    ///
    /// `None` rather than an eagerly-stored default: `is_default()` decides the
    /// byte-identical path in `render.rs`, and a caller who has never touched a
    /// layer should not be able to reach it through a stored value at all.
    appearance_layers: Option<render::LayerStack>,
    ...
    /// The display device `build_color_texture` encodes its finished raster for
    /// (`LARGE_ITEM_RULINGS.md`, **Colour management**). `Srgb` on a fresh
    /// session, and `render::apply_color_space` early-returns on it, so an
    /// untouched `WorldGen` emits the same bytes it emitted before this field
    /// existed -- pinned as a hash of the finished default render in
    /// `tests/color_space.rs`, not merely asserted.
    ///
    /// **Deliberately not a field on `TerrainAppearance`, and not in
    /// `appearance_over`.** Everything in those describes the *picture*, is
    /// serialized into a saved look and into the project's `AppearanceDoc`, and
    /// is meant to travel: a look carried to another machine should draw the
    /// same map there. A display device is the opposite -- it describes the
    /// *monitor in front of this session*, and carrying one into a file would
    /// make a look saved on a wide-gamut panel render wrong on the sRGB laptop
    /// that opened it. `GUI_GAP_REGISTER.md`'s own split, verbatim: *"display =
    ...
    /// `generate()`/`generate_sized()` call, and after `load_save()`.
    ///
    /// **`project_open` puts one back**, when and only when the archive
    /// carries a `drafts/sculpt.json` for this world's own grid. This
    /// paragraph used to say a draft "only ever exists over a freshly
    /// generated `WorldState`" because a loaded save carries no
    /// `river_mask`/`river_floor` — true about the *masks*, and the wrong
    /// conclusion: having none is the legal empty lock state
    /// (`WaterState::new`, what a world generated with river carving off
    /// gets), not a missing value. What a loaded project still cannot do is
    /// **commit** the draft; `sculpt_commit` pattern-matches
    /// `WorldSource::Generated` and is unchanged.
    ...
    ///
    /// **`project_open` puts one back**, when and only when the archive
    /// carries a `drafts/paint.json` for this world's own grid. This
    /// paragraph used to say the land-only gate's water-body classification
    /// could only come from "a freshly generated `WorldState`'s
    /// `field`/`rainfall`"; it comes from `build_water_bodies` over
    /// `rasters/heightmap.f32` and `rasters/rainfall.f32`, which
    /// `SAVEFILE_COMPAT.md` §5.1 requires every archive to carry. See
    /// `project_bridge.rs`'s `loaded_water_mask`.
    ...
    /// `civ_delete_settlement` (`ED-03d`'s own three), cleared by `absorb` (a
    /// freshly generated world's civ layer is derived from its own
    /// settlements by construction) and by the `civ_rebuild` modes that
    /// actually re-derive the whole layer — `Downstream` and `Replace`, but
    /// **not** `Routes`, which rebuilds only the route network and leaves
    /// territory, provinces, trade balances and explanations pre-run
    /// (`civ_merge` returns that answer; `civ_merge_tests` asserts it).
    /// Read only by `stale_stages`, so the indicator does not claim "up to
    /// date" straight after a hand-dropped town.
    civ_dirty: bool,
    ...
    /// `project_save_with_documents` so the save can put it back.
    ///
    /// `SAVEFILE_COMPAT.md` §6.2 gives a reader that writes an archive back
    /// two choices: re-emit what it did not consume, or refuse to overwrite.
    /// This field is the first choice. Without it, an older build opening a
    /// newer build's project and pressing Save deleted the newer payloads
    /// silently -- §17 named that as the format's one open conformance gap.
    ///
    /// **Cleared wherever a world ends**, and it has to be, because these
    /// bytes belong to *one file*: `release_world()` (every generate path)
    /// and `load_save()` (every open, including the one `project_open` makes
    /// before restoring on top). Left uncleared it would graft the last
    /// project's foreign entries onto the next one, which is the same class
    /// of bug `landmark_store.settings` and `vault.store.links` were both
    /// fixed for.
    ///
    /// Empty for a generated world, which is the honest answer: nothing was
    /// read, so nothing is being carried.
    carried_foreign: std::collections::BTreeMap<String, Vec<u8>>,
    ...
    /// `region_new_world()` runs on the shell's worker thread, where every
    /// `Dictionary`/`Array`/`GString` operation goes through
    /// `sys::get_binding()` and panics. It is read back and converted by
    /// `region_new_world_error()`, on the main thread.
    ///
    /// Its five values are the five refusals `region_new_world` can give --
    /// the finalize lock, no marquee, no world, a source shorter than its own
    /// grid, and a resample below `MIN_REGION_WORLD_AXIS`. Each names what to
    /// change, because a modal that says only "failed" is the shape this
    /// project has repeatedly had to go back and fix. Not saved: it describes
    /// one press, not the world.
    region_error: String,

    /// **How this world's height field was produced** — one of
    /// `bake_bridge::ORIGIN_GENERATED`/`ORIGIN_IMPORTED`/`ORIGIN_REGION`, and
    /// an element of the atlas world key.
    ///
    /// It is here because nothing else on this struct answers the question.
    /// `source` distinguishes `Generated` from `Loaded`, but an imported
    /// heightmap and a region resample are both `WorldSource::Generated`, and
    /// every other element of `world_key()` is a generation *input* — so at one
    /// parameter tuple those three worlds hashed to a single atlas namespace
    /// and one served another's baked tiles. See
    /// [`bake_bridge::world_key_signature`] for the whole argument.
    ///
    /// Set by [`absorb`], which takes it as an argument precisely so a new
    /// world-replacement path cannot reach this field by forgetting to, and
...
impl WorldGen {
    ...
    fn release_world(&mut self) {
        ...
        // first save. See the field's own doc comment.
        self.carried_foreign.clear();
        ...
        // path into B's `vault.json`.
        //
        // These two are **not** the authoritative clear any more, and a new
        // `LinkStore` member that is world state should follow `absorb`'s copy
        // rather than this one: `release_world` is not on every path that
        // replaces a world (`import_heightmap` is not, by design), which was
        // the 2026-09-03 defect. What this copy is for is the window
        // `release_world` opens on purpose -- from here until `absorb`
        // returns there is no world, and if the generate in between never
        // returns, the store must not still hold the released world's links.
    ...
    /// on GPU.
    ///
    /// `origin` is how the incoming field was produced — one of
    /// `bake_bridge::ORIGIN_GENERATED`/`ORIGIN_IMPORTED`/`ORIGIN_REGION`. It is
    /// a parameter rather than something this function could infer because it
    /// genuinely cannot: all three arrive here as a `WorldState`, and the atlas
    /// world key has to tell them apart (see the `world_origin` field).
    fn absorb(
        &mut self,
        ws: cartalith_engine::WorldState,
        p: &WorldParams,
        ...
        // *outgoing* civ layer, and `self.civ` was replaced wholesale at the
        // top of this function.
        //
        // Here rather than only in `release_world()`, which is where these two
        // lines used to live alone. `release_world` is not on every path that
        // replaces a world: `import_heightmap` calls `absorb` and nothing else,
        // deliberately -- it can fail recoverably on an unreadable file and
        // promises to leave the previous world untouched, so it cannot release
        // anything before it knows the import succeeded. The result was that
        // links and snapshots taken against the world an import replaced
        // survived into the imported one, pointing at settlements that no
        // longer existed, and `project_save_with_documents` would then write
        // them into the imported world's `vault.json`. `absorb` is the one
        // funnel all four replacement paths pass through, so clearing here
        // means the fifth cannot repeat it.
        //
        // `release_world`'s copy stays: its job is to leave no stale world
        // state behind even if the generate that follows it never returns, and
        // clearing an already-empty `Vec`/`BTreeMap` costs nothing.
        //
        // `store.vaults` is untouched -- it is the device's binding registry
        // (`vault_info()`), not world state. `load_save`'s copy of this clear
        // carries the longer version. Any future member of `LinkStore` that is
        // world state belongs in all three.
        self.vault.store.links.clear();
        self.vault.store.snapshots.clear();
...
impl WorldGen {
    ...
    /// One civ parameter, written through the same [`params::set`] the
    /// dictionary API uses and marking the same stage — so the four
    /// hand-written civ sugar setters (`set_villages_enabled` and friends,
    /// which `engine_bridge.gd` drives) cannot clamp, validate or invalidate
    /// differently from `set_params({"civ.villages": …})`.
    ///
    /// Deliberately not `#[func]`: it takes a `params::Value`, and the
    /// Variant-facing entry point is `set_params`.
    fn set_civ_param(&mut self, key: &str, value: params::Value) {
        if params::set(&mut self.params, key, value) == params::Outcome::Rejected {
            godot_print!("cartalith-godot: set_civ_param rejected '{key}'");
            return;
        }
        if let Some(stage) = params::invalidates(key) {
            self.mark_param_change(stage, &format!("param:{key}"));
        }
    }
    ...
    /// numbers. Does not touch anything about an already-generated world.
    ///
    /// **Does** reset the civ group now (`civ.villages` and its six
    /// siblings). It did not before the civ `PARAMS` group existed, when
    /// those four flags lived on `WorldGen` instead of in `WorldParams` —
    /// this doc comment used to say so ("Does not touch
    /// `set_villages_enabled`, civ-layer, not a `WorldParams` field") and
    /// that sentence stopped being true with the group.
    ///
    ...
    // -- CPU worker threads (owner ruling, `LARGE_ITEM_RULINGS.md` "CPU
    //    worker threads": "Call ThreadPoolBuilder at engine init with a
    //    count from settings; expose a #[func] to read and set it") -------
    //
    // Same shape as the Multi-GPU block above and for the same reason: how
    // many workers Rayon's global pool spans describes the machine, not the
    // world, so these are `&self` methods backed by a process-global in
    // `cartalith-engine` rather than a `WorldParams` field, and are absent
    // from `params.rs`'s table. See `cartalith_engine`'s own "CPU worker
    // threads" section for why the pool is process-global rather than a
    // rebuildable scoped one, and for what "set" actually does once a
    // generation has already run this process.

    /// Logical cores this machine reports -- the ceiling `set_cpu_thread_
    /// count` clamps to, and what a ladder UI (1 / quarter / half / cores-4
    /// / all, `menus.gd`'s own SS2.5 note) needs to compute its rungs rather
    /// than guess.
    #[func]
    fn cpu_logical_core_count(&self) -> i32 {
        cartalith_engine::logical_core_count() as i32
    }

    /// The worker count Rayon's global pool will build with, or already
    /// built with — `0` for "follow Rayon's own default" (an untouched
    /// install's answer). Compare against `cpu_thread_count_active` to tell
    /// a value that already took effect from one still waiting on a restart.
    #[func]
    fn cpu_thread_count(&self) -> i32 {
        cartalith_engine::configured_thread_count() as i32
    }

    /// The pool's **real, running** worker count this process — `0` before
    /// the first `generate()`/`set_cpu_thread_count` call, never `0` after.
    /// Measured, not requested, for `gpu_last_backend`'s own reason above:
    /// what was asked for and what is actually running can disagree.
    #[func]
    fn cpu_thread_count_active(&self) -> i32 {
        cartalith_engine::thread_pool_active_count() as i32
    }

    /// Set the preferred worker count, clamped to `1..=cpu_logical_core_
    /// count()` (`0` resets to "follow Rayon's own default"). The preference
    /// is always stored — `cpu_thread_count` reads back exactly what was
    /// passed — and the return value says only whether **the pool now
    /// running is the one this call asked for**: `true` when this call built
    /// it, `false` when anything else had already built it, which includes
    /// the implicit build any `par_iter()` performs. **Rayon's global pool
    /// cannot be rebuilt once anything has used it**, so in a shell that has
    /// drawn a single frame the honest answer is `false` — "recorded, takes
    /// effect at next start" — and `menus.gd`'s CPU worker threads row says
    /// exactly that rather than claiming a change landed when it did not.
    ///
    /// This return value **was** wrong: it reported `true` unconditionally in
    /// a warm process. See `cartalith_engine`'s own "CPU worker threads"
    /// section for the measurement and the fix.
    #[func]
    fn set_cpu_thread_count(&self, threads: i32) -> bool {
        cartalith_engine::set_configured_thread_count(threads.max(0) as usize)
    }
    ...
    fn get_recovery_phase(&self) -> i32 {
        self.params.civ.recovery_phase
    }
    ...
    /// seeding pass (Phase 2 milestone 15).
    ///
    /// **Sugar over the `civ.villages` parameter**, kept because
    /// `engine_bridge.gd` drives it: it and its three siblings below write
    /// through the same `params::set` the dictionary API uses (see
    /// [`Self::set_civ_param`]), so neither can clamp, validate or invalidate
    /// differently from `set_params({"civ.villages": …})`. Before the civ
    /// `PARAMS` group these four were the *only* way to reach these flags,
    /// they lived on `WorldGen` rather than in `WorldParams`, and none of them
    /// survived a save.
    ///
    /// Applies on the next `generate()` **or** [`Self::civ_populate`], which
    /// is the cheap way to see it: re-places from the current suitability
    /// field without re-rolling the terrain.
    #[func]
    fn set_villages_enabled(&mut self, enabled: bool) {
        self.set_civ_param("civ.villages", params::Value::Bool(enabled));
    }
    ...
    /// as `kind == "metropolis"`. [`Self::civ_populate`] applies it too --
    /// see [`Self::set_villages_enabled`] for the shared contract.
    #[func]
    fn set_metropolis_enabled(&mut self, enabled: bool) {
        self.set_civ_param("civ.metropolis", params::Value::Bool(enabled));
    }
    ...
    fn set_recovery_phase(&mut self, phase: i32) {
        self.set_civ_param("civ.recovery_phase", params::Value::Num(f64::from(phase)));
    }
    ...
    fn generate_sized(&mut self, seed: i32, width_km: f64, grid_w: i32, grid_h: i32) {
        ...
        self.absorb(ws, &p, seed, bake_bridge::ORIGIN_GENERATED);
    }
    ...
    fn import_heightmap(&mut self, path: GString, seed: i32, width_km: f64, grid_w: i32) -> bool {
        ...
        match cartalith_engine::import::import_heightmap(&bytes, &base) {
            Ok(out) => {
                ...
                // `ORIGIN_IMPORTED`, not `ORIGIN_GENERATED`: this field came
                // off disk and its substrate was inverted from it, so the
                // parameter tuple in the atlas key does not determine it and
                // must not name the namespace a generated world at the same
                // tuple owns.
                self.absorb(out.state, &out.params, seed, bake_bridge::ORIGIN_IMPORTED);
                true
            }
            Err(e) => {
    ...
    /// for re-deriving placement from terrain are `generate()` and
    /// [`Self::civ_populate`].
    ///
    ...
    fn recompute_civilisation(&mut self) -> VarDictionary {
        self.civ_rebuild(CivRebuild::Downstream)
    }

    /// The reference's **Auto-populate world** (`#civAutoPopulateBtn` ->
    /// `_civAutoWorld()`, reference 21519), re-entrant over the world already
    /// on screen — `LARGE_ITEM_RULINGS.md`'s civ-authoring ruling, stage 1 of
    /// 5.
    ///
    /// Re-derives settlement placement from the current suitability field and
    /// rebuilds everything below it, without re-generating terrain. So the
    /// three placement dials in the civ `PARAMS` group (`civ.seed_thresh`,
    /// `civ.seed_suppress_div`, `civ.factions`) and the four pass flags
    /// (`civ.villages`, `civ.metropolis`, `civ.recovery_phase`,
    /// `civ.biome_k`) are adjustable and *observable*: move one, press this,
    /// see the world it makes. Before this they were reachable only through a
    /// full `generate()`, which also re-rolls the terrain underneath them.
    ///
    /// **This replaces every settlement**, exactly as the reference's own
    /// button does: names, tiers, populations and `tid`s are all new. What
    /// that costs, stated rather than discovered:
    ///
    /// - **`place_extras` is dropped** (traits, specialisation, history,
    ///   age/walls overrides). It is keyed by `tid`, and `tid`s restart at 1
    ///   here, so keeping it would silently attach the old settlement 3's
    ///   notes to an unrelated new town. `civ_delete_settlement` already
    ///   forgets one row this way; this forgets all of them.
    /// - **The recorded timeline is dropped** for the same reason: every
    ///   snapshot references settlements that no longer exist.
    /// - **The faction roster survives.** It is keyed by faction index, not
    ///   `tid`, so renames, cultures, religions and identity colours carry
    ///   across — the reference's `_civAutoWorld` likewise never touches
    ///   `CIV_FACTIONS`. It is grown if `civ.factions` was raised, so no
    ///   settlement ends up in a faction the roster has no row for.
    /// - **Hand-painted territory survives**, re-anchored onto the new
    ///   borders by [`civ_tools_bridge::CivTools::rebase`], the same as a
    ///   plain recompute.
    ///
    /// Same return dictionary as [`Self::recompute_civilisation`], and the
    /// same cost — it is the identical pass with placement re-derived rather
    /// than held, so it is if anything slightly *more* expensive than the
    /// 0.94 s / 1.60 s / 4.22 s measured there.
    #[func]
    fn civ_populate(&mut self) -> VarDictionary {
        self.civ_rebuild(CivRebuild::Replace)
    }

    /// The reference's **Generate Roads** (`#civAutoRoutesBtn` ->
    /// `_civAutoRoutes()`, reference 21367) — stage 2 of 5.
    ///
    /// Rebuilds the route network alone: the hierarchical land topology, the
    /// consolidated and smoothed ways it becomes, and the port-MST sea lanes.
    /// Settlements, territory, provinces, trade balances, explanations and
    /// the timeline are all left exactly as they are — this is the one civ
    /// stage genuinely narrower than a recompute, and it is narrow because
    /// the reference's own button is.
    ///
    /// Refuses with `ok: false` when there are no settlements to connect,
    /// which is the reference's own guard (`if(!state.places.length){
    /// alert('Auto-populate the world first.'); return; }`).
    ///
    /// It still costs a full `compute_civilisation` internally — the road
    /// builder reads river order, the biome raster and the water-body
    /// classification, none of which is retained past that function — so the
    /// price is the recompute's and only the *result* is narrowed. Said here
    /// rather than left for someone to measure and be surprised by.
    ///
    /// **This does not clear the civ staleness indicator, and must not.**
    /// Because the result is narrowed, a world that was stale before this ran
    /// is still stale after it: SG-02's "Recompute now" keeps reporting `civ`,
    /// `civ_dirty` survives, and `PipelineStage::Civ` stays unmarked. Only
    /// [`Self::recompute_civilisation`] and [`Self::civ_populate`] re-derive
    /// enough to say otherwise — see `civ_merge`.
    ///
    /// Same return dictionary as [`Self::recompute_civilisation`], minus
    /// `civ` in `recomputed` for that reason.
    #[func]
    fn civ_auto_routes(&mut self) -> VarDictionary {
        self.civ_rebuild(CivRebuild::Routes)
    }

    /// The reference's **Clear ways & journeys** (`#civClearRoadsBtn`,
    /// reference 26673) — stage 3 of 5.
    ///
    /// Empties the generated route network (`ways`, the raw `road_edges`
    /// behind them, and `sea_routes`) *and* the manually drawn ways and
    /// committed journeys in [`infra_tools_bridge::InfraTools`], because the
    /// reference's own handler clears both of its equivalents in one press
    /// (`civWays=[]; civJourneys=[]`). The disabled control this replaces
    /// said there was "nothing here that could honestly claim to clear both";
    /// this clears both.
    ///
    /// Settlements are untouched. Returns `{"ways": int, "sea_routes": int,
    /// "manual_ways": int, "journeys": int}` — what was actually removed, so
    /// a status line can report it rather than guess. All zero is a
    /// legitimate answer, not an error.
    ///
    /// Not undoable, and recorded as such in the ledger: the polylines are
    /// gone, and [`Self::civ_auto_routes`] regenerates a network rather than
    /// restoring this one.
    #[func]
    fn civ_clear_ways(&mut self) -> VarDictionary {
        let (mut ways, mut lanes, mut manual, mut journeys) = (0usize, 0usize, 0usize, 0usize);
        if let Some(civ) = self.civ.as_mut() {
            ways = civ.ways.len();
            lanes = civ.sea_routes.len();
            civ.ways.clear();
            civ.road_edges.clear();
            civ.sea_routes.clear();
        }
        if let Some(tools) = self.infra.as_mut() {
            manual = tools.ways.len();
            journeys = tools.routes.len();
            tools.ways.clear();
            tools.routes.clear();
        }
        if ways + lanes + manual + journeys > 0 {
            self.ledger.record(
                "civ",
                "Clear ways & journeys",
                format!("{ways} way(s), {lanes} sea lane(s), {manual} manual way(s), {journeys} journey(s)"),
                undo::EntryKind::Recorded(
                    "the polylines are not retained; Generate roads rebuilds a network rather than restoring this one",
                ),
            );
        }
        dict! {
            "ways" => ways as i64,
            "sea_routes" => lanes as i64,
            "manual_ways" => manual as i64,
            "journeys" => journeys as i64,
        }
    }

    /// The reference's **Clear places & routes** (`#civClearPlacesBtn`,
    /// reference 26679) — stage 4 of 5.
    ///
    /// Empties the settlement list and, with it, everything indexed by it:
    /// `place_extras`, trade balances, suitability explanations, provinces,
    /// and the territory `assign_territory` derived from the capitals. Ways
    /// and journeys go too — the reference's own v0.92 fix, quoted from its
    /// handler: "a route network with no places is meaningless — one click
    /// removes settlements AND their routes" — so this calls
    /// [`Self::civ_clear_ways`] rather than reimplementing it.
    ///
    /// The recorded timeline is dropped: every snapshot references
    ...
    fn civ_clear_places(&mut self) -> VarDictionary {
        ...
        out.set("settlements", removed as i64);
        out
    }

    /// The reference's **Clear territory** (`#civClearTerrBtn`, reference
    /// 26664) — stage 5 of 5, and the one its disabled control named
    /// specifically: *"there is no way to leave the claim map empty"*.
    ///
    /// Zeroes the claim map and the provinces derived from it, and — unlike
    /// the reference, whose `civTerritory` is paint and nothing else — also
    /// drops the accumulated `CivTools` paint layer and re-bases it on the
    /// now-empty map. Both halves are necessary here: clearing only
    /// `CivData::territory` would leave the next paint commit rebuilding the
    /// old claims out of `territory_base`, and clearing only the paint would
    /// leave `assign_territory`'s computed borders on screen.
    ///
    /// Settlements, ways and the timeline are untouched.
    ...
    /// ([`Self::recompute_civilisation`], [`Self::civ_populate`],
    /// [`Self::civ_auto_routes`]) — the ruling's "find out what actually
    /// prevents re-running each stage and fix that, rather than adding a
    /// parallel path beside it".
    ///
    /// What prevented it was never the algorithms. `compute_civilisation`
    /// builds some twenty rasters as locals and drops them on return, so
    /// nothing below it could be re-run on its own; its `keep` parameter had
    /// already carved out one axis of that (settlements as an input, versus
    /// derived from suitability). [`CivRebuild`] is that same axis plus a
    /// second one — how much of the fresh result is actually taken.
    fn civ_rebuild(&mut self, mode: CivRebuild) -> VarDictionary {
        let t0 = std::time::Instant::now();
        ...
        // The reference's own guard on Generate Roads, verbatim: "Auto-populate
        // the world first." A network over an empty place list is not an empty
        // network, it is a question with no answer.
        if mode == CivRebuild::Routes && civ.settlements.is_empty() {
            return refuse("Generate roads needs settlements to connect. Auto-populate the world first.");
        }
        let keep = match mode {
            // Placement is an input on both of these: the settlement list is
            // taken verbatim and only what hangs off it is rebuilt.
            CivRebuild::Downstream | CivRebuild::Routes => Some(KeptCiv {
            ...
            // phase applied, all from the current suitability field.
        ...
        // merge that knows, never re-derived from `mode` down here. See
        // `civ_merge`.
        let (mut fresh, rederived) = civ_merge(mode, computed, old);
        ...
        self.civ = Some(fresh);
        // SG-01, and only for a mode that earned it: everything derived from
        // the settlement list has just been re-derived from the settlement
        // list. `Routes` did not do that -- it kept territory, provinces,
        // trade balances, explanations and density at their pre-run values --
        // so it leaves the flag and the stage mark alone rather than
        // silencing SG-02's "Recompute now" over data that is still stale,
        // and reports no `civ` in `recomputed` for the same reason.
        civ_settle_staleness(&mut self.stages, &mut self.civ_dirty, rederived);
        if rederived {
    ...
    fn generate_world_structure_sized(
        ...
        self.release_world();
        let ws = generate_terrain(&p);
        // An archetype is a parameter preset, not a different kind of world:
        // `generate_terrain` still produced this field from the tuple the key
        // hashes, so it shares `generate_sized`'s origin rather than getting a
        // fourth one.
        self.absorb(ws, &p, seed, bake_bridge::ORIGIN_GENERATED);
        true
    }
    ...
    fn load_save(&mut self, path: GString) -> bool {
        ...
        // this path never calls `generate_terrain`, so it needs its own call
        // to reach `par_iter()` downstream (e.g. a texture rebuild) with the
        // pool already sized from settings rather than Rayon's implicit
        // default winning the race. A no-op if `generate()` already ran
        // first this process.
        cartalith_engine::ensure_thread_pool();
        ...
        // Dropped because any in-progress draft over the *previous* world
        // would silently apply to the wrong dimensions if kept.
        //
        // This comment used to add "a draft's water hooks need
        // `river_mask`/`river_floor`, which the save format doesn't carry",
        // as though that made a draft impossible over a loaded world. It does
        // not: having no lock masks is `WaterState::new`, the same state a
        // world generated with river carving off has. `project_open` builds
        // an editor here (after this function) whenever the archive carries a
        // `drafts/sculpt.json` for this grid. Committing one still needs the
        ...
        // dimensions if kept.
        self.civ_tools = None;
        // Dropped for the surviving half of `sculpt`'s reason above: an
        // in-progress paint draft from the *previous* world would silently
        // apply to the wrong dimensions if kept.
        //
        // Not for the other half this comment used to give -- that the
        // land-only gate could only be computed from a freshly generated
        // `WorldState`. `project_open` computes it from the archive's own
        // heightmap and rainfall (`project_bridge.rs::loaded_water_mask`) and
        // builds a Paint editor here whenever the archive carries a
        // `drafts/paint.json`, which is why a painted project now reopens
        // painted. That restore runs *after* this function, so this clear
        ...
        // The previous project's unmodelled entries, for exactly the reason
        // the landmark settings above are reset: they belong to the file that
        // is being closed. `project_open` calls this function first and then
        // files *this* archive's own foreign entries on top, so the field is
        // never empty when it should not be, and never the last file's.
        self.carried_foreign.clear();
    ...
    /// reference_pack.zip`) and decode the four families this port composites
    /// — scattered feature icons, the six ground-texture splat channels, and
    /// the two painted-layer ground families (`biomes`/`terrains`, which the
    /// paint blend prefers over its flat palette swatch). `pack.rs`'s own doc
    /// comment explains why `structures`/`custom`/`seamarks` are parsed but
    /// not decoded here.
    ///
    ...
    fn appearance(&self) -> TerrainAppearance {
        ...
        // slot: a caller's own arrangement wins over whatever the tier or a
        // loaded preset carried, and `None` leaves the base's stack alone
        // (which for every tier and every look is `LayerStack::DEFAULT`, the
        // byte-identical path).
    ...
    ///
    /// All four layers, not just the override map: after a preset load the
    ...
    fn reset_appearance(&mut self) -> i32 {
        let n = self.appearance_over.len() as i32
            + self.appearance_ramp.is_some() as i32
            + self.appearance_layers.is_some() as i32
            + self.appearance_preset.is_some() as i32;
        self.appearance_over.clear();
        self.appearance_ramp = None;
        // The layer stack is one of the things "reset" means, for the reason
        // the preset is: a reordered or blended stack is the loudest possible
        // state to leave behind a button that says it restored the tier.
    ...
    //
    // The precondition, bound. `render.rs`'s section above `RasterLayer` is
    // where the architecture and the byte-identity argument live; this is the
    // three calls a `Layers` panel needs and nothing more. **No panel draws it
    // yet** -- `cartography_workspace.gd`'s Layers ▸ Not built note still says
    // the categories cannot separate, and that reason is now false.

    /// The terrain raster's three separable categories, **top-first** — the
    /// order §7's layer list draws them, which is the reverse of the order the
    /// renderer composites them in.
    ///
    /// Each row is a `Dictionary`: `id` (the stable key `set_layer_stack`
    /// reads), `label`, `visible`, `opacity` and `blend` (one of
    /// `list_blend_modes()`). A panel builds itself from this rather than from
    /// a second copy of the vocabulary in GDScript — the rule
    /// `list_appearance_tunables` and `list_ramp_presets` already follow.
    ///
    /// **The engine stores bottom-first and this reverses once, here.** Doing
    /// it at the boundary rather than in the renderer keeps the compositing
    /// loop reading in the direction it composites, and keeps the panel reading
    /// in the direction it draws; the round trip is asserted in
    /// `tests/layer_stack.rs`.
    #[func]
    fn get_layer_stack(&self) -> VarArray {
        let a = self.appearance();
        let mut out = VarArray::new();
        for e in a.layers.entries().iter().rev() {
            let mut row = VarDictionary::new();
            row.set("id", &GString::from(e.layer.id()));
            row.set("label", &GString::from(e.layer.label()));
            row.set("visible", e.visible);
            row.set("opacity", e.opacity);
            row.set("blend", &GString::from(e.blend.name()));
            out.push(&row.to_variant());
        }
        out
    }

    /// Replace the whole stack from `get_layer_stack()`'s own row shape,
    /// top-first. Returns the number of rows applied — `3` on success and `0`
    /// on any refusal, with nothing changed.
    ///
    /// **All three rows or none.** A call that omits a category, names one
    /// twice, or carries an id this build does not have is refused rather than
    /// completed with an invented row: a stack missing `hillshade` is not a
    /// stack with the hillshade hidden, and guessing which was meant is the
    /// "encode no value as a plausible value" mistake this repository has
    /// recorded five times. `visible`, `opacity` and `blend` are each optional
    /// *within* a row and keep the current value when absent, so a panel can
    /// send a visibility toggle without restating the opacities.
    ///
    /// Reordering, blending, opacity and visibility are all this one call, on
    /// `set_color_ramp`'s own precedent: the panel sends the arrangement it
    /// wants and the engine takes it, so dragging a row past its neighbour *is*
    /// the reorder.
    ///
    ...
    /// or the seed — call `build_color_texture()` again to see it, with no
    /// regeneration.
    #[func]
    fn set_layer_stack(&mut self, rows: VarArray) -> i32 {
        if rows.len() != 3 {
            return 0;
        }
        let current = self.appearance().layers;
        let mut parsed: Vec<render::LayerEntry> = Vec::with_capacity(3);
        for row in rows.iter_shared() {
            let Ok(row) = row.try_to::<VarDictionary>() else { return 0 };
            let Some(id) = row.get("id").and_then(|v| v.try_to::<GString>().ok()) else { return 0 };
            let Some(layer) = render::RasterLayer::from_id(&id.to_string()) else { return 0 };
            // The row this layer has now, as the base every absent key falls
            // back to -- so an omitted key means "unchanged", never "default".
            let Some(&was) = current.entries().iter().find(|e| e.layer == layer) else { return 0 };
            let mut e = was;
            if let Some(v) = row.get("visible")
                && let Ok(b) = v.try_to::<bool>()
            {
                e.visible = b;
            }
            if let Some(v) = row.get("opacity")
                && let Ok(f) = v.try_to::<f64>()
                && f.is_finite()
            {
                e.opacity = f;
            }
            if let Some(v) = row.get("blend")
                && let Ok(s) = v.try_to::<GString>()
            {
                // An unrecognised blend name is refused rather than falling
                // back to Normal: a panel written against a newer engine must
                // lose the call, not silently draw a different picture.
                let Some(m) = render::BlendMode::from_name(&s.to_string()) else { return 0 };
                e.blend = m;
            }
            parsed.push(e);
        }
        // The caller sent top-first; the renderer walks bottom-first.
        parsed.reverse();
        let Ok(entries) = <[render::LayerEntry; 3]>::try_from(parsed) else { return 0 };
        let mut stack = current;
        if !stack.set(entries) {
            return 0;
        }
        self.appearance_layers = Some(stack);
        3
    }

    /// The blend modes this build has, in the engine's own order —
    /// `["Normal", "Multiply", "Screen", "Overlay", "Add"]`. The picker is this
    /// list, not a second copy of it in GDScript.
    #[func]
    fn list_blend_modes(&self) -> PackedStringArray {
        render::BLEND_MODES.iter().map(|n| GString::from(*n)).collect()
    }

    // -- The output colour space (`LARGE_ITEM_RULINGS.md`, Colour management) --

    /// The display devices this build can encode the map raster for --
    /// `["sRGB", "Display P3"]`. The picker is this list, not a second copy of
    /// it in GDScript, the same rule `list_ramp_modes` follows.
    ///
    /// **Two entries, where `OUTSTANDING_WORK.md` §2.5's design row asks for
    /// three.** `GUI_GAP_REGISTER.md` §7.6 found the third one -- *linear* --
    /// is a working space rather than a display device, and offering the two
    /// axes as one list is a category error. The working space here is
    /// measurably 8-bit sRGB and is a readout in the panel, not a choice; see
    /// `render::ColorSpace` for why 8-bit linear is unshippable rather than
    /// merely unbuilt.
    #[func]
    fn list_color_spaces(&self) -> PackedStringArray {
        render::COLOR_SPACES.iter().map(|n| GString::from(*n)).collect()
    }

    /// The space currently in force, as one of `list_color_spaces()`'s names.
    #[func]
    fn get_color_space(&self) -> GString {
        GString::from(self.color_space.name())
    }

    /// Set the display device by name. `false` for a name this build does not
    /// have, so a panel written against a newer engine loses a row rather than
    /// silently emitting the wrong encoding.
    ///
    /// Presentation only, on `set_appearance`'s exact terms: call
    /// `build_color_texture()` again to see it, with no regeneration. **And
    /// session-only** -- unlike every other control in that family this is not
    /// saved with the look or the project, because it describes the monitor
    /// rather than the map (see the `color_space` field's own doc).
    #[func]
    fn set_color_space(&mut self, name: GString) -> bool {
        let Some(space) = render::ColorSpace::from_name(&name.to_string()) else {
            return false;
        };
    ...
    fn build_color_texture(&self) -> Option<Gd<ImageTexture>> {
        ...
        if let Some(loaded) = self.asset_pack.as_ref() {
            ...
            // The same milestone's painted-ground families, decoded by
            // `pack::load_pack_from_bytes` and preferred over the flat
            // palette swatch by `land_color`'s paint blend (the reference's
            // v1.28 `_paintedTex`). Attaching them moves no pixel on its own:
            // a tile is reachable only through a cell the brush below has
            // painted.
        ...
        // The output colour space (`LARGE_ITEM_RULINGS.md`, Colour management),
        // last of all -- *after* the icon composite rather than before it, on
        // the opposite argument to local contrast and the grade. Those two are
        // about the terrain image and have no business touching drawn artwork;
        // this one is about the *encoding of the buffer*, and an icon whose
        // pixels were left in sRGB while the ground around them was re-encoded
        // would be the one thing on the plate that reads wrong. Everything in
        // `bytes` goes to the same panel, so everything in `bytes` is
    ...
    ///
    /// - `religion` (String) — the settlement's plurality religion, a
    ///   `cartalith_civ::roster::CIV_RELIGIONS` key. **`"none"` is a real
    ///   answer here**, meaning a majority of these people follow no
    ///   religion, and it is emitted as such rather than suppressed.
    /// - `adherents` (Dictionary, religion key → int) — head-counts summing
    ...
    fn get_settlements(&self) -> Array<VarDictionary> {
        ...
        civ.settlements
            ...
            .map(|(i, s)| {
                ...
                if let Some(state) = belief.and_then(|b| b.get(i)) {
                    let key = cartalith_civ::belief::religion_key(state.plurality()).unwrap_or("none");
                    let counts = state.adherents(s.pop);
                    let mut adherents = VarDictionary::new();
                    for (r, &n) in counts.iter().enumerate() {
                        // A zero-adherent religion is omitted rather than
                        // written as `0`, so the inner dictionary's size is
                        // the number of faiths actually present.
                        if let (true, Some(k)) = (n > 0, cartalith_civ::belief::religion_key(r)) {
    ...
    /// `RELIGION_DIFFUSION_SCOPE.md` §3 milestone 1's entry point: run
    /// `years` diffusion steps over the generated road network, seeding the
    /// layer from the faction roster first if it is missing or stale.
    ///
    /// Returns a status `Dictionary`, and **the status is the point** — a
    /// caller that only reads `get_settlements` cannot tell the three states
    /// apart, and telling them apart is what stops a Religion screen showing
    /// a blank panel for three different reasons:
    ///
    /// - `settlements` (int) — how many the layer covers. `0` means there is
    ///   no world yet.
    /// - `seeded` (bool) — whether this call re-seeded (first run, or the
    ///   settlement list changed under a previous one).
    /// - `years` (int) — steps actually taken.
    /// - `any_faith` (bool) — whether **any** settlement holds any share of
    ///   any religion. `false` is not an error and not an empty result: it
    ///   means no faction has been given a religion, and the fix is in the
    ///   Faction Inspector's Religion dropdown.
    /// - `reason` (String) — **present only when `any_faith` is false**, and
    ///   carrying the sentence a surface should show instead of an empty
    ///   breakdown. Omitted, not blanked, on the normal path.
    ///
    /// A run is idempotent in the sense that matters: it is deterministic, so
    /// re-running the same years over the same world gives the same answer.
    /// It is *not* a reset — a second call continues from where the first
    /// left off, which is what makes "advance the world by ten years" a
    /// natural call rather than a re-simulation from the seed.
    #[func]
    fn civ_belief_run(&mut self, years: i64) -> VarDictionary {
        let years = years.clamp(0, 100_000) as u32;
        let Some(civ) = self.civ.as_mut() else {
            return vdict! { "settlements" => 0, "seeded" => false, "years" => 0, "any_faith" => false,
                            "reason" => "No world has been generated yet." };
        };
        let (n, seeded, any_faith) = civ.civ_belief_run(years);
        let mut d = vdict! {
            "settlements" => n as i64,
            "seeded" => seeded,
            "years" => years as i64,
            "any_faith" => any_faith,
        };
        if !any_faith {
            d.set(
                "reason",
                if n == 0 {
                    "This world has no settlements to hold a religion."
                } else {
                    "Every faction's religion is set to None, so there is nothing to spread. \
                     Set a faction's religion in the Faction Inspector, then run again."
                },
            );
        }
        d
    ...
    /// The river network as **entities** (`cartalith_hydrology::river_entities`,
    /// `OUTSTANDING_WORK.md` §2.2) -- one `Dictionary` per drawable river run,
    /// in `trace_river_polylines`' own main-stems-first order.
    ///
    /// # What a "river" is here, and why it is not a new invention
    ///
    /// The reference has no river object: `buildRiverNetwork` returns rasters.
    /// What it *draws* as one river is a traced receiver chain, and
    /// `drawRiverWays` (reference line 9473) rescans each of those chains for
    /// its own `maxO` and colours the stroke by it. That is the entity this
    /// returns -- see [`cartalith_hydrology::River`] for the full statement,
    /// including the two consequences a caller has to know: a main stem is the
    /// *continuation of its first-traced headwater arm* (a trunk cell has an
    /// upstream donor, so it is never a source of its own), and a tributary's
    /// `order` counts the junction cell it ends on, so it can equal its
    /// trunk's.
    ///
    /// # Keys
    ///
    /// * `index` (int) -- position in this array. Stable only for the same
    ///   `min_order` on the same world; see `river_at()`.
    ...
    /// * `discharge` (float) -- the largest `WorldState::flow_discharge`
    ///   anywhere on the run, rescanned exactly as `order` is.
    /// * `mouth_discharge` (float) -- the reading at the outlet cell. It is
    ///   **not** the same number, because the polyline follows
    ///   `build_channels`' D-infinity aspect-projected receiver tree while
    ///   `flow_discharge` accumulated along `compute_flow`'s plain D8 one, so
    ///   discharge is not monotone downstream on a traced run. Measured on a
    ///   192x144 world: a run with 11.29 at its head and 3.32 at its mouth.
    ///   See [`cartalith_hydrology::River::discharge`].
    /// * `catchment_km2` (float) -- `discharge` as an area. **It is the same
    ///   reading in different units, and deliberately so.**
    ///   `compute_flow(use_rain: true)` seeds each cell with its rainfall
    ///   rescaled so the mean seed is exactly `1.0`, so the accumulation at
    ///   any cell is an upstream cell count weighted by *relative* rainfall:
    ///   a wetter-than-average basin reads larger than its true area, a drier
    ///   one smaller. A true unweighted area would need a second
    ///   `compute_flow` pass over the whole grid, which is the measured
    ///   hottest line in `generate()` and not something to run on a dock
    ///   rebuild.
    /// * `tributaries` (int) -- runs ending on a cell of this one.
    ...
    /// no channel topology and `WorldState::stream_order`/`channels` are
    /// `None` for a loaded save. That is the same condition `export_geojson`'s
    /// river block already tests, and it is narrower than `get_settlements()`'
    /// (a save can have settlements and no rivers).
    ///
    /// `min_order` filters headwaters out: `1` traces everything (thousands of
    /// order-1 trickles on a large world), `2` is what `EXPORT_MIN_RIVER_ORDER`
    /// uses. Values below 1 are clamped up by `trace_river_polylines`.
    #[func]
    ...
    /// coordinates, within `radius_cells` of its polyline, as the same
    /// `Dictionary` `get_rivers(min_order)` builds -- or an empty
    /// `Dictionary` when nothing is in range.
    ///
    /// **One call, not two.** The obvious shape would be an index into
    /// `get_rivers()`, but a click would then trace the network twice and
    /// marshal every river across the boundary to use one. The entity comes
    /// back whole instead; its `index` is still the position it holds in
    /// `get_rivers(min_order)` *for the same `min_order`*, for a caller that
    /// wants to cross-reference.
    ///
    /// `radius_cells` is the caller's, because only the caller knows the zoom:
    /// a fixed grid-space radius is a shrinking screen target as you zoom in
    /// and an unusable one as you zoom out. `right_dock.gd` derives it from
    /// the on-screen cell size, the same way `map_overlay.gd`'s
    /// `_hit_test_settlement` works in screen space rather than grid space.
    #[func]
    fn river_at(&self, gx: f64, gy: f64, radius_cells: f64, min_order: i64) -> VarDictionary {
        let Some(f) = self.sample_refs() else { return VarDictionary::new() };
        let rivers = self.rivers_now(min_order);
        let Some(i) = cartalith_hydrology::pick_river(&rivers, gx, gy, radius_cells) else {
            return VarDictionary::new();
        };
        river_dict(&f, &rivers[i], i)
    }

    /// The traced network behind both river bindings.
    ///
    /// Deliberately **uncached**: `WorldGen` keeps no river-entity slot, so
    /// there is nothing to invalidate on a sculpt commit, a carve, a hand
    /// paint or a regenerate — every one of which moves `flow_discharge` and
    /// `channels`. A stale river list is worse than a re-trace, and the
    /// re-trace is a graph walk over channel cells (not the grid), which is
    /// what the GeoJSON exporter and the urban pass already pay per call.
    ///
    /// Empty for a loaded save, whose `stream_order`/`channels` are `None`.
    fn rivers_now(&self, min_order: i64) -> Vec<cartalith_hydrology::River> {
        let Some(WorldSource::Generated(ws)) = self.source.as_ref() else { return Vec::new() };
        let (gw, gh) = (self.gw.max(0) as usize, self.gh.max(0) as usize);
        let (Some(order), Some(ch)) = (ws.stream_order.as_ref(), ws.channels.as_ref()) else {
            return Vec::new();
        };
        cartalith_hydrology::river_entities(
            order,
            &ch.recv,
            &ws.flow_discharge,
            &ws.field,
            gw,
            gh,
            min_order.clamp(1, i32::MAX as i64) as i32,
            cartalith_hydrology::river_flow_thresh(gw, gh, gw, self.map_width_km),
    ...
    /// **primary** -- or `-1` for none. Unchanged for every caller it already
    /// had: at zero or one selected stamp this answers exactly what the old
    /// scalar answered. Use `sculpt_get_selection` for the whole set.
    #[func]
    fn sculpt_get_selected_stamp(&self) -> i32 {
        self.sculpt.as_ref().and_then(|s| s.selected()).map_or(-1, |i| i as i32)
    }

    /// Every selected stamp's draft index, ascending -- step one of the
    /// owner's selection-sets ruling (`crate::selection`'s own module doc).
    /// Empty for no selection or before any `generate()` call.
    ///
    /// **No canvas gesture creates a multi-selection of stamps.** A stamp is
    /// picked from the right dock's stack list, not by clicking the map, and
    /// that list (`right_dock.gd::_on_stamp_select`) is outside this lane's
    /// files. The set is here and `sculpt_select_set` drives it; the modifier
    /// click on those rows is the one interaction still to be wired.
    #[func]
    fn sculpt_get_selection(&self) -> PackedInt64Array {
        let Some(s) = self.sculpt.as_ref() else { return PackedInt64Array::new() };
        s.selection.sorted().into_iter().map(|i| i as i64).collect()
    }

    /// Replaces the whole stamp selection. Indices past the end of the draft
    /// are dropped rather than rejecting the call; `false` reports that at
    /// least one was. An empty array deselects.
    #[func]
    fn sculpt_select_set(&mut self, indices: PackedInt64Array) -> bool {
        let Some(s) = self.sculpt.as_mut() else { return false };
        let len = s.draft.len();
        s.selection.set_from(indices.as_slice().iter().map(|&i| usize::try_from(i).unwrap_or(usize::MAX)), len)
    }

    /// Every stamp on the draft at once (`Edit ▸ Select all`'s engine half --
    /// the menu row itself is step three of the ruling and is still `_todo`).
    /// Returns how many are now selected; `0` before any `generate()` call or
    /// with an empty draft.
    #[func]
    fn sculpt_select_all_stamps(&mut self) -> i64 {
        let Some(s) = self.sculpt.as_mut() else { return 0 };
        let n = s.draft.len();
        s.selection.select_all(n);
        n as i64
    }
    ...
    /// Removes a stamp from the draft. Clears the selection if it pointed at
    /// the removed stamp, and **shifts a selection past it down by one** so it
    /// keeps naming the same logical stamp. `false` for an out-of-range index
    /// or before any `generate()` call.
    ///
    /// # The shift is new, and it fixes a real hole this comment used to have
    ///
    /// This read *"Clears the selection if it pointed at the removed stamp (an
    /// index into a now-shorter stack would otherwise silently refer to a
    /// different stamp)"* -- and guarded only the equal case. `PassBuffer::
    /// remove` renumbers everything after `i`, so a selection at 5 with stamp
    /// 2 deleted silently became the stamp formerly at 6: exactly the failure
    /// the parenthesis names, one index over. `SelectionSet::
    /// retain_after_remove` closes it with `IconEditor::delete`'s own rule,
    /// which `icon_bridge.rs` had all along and contrasted against this one.
    /// At zero or one selected stamp the equal case is unchanged (removed ->
    /// nothing selected); the later case now reports 4 where it reported 5.
    #[func]
...
impl WorldGen {
    ...
    /// set's **primary** -- or `-1` for none (or before any `generate()`
    /// call). `label_get_selected`'s own icon counterpart: `icon_resize`
    /// already requires `index` to match it, and `icon_handles`'s caller
    /// needs to know which index to ask for and when to draw a handle at all.
    ///
    /// Unchanged for every caller it already had: at zero or one selected
    /// icon this answers exactly what the old scalar answered. Use
    /// `icon_get_selection` for the whole set.
    #[func]
    fn icon_get_selected(&self) -> i64 {
        self.icons.as_ref().and_then(|i| i.selected()).map_or(-1, |i| i as i64)
    }

    /// Every selected icon's index, ascending -- step one of the owner's
    /// selection-sets ruling (`crate::selection`'s own module doc). Empty for
    /// no selection or before any `generate()` call.
    #[func]
    fn icon_get_selection(&self) -> PackedInt64Array {
        let Some(icons) = self.icons.as_ref() else { return PackedInt64Array::new() };
        icons.selection.sorted().into_iter().map(|i| i as i64).collect()
    }

    /// Replaces the whole icon selection. Indices past the end of the placed
    /// list are dropped rather than rejecting the call; `false` reports that
    /// at least one was. An empty array deselects.
    #[func]
    fn icon_select_set(&mut self, indices: PackedInt64Array) -> bool {
        let Some(icons) = self.icons.as_mut() else { return false };
        icons.select_set(indices.as_slice().iter().map(|&i| usize::try_from(i).unwrap_or(usize::MAX)))
    }

    /// Every placed icon at once (`Edit ▸ Select all`'s engine half -- the
    /// menu row itself is step three of the ruling and is still `_todo`).
    /// Returns how many are now selected; `0` before any `generate()` call or
    /// with nothing placed.
    #[func]
    fn icon_select_all(&mut self) -> i64 {
        let Some(icons) = self.icons.as_mut() else { return 0 };
        let n = icons.icons.len();
        icons.select_set(0..n);
        n as i64
    }
    ...
    fn icon_hit_test(&mut self, gx: f64, gy: f64) -> i64 {
        self.icon_hit_test_mode(gx, gy, 0)
    }

    /// [`Self::icon_hit_test`] with the modifier a click carried: `mode` is
    /// `0` replace (a plain click -- exactly what `icon_hit_test` does and
    /// always did), `1` toggle (Ctrl/Cmd-click), `2` extend (Shift-click).
    /// Anything else is treated as `0`. A miss changes nothing in any mode.
    ///
    /// A **separate** binding rather than a parameter on `icon_hit_test`, for
    /// the reason [`Self::label_hit_test_mode`] gives.
    #[func]
    fn icon_hit_test_mode(&mut self, gx: f64, gy: f64, mode: i64) -> i64 {
        let grid_w = self.gw as usize;
        let Some(icons) = self.icons.as_mut() else { return -1 };
        let env = cartalith_assets::manual::IconViewEnv { grid_w, zoom_scale: 1.0, icon_scale: 1.0 };
        icons.hit_test(gx, gy, &env, selection::SelectMode::from_i64(mode)).map_or(-1, |i| i as i64)
    }
    ...
    /// own `index`, `selected` (bool) and `primary` (bool). Empty before any
    /// `generate()` call or while nothing is placed.
    ///
    /// The two bools are `label_list`'s own pair and mean the same thing:
    /// `selected` is membership of the selection set, `primary` is the one
    /// member the resize handle belongs to. Both are additions -- this list
    /// carried neither before, and `cartography_workspace.gd` highlighted its
    /// rows by comparing each `index` against `icon_get_selected()`, which
    /// still works and is exactly `primary`.
    #[func]
    fn icon_list(&self) -> Array<VarDictionary> {
        let Some(icons) = self.icons.as_ref() else { return Array::new() };
        let primary = icons.selected();
        icons
            .icons
            .iter()
            .enumerate()
            .map(|(i, ic)| {
                ...
                d.set("selected", icons.selection.contains(i));
                d.set("primary", primary == Some(i));
                d
            })
            .collect()
    }
...
impl WorldGen {
    ...
    fn world_key(&self) -> String {
        ...
        cartalith_io::world_key(&bake_bridge::world_key_signature(
            ...
            self.sea_level,
            self.world,
            // How the field was produced. Every other element of this
            // signature is a generation *input*, so without this one an
            ...
            // byte-identical either way. See that function's own doc.
            params::world_key_state(&self.params),
        ))
    }
...
impl WorldGen {
    ...
    fn label_select(&mut self, index: i64) -> bool {
        ...
        let Ok(i) = usize::try_from(index) else { return false };
        labels.select(i, selection::SelectMode::Replace)
    }

    /// The selected label the edit session belongs to — the selection set's
    /// **primary** — or `-1` for none (or before any `generate()` call).
    ///
    /// Unchanged for every caller it already had: at zero or one selected
    /// label this answers exactly what the old scalar answered. Use
    /// `label_get_selection` for the whole set.
    #[func]
    fn label_get_selected(&self) -> i64 {
        self.labels.as_ref().and_then(|l| l.selected()).map_or(-1, |i| i as i64)
    }

    /// Every selected label's index, ascending -- step one of the owner's
    /// selection-sets ruling (`crate::selection`'s own module doc). Empty for
    /// no selection or before any `generate()` call; a one-element array
    /// whose only member is `label_get_selected()` in every case that getter
    /// used to be the whole story.
    #[func]
    fn label_get_selection(&self) -> PackedInt64Array {
        let Some(labels) = self.labels.as_ref() else { return PackedInt64Array::new() };
        labels.selection.sorted().into_iter().map(|i| i as i64).collect()
    }

    /// Replaces the whole label selection. Indices past the end of the list
    /// are dropped rather than rejecting the call (a shell holding a stale
    /// index must not be able to leave a member pointing at nothing);
    /// `false` reports that at least one was. An empty array deselects.
    ///
    /// The edit session follows the set's primary, so a call that lands on a
    /// different primary starts a fresh snapshot — the same thing
    /// `label_select` on a different label has always done.
    #[func]
    fn label_select_set(&mut self, indices: PackedInt64Array) -> bool {
        let Some(labels) = self.labels.as_mut() else { return false };
        labels.select_set(indices.as_slice().iter().map(|&i| usize::try_from(i).unwrap_or(usize::MAX)))
    }

    /// Every label at once (`Edit ▸ Select all`'s engine half — the menu row
    /// itself is step three of the ruling and is still `_todo`). Returns how
    /// many are now selected; `0` before any `generate()` call or with no
    /// labels placed.
    #[func]
    fn label_select_all(&mut self) -> i64 {
        let Some(labels) = self.labels.as_mut() else { return 0 };
        let n = labels.labels.len();
        labels.select_set(0..n);
        n as i64
    }
    ...
    /// own `index`, `selected` (bool) and `primary` (bool). Empty before any
    /// `generate()` call or while no labels are placed.
    ///
    /// `selected` is **membership of the selection set**, and `primary` is
    /// the one member the edit session belongs to. At zero or one selected
    /// label they are the same bool and `selected` means what it always did;
    /// a caller that only ever read `selected` therefore keeps working, and
    /// one that highlights a multi-selection has the distinction it needs
    /// without a second call.
    #[func]
    fn label_list(&self) -> Array<VarDictionary> {
        let Some(labels) = self.labels.as_ref() else { return Array::new() };
        let primary = labels.selected();
        labels
            .labels
            .iter()
            .enumerate()
            .map(|(i, lb)| {
                ...
                d.set("index", i as i64);
                d.set("selected", labels.selection.contains(i));
                d.set("primary", primary == Some(i));
                d
            })
            .collect()
    ...
    /// `0` replace (a plain click -- exactly what `label_hit_test` does and
    /// always did), `1` toggle (Ctrl/Cmd-click), `2` extend (Shift-click).
...
impl WorldGen {
    ...
    /// **The resource- and density-fed half of `_civFactionAggregates`**, per
    /// faction — `OUTSTANDING_WORK.md` §2.3's "the aggregate is ported and
    /// has three callers; nothing shows this half".
    ///
    /// That was accurate. `civ_faction_terrain_fits` above runs the aggregate
    /// with `resources: None, density: None` (it needs only the terrain mix),
    /// and `civ_military_bridge`'s own private `aggregates()` feeds
    /// `resources` but not `density` and reads only manpower's fields off the
    /// result. So the six fields below — every one of which exists only
    /// because `pots`/`dens` were threaded in — reached nothing.
    ///
    ...
    /// milestone 5.
    ///
    /// One call for every faction, and a modal-open call rather than a
    /// per-frame one, for the reason `civ_faction_terrain_fits` states: the
    /// aggregate is O(cells) and the resource rebuild is a second full pass.
    ///
    /// Each entry, indexed `1..faction_count`: `faction`, `territory_km2`,
    /// `pop`, `food_capacity` and `food_surplus` (the density-fed pair —
    /// carrying capacity over the faction's own cells against the population
    /// actually living on them, so a negative surplus is a faction that
    /// cannot feed itself), `resources` (a `Dictionary` of all fifteen
    /// `CIV_RESOURCE_KEYS` catchment means), `strategic` (those above the
    /// reference's own `0.4` bar), and `exports`/`imports` — the trade
    /// balance against the world mean, with `"food"` appended by the
    /// surplus sign exactly as the reference does. `world_mean` carries the
    /// same fifteen keys world-wide, so a row is comparable rather than a
    /// bare number.
    ///
    ...
    fn civ_faction_economy(&self) -> Array<VarDictionary> {
        ...
        let keyed = |m: &std::collections::HashMap<&'static str, f64>| -> VarDictionary {
            let mut d = VarDictionary::new();
            for k in cartalith_civ::CIV_RESOURCE_KEYS {
                d.set(k, *m.get(k).unwrap_or(&0.0));
            }
            d
        };
        ...
        (1..civ.faction_roster.0.len())
            .map(|f| {
                ...
                vdict! {
                    ...
                    "pop" => a.pop,
                    "food_capacity" => a.food_production_capacity,
                    "food_surplus" => a.food_surplus,
                    "resources" => &keyed(&a.resource_potential),
                    "strategic" => &names(&a.strategic_resources),
    ...
    /// contribution.
    ///
    /// Applies on the **next** `generate()` or [`Self::civ_populate`], like
    /// every other `civ.*` parameter — K feeds settlement suitability, so
    /// flipping it mid-world would leave placed settlements sitting on a
    /// suitability raster they were never scored against. `civ_populate` is
    /// the cheap way to see it: it re-places from the new suitability field
    /// without re-rolling the terrain.
    #[func]
    fn set_biome_k_enabled(&mut self, enabled: bool) {
        self.set_civ_param("civ.biome_k", params::Value::Bool(enabled));
    }
