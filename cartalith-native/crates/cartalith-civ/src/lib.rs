...
///   (the research's `ENDOREIC` case, §6/§7 -- "a water body occupying one
...
/// were never part of the largest-wins rule to begin with (a separate pass,
/// always lake) and the research's `ENDOREIC`/closed-basin framing already
...
pub fn build_water_bodies(
    ...
    // Per-component: does any cell touch the grid's real boundary? Same
    // world-aware edge test [`water_body_topology`] uses on the raster
    // afterwards (X counts only when `!world`; Y always counts) -- computed
    // here, on the raw below-sea mask, because the ocean/lake decision this
    // pass makes now DEPENDS on it (Ruling Q, see this function's own doc
    // comment above).
    let mut touches_boundary: Vec<bool> = Vec::new();
    ...
    for i in 0..n {
        if (field[i] as f64) < sea {
            out[i] = if is_ocean_comp[lab[i] as usize] { 1 } else { 2 };
        }
    }
...
///
/// `OUTSTANDING_WORK.md` §2.3 named this `HYDROLOGY_CLASSIFICATION_RESEARCH.md`'s
/// own "cheapest honest first step" when it was built (commit `c6de2a2`):
/// the paper's central argument is that a water body's classification
/// should describe its physical relationship to the world's hydrological
...
/// (`ocean_connected`, restating [`build_water_bodies`]'s own `kind == 1`
/// as a fact about the body rather than inferred from its value each
/// time), and whether it touches the generated region's own edge
...
pub struct Continent {
    ...
    /// holding the most cells on this landmass (ties to the lowest id),
    /// resolved through the roster's culture column exactly as settlement
    /// naming resolves it ([`civ_faction_culture`]) -- so an edited faction
    /// culture reaches the name. A landmass no faction holds is named in
    /// `civ_default_culture(1)`, the default this function has always used
    /// for it; see [`civ_continents_with_cultures`]. The reference has no separate
...
/// faction, and passing `None` names every landmass in faction 1's default
/// culture.
///
/// This entry point holds no roster, so it names in each faction's
/// **default** culture — [`civ_continents_with_cultures`] with an empty
/// culture column, which is what an unedited roster names as too.
///
...
pub fn civ_continents(lq: &LandmassQuality, gw: usize, gh: usize, min_cells: usize, territory: Option<&[i32]>) -> Vec<Continent> {
    civ_continents_with_cultures(lq, gw, gh, min_cells, territory, &[])
}

/// [`civ_continents`], naming each landmass in the roster culture of the
/// faction that holds the most of it.
///
/// Port-only naming — the reference has no continents — so the rule is this
/// port's own, stated here so it can be checked:
///
/// - **Which faction:** the one holding the most `territory` cells on the
///   landmass; a tie goes to the **lowest faction id**. Unclaimed (`0`)
///   cells never count.
/// - **Which culture:** `cultures[faction]` through [`civ_faction_culture`],
///   the same resolver settlement naming uses — so a faction the column does
///   not cover (including every faction, for `cultures = &[]`) takes its
///   [`civ_default_culture`], and an unedited roster names exactly as no
///   roster.
/// - **No faction holds any of it:** `civ_default_culture(1)`, the fixed
///   default this function used before it took a roster. Deliberately **not**
///   `cultures[1]`: editing faction 1 must not rename land it does not hold.
pub fn civ_continents_with_cultures(
    lq: &LandmassQuality,
    gw: usize,
    gh: usize,
    min_cells: usize,
    territory: Option<&[i32]>,
    cultures: &[&str],
) -> Vec<Continent> {
    if lq.count == 0 {
        return Vec::new();
    ...
    order
        ...
        .map(|(rank, c)| {
            ...
            // Unowned land keeps the fixed default; see this function's doc.
            let culture = if faction > 0 { civ_faction_culture(cultures, faction) } else { civ_default_culture(1) };
            Continent {
                id: rank as i32 + 1,
                ...
                name: crate::naming::decorate(
                    &crate::naming::civ_settle_name_bounded_in(&mut rng, culture, &mut seen_names),
...
/// inland-sea/lake shore is waterside but not a sea-lane port.
///
/// **x wraps only when `world` is set** (a whole-world map that wraps in
/// longitude), the same conditional wrap `civ_snap_coast` uses. The
/// reference wraps unconditionally (`nx=((gx+dx)+GW)%GW`, no `state.world`
/// guard), so on a flat map a settlement near the east edge read as coastal
/// because of ocean at the west edge, with no coastline between them. The
/// owner ruled that a defect to fix, not a quirk to keep (Ruling AR,
/// `LARGE_ITEM_RULINGS.md`, 2026-09-24), with the goldens it moves
/// re-baselined and disclosed beside the values. On a world map the answer
/// is unchanged.
///
...
///    not reach.
///
/// **The one real defect that measurement found was the wrap itself**: on
/// a non-wrapping map the unconditional wrap let a settlement five cells
/// from the east edge read as coastal because of ocean at the *west* edge,
/// with no continuous coastline between them. Ruling N did not authorise
/// that change; Ruling AR (2026-09-24) did, and the `world` guard above is
/// it. Points 1 and 3 still hold for the proxy itself.
#[allow(clippy::too_many_arguments)]
fn civ_is_coastal(
    x: usize,
    y: usize,
    ...
    for dy in -r..=r {
        ...
        for dx in -r..=r {
            ...
            let nx = x as isize + dx;
            let nx = if world {
                (nx % gw as isize + gw as isize) % gw as isize
            } else if nx < 0 || nx >= gw as isize {
                continue;
            } else {
                nx
...
pub fn place_settlements_with_counts(
    ...
    candidates
        ...
        .map(|(rank, c)| {
            ...
            .unwrap_or((c.x, c.y));
            let coastal = civ_is_coastal(fx, fy, coast_r, true, field, Some(wb), gw, gh, sea, world);
            SettlementPlacement {
                x: fx,
                y: fy,
                suit: c.suit,
...
pub fn place_settlements(
    ...
    candidates
        ...
        .map(|(rank, c)| {
            ...
            let coastal = civ_is_coastal(c.x, c.y, coast_r, true, field, Some(wb), gw, gh, sea, world);
            SettlementPlacement {
                x: c.x,
                y: c.y,
                suit: c.suit,
...
///
/// This is a faction's culture **until someone edits it**, not its culture.
/// The reference seeds `civFactionCulture` with it at module load
/// (`.map(_civDefaultCulture)`) and then lets the user reassign it; this
/// port does the same through `cartalith-godot`'s faction roster
/// (`FactionEntry::culture`, seeded from this function and written by the
/// roster and Culture profiles windows). So naming reads the roster through
/// [`civ_faction_culture`], and this function answers directly only for a
/// caller that holds no roster at all.
pub fn civ_default_culture(faction: i32) -> &'static Culture {
    let idx = (faction as usize) % CIV_CULTURES.len();
    &CIV_CULTURES[idx]
}

/// `_civCultureByKey(key)` (reference v2.10 line 14635): the culture with
/// this key, or `CIV_CULTURES[0]` ("common") for an unknown one -- the
/// reference's own fallback, kept rather than turned into an error because
/// the roster only ever stores keys `set_field` has already validated.
pub fn civ_culture_by_key(key: &str) -> &'static Culture {
    CIV_CULTURES.iter().find(|c| c.key == key).unwrap_or(&CIV_CULTURES[0])
}

/// `civFactionCulture[faction]`, resolved the way `_civSettleName` reads it
/// (reference v2.10 line 20718:
/// `_civCultureByKey((faction!=null&&civFactionCulture[faction])||'common')`).
///
/// `cultures` is the roster's culture column indexed by faction id
/// (`cartalith-godot`'s `FactionRoster::cultures`). A faction with no entry
/// -- every faction, for a caller that passes `&[]` because it holds no
/// roster -- falls back to [`civ_default_culture`], which is exactly what the
/// roster seeds that row with. So an unedited roster and no roster name
/// identically, and only an edited culture moves a name. (The reference's
/// missing-entry fallback is `'common'` instead; that branch is unreachable
/// with a roster, which is grown to cover every faction a settlement can
/// carry.)
pub fn civ_faction_culture(cultures: &[&str], faction: i32) -> &'static Culture {
    match usize::try_from(faction).ok().and_then(|f| cultures.get(f)) {
        Some(key) => civ_culture_by_key(key),
        None => civ_default_culture(faction),
    }
}
...
/// pool) plus one RNG-chosen suffix, first letter capitalised.
///
/// Takes the culture, not the faction: the reference resolves the faction's
/// *editable* culture first, so every caller says which culture it means --
/// [`civ_faction_culture`] over the roster for a settlement, or
/// [`civ_default_culture`] where no roster exists. Consumes
...
pub fn civ_settle_name(rng: &mut cartalith_rng::Mulberry32, cul: &Culture) -> String {
...
///
/// `cultures` is the roster's culture column ([`civ_faction_culture`]);
/// `&[]` names every faction in its default culture, which is what the
/// plain [`name_and_populate_settlements`] does.
pub fn name_and_populate_settlements_with_rng(
    ...
    placements
        .iter()
        .map(|p| {
            let name = civ_settle_name(rng, civ_faction_culture(cultures, p.faction));
...
/// `cultures` names each village in its faction's culture exactly as
/// [`name_and_populate_settlements_with_rng`] does (`&[]` = defaults).
#[allow(clippy::too_many_arguments)]
pub fn civ_seed_villages(
    ...
    for c in cands {
        ...
        let name = civ_settle_name(rng, civ_faction_culture(cultures, faction));
...
mod tests {
    ...
    // ---- Ruling Q re-baseline (2026-09-21): build_water_bodies moved from
    // a size-primary ocean/lake rule to a topology-primary one
    // (HYDROLOGY_CLASSIFICATION_RESEARCH.md). Renamed/re-derived tests
    // below; each states old value -> new value and why, per
    // cartalith-porting-discipline's golden-re-baseline disclosure rule.
    ...
    fn build_water_bodies_boundary_touching_component_is_ocean() {
        ...
        // agree here -- this test alone cannot tell them apart (see the
        // discriminating tests below for that).
    ...
    fn build_water_bodies_small_boundary_touching_component_is_now_ocean_not_lake() {
        ...
        // size-primary rule cell 4 was classification[4] == 2 (lake, because
        // it was the smaller component). Under the NEW topology-primary
        // rule BOTH components touch the grid's real boundary (x=0 and
        // x=gw-1 both count when `world` is false), so BOTH become ocean:
        // classification[4] moves 2 -> 1. This is Ruling Q's failure mode 2
        // fixed directly -- "a genuinely marine body ... that is not the
        // largest component" no longer becomes a lake.
        ...
        assert_eq!(wb.classification[2], 1);
        assert_eq!(
            wb.classification[4], 1,
            "moved 2 -> 1: a small but boundary-touching component is ocean, not lake"
        );
    }
    ...
    fn build_water_bodies_interior_component_is_lake_even_if_it_is_the_largest() {
        ...
        // `y` never collides with a real edge for row 1's cells (a 9x1
        // grid was tried first and rejected: with `gh=1`, `y == 0` and
        // `y == gh-1` are the SAME row, so every cell trivially "touches"
        // the y-boundary and the discriminating shape can't be built).
        // Row 1 (index = 9 + x):
        //   x=0:   0.1  -- lone below-sea cell touching the real x=0 edge
        //   x=1:   0.9  -- land, isolates it from the interior body
        //   x=2-6: 0.1 x5 -- a 5-cell interior below-sea component, one
        //                    full cell clear of every edge -- the
        //                    "huge inland basin"
        //   x=7:   0.9  -- land, isolates it from the far edge
        //   x=8:   0.1  -- lone below-sea cell touching the real x=8 edge
        // Under the OLD size-primary rule the 5-cell interior body was the
        // LARGEST component, so it alone won the ocean label and both
        // boundary slivers were classified lake -- exactly the bug the
        // research names. Under the NEW rule the two boundary-touching
        // slivers (size 1 each) are ocean and the 5-cell interior body,
        // despite being five times larger, is LAKE.
        let mut field = vec![0.9f32; 27]; // 9x3, all land
        for &x in &[0usize, 2, 3, 4, 5, 6, 8] {
            field[9 + x] = 0.1;
        }
        let wb = build_water_bodies(&field, 9, 3, 0.4, false, None);
        assert_eq!(wb.classification[9], 1, "boundary sliver at x=0 is ocean");
        for x in 2..=6 {
            assert_eq!(
                wb.classification[9 + x],
                2,
                "x={x}: the larger interior basin is a lake despite its size"
            );
        }
        assert_eq!(wb.classification[17], 1, "boundary sliver at x=8 is ocean");
        // Control: row 0 and row 2 must stay land, unaffected.
        assert_eq!(wb.classification[0], 0);
        assert_eq!(wb.classification[26], 0);
    }
    ...
    fn build_water_bodies_falls_back_to_largest_when_nothing_touches_boundary() {
        ...
        // largest-component rule rather than leaving the map with zero
        // ocean cells. 7x3, sea=0.4, `world=false`: one below-sea component
        // (row 1, x=2..5) sitting entirely inside the grid, one cell clear
        // of every edge (x=0/x=6/y=0/y=2).
        let mut field = vec![0.9f32; 21]; // 7x3, all land
        for x in 2..=5 {
            field[7 + x] = 0.1; // interior component, row 1, cols 2..5
        }
        let wb = build_water_bodies(&field, 7, 3, 0.4, false, None);
        for x in 2..=5 {
            assert_eq!(
                wb.classification[7 + x],
                1,
                "x={x}: sole below-sea component, no boundary contact anywhere -> \
                 fallback picks it as ocean so the map is not left without one"
            );
        }
        assert_eq!(wb.classification[0], 0);
    ...
    fn water_body_topology_multiple_disconnected_boundary_touching_components_are_both_ocean() {
        ...
        // classifies BOTH components ocean (see
        // `build_water_bodies_small_boundary_touching_component_is_now_ocean_not_lake`),
        // so `water_body_topology` -- which only re-derives facts from the
        // raster's own output -- now reports TWO physically disconnected
        // bodies that are both `kind == 1` / `ocean_connected`. This is the
        // research's own point: multiple marine basins can coexist with
        // (or here, stand in for) the world ocean, rather than being forced
        ...
        assert_eq!(
            bodies.len(),
            2,
            "two disconnected components, both classified ocean"
        );

        let small = &bodies[0];
        assert_eq!(small.kind, 1, "moved from kind 2 (lake) to kind 1 (ocean)");
        assert_eq!(small.cell_count, 1);
        assert!(small.ocean_connected);
        assert!(
            small.map_boundary_contact,
            "the lone cell sits at x=gw-1, which is exactly why it is now ocean"
        );
        assert_eq!(small.basin_type, WaterBodyBasin::Ocean);

        let large = &bodies[1];
        assert_eq!(large.kind, 1);
        assert_eq!(large.cell_count, 3);
        assert!(large.ocean_connected);
        assert_eq!(large.basin_type, WaterBodyBasin::Ocean);
    }
    ...
    /// The pre-roster naming, re-derived independently: every landmass named
    /// in `civ_default_culture(faction.max(1))`, one shared stream. The
    /// oracle the roster path must equal whenever the roster is unedited.
    fn continent_names_before_the_roster(factions: &[i32]) -> Vec<String> {
        let mut rng = civ_continent_name_rng();
        let mut seen = std::collections::BTreeSet::new();
        factions
            .iter()
            .map(|&f| {
                let stem = crate::naming::civ_settle_name_bounded(&mut rng, f.max(1), &mut seen);
                crate::naming::decorate(&stem, crate::naming::FeatureKind::Continent, &mut rng)
            })
            .collect()
    }

    /// Big landmass: faction 3 holds 12 cells, faction 2 holds 8.
    fn plurality_territory(gw: usize, gh: usize) -> Vec<i32> {
        let mut territory = vec![0i32; gw * gh];
        for y in 0..4 {
            for x in 0..5 {
                territory[y * gw + x] = if x < 2 { 2 } else { 3 };
    ...
    fn an_unedited_roster_names_continents_exactly_as_before() {
        ...
        let cul = default_roster_cultures();
        let with = civ_continents_with_cultures(&lq, gw, gh, 1, Some(&territory), &cul);
        assert_eq!(with.iter().map(|c| c.faction).collect::<Vec<_>>(), [3, 0, 0]);
        assert_eq!(names(&with), continent_names_before_the_roster(&[3, 0, 0]));
        assert_eq!(names(&with), names(&civ_continents(&lq, gw, gh, 1, Some(&territory))), "no roster == unedited roster");
        // Literal pin -- see the constant for where it came from.
        assert_eq!(names(&with), CONTINENT_NAMES_PLURALITY_FIXTURE);
    }

    /// Captured 2026-09-24 from `continent_names_before_the_roster(&[3, 0, 0])`
    /// (the pre-roster algorithm) on the fixture above. Faction 3's default
    /// is `desert` (`-march`); the unowned two are `imperial`.
    const CONTINENT_NAMES_PLURALITY_FIXTURE: [&str; 3] =
        ["Abedsahmarch", "The Junquinluccasum Reach", "Sevmarcvaldomcastra"];
    ...
    fn an_edited_dominant_faction_culture_renames_its_continent() {
        ...
        let lq = build_landmass_quality(&field, None, gw, gh, 0.5, false);
        let territory = plurality_territory(gw, gh);
        let before = civ_continents_with_cultures(&lq, gw, gh, 1, Some(&territory), &default_roster_cultures());
        let mut cul = default_roster_cultures();
        assert_eq!(cul[3], "desert", "the fixture edits away from faction 3's real default");
        cul[3] = "maritime";
        let after = civ_continents_with_cultures(&lq, gw, gh, 1, Some(&territory), &cul);
        assert_ne!(after[0].name, before[0].name, "faction 3 holds the big landmass");
        // And the name is drawn from the edited culture, not merely different.
        let mut rng = civ_continent_name_rng();
        let stem = crate::naming::civ_settle_name_bounded_in(&mut rng, civ_culture_by_key("maritime"), &mut Default::default());
        assert_eq!(after[0].name, crate::naming::decorate(&stem, crate::naming::FeatureKind::Continent, &mut rng));

        // Editing the minority holder (faction 2) moves nothing.
        let mut minority = default_roster_cultures();
        minority[2] = "maritime";
        assert_eq!(names(&civ_continents_with_cultures(&lq, gw, gh, 1, Some(&territory), &minority)), names(&before));
    }
    ...
    fn an_unowned_continent_keeps_the_default_culture_whatever_faction_1_says() {
        ...
        // Every landmass unowned; faction 1's culture is the tempting wrong
        // fallback (it was `faction.max(1)`), so edit exactly that.
        let mut cul = default_roster_cultures();
        assert_eq!(cul[1], "imperial");
        cul[1] = "highland";
        let after = civ_continents_with_cultures(&lq, gw, gh, 1, None, &cul);
        assert_eq!(after.iter().map(|c| c.faction).collect::<Vec<_>>(), [0, 0, 0]);
        assert_eq!(names(&after), names(&before));
        assert_eq!(names(&after), continent_names_before_the_roster(&[0, 0, 0]));
    }
    ...
    fn a_tied_landmass_is_named_by_the_lowest_faction_id() {
        ...
        for y in 0..4 {
            for x in 0..5 {
                territory[y * gw + x] = if y < 2 { 5 } else { 4 };
            }
        }
        let base = civ_continents_with_cultures(&lq, gw, gh, 1, Some(&territory), &default_roster_cultures());
        assert_eq!(base[0].faction, 4);
        let mut edit4 = default_roster_cultures();
        edit4[4] = if edit4[4] == "maritime" { "desert" } else { "maritime" };
        let mut edit5 = default_roster_cultures();
        edit5[5] = if edit5[5] == "maritime" { "desert" } else { "maritime" };
        assert_ne!(civ_continents_with_cultures(&lq, gw, gh, 1, Some(&territory), &edit4)[0].name, base[0].name);
        assert_eq!(civ_continents_with_cultures(&lq, gw, gh, 1, Some(&territory), &edit5)[0].name, base[0].name);
    }
    ...
    fn a_continent_is_not_named_after_the_first_settlement() {
        ...
        let first_settlement_name = civ_settle_name(&mut civ_name_rng(), civ_default_culture(1));
        assert_ne!(continents[0].name, first_settlement_name);
        assert_eq!(
            continents[0].name,
            civ_settle_name(&mut civ_continent_name_rng(), civ_default_culture(1)),
            "and it does come from the continent stream, so this is a different start rather than a different scheme"
        );
    }

    /// Sixteen placements, faction ids `0..8` twice over, so every default
    /// culture and the wrap past `CIV_CULTURES.len()` (faction 7) are named.
    fn naming_fixture() -> Vec<SettlementPlacement> {
        (0..16)
            .map(|i| SettlementPlacement {
                x: i,
                y: 0,
                suit: 0.5,
                faction: (i % 8) as i32,
    ...
    /// Audit 2026-09-24 Part 1 A2: a roster nobody edited must name exactly
    /// as no roster does, or every naming golden would move the day the
    ...
    fn an_unedited_roster_names_exactly_as_no_roster() {
        ...
        let rostered = name_and_populate_settlements_with_rng(&placements, &mut civ_name_rng(), &defaults);
        assert_eq!(bare, rostered);
        assert_eq!(bare, name_and_populate_settlements(&placements));
    }

    /// The defect itself: the reference names from the *editable*
    /// `civFactionCulture[faction]` (v2.10 20718), so reassigning faction 1
    /// from `imperial` to `desert` must move faction 1's names into the
    /// desert pool -- and nothing else, since a name draws `1 + n + 1`
    /// values whatever the culture, so every later name and population on
    ...
    fn an_edited_faction_culture_renames_that_faction_only() {
        ...
        assert_eq!(edited[1], "imperial", "the fixture's premise: faction 1 defaults to imperial");
        edited[1] = "desert";
        let renamed = name_and_populate_settlements_with_rng(&placements, &mut civ_name_rng(), &edited);
        // Literal suffix lists, not a read of `CIV_CULTURES`: the pools are
        // what this pins.
        let desert_sfx = ["abad", "sar", "ir", "oasis", "dune", "well", "rest", "march", "gate", "span"];
        let imperial_sfx = ["ium", "ora", "ara", "um", "opolis", "ica", "iana", "forum", "portus", "castra"];
        for (b, r) in bare.iter().zip(renamed.iter()) {
            assert_eq!(b.pop, r.pop, "population draws must not shift");
            if b.placement.faction == 1 {
                assert_ne!(b.name, r.name);
                assert!(imperial_sfx.iter().any(|s| b.name.ends_with(s)), "{} is not imperial", b.name);
                assert!(desert_sfx.iter().any(|s| r.name.ends_with(s)), "{} is not desert", r.name);
            } else {
                assert_eq!(b.name, r.name, "faction {} was not edited", b.placement.faction);
            }
        }
    }
    ...
    fn faction_culture_falls_back_like_the_reference_and_the_roster() {
        ...
        assert_eq!(civ_faction_culture(&["common"], 3).key, "desert");
        assert_eq!(civ_faction_culture(&[], 1).key, "imperial");
        assert_eq!(civ_faction_culture(&["common", "maritime"], 1).key, "maritime");
    }
