...
pub struct Landmark {
    ...
    /// the emission order is — and **that is all it is**: a position in
    /// [`LandmarkResult::landmarks`]. It moves when a cap moves, when a kind
    /// is disarmed, and when a filter changes the list, so nothing outside
    /// one run's own list may store it. Store [`key`](Self::key) instead.
    pub id: u64,
...
impl Landmark {
    /// **The stable identity.** `"<kind>@<x>,<y>"` — e.g. `"waterfall@120,64"`.
    ///
    /// Composed from three members this record already carries, so no field
    /// was added for it and there is exactly one home for the value. What it
    /// survives, and why:
    ///
    /// * **A regenerate of the same world at the same seed.** Every detector
    ///   reads the field and emits a candidate *per cell*; the walk is sorted
    ///   by score with the cell index as the total tie-break. Same field, same
    ///   settings, same cells — `two_runs_of_the_same_world_are_identical`
    ///   already asserts the whole list, so it holds a fortiori for the key.
    /// * **A cap change.** The kind and the cell do not move when the cap
    ///   does; only how many of them there are. That is the same property
    ///   [`landmark_seed`] is built for and
    ///   `a_landmark_seed_survives_a_cap_change` measures — and note that
    ///   that test **already identifies a landmark across two runs by
    ///   `(kind, x, y)`**, which is this key. This method names what the code
    ///   was doing anyway.
    /// * **A kind being disarmed.** A disarmed kind's rows leave the list;
    ///   the survivors keep their kind and their cell. (With
    ///   [`LandmarkSettings::cross_type_competition`] on, a disarm frees the
    ///   shared spacing field and *other* kinds may then place where they
    ///   could not before — that changes which landmarks exist, not the key
    ///   of one that still does.)
    ///
    /// It is unique within a run, which is asserted rather than argued:
    /// `the_stable_key_is_unique_within_a_run` measures it on a real world,
    /// including with every class radius at zero — the one setting that
    /// disables the spacing field that would otherwise enforce it.
    ///
    /// **Not [`seed`](Self::seed), which is not an identity.**
    /// [`landmark_seed`] takes no `kind`, so two same-class kinds placed at
    /// one cell hash to the same number. It is a stable *stream seed* for a
    /// naming or cultural pass, which is what its own doc claims and all it
    /// claims. Widening it would change generated output and needs an owner
    /// ruling; the key above needs neither, which is why it is the one built.
...
pub enum LandmarkLimit {
    ...
    NotBuildable,
    /// **The counters are not known**, and this says so instead of picking a
    /// reason that looks like one.
    ///
    /// [`generate`] never produces it — every path there ends on one of the
    /// six above, which is why adding this variant changes no generated
    /// output. It exists for a [`LandmarkResult`] rebuilt from a project
    /// archive (`cartalith-godot`'s `project_bridge.rs`,
    /// `entities/landmarks.json`) for a kind that archive carries no funnel
    /// for — a document written by a build whose [`kinds`] table differed.
    /// The placements for that kind are equally absent, so `candidates`,
    /// `placed` and every reject bucket are `0` and the funnel's own identity
    /// still closes; the one thing that would have to be invented is *why*,
    /// and this is that answer written out.
    ///
    /// The alternative was [`NoTerrain`](Self::NoTerrain), which reads as
    /// "every candidate failed" — a measurement nobody took.
    Unrecorded,
}
...
impl LandmarkLimit {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            LandmarkLimit::Unrecorded => "unrecorded",
        }
    }

    /// Every variant, the way [`LandmarkClass::all`] already does it —
    /// **seven**. It is a hand-written list, so it can fall behind the enum;
    /// `every_limit_token_round_trips` is the guard, and what it actually
    /// catches is worth stating exactly rather than generally. Adding an
    /// eighth variant **stops that test compiling** (its `match` over this
    /// array's item type is exhaustive), and adding it here as well then
    /// fails the test's literal length assertion until that is moved too. So
    /// a new variant cannot land without a reader standing in this function.
    /// What it does not catch on its own is a variant given a token and never
    /// added to this array: the arithmetic still agrees with itself. That is
    /// why the list is three lines above the assertion rather than elsewhere.
    pub fn all() -> [LandmarkLimit; 7] {
        [
            LandmarkLimit::AtCap,
            ...
            LandmarkLimit::Unrecorded,
        ]
    }

    /// The inverse of [`as_str`](Self::as_str) — the wire format read back,
    /// for `project_bridge.rs`'s `entities/landmarks.json`.
    ///
    /// `None` for a token this build does not know, which the archive reader
    /// turns into [`Unrecorded`](Self::Unrecorded) rather than a guess.
...
pub struct LandmarkResult {
    ...
    ///
    /// **Empty is "this result carries no reject rows", never "nothing was
    /// rejected"** — `funnels` answers the second question and is always
    /// populated. Two results are legitimately empty here: one from
    /// `all_skipped`, whose own comment says why, and one restored from a
    /// project archive, which does not carry the list at all (see
    /// `project_bridge.rs`'s `LandmarkRunDto`).
...
pub struct LandmarkStore {
    ...
    /// claims.
    ///
    /// **A second writer since owner ruling 10 (2026-09-06):** opening a
    /// project restores it from `entities/landmarks.json`
    /// (`cartalith-godot`'s `project_bridge.rs`, `LandmarksDoc`), so `Some`
    /// no longer implies that [`run`](LandmarkStore::run) was called in this
...
/// is wanted here is a stable *starting point* for a later stream, not a
/// stream. Deriving it from the cell index rather than from the placement
/// order is what makes it survive a cap change — move Waterfall from 40 to 41
/// and the forty that were already there keep their seeds.
///
/// **It is not the landmark's identity, and the line above used to call it
/// one.** The four arguments carry no `kind`, so two kinds of the same class
/// placed on one cell hash to the same number — reachable, because
/// `cross_type_competition == false` gives each kind its own exclusion field.
/// [`Landmark::key`] is the identity; this is the seed, and widening it would
/// move every existing landmark's value for no gain this port can yet spend.
...
/// [`Landmark::seed`] recomputed from what a project archive carries, for
/// `cartalith-godot`'s `project_bridge.rs`.
///
/// **`entities/landmarks.json` does not store the seed, and cannot.**
/// `SAVEFILE_COMPAT.md` §14.1 puts every integer in the format inside
/// ±(2^53 − 1), because a JavaScript reader would silently corrupt anything
/// larger, and its last paragraph closes the other door too: *"There are no
/// string-encoded integers anywhere in this format and none should be
/// added."* A 64-bit hash fits neither rule. So the archive carries the three
/// things this is a function of — the world seed in `project.json`, the grid
/// width, and the row's own `kind`/`x`/`y` — and the value is rebuilt here,
/// beside the definition it must agree with rather than in the reader.
///
/// `None` when `kind` is not in [`kinds`], which is also the condition under
/// which the archive reader drops the row: there is no class for it, so there
/// is no seed either.
///
/// `seed_for_matches_every_seed_a_real_run_produced` asserts the agreement
/// over every landmark of a real world — including the `feature == y * gw + x`
/// step, which is otherwise an assumption about eighteen detectors.
///
/// The one thing it does **not** survive is a [`GENERATOR_VERSION`] bump: this
/// recomputes at the version the reading build carries, not the writing one.
/// That is §27's versioned-generator rule doing exactly what it says — a
/// version bump is a declaration that landmarks may move — but it is a real
/// difference from a stored value and is stated rather than left to be found.
pub fn seed_for(world_seed: u64, gw: usize, kind: &str, x: usize, y: usize) -> Option<u64> {
    let spec = kind_spec(kind)?;
    Some(landmark_seed(world_seed, (y * gw + x) as u64, spec.class, GENERATOR_VERSION))
}
...
mod tests {
    ...
    /// [`LandmarkLimit::as_str`] and [`LandmarkLimit::from_key`] are one wire
    /// format read in both directions, and this is what keeps them one.
    ///
    /// The `match` below is exhaustive over the enum, so an eighth variant
    /// does not compile until it has a token here; the length assertion is a
    /// **literal**, so adding it to [`LandmarkLimit::all`] then fails until
    /// the count is moved with it. [`LandmarkLimit::all`]'s own doc says what
    ...
    fn every_limit_token_round_trips() {
        ...
        for l in LandmarkLimit::all() {
            ...
            assert_eq!(l.as_str(), token, "{l:?}");
            assert_eq!(LandmarkLimit::from_key(token), Some(l), "{l:?}");
            assert!(seen.insert(token), "two variants share the token {token}");
        }
        assert_eq!(LandmarkLimit::from_key("no_such_limit"), None);
    }

    /// `Unrecorded` is the archive reader's answer, never the generator's:
    ...
    fn generate_never_emits_unrecorded() {
        ...
        let pairs = real_resource_pairs(&r.resources);
        let inp = real_inputs(&r, &pairs);
        // A run that places things, and a run that places nothing at all --
        // `all_skipped`'s path, which is the one that fills every funnel from
        // `skipped_limit` and so is the likeliest place for a wrong default.
        let mut off = LandmarkSettings::default();
        for k in kinds() {
            off.set_armed(k.key, false);
        }
        for (label, s) in [("default", LandmarkSettings::default()), ("all disarmed", off)] {
            let out = generate(&inp, &s, 7);
            for f in &out.funnels {
                assert_ne!(f.limit, LandmarkLimit::Unrecorded, "{label}: {}", f.kind);
            }
        }
    }

    /// [`seed_for`] is what `project_bridge.rs` rebuilds a loaded landmark's
    /// seed with, so it has to agree with the generator on every row -- and
    /// the step worth measuring is `feature == y * gw + x`, which is an
    ...
    fn seed_for_matches_every_seed_a_real_run_produced() {
        ...
        for lm in &out.landmarks {
            assert_eq!(
                seed_for(WORLD_SEED, r.gw, &lm.kind, lm.x, lm.y),
                Some(lm.seed),
                "{} at ({}, {})",
                lm.kind,
                lm.x,
            ...
            kinds_seen.insert(lm.kind.as_str());
        }
        // Several kinds, not one detector's worth: the `y * gw + x` claim is
        // about all of them.
        assert!(kinds_seen.len() >= 8, "only {} kinds placed: {:?}", kinds_seen.len(), kinds_seen);
        assert_eq!(seed_for(WORLD_SEED, r.gw, "not_a_landmark_kind", 0, 0), None);
        // The world seed is really an input, so a project opened against the
        // wrong seed would not quietly agree.
        assert_ne!(
            seed_for(WORLD_SEED + 1, r.gw, &out.landmarks[0].kind, out.landmarks[0].x, out.landmarks[0].y),
            Some(out.landmarks[0].seed)
        );
    }

    // -- the stable key (`Landmark::key`) -----------------------------------

    /// The precondition [`Landmark::key`] rests on, measured rather than
    /// argued: no two rows of one run share `(kind, x, y)`.
    ///
    /// Two settings, because the spacing field is what normally enforces it
    /// and one of them switches that field off. `Buckets::fits` returns
    /// `true` unconditionally for `r <= 0` (`let r = if r.is_finite() && r >
    /// 0.0 { r } else { 0.0 }`, then every comparison is guarded on `r >
    /// 0.0`), and `set_class_radius` floors at zero rather than rejecting, so
    /// **all four radii at 0 km is a reachable user state** — the case where
    /// uniqueness can only come from the detectors emitting one candidate per
    ...
    fn the_stable_key_is_unique_within_a_run() {
        ...
        let spaced = LandmarkSettings { cross_type_competition: false, ..Default::default() };
        let mut unspaced = spaced.clone();
        unspaced.class_radius_km = [0.0; 4];

        let mut counts = Vec::new();
        for (label, s) in [("default radii", &spaced), ("all radii at 0 km", &unspaced)] {
            let out = generate(&inp, s, 7);
            assert!(!out.landmarks.is_empty(), "{label}: nothing placed, so nothing is proven");
            let mut seen: std::collections::BTreeSet<String> = std::collections::BTreeSet::new();
            for lm in &out.landmarks {
                assert!(seen.insert(lm.key()), "{label}: duplicate key {}", lm.key());
            }
            assert_eq!(seen.len(), out.landmarks.len(), "{label}");
            counts.push(out.landmarks.len());
        }
        // Removing the spacing field can only free placements — same pools,
        // same ranked walk, same caps, no spacing rejection — so the second
        // run is the strictly harder case and not a repeat of the first.
        assert!(
            counts[1] > counts[0],
            "zeroing the radii placed no more than the default did ({} vs {}), \
             so the second arm is not exercising the unspaced path",
            counts[1],
            counts[0]
        );
    }

    /// The three transitions [`Landmark::key`]'s doc claims it survives, in
    /// one test, against the one thing it must beat: [`Landmark::id`].
    ///
    /// Each arm asserts both halves — the key of a landmark that is still
    /// there is unchanged, **and** the run really did move something, so the
    ...
    fn the_stable_key_survives_a_cap_change_and_a_disarm_where_the_id_does_not() {
        ...
        let pairs = real_resource_pairs(&r.resources);
        let inp = real_inputs(&r, &pairs);
        let base = LandmarkSettings { cross_type_competition: false, ..Default::default() };
        let a = generate(&inp, &base, 7);

        let keyed = |res: &LandmarkResult| -> std::collections::BTreeMap<String, u64> {
            res.landmarks.iter().map(|l| (l.key(), l.id)).collect()
        };
        let ka = keyed(&a);
        assert!(ka.len() > 2, "too few landmarks to say anything: {}", ka.len());

        // (1) A regenerate of the same world at the same settings.
        let same = keyed(&generate(&inp, &base, 7));
        assert_eq!(same.keys().collect::<Vec<_>>(), ka.keys().collect::<Vec<_>>());

        // (2) A cap change on the most-placed kind, downwards. The kind is
        // read off the run rather than named, so this does not go vacuous
        // when a detector's yield changes.
        let busiest = a
            .funnels
            .iter()
            .max_by_key(|f| f.placed)
            .map(|f| f.kind.clone())
            .expect("every kind has a funnel");
        let placed = a.placed(&busiest);
        assert!(placed >= 2, "the busiest kind placed {placed}; nothing to cut");
        let mut cut = base.clone();
        cut.set_cap(&busiest, (placed / 2) as u32);
        let kc = keyed(&generate(&inp, &cut, 7));
        assert!(kc.len() < ka.len(), "the cap change moved nothing");
        for key in kc.keys() {
            assert!(ka.contains_key(key), "{key} appeared out of nowhere when only a cap fell");
        }
        // The id is what this key exists to replace: at least one survivor's
        // position in the list moved. (If this ever stops holding, the key is
        // still right and this assertion is the thing to re-examine.)
        assert!(
            kc.iter().any(|(k, id)| ka.get(k) != Some(id)),
            "no surviving landmark's id moved, so this test is not measuring what it says"
        );

        // (3) The same kind disarmed entirely.
        let mut off = base.clone();
        off.set_armed(&busiest, false);
        let kd = keyed(&generate(&inp, &off, 7));
        assert!(kd.len() < ka.len(), "the disarm removed nothing");
        for key in kd.keys() {
            assert!(!key.starts_with(&format!("{busiest}@")), "{key} survived its kind's disarm");
            assert!(ka.contains_key(key), "{key} appeared out of nowhere when a kind was disarmed");
        }
    }
