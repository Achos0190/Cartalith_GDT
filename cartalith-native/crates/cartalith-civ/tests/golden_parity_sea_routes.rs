...
//! `LARGE_ITEM_RULINGS.md`'s Ruling Q briefly moved case1's water-body
//! counts (116/14/62), route count (3, down from 4) and route points/`km`
//! values (`build_water_bodies`' topology-primary rule --
//! `golden_parity_waterbodies.rs`'s header has the full account). Ruling T
//! then special-cased case1's `world=true` map back to the original
//! size-primary rule, reverting all of it a second time to this file's own
//! pre-Ruling-Q content (`git show
//! c6de2a2:crates/cartalith-civ/tests/golden_parity_sea_routes.rs`) --
//! re-run and confirmed against `civ_sea_routes`' own current actual
//! output. case0 (`world=false`) is unaffected by either ruling.
//!
