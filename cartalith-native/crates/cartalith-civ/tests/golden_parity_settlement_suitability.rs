...
fn settlement_suitability_case_1_world_wrap() {
    ...
    // RE-BASELINED 2026-09-21 (`LARGE_ITEM_RULINGS.md`'s Ruling Q), THEN
    // RE-REVERTED the same day (Ruling T). Ruling Q briefly superseded the
    // Ruling N re-baseline this comment used to describe alone:
    ...
    // reached this arm through `carrying_cap`/`resources`/`landmass`/
    // `ctx.water_bodies`/`ctx.coast_reach` -- five real inputs, not one term
    // -- moving `expected_suit`/`expected_seeds` on top of Ruling N's
    // original river/coastal re-baseline. Ruling T then special-cased this
    // fixture's `world=true` map back to the original size-primary rule,
    // which is EXACTLY what `legacy_build_water_bodies` (below) already
    // reproduces -- so the production arm's `build_water_bodies` call and
    // the legacy arm's frozen one now compute bit-identical classification
    // for this fixture, and `expected_suit`/`expected_seeds` revert a
    // second time to Ruling N's own values (the ones this comment
    // originally described), matching this file's own pre-Ruling-Q content
    // (`git show
    // c6de2a2:crates/cartalith-civ/tests/golden_parity_settlement_suitability.rs`).
    // `reference_suit`/`reference_seeds` above are UNCHANGED throughout --
    // still the true JS capture, never moved by any of this. Re-run and
    // confirmed against `build_settlement_suitability`/
    // `find_settlement_seeds`'s own current actual production output.
...
// derivation needs.
//
// **Still needed after Ruling T (2026-09-21), unchanged.** Ruling T
// special-cased `cartalith_civ::build_water_bodies` for `world=true` back
// to this exact size-primary rule, so this file's `case_1_world_wrap`
// legacy and production arms now happen to compute bit-identical water-body
// classification -- but that is a THIRD variant existing (bounded
// topology-primary, wrapped size-primary, and this frozen pure size-primary
// one), not this copy becoming redundant. `case_0_region` is `world=false`,
// where the production function is still Ruling Q's topology-primary rule
// and this frozen copy is the only thing in this file proving the legacy
// arm still matches the reference there. And this copy's whole job is to
// be independent of whatever `cartalith_civ::build_water_bodies` does
// TODAY OR LATER (that is the entire reason it is frozen rather than
// calling through) -- a future change to the production rule, for either
// `world` value, must not silently move `reference_suit`/`reference_seeds`'
// own proof, which is exactly the coincidence-vs-dependency distinction
