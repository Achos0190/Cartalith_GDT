extends Node
## VERIFIER COPY. Constraint 2 of the batch-24 Lane A brief: *desktop must come out
## VERIFIER COPY. byte-identical except where it was already rendering outside its slot*.
## VERIFIER COPY. Reasoning from the node tree cannot show that (MISTAKES.md: "flip the flag
## VERIFIER COPY. and diff the framebuffer"), so this renders the right dock with the Journey
## VERIFIER COPY. section appended at a width where EVERY row already fitted before the
## VERIFIER COPY. reflow -- 620 px, comfortably past the 456 px the panel used to demand --
## VERIFIER COPY. crops the dock's own rect and writes it as a PNG for the caller to hash.
## VERIFIER COPY. The 620 px it asks for is not what it gets: at `--resolution 1600x1000` the
## VERIFIER COPY. row settles the dock at **597 px** (printed with every capture), which is
## VERIFIER COPY. still past 456 and is the width the identical hashes were taken at.
##
## VERIFIER COPY. Identical hashes before and after the change is the proof, and the rect
## VERIFIER COPY. printed at the shipped width is its positive control -- 456 x 867 at HEAD,
## VERIFIER COPY. 280 x 867 with the reflow, so a run that produced identical hashes is known
## VERIFIER COPY. to have been capable of showing a difference. Two runs of the same code gave
## VERIFIER COPY. identical hashes on all three seeds, so the comparison is not merely noise.
## VERIFIER COPY. The minimum-size figures are a separate probe (`_jpwidth_probe.gd`).
##
## VERIFIER COPY. Windowed, not headless -- `--headless` renders nothing to diff.
## VERIFIER COPY.   Godot_v4.7.1-stable_win64_console.exe --path . --resolution 1600x1000 _jpwshot_probe.tscn
...
func _ready() -> void:
	...
	for seed_v in SEEDS:
		...
		var out := "user://vfys_%d.png" % seed_v
		sub.save_png(out)
