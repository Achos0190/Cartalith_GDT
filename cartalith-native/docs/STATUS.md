...
Last updated: 2026-08-17 (post real Android device pass: builds/installs/launches confirmed on a real OnePlus 6T; golden-path exercise blocked by the device's own lock screen, not by this project's code).
...
| 4 | Android `.apk` builds + owner has installed/run | **Builds, installs, and launches on real hardware, confirmed 2026-08-17** — a genuine first for this project (a real OnePlus 6T, Android 14, connected this session). `cargo ndk` cross-build (debug + release) → `godot4 --headless --export-debug` → `adb install` → launch all succeeded; logcat confirms the GDExtension loaded and a real OpenGL ES 3.2 context was created against the device's actual Adreno 630 GPU, no crash/ANR. **Not reached**: driving the golden path (tap Generate, confirm render, on-device memory during generation) — the device has a real, enabled lock-screen credential (confirmed via `locksettings get-disabled=false`) that `adb` cannot dismiss without the owner physically unlocking it; this is a physical-access blocker, not a code or toolchain gap. See `ANDROID_BUILD_SCOPE.md` for the full record, including that `TOOLCHAIN.md`'s own "highest-risk item" framing turned out to already be a non-issue — every piece of the Android toolchain was already correctly installed and wired from earlier work. |
...
- Criterion 4's remaining half: the app is confirmed installed and launching on the owner's real OnePlus 6T (2026-08-17), but driving the actual golden path (tap Generate, confirm render, on-device memory during generation) needs the owner to physically unlock the phone first — `adb` cannot dismiss its real lock credential. See `ANDROID_BUILD_SCOPE.md`.
