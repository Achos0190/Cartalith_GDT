...
Last updated: 2026-08-17 (post real Android device pass: build/install/launch AND a real driven golden-path generation both confirmed on a real OnePlus 6T — peak ~277MB PSS at 512x512, no ANR/crash. MVP criterion 4 fully closed).
...
| 1 — Terrain MVP | **7/7, all done.** Criteria 1/2/3/5/6/7 done; criterion 4 (see its own row above) fully closed 2026-08-17 — real device build/install/launch plus a real driven golden-path generation, both confirmed. Two "easy to forget" Phase-1 closeout items from `ROADMAP.md` are still **not started**: a credits screen, and a licence audit of the crates pulled in (`PROVENANCE.md`). |
