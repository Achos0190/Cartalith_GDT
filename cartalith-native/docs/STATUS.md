...
| ID | Milestone | Status | Evidence |
|---|---|---|---|
| RD-0 | Foundation — culture and religion as quantitative traits, and the compatibility relation | done, **no consumer** | `cartalith-civ/src/belief.rs`, 945 lines — `culture_domain`, `ReligionDomain`, `CIV_RELIGION_DOMAIN`, `religion_domain`, `CompatBasis`, `Compat`, `compat`, `compat_value`, `NEUTRAL_COMPAT`, `COMPAT_WEIGHTS`. Its module doc says it is "the foundation both milestone 1 and milestone 3 need, and nothing above it". **`grep 'belief::'` across `crates/` excluding the file itself returns zero hits** — nothing calls it, and no Godot binding exposes it |
| RD-1 | 1 — MVP: network exposure and conversion, read-only | not started | `SettlementReligionState`, the type milestone 1 is defined by, exists only as a name in `belief.rs`'s module doc. No diffusion step, no adherent counts. `get_settlements()` emits no religion field |
