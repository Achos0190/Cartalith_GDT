...
| 7 | Opens a real HTML-app `.zip`, renders it, checked against the HTML app's own output | **Wired but not fully verified.** `cartalith-io::load_save` reads the 7 entries it needs (`params.json`, `heightmap.f32`, `temperature.f32`, `rainfall.f32`, `volcanic_field.f32`, `impact_field.f32`, `strahler_order.bin`) and round-trips in its own unit tests. **Not yet checked against a real export from the actual reference engine** — no browser available, but the reference's `exportZip()` (line ~12418) is pure-data JS reachable via the same Node `vm` harness already used for every other golden test; producing one real `.zip` this way and comparing is in progress. |
