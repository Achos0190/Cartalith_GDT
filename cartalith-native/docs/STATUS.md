...
Last updated: 2026-08-16 (post Phase 2 milestone 1: affordance fields).
...
| 2 — Civilisation layer | **Started 2026-08-16, milestone 1 of an unknown-but-large number done.** `PHASE2_SCOPE.md` traced `currentSettlementSuitability`'s (the "v1.30 one function" `ROADMAP.md` names) real dependency chain first and found it several milestones away — matching the reference's own v0.104 history, which drew the same boundary. Milestone 1 (new `cartalith-civ` crate, zero `gdext` dep): lithology classification (bit-exact golden-verified, categorical), soil fertility, water access — both continuous fields at `1e-4` tolerance. Both real fixture cases (reused from `golden_parity_carve.rs`) passed on the first attempt. A real gap fixed along the way: `WorldState` never retained the raw per-cell plate-crust array (`plateCrust()`'s Rust equivalent) past `generate_terrain`; added as `WorldState.crust_field` (pure addition). **Still not reachable**: resource potentials, carrying capacity, population density (reference's own next milestone, v0.105–0.106); settlement suitability/seed-finding itself (needs those plus route corridors, landmass quality, coast SDF, water-body classification, river network order — none built); factions, territory, provinces, culture, economy, roads, the Journey Planner (all untouched, block-2 proper). See `CHANGELOG.md`'s "Phase 2 milestone 1" entry. |
