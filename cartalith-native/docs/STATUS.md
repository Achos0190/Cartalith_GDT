...
| MM-6 | Per-settlement garrisons | not started — reopened by Ruling AW (2026-09-24), derived | `manpower.rs` produces per-faction headcounts only — no settlement-keyed output type exists. Disclosed on screen in CIVIL ▸ Military ▸ Not built |
| MM-7 | Campaigns, unit movement, combat resolution | not started — reopened by Ruling AW (2026-09-24) as territory over time and siege lines in a CARTO Conflict layer; unit movement and combat resolution not asked for | No combat or campaign type anywhere in `cartalith-civ`; still true after SP-4: `cartalith_civ::conflict` (2026-09-23) annotates a drawn conflict with authored free-text outcome and reads manpower, and resolves nothing (`STORY_PLANNING_SCOPE.md` §5). CIVIL ▸ Military ▸ Not built says so in Ruling AV's one line (`_fill_military`: "War campaigns over time were considered and declined; the conflict overlay shows each side's manpower.") — 2026-09-24, verified 2026-09-24 |
| MM-8 | Change over time (manpower across the year cursor) | not started — reopened by Ruling AW (2026-09-24) | `manpower.rs` takes no year argument and `TimelineSnapshot` carries no manpower field; the model reads the world as it stands, as §4 states |
