...
- **The New world dialog's default resolution (2048×1311, 2.68 M cells) costs ~875 MB peak on a real phone**, with no progress indication. Re-measured 2026-08-20: peak **899,089 KB (878 MB)** against 2026-08-18's 894,968 KB (874 MB) — **flat**, so everything landed between the two passes cost essentially nothing at the transient peak. Steady-state did grow, 500-538 MB → **662,793 KB (647 MB, +23%)**, which is the phone shell's second chrome tree plus resident new windows; not a leak (held across seven consecutive samples). Wall-clock improved to ~16-18 s from ~31 s. It completes and renders correctly and nothing kills it, but 878 MB is still a large fraction of a mid-range Android per-app budget. Worth revisiting before Android is treated as a supported target rather than a verified one. **Revisited 2026-08-25 and this is now a plan rather than a question** — `MEMORY_OPTIMIZATION_SCOPE.md`'s "The generation peak, measured field by field", a `#[global_allocator]` audit run **on the handset itself** (probes `cartalith-native/crates/cartalith-civ/examples/_peakaudit_*.rs`), with the seed fixed as §52 demands. **The pipeline peaks at 618.28 MiB at 2048×1311 — 241.5 bytes per cell, exactly linear above 1024, and moving only 0.6 % across three seeds**, so none of §52's 160 MB of seed spread is generation. Windows and the phone agree to 0.15 %, and the phone's own `VmHWM` (616.90 MiB) is within 0.23 % of it. 422 MB no-world PSS + 618 MiB = **1 040 MB against §50's measured 1 033 MB peak**, so the pipeline is ~60 % of the app's peak and is now accounted for to the megabyte. Three changes, in order: **(1) free the previous world before generating the next** — `generate_sized` calls `generate_terrain` while `self.source`/`self.civ` still hold the last one, measured at **+209.96 MiB for the `WorldState` alone** and ≈269 MiB with `CivData` and `absorb`'s clones, **0 ms, a reordering** (and the explanation for the 2026-08-16 "double peak" that pass put out of scope); **(2) delete four dead resident grids** — `flexure_field`, `heterogeneity_field`, `flow_area` and `ChannelResult::slope` each have exactly one reader inside `generate_terrain` and none anywhere else in the workspace, **40.96 MiB off peak *and* resident, 0 ms, a deletion**; **(3) block `build_resource_potentials`' `per_cell: Vec<[f32;15]>`** — 60 B/cell of scratch on top of its own output, **153.63 MiB for a measured +38–50 ms** on the handset (+0.15 % of a 30 s generate), parity-safe by construction. Eight ranked options together take 618.28 → **469.56 MiB (−24 %)**; past that **the plateau is the algorithm** — 478.82 MiB of individually-justified live fields at the `SuitabilityCtx` point — and **streaming any of it from disk buys nothing**, because every field is read by a whole-grid stage that touches every cell once. Also settled: **`RESOLUTION_PRESETS`' 4096 needs 2.41 GiB and 8192 needs 9.65 GiB**, so on Android 2048×1311 is the last preset that fits. **The first three landed the same day, and the audit's own numbers held: 618.81 → 518.92 MiB on Windows and 618.28 → 518.86 on the handset, −16.1 %**, with `WorldState` at 66.0 bytes a cell rather than 82.0 and the binding stage moved from `build_resource_potentials` to `civ_hierarchical_network_topology`. Desktop and phone now agree to 0.01 %. The 469.56 figure above is for **all eight** changes; R1–R3 alone were always going to stop at ~518.9, and did, to about 60 KiB of the prediction. Two corrections to the audit, both recorded in the scope document: **all four "dead" grids were in fact asserted against the JS reference by golden tests the audit's grep did not cover**, so `ChannelResult::slope` was released rather than deleted (keeping its three assertions for the same 10.24 MiB) and the other three cost six pipeline-test assertions, each judged individually against its own dedicated golden; and **R1 had a second call site**, `generate_world_structure_sized`, which every archetype generate goes through. Output proved byte-identical field by field, not merely test-passing (`_peakaudit_hash.rs`). R4–R8 remain on the table and would take it to ~469.
