...
- **Real bug found 2026-08-17, not fixed (out of scope for the task that found it)**: `main.gd`'s `_on_generate_pressed` synchronously calls `world_gen.generate()`/`generate_world_structure()` once on the main thread (blocking, wasted work — full generation runs and is thrown away), then `_generate_worker` (the actual background-thread call whose result is what's displayed) unconditionally calls plain `world_gen.generate(...)` — **never `generate_world_structure(...)`, regardless of which World Shape is selected**. Net effect: selecting Earth-like/Supercontinent/Archipelago/Volcanic/Rift in the UI currently has **no effect on the displayed map** — every generation actually runs the Classic (non-archetype) path. Needs its own fix-and-verify pass (make the worker thread call the correct one of the two based on `archetype`, and drop the wasted synchronous pre-call) — not attempted here since it was found while wiring an unrelated feature (sea level) and deserved dedicated verification, not a same-commit fix.
