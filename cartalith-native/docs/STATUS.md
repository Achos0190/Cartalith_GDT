...
| CV-23 · CV-24 · CV-25 · CV-26 · IN-13 · VA-01 | **open**, sharpened |
...
- **CV-23** — the influence field is `assign_territory`'s `best_effective`,
  computed today and thrown away. Contested is one more array, not one more
  Dijkstra. Blocked on memory (268 MB at 8192², so on-demand like
  `wildlife_regions`) and on `compute_civilisation` freeing the `cost` field
  that recompute needs.
...
- **CV-24, CV-25, CV-26, ED-02** — all want an owner decision, not wiring.
