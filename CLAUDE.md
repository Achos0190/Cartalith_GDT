...
| `ELEVATION_FIELD_ARCHITECTURE_RESEARCH.md` | owner-requested 2026-09-20, in conversation — real detail (not upsampling) on zoom for rivers and mountains: a raster elevation field stays the simulation substrate, but gains a queryable multi-resolution refinement primitive (extends `amplify_region`) plus a NEW boundary-condition-seeded local hydrology re-accumulation for genuine tributary detail (`compute_flow`/`build_channels`/`trace_river_polylines` are currently whole-world, one-shot). Deliberately not a vertex/TIN replacement of the simulation grid — every existing erosion/hydrology/climate algorithm is raster-native. Research and a proposed design (EF-0…EF-5), **not a scope**: four owner questions gate the build rows |
| **Directories** | |
