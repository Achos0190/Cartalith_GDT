...
- **Do not edit `reference/Cartalith Gen1 v2.10.html`.** It is the frozen
  ...
  **The freeze has drifted, and the old "no drift" note here was wrong.** This
  ...
  - **Resolved 2026-09-17 — the root v2.11 is a copy the source has long since
    ...
    **35** DCC-line files, newest **v2.57**. The port is measured against a
  ...
  - **The interval is specified, not unknown.** `RC_ENGINE_CHANGES.md` covers
    v2.11 → v2.57 change by change. Until 2026-09-17 nothing referenced it but
...
| `RC_ENGINE_CHANGES.md` | **the HTML's simulation/generation changes from v2.11 to v2.57**, as a porting spec — names the function and constant each change lives in, why each number is that number, and which harness verified it. Claims nothing about port status; that is `STATUS.md`'s. Note its first section: the HTML forked at v2.22 and every engine change from v2.25 on exists only on the DCC line. §6i is the newest block — the coastline was the level set of a blur of a piecewise-constant plate Voronoi map, and `PLATE_BASE_BLUR_K` is the single highest-leverage constant in the height formula; §6h holds the three independent floors (storage, the relief gate, the global ramp) that each flattened a deep-zoom plain. **Read §8's heading before appending to it** — it claimed "not simulation" while holding three deliberate height-field re-baselines, and was split into §8.1/§8.2 on 2026-09-17 |
