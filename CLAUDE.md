...
| `RC_ENGINE_CHANGES.md` | **the HTML's simulation/generation changes from v2.11 to v2.34**, as a porting spec — names the function and constant each change lives in, why each number is that number, and which harness verified it. Claims nothing about port status; that is `STATUS.md`'s. Note its first section: the HTML forked at v2.22 and every engine change from v2.25 on exists only on the DCC line |
