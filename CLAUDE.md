...
| `ELEVATION_FIELD_ARCHITECTURE_RESEARCH.md` | owner-requested 2026-09-20, in conversation — a hard, numbered acceptance bar ("always correctly detailed, no pixelated/square artifacts, no matter the zoom") for real detail on zoom, for rivers and mountains, without upsampling. Raster stays the simulation substrate; EF-0…EF-8 propose a queryable multi-resolution refinement primitive (extends `amplify_region`), boundary-seeded local hydrology re-accumulation for real river tributaries (new — `compute_flow`/`build_channels`/`trace_river_polylines` are currently whole-world, one-shot), a generalized vector layer (coastlines/faults/ridges, beyond the rivers this engine already vectorizes), importance-driven refinement (flagged, not designed), and a tile data structure grounded in the already-real, currently-unwired `ChunkId`/`DirtyTracker`. Deliberately not a vertex/TIN replacement of the simulation grid. Research and a proposed design, **not a scope**: six owner questions gate the build rows |
| **Directories** | |
