...
| States: `resolved` / `editing` / `stale` | §5.1 | — | `StageGraph::{is_stale, staleness, stale_stages}` | engine gap | Real for four stages, at **tile** granularity, and only after a committed tool pass marks height changed. *(Corrected 2026-09-24: a parameter change now marks too. Since `GUI_GAP_REGISTER.md` SG-03, `set_params`, `set_civ_param` and `reset_params` call `mark_param_change`, which marks without recomputing.)* DCC milestone 2 decided, deliberately and on the record, not to show a fake per-stage pip. |
...
| Stale propagation: editing *n* marks every downstream stage stale | §5.1 | — | `StageGraph::mark_changed_tiles` + lazy version comparison | engine gap | Real mechanism, wrong shape: four stages, tile-granular. It is triggered by a committed **tool pass**. *(Corrected 2026-09-24: a parameter edit also marks, through `mark_param_change`, since SG-03.)* |
...
| …*and re-runs erosion, hydrology and climate once* | §5.2 | — | `WorldGen::sculpt_commit` → `mark_and_recompute` → `staleness::recompute_stale` | engine gap | *Re-checked 2026-09-24; this row said "marks stale, does not re-run", which has been false since `8e666ac` (2026-08-24).* A commit marks the height tiles changed, then re-runs **hydrology and climate once** (`refresh_climate`). **Erosion is not re-run**, and the civ stage stays stale until `recompute_civilisation` / `recompute_stale_stages`. (The ~7.07 s figure once cited here is `CPU_MULTITHREADING_SCOPE.md`'s *full generation* at 2048², not a per-stroke commit.) |
...
| Layer list — Terrain (with children) | §7 | — | `get_layer_stack()` / `set_layer_stack()` / `list_blend_modes()` | backed, unwired | **Corrected 2026-09-03: the children ARE separable now.** `render::LayerStack` carries Terrain / Colour relief / Hillshade with visibility, opacity, blend mode and order, composited per-pixel inside `land_color` and reaching every raster consumer through `TerrainAppearance`. This cell read "the children below are not separable outputs today" until the stack landed. What is still owed is §7's row UI — visibility dot, opacity bar, blend picker, drag reorder. |
| Layer child — Hand-drawn hillshade (off) | §7 | — | `WorldGen::set_npr` | backed, unwired | **Corrected 2026-09-03.** This cell called NPR unported and quoted `FUNCTIONAL_CONTRACT.md` §6 saying "Absent"; `apply_npr` shipped with Phase 3's NPR block and is independently switchable. The quotation was accurate when written and the thing it described has changed. |
...
| Layer list footer tabs — Blocks / Verticality | §7 | — | none | **not built, deliberately** | **Ruled 2026-09-06 (CA-09, reading C: both are dead names).** Each names something the shell already does under a better name, so a footer tab would be a *third* route to controls that already have two. **Verticality** = vertical exaggeration = `exag`, a live slider in CARTO ▸ Map style ▸ § Map view (`render_workspace.gd`'s `APPEARANCE_VIEW`, backed by `render.rs`'s `TerrainAppearance::exag`), tooltipped as *"the reference's own Relief slider"*. **Blocks** = a 2.5D block diagram, which this 2D port has no view to switch to — and the word is already taken elsewhere in the shell, for a town layout's block count (`city_viewer_window.gd`). Read instead as tiles or as a style bundle, both are built and named: Preferences ▸ Tiles & LOD (`dcc_settings.gd` §2.5) and `STYLE_PRESETS` in Map style. The reasoning is kept in `cartography_workspace.gd`, beside the Layers category where the tabs would have gone. CA-09's **search field shipped** the same day, in `layers_popover.gd`. |
...
| Rule: no control here marks a generation stage stale | §7 | — | `StageGraph` is marked by committed tool passes and by generation-parameter writes (`set_params` → `mark_param_change`, which `params::invalidates` routes) | wired | Holds as long as no style key routes to a generation stage in `params::invalidates`. Re-checked 2026-09-24 only as far as the mechanism; the key-by-key routing was not audited. |
