...
| ~~Previews re-upload the whole texture~~ — **paint half CLOSED 2026-09-21 (was already fully wired, this row was stale); sculpt half still declined** | `cartalith-spatial/src/pass.rs:193,199` (`touched_tiles`/`touched_bounds`, unmoved); consumers `lib.rs:9933` (`build_paint_preview_patch`, drifted from `:8936`) and the still-declined `build_sculpt_preview_texture` | **The sculpt half is unchanged and still honestly declined** — `build_sculpt_preview_texture`'s doc still argues a bounded preview needs `render.rs`'s AO/wetness/sea passes reworked over a caller-supplied window; code `golden_parity_render.rs` pins bit-for-bit; tracked as SL-1. **Paint half: re-verified at the symbol 2026-09-21, and the shell caller this row said was missing has existed since commit `45df3019` the same day the patch API landed (2026-09-04) — this row was simply never updated.** `engine_bridge.gd:3071` wraps `build_paint_preview_patch`; `viewport_host.gd:2295` adds `set_preview_patch(patch: Dictionary) -> bool` (a sibling to `set_preview_texture`, not a signature change — `Image.blit_rect` + `ImageTexture.update()`, chosen so erase can replace pixels rather than blend); `world_workspace.gd:3089`'s `_paint_show_preview()` calls `set_preview_patch(bridge.build_paint_preview_patch())` and only falls back to a full re-upload when that returns `false` (no patchable base yet, format mismatch, mid-stroke resize, or an out-of-bounds window — all explicit, not silent). Re-measured 2026-09-21, still current: 2048² (shipped default) **14.889 ms → 1.907 ms** median per dab over 20 dabs. Verified via three pre-existing probes, independently re-run: `_paintpatch_probe.gd` (headless, PASS 0 failures — confirmed a second time by the coordinating session itself), `_paintwire_probe.gd` (windowed, PASS 0 failures, 20-dab drag byte-identical to a full re-upload after every dab), `_paintwire_bench.gd` (perf, re-measured). | medium |
...
Saved measurements (2026-09-03) and Region ▸ New world from selection
(2026-09-20, commit `76cbf8b`), sixteen of the eighteen are built. **Header
count corrected 2026-09-21** — the "Open" table below already showed Region ▸
New world from selection as `CLOSED`; only the header had not caught up.

### Open

| Item | Where (`file:line`) | Ruling | State |
|---|---|---|---|
| ~~`Cut` · `Copy` · `Paste` · `Select all`~~ — **CLOSED — was already built the same day this row was last written** | `menus.gd:1558` (`_cut_selection`, drifted from the `:771` `_todo`), `:1565` (`_copy_selection`), `:1579` (`_paste_clipboard`), `:1663` (`_select_all`), dispatched from `_on_edit()` `:1343-1346` | Ruled: selection sets → clipboard → commands, **in that order**. | **The clipboard this row said did not exist shipped `686cd2a`, 2026-09-03, "The clipboard lands, and a fourth row that was already built" — hours after this row's own prose was written, and never revisited since.** Re-verified at the symbol 2026-09-21: a GDScript-side, session-lived `Dictionary` on `DccMenus` (`:1414`) holding the engine's own `icon_get()`/`label_get()` records verbatim (an omit-the-key rule, not an empty-Array one), so a paste after a world change re-creates entities from scratch rather than referencing stale indices. Paste offset is `PASTE_OFFSET_CELLS := 4.0`, clamped into the grid. Cut is Copy + delete-descending (highest index first, so `Vec::remove` doesn't shift the rest). **The "scoped to the active layer" stretch goal is also built**: `_select_all()` matches on `_active_domain()` and `_refresh_clipboard_rows()` reason-dashes all four rows per domain. Icons and labels only, by design — sculpt stamps (`sculpt_list_stamps` reports a count, not points, so a stamp can't be read back) and settlements (no selection set) are named out of scope in the code itself. **The one real gap was in the probe, not the feature**: `_clipboard_probe.gd`'s icon leg never ran ("no asset pack ships" misread as "no pack reachable" — false, `crates/cartalith-assets/tests/fixtures/reference_pack.zip` is reachable and is exactly what the asset-pack milestone's own tests load from). Extended 2026-09-21 with 25 new assertions covering icon copy/cut/paste; `_clipboard_probe: PASS`, independently re-run. | large |
...
| ~~`Region ▸ New world from selection`~~ — **CLOSED 2026-09-20** | engine `ops_bridge.rs:317` (`region_new_world`); wrapper `engine_bridge.gd:3281` (drifted from `:2989`) | Built and verified | File menu ID 18 calls `EngineBridge.region_new_world()` with a mandatory confirm naming what a resample destroys (civ layer, labels, icons, ways, routes, painted layers, sculpt draft, journeys, measurements, landmark store, vault snapshots and links, undo history). Gated on the engine binding existing, a world existing, and a real marquee. Cancel is provably inert. 55/55 probe checks. Commit `76cbf8b`. | large |
...
## The dangerous class — 0 open, 9 closed (2026-09-21)

**All nine are fixed in the tree.** A 2026-09-13 triage session
(`scratchpad/unwiredtriage_report.md`) first found this; this pass re-opened
each at its own cited symbol rather than trusting that report, and confirms
it for eight of the nine directly (b-1 through b-7 and b-9 — each string
below now reads the corrected text, several with their own "corrected
2026-09-0x" self-annotation already in place) and for the ninth (b-8) by the
false string's total absence from the tree — `performance_window.gd` itself
no longer exists (its content moved during later GUI work), and `menus.gd`'s
own already-correct build-conditional wording (`:3038`) is now the only
surviving version of the claim. **This table is history, not a live list —
kept so the defect class and its method are not re-discovered.**
