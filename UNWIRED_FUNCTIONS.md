...
| Previews re-upload the whole texture — **paint half closed 2026-09-04, sculpt half still declined** | `cartalith-spatial/src/pass.rs:193,199` (`touched_tiles`/`touched_bounds`, **unmoved** — the bare `pass.rs` of earlier cuts is ambiguous, this workspace has one `pass.rs` and it is in `cartalith-spatial`, not `cartalith-godot`); consumers `lib.rs:8033` (`build_sculpt_preview_texture`; `:6128-6179` two cuts ago, `:8027` at the first 2026-09-03 pass) and `:8936` (`build_paint_preview_texture`; `:7000-7023`, then `:8926`) | **The sculpt half is unchanged and still honestly declined; the paint half was built 2026-09-04.** `build_sculpt_preview_texture`'s doc still argues a bounded preview needs `render.rs`'s AO/wetness/sea passes reworked over a caller-supplied window — code `golden_parity_render.rs` pins bit-for-bit — and that restricting only the final pixel loop would be "a cosmetic optimisation reported as a real one". That stands, and is tracked as SL-1. **Paint:** the doc's "paint does not need this at all" argument was killed by measurement on 2026-09-04 (0.667/1.540/4.551/16.982 ms per dab at 512/1024/2048/4096²; 1.05 MB to 67.1 MB across the FFI each time) and `WorldGen::build_paint_preview_patch` now returns `{texture, x, y, w, h}` over `PassBuffer::preview_touched_into` — **1.3x/3.0x/7.8x/19.0x less CPU and 15.5x/33.1x/55.5x/75.9x fewer bytes**, byte-identical to a full re-upload inside the window. **No shell caller yet:** `world_workspace.gd::_paint_apply_dab` still calls `build_paint_preview_texture` and `viewport.set_preview_texture()` takes no offset, so the win is available and not yet taken. | medium |
