...
> this port.
>
> The passes are numbered 1-3 below; the retired
> `cartalith-native/docs/CHANGELOG.md` recorded them as "CPU multithreading
> milestone 1/2/3", and "milestone *N*" in older citations means pass *N*.
> "GPU milestone *N*" always means `GPU_LAYER_INTEGRATION_SCOPE.md`'s.
...
GPU work, and — given GPU milestone 6's finding, on the day this was
written, that the GPU path *lost* to CPU below 2048² to per-call
context-creation overhead — was likely to deliver more real, immediate
speedup than the GPU path could at the time.
...
- `cartalith-climate`: temperature/rainfall formulas — verify
  `simulate_weather`'s wind-iteration loop specifically (GPU milestone 7's
  investigation found it "gather-shaped" per-iteration, meaning each
...
would, and needs the same gather reformulation), orogeny's graph-tracing.
*The gather reformulations for `compute_stress` and `erode_thermal` have
since been written for the GPU (`GPU_LAYER_INTEGRATION_SCOPE.md` D1 and E);
they are the shape a Rayon version of either would take. Those GPU passes
left both CPU loops as they were.*
...
stays fully sequential and sets the real ceiling. Full account: the
retired `CHANGELOG.md`'s "CPU multithreading milestone 1" entry.

**Natural follow-up passes** (not scoped here, same "one subsystem at a
time" discipline):

1. `cartalith-climate`/`cartalith-erosion`/`cartalith-hydrology` — each
   needs its own independence read before touching. This became pass 3.
2. GPU milestone 6's flagged next step (`GpuContext` reuse across stages,
   which became GPU milestone 8) and the integrated-GPU idea below are
   GPU-side follow-ups, not CPU-multithreading scope.

## Pass 2 (2026-08-17) — `cartalith-civ`

The concurrent forks that blocked this crate during pass 1 (sea routes,
memory investigation) both landed (`71da1d5`, `62b9b51`), so this was
unblocked and scoped as its own pass rather than left further deferred.

Read every named candidate function's body in full before touching it
(same discipline as pass 1), confirming each is genuinely
...
**Left sequential, and why** (the same "genuine cross-cell state, not
just 'hasn't been tried'" bar pass 1 and `GPU_LAYER_INTEGRATION_SCOPE.md`
both used): `chamfer_dist` (two-pass raster scan -- each cell reads its
immediate predecessor in the SAME pass, a genuine wavefront/scan
dependency, not independent per-cell); `jfa_dist` and therefore
`build_coast_sdf` (iterative Jump Flooding, the algorithm this document's
"not safe" list flags for plate assignment); `build_water_bodies`
(priority-flood); `label_land_components` and `build_landmass_quality`'s own flood-fill (connected
...
*The example was first named `timing_bench.rs`, and renamed on 2026-08-17
during GPU milestone 7: it collided with `cartalith-engine`'s
`examples/timing_bench.rs` at one output path
(`target/debug/examples/timing_bench.exe`), which broke `cargo test
--workspace` and `cargo build --workspace --examples`. `-p` disambiguates
`cargo run --example`, but not a workspace-wide build writing both to the
same filename. This crate's example was renamed rather than the engine's,
which was one commit older.*

Real, honest, and better-scaling than pass 1's terrain result
...
compress the ceiling), plus 128x128 shows the same small-size floor GPU milestone 6 found, for
a different reason (fixed per-`par_iter_mut` dispatch overhead not yet
amortized by so little work per cell at that size).

Combined with pass 1's `cartalith-terrain` result, a full
...
**Verification**: full account above; the retired `CHANGELOG.md`'s "CPU
multithreading milestone 2" entry has the same numbers.

## Pass 3 (2026-08-17) — `cartalith-climate`/`cartalith-erosion`/`cartalith-hydrology`

Covers pass 1's "Natural follow-up passes" note. Read every candidate
function's body in full in all three crates before touching it (same
discipline as passes 1-2), checking
...
`GPU_LAYER_INTEGRATION_SCOPE.md` milestone 7's "gather-shaped" finding
applies equally to the CPU path). **The rule that decided every reduction
here: a max is safe, a sum gating a branch is not.** Every max-style
reduction (`build_wind`'s `mx`, `apply_climate_moisture_correctors`'s
`f_max`) was parallelised via `reduce(f64::max)` — order-independent and
bit-exact — and nothing in this crate has a running *sum* gating a branch
decision, the case that keeps erosion's `stream_power_kernel::ss`
sequential (below).
...
to parallelize safely, not attempted here; the GPU version later built
exactly that gather, `GPU_LAYER_INTEGRATION_SCOPE.md` E); `stream_power_kernel`'s
...
With passes 1-2's terrain and civ speedups, this is the third and final
subsystem this document covers. What it named as the remaining ceiling —
`cartalith-godot`'s own sequential orchestration and the hard-hazard
functions in every crate (flow accumulation, priority-flood,
scatter-writes, per-droplet/per-iteration wavefronts) — is the "Out of
scope" list from pass 1.

**The orchestration half of that ceiling, measured (2026-09-23).** A
scoping pass timed `WorldGen::generate_sized` → `generate_terrain` →
`absorb()` with real `Instant` timers (16 threads, seed 12345, 512×512
through 2048×1024, debug and release): `absorb()`'s glue outside
`generate_terrain`/`compute_civilisation` is **under 30 ms at every size**
against 0.7-4.2 s wall totals, and no reordering is available —
`compute_civilisation` depends on `generate_terrain`'s output,
`coarse_ocean_wind_fields` feeds `compute_civilisation`, and
`recompute_stale`'s merged `refresh_climate()` is deliberately ordered after
hydrology. So the real remaining sequential cost is the hard-hazard
functions alone. Full record: `OUTSTANDING_WORK.md` §2.6's closed
"`cartalith-godot`'s own sequential orchestration" row.

## Separate idea, recorded here and built elsewhere: using the integrated GPU too

Also raised by the owner on 2026-08-16: this machine has an integrated GPU
alongside the dedicated one, and `cartalith-gpu` then requested exactly one
`PowerPreference::HighPerformance` adapter (correctly the dedicated GPU,
per the pilot's results), never enumerating the integrated one.

The idea — running smaller or latency-tolerant work on the integrated GPU
while the dedicated GPU carries the main pipeline — is valid, but it is a
multi-adapter architecture question, and **this document deliberately does
not scope it**. It was recorded in `HARDWARE_ACCELERATION.md` for later, on
the reasoning that CPU multithreading was the higher-value, lower-risk win
to take first, given GPU milestone 6's finding that the single
dedicated-GPU path then lost to CPU below 2048².

**It shipped under another document four days later**:
`cartalith-gpu/src/multi.rs` (`0de790a`, 2026-08-20) — adapter enumeration
(`enumerate_devices`), a split-tiles mode running more than one physical
device at once (`GpuDeviceSet`), weighted by a measured per-device-type
throughput share that names `wgpu::DeviceType::IntegratedGpu`, and a VRAM
cap. Its tests open this machine's real integrated Radeon, not a mock
(`multi_gpu.rs::the_integrated_gpu_at_8192_falls_back_instead_of_panicking`).
The design and its measurements are `HARDWARE_ACCELERATION.md`'s
"2026-08-20 — Multi-GPU" section; `GPU_LAYER_INTEGRATION_SCOPE.md` indexes
it as milestone M.
...
itself; nothing was reverted from passes 1-3; no code changes were kept
...
`Cargo.toml` (six crates, matching every crate passes 1-3 named).
...
`git log` on each crate's `src/lib.rs` since the pass-3 commit
...
passes 1-3 correctly left sequential: flood-fill/priority-flood
...
proportionally more at small sizes -- 1.11s with `use_gpu=true` vs 0.90s
with it off, at 512x512. (This entry originally cited GPU milestone 6's
"GPU loses below 2048²"; by 2026-08-19 milestones 8-9 had moved the
end-to-end crossover to 1024², and a GPU loss at 512² is what their 0.83×
predicts.)
...
GPU/CPU stage branching itself is a legitimate design (the GPU path wins at
large grids — from 1024² end to end after GPU milestone 9 — and CPU+Rayon is
correct below that and for the GPU-ineligible stages) and reworking *when* GPU vs. CPU runs, or
...
**Resolved 2026-08-25 (the `/ponytail` pass)**: the incidental finding above
...
document's list, on exactly the `output[i] = f(input, i)` bar passes 1-3
