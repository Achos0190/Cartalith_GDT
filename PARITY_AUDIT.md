...
| 14 | **Export controls in the header bar**, itemised | `bakeRes` (2K/4K/8K), `bakeTiles`, `chanAtlasChk`, `layersPreviewChk` | **Re-scoped again 2026-08-24, and the earlier scoping was wrong.** These four do *not* belong to the tile-pyramid bake. The reference has **two separate systems both called "bake"**, and the 2026-08-23 note conflated them: (a) `bakeAllTiles`/`atlas*` is the LOD tile pyramid, now built (the row above); (b) `bakeDims`/`bakePixel`/`bakeSingle`/`bakeTiled` is the **export raster**, `exportZip`'s `map.png` at 2K/4K/8K. `bakeRes` and `bakeTiles` are (b) and are still absent — `bakePixel` is the *full material path at a fractional sample position*, and this port's `render::cell_color` takes integer cell indices only, so it is a rendering milestone rather than an export one. `chanAtlasChk` is a third thing again and much cheaper: `_chanEnc`/`packRGB8`/`channelAtlasGroups` pack up to three affordance fields into one RGB8 PNG, and every input already exists (`cartalith_civ::build_soil_fertility`/`build_water_access`/`build_carrying_capacity`/`build_settlement_suitability`/`build_resource_potentials`). `layersPreviewChk` is the f32 layer previews, likewise unbuilt. **None of the four was built in the 2026-08-24 pass**, which scoped and verified (a) instead |
