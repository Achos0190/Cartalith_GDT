...
| Velocity erosion (Mei virtual pipes) + coastal + glacial + hillslope **as user-run passes** | ~16 | kernels partly absent, no run-button path | WW-02, itemised per-parameter in `GENERATION_PARAMETERS.md` |
| Evolve coupled + sediment routing/deposition + tidal flats | 5 | absent | MS-04 / MS-05, stage 06's `gap` string |
| Geoid (`buildGeoid`/`refreshGeoid`/`geoAt`/`currentGeoidPreview`) | 4 | absent; `cartalith-climate/src/lib.rs:132-133` says so in-code | WW-07 |
| Tides (`tidalForcing`/`computeTideField`/`buildTideField`/`refreshTides`/`currentTideField`) | 5 | absent | WW-07 |
| Seasons + Köppen (`computeSeasons`/`classifyKoppen`/`buildKoppen`/`koppenColor`…) | 7 | absent; `sample_bridge.rs:496` names it | WW-09 |
| Wildlife + ecoregions (`buildTRI`/`guildTrophic`/`buildEcoregions`/`assignWildlife`/`regionRichness`/`wildRegionColor`/`currentWildlife`) | ~9 (`buildNPP` **is** ported) | absent | `sample_bridge.rs:613` gap list only — **not** in `GUI_GAP_REGISTER.md`; see §5 item 8 |
...
| NPR "Painter" styles, plus rendering-advanced toggles, waves, animated water, multi-sun as *controls* | ~15 render paths | absent. `render.rs:18` says the Painter block is not ported | `render_workspace.gd:55-68` — a real in-product prose disclosure naming every one of them; register RN-01 |
...
| 8 | **Wildlife roster click popup** | `showWildInfo`, `wildFmtPop` | Follows from ecoregions being unported — disclosed as a *debug-layer* gap in `sample_bridge.rs`, never as a missing interaction |
