...
| Journey planner reads claimed land as claimed | Improved | A stage's claimed fraction counts only cells a faction owns, so tolls fall at real political frontiers and the claimed-land infrastructure floor applies only on claimed land. The legacy `_jpClaimedAt` tests `>= 0` on a `Uint8Array` whose unowned value is `0`, so every cell (open sea included) reads as claimed and the toll count is always 0. Added 2026-09-26 | `cartalith-civ/src/lib.rs::jp_claimed_at` (`> 0`) | Shipped; pending independent verification |
| Physical crater model | New | Optional crater count and size by area density, plus wear over a stated surface age in millions of years | `params.rs` rows `crater.physical_model`, `crater.surface_age_myr`; `cartalith_terrain::crater_degradation_tau` | Shipped; **on in the app** (`cartalith_godot::params::defaults` sets `crater.physical_model = true`, since `cff1edc`, 2026-09-02), off only in the golden baseline `WorldParams::defaults` (§7l). This row said "off by default" until 2026-09-24 |
| Depression-filled flow routing | Improved | Flow is routed over a Priority-Flood+ε routing surface, so interior pits drain to the sea instead of stopping rivers. The legacy v2.11 `computeFlow` sorts by raw height with no fill (the source added its own fill in v2.41, after both frozen snapshots). A standard-algorithm build, not a JS port (`DECISIONS.md` §7o). Added to this list 2026-09-24 | `cartalith-hydrology/src/lib.rs::build_routing_surface`; `integrate_drainage` row in `params.rs` | Shipped; on in the app (`params::defaults`), off in the golden baseline (`WorldParams::defaults`) |
| Erosion passes as generation parameters | Improved | Velocity erosion, glacial carving, coastal processes and hillslope diffusion are one-off manual buttons in the legacy file. Here they are saved, toggleable parameters that run inside generation | `cartalith-godot/src/params.rs` (`passes.*`) | Shipped; the passes default to off in `WorldParams::defaults`. The shipped app turns glacial carving on for new worlds (`params::defaults()`, owner Ruling AU, 2026-09-24) |
...
| Colour grade | New | Exposure, contrast, saturation, temperature, split toning and gamma, weighted by biome, elevation, moisture and geology | `render.rs::apply_color_grade`, `build_grade_influence` | Shipped and reachable: the `grade_*` keys are appearance keys in `render.rs`'s appearance table, and `render_workspace.gd` draws them as its "Colour grade" and "Grade field influence" rows (since `23d2779`, 2026-08-24). This row said "Engine only: no bridge" until 2026-09-24 |
...
| Interactive deep-zoom tiles | Improved | Camera-driven tile synthesis with added fBm/ridged detail when zooming in. Tiles are coloured by `render::render_biome_tile_rgba`, the port of the legacy per-tile renderer `renderBiomeTileRGBA` (LOD-D1, `f6d1bd5`, 2026-09-21), called from `lod_bridge.rs`'s tile synthesis. On 2026-09-13 this row said that renderer was unported; how far the rest of `LOD_DETAIL_SCOPE.md` has got is `STATUS.md`'s answer | `lod_bridge.rs::lod_synthesize_tile`; `cartalith_terrain::amplify::amplify_region`; `render.rs::render_biome_tile_rgba` | Shipped |
