...
| Item | Owns it | Size | Next step |
|---|---|---|---|
| Project archive remainder — project-layer panels, the `library/` and `drafts/` slots, foreign-entry preservation | `STATUS.md`, `SAVEFILE_COMPAT.md` §17 | medium | Nothing draws any of it; foreign entries are reported rather than preserved. **The `preview.png` producer half of this row is CLOSED 2026-09-21 (verified)** — `DccApp._capture_preview_png()` captures and PNG-encodes the map viewport at save time (256px on the longer edge), `project_save_with_documents` gained a caller-optional `PackedByteArray` parameter (backward compatible — every existing 2-arg call site keeps working unchanged) that writes `write.preview_png` when non-empty. Verified independently: full diff read, `cargo test --workspace --no-fail-fast` reproduced exactly (163/3488/0/34), both touched `.gd` files parse clean, the windowed capture probe's own review PNG viewed directly (a real rendered map frame, not blank). Commit `cf1eeec`. The rest of this row (panels, `library/`/`drafts/` slots, foreign-entry preservation) stays open. |
