...
| **Ruling 13 (landmarks as a vault `EntityKind`) — UNBLOCKED 2026-09-06, one change remains outside the two granted files** | `MARKDOWN_VAULT_SCOPE.md` | small | **Ruling 10 built, and it removed the blocker.** Landmarks now have a stable identity: `Landmark::key()` = `"<kind>@<x>,<y>"`, composed from three fields the record already carried — **no field added**. Not the `id` (a position: `lm.id = k as u64 + 1`) and not the `seed` (`landmark_seed` takes no `kind`, so same-class kinds at one cell collide). **Stability was measured, not argued**, and the verifier re-derived it on a different world at the shipped default `cross_type_competition: true` — which the lane's own tests never exercised — getting 240/240 distinct keys at default radii and 314/314 with all four class radii at 0 km, the setting that switches off the spacing field that would otherwise enforce uniqueness. Results now persist in `LandmarksDoc.results`. **What remains:** `vault_bridge::entity_values` and `entity_cell` end at `EntityKind::Culture` with **no catch-all**, so adding the variant needs an edit in that file — small, and now the only thing in the way |
