...
| **Ruling 14's spacing consequence is BUILT BUT INERT — two lines in `lib.rs` away** | `URBAN_MORPHOLOGY_SCOPE.md` | small | **The engine half landed 2026-09-06 and the app never uses it.** `LandmarkInputs::manual_icons` exists, `generate` honours it with a `MANUAL_ICON_EXCLUSION_KM = 3.0` ring (deliberately weaker than the smallest class radius, 6 km), and `landmark_bridge::icon_to_mark` filters to `Manual` origins. **But `grep -rn "icon_to_mark"` returns 5 hits — the definition, three of its own tests, and a doc mention. ZERO shipping callers**, and `lib.rs`'s `landmark_run_inner` assigns only `inputs.settlements`. So generation can still place a landmark on a hand-placed icon in the running app. **The lane declined this deliberately and disclosed it — `lib.rs` was not in its grant.** This is the last consequence of ruling 14, and the only one of the three still open |
