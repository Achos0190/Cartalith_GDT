...
| **History's `COMMITTED` rule** — **BUILT 2026-09-06, BUT ITS LANE DIED BEFORE REPORTING; NOT ADVERSARIALLY VERIFIED** | `GUI_GAP_REGISTER.md` ED-02 | small | **The work is in the tree and healthy; what is missing is the verification, and that is the honest state to record.** The lane hit the StructuredOutput retry cap and filed nothing, which killed the whole workflow, so **no verifier ran at all** — unlike the timeline lane, which failed the same way but had its verifier survive to check it independently. **What I checked myself, and only this:** `cargo test --workspace` 157 lines / **3 240 passed / 0 failed** (+10); all three touched `.gd` parse clean; the mark is stamped by `HistoryLedger::mark_saved_now()` called **only on `Ok`** in the save path, with the reason written beside it (a refused write leaves the previous file in place and must not move a rule that says what is on disk); and `saved_seq`/`saved_at_ms` are `Option`, with the doc stating in terms that **`None` and "seq 0" are different claims and are spelled differently**. **Not checked by anyone adversarial:** the cross-session reopen, the never-saved panel state, and whether the confirmation was duplicated. **Next verifier should take this as its first item** |
