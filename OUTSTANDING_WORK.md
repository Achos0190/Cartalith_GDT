...
rows mechanically, 2026-09-05 (thirty-eighth pass)**, after an earlier pass left four
...
|---|---|---|---|
| ~~The design fix pass — 7 of 13 round-2 boards carry factual defects~~ — **CLOSED 2026-09-05** | `DESIGN_HANDOFF.md` | medium | All eight edited boards verified; scope held (**nothing touched outside `design/proposed-2026-09-05-round2/`**, every hunk read). **Two of the critic's own defects were refuted by the lanes and correctly left unfixed** — which is the result worth keeping. (1) The *missing `poi` chip*: `geojson.rs:309` emits it in the **true arm of `if p.is_poi`**, not unconditionally, and `geojson_bridge.rs:170` builds every `GeoPlace` with `is_poi: false` — so the exporter *can express* `poi` and no document this port writes can *contain* one. Adding the chip would have drawn a group that can never populate. (2) *"No dry-run path exists"*: `data_manager_window.gd:2423` wires a live Dry run on Export ▸ Maps; the defect held only on the GIS route this board draws, and the lane fixed it on the true ground rather than the stated one. **Fixed for real:** `CPU 38`/`GPU 71` removed (`cartalith-gpu/src/multi.rs:19` names live utilisation as the one §2.5 phrase it deliberately does not implement, because wgpu 30 has no such query on any backend — the board drew that refused fiction verbatim); menu bar 34→36 px; the divider and undo/redo squares added; the lit title un-underlined; two underivable estimate figures ghosted; the fjord swatch `#3f6f8c`→`#3cc8f0`; Sea marks anchor bottom→center; landmark families four→six; and the owner's washed-pill ruling applied identically on Timeline, WorldDockB and BiomePaint |
| **The Asset Library window has no internal search** | `GUI_GAP_REGISTER.md` | medium | **Named 2026-09-05 by the pass that decided NOT to add `EXTRAS` rows for the fifteen dropped Asset-pack menu rows.** That decision rests on every one of those actions being a labelled control already visible in the library window — `_build_slot_grid()`'s batch-verb row and `_build_inspector()`'s row — which is true, and which makes the window the single home for them. **The window has no search of its own**, so "it lives in the library" is only as good as a user's ability to find it there. `asset_library_window.gd`, and a materially bigger build than the documentation pass that surfaced it |
...
| **A phone control measures 35 x 27 against a 115 px tap floor** | `DESIGN_HANDOFF.md` | small | **Pre-existing, and newly legible rather than newly caused — established by measurement, not assumed.** `_phonechrome_probe.tscn -- --force-touch` reports it at HEAD with the 2026-09-06 phone-bar work reverted, so the batch that surfaced it did not introduce it; that batch in fact took the probe's *unlaid* count from 5 to 3. The probe had been reporting four failures as one undifferentiated number, so **a zero-width control that has never been laid out was being counted as a control too small to hit.** Those are now split — `UNLAID` versus `VIOLATION` — leaving exactly one real violation, at `@Control@4906/@VBoxContainer@4907/@PanelContainer@4909/@VBoxContainer@4908/@Control@4910/@Button@4913`. Identify that button and either floor it or record why it is exempt |
| **Three phone Controls never get a layout pass, so their width cannot be measured** | `DESIGN_HANDOFF.md` | small | **Found 2026-09-06 by splitting `UNLAID` from `VIOLATION` in `_phonechrome_probe`.** Three buttons under one `VBoxContainer` report `(0.0, 115.0)` — a correct height against a width that was never resolved. The probe warms the phone tree by opening each overlay once, precisely because *"nothing had ever asked a `VBoxContainer` inside a `visible=false` subtree to lay out its children's cross-axis width"*; one of those warm-ups was `_set_panel_picker_open`, deleted with the `▤` glyph under ruling 20. **The count improved (5 → 3) rather than worsening**, so this is an older gap the deletion narrowed. Either find what still needs warming, or establish that the subtree is unreachable and should not be measured |
| **An unrecognised `entity_kind` fails the WHOLE vault link store, against `SAVEFILE_COMPAT` §13.3.3's own MUST** | `SAVEFILE_COMPAT.md` §13.3.3 | small | **Reproduced independently by the verifier, 2026-09-06, not taken from the report.** Doctoring one link's `"settlement"` to `"river"` in a two-link store gives `unknown variant \`river\`, expected one of \`settlement\`, \`province\`, \`continent\`, \`faction\`, \`culture\`, \`landmark\`` and **`LinkStore::from_json` fails entirely, taking the valid settlement link with it.** §13.3.3 says in terms: *"An unrecognised value MUST NOT drop the link."* **Pre-existing since faction/culture landed 2026-08-25** — adding the sixth token made it concretely reachable rather than causing it. Not silent: `project_open` matches with an explicit `Err(e) => godot_warn!` arm, so links are left out loudly. The fix is the shape `IconsDoc` already uses — drop the unresolvable row, keep the document |
...
| **After POI placement every landmark draws TWICE, and the copies survive turning the Landmarks layer off** | `GUI_GAP_REGISTER.md` | small | **Observable today, found 2026-09-06 while establishing what draws each layer.** `map_overlay.gd::_draw()` runs two independent passes — `_draw_manual_icons` (from `icon_list()`) and `_draw_landmarks` (from `EngineBridge.landmarks()`). Run POI automatic placement and every landmark gets a landmark ring **and** a poi icon glyph at the same cell. Worse, their visibility differs: `_draw_landmarks` honours `layer_visible("landmarks")` and manual icons have **no flag at all** — the file's own comment says they *"always draw once placed"* — so switching Landmarks off leaves the duplicates on screen. **This is exactly what ruling 14's "the renderer draws one layer" removes**, and it is downstream of the `origin` field below |
| **Ruling 14's merge needs a wider file grant than it was given — and its stated benefit is unrealised** | `URBAN_MORPHOLOGY_SCOPE.md` | medium | **Declined 2026-09-06 inside the grant rather than deferred by preference, and the decline was right.** `origin` has no runtime home: `cartalith_assets::manual::ManualIcon` records no provenance and `IconEditor::icons` is one flat `Vec`, so `project_bridge.rs` alone could write no true origin and keep none it read — a save stamping every row `"manual"` would mislabel the generated ones, since `IconEditor::generate` builds POI icons straight out of `landmark_store.last.landmarks`. **The shape to build is written into `IconsDoc`'s own doc comment** so it is not re-derived: `IconOrigin { Manual, Generated }` on `ManualIcon` defaulting to Manual, set Generated only in `generate`, carried as `IconDto.origin: String` with `#[serde(default)]`. **Grant needed:** `cartalith-assets/src/manual.rs` + `cartalith-godot/src/icon_bridge.rs` (19 struct literals across 4 files, one of them a golden-parity test). **And the benefit is unrealised:** M6 spacing is `cartalith_civ::landmark::generate`, whose only input is `LandmarkInputs` — manual icons are not among its members, so **spacing still cannot see them** and generation can still place a landmark on a hand-placed icon. Ruling 14 chose one layer over two for that reason; closing it needs the spacing input too |
...
| **Consolidate landmarks into `cartalith-civ`** (ruling 24) | `ARCHITECTURE.md` | medium | **Owner ruling 2026-09-06, against the recommendation to ratify the existing split.** The terrain-derived half (`cartalith-terrain/src/`) moves into `cartalith-civ/src/landmark.rs`'s crate. **Watch the dependency direction** — `cartalith-civ` will lean harder on `cartalith-terrain` internals, and if that forces `pub` on something private, say so rather than widening quietly. **Golden tests must not move**: a crate move that changes a value is a re-baseline and ruling 24 does not grant one |
| **Un-shelve the 16K/32K export** (ruling 15) | `EXPORT_SCOPE.md` | large | **Owner ruling 2026-09-06, against the recommendation to stay shelved.** §5's four costs are unchanged: **the render-once decision must be reversed** — establish what depends on it *before* touching it; four measured gaps close; and the banded renderer that was prototyped, measured byte-identical and reverted should be **recovered from history rather than rewritten**. Note the reference's own bake draws **terrain and nothing else**, so parity here is a much smaller target than the feature implies |
| **The 16K/32K codec question is now live and may need its own owner decision** | `EXPORT_SCOPE.md` | small | **Created by ruling 15.** The survey already eliminated two: **WebP dies at 16 383 px** and **JPEG XL dies on its AGPL encoder**. Neither remaining option is obvious, so **bring the owner a shortlist rather than picking** — this is the kind of choice that is cheap to make now and expensive to reverse once files exist |
| **The prototype's segment on-state is `--wash2` (0.16); the shell paints `accent_wash` (0.09)** | `DESIGN_HANDOFF.md` | medium | **Found 2026-09-06, and nobody had ever compared them because the line was believed lost to truncation.** `ldSwABg` is `var(--wash2)` at `Cartalith DCC Environment.dc.html:1941`; `DccWidgets.set_segment_on()` builds `box("accent", "accent_wash")` and `dcc_theme.gd` has `accent_wash` at alpha **0.09** against `accent_wash_2`'s **0.16**. The canvas and the shell disagree by one alpha step **on the exact control the owner approved as Option B**. `set_segment_on()` has **141+ call sites**, so this is a token pass rather than a patch. **Measure whether the difference is visible before proposing it** |
| **The timeline scrub row's 44 px touch floor is declared, not drawn-measured** | `DESIGN_HANDOFF.md` | small | **Disclosed as partial by the lane that built it, 2026-09-06 — the honest half of an otherwise green board.** `track.custom_minimum_size.y = maxf(role_px("timeline_track_h"), role_px("btn_min_h"))` is right, and probe §6 verifies **12.0 px at POINTER only**. Under `--force-touch` the probe hit ten undiagnosed failures because the tablet composition builds a different strip (the pill band measures 131 px there), so **the probe now REFUSES `--force-touch`** rather than defaulting through it and printing greens that would read as touch coverage. 44 is the role table's figure, not a drawn measurement. Needs a probe written for the tablet composition |
| **6 of `WorldGen`'s `#[func]`s are unreachable from GDScript entirely** | `UNWIRED_FUNCTIONS.md` | small | **The figure this row shipped with — "27 without a forwarder, 12 unreachable" — did not reproduce, and I wrote it here without re-deriving it.** A lane re-derived over all 448 `#[func]`s in `crates/cartalith-godot/src/*.rs` against every `.gd` call site outside a comment and got **6** (7 counting `get_rivers`, now wired); the verifier reproduced that independently. The gap was method: a plain substring search over `godot-project/` returns 1, because a `#[func]` merely *named* in a comment counts as reached. **Corrected 2026-09-05.** The original was carried from a previous verifier's number into this table unchecked — the exact failure this row's own subject matter is about. Below the old text, kept because the mechanism is still right: **Measured 2026-09-05 by the lane that fixed two of them, and re-derived independently by the verifier.** `EngineBridge` is the GDScript wrapper, so a `#[func]` with no forwarder is invisible to the shell no matter how healthy the `.dll` is — the failure reads as `Nonexistent function 'X' in base 'Node (EngineBridge)'` and is routinely misdiagnosed as a stale build (it was, twice, this week). Of the 27, **15 are reached directly** by callers that skip the wrapper and **12 are unreached entirely**. `get_rivers` is on that list and is the reason the Data-manager's `rivers` chip is dashed — the count exists, the forwarder does not. Also measured: **1 861 `bridge.*` call sites, 4 naming an undefined method before this batch and 2 after**, both survivors already guarded. Do not fix all 27 blind — the 12 unreached ones need a caller before a forwarder means anything |
| **History draws its undone steps from `RedoTail`, which names only one** | `GUI_GAP_REGISTER.md` ED-02 | medium | **Disclosed 2026-09-05 by the lane that built the context, with the measurement.** The approved artboard draws undone steps as a list that *"are not deleted"*. They are: `undo_last()` calls `ledger.pop_newest_height()` and `undo_revert_to()` calls `ledger.truncate_to(seq)` — measured `after undo: ledger=3 -> 2  redo_available=true redo_depth=1`. What survives is `RedoTail`, and `RedoTail::label()` names exactly **one** step. So the dock honestly draws one named undone row from `redo_label()` and *counts* the rest from `redo_depth` without naming them. Drawing the full stack the design shows needs the ledger to stop truncating on undo and to expose labels for the whole redo tail — **a Rust change**, in `cartalith-civ`/the ledger, not in the shell |
| **History's `COMMITTED` rule is session-scoped, because the ledger stores no save marker** | `GUI_GAP_REGISTER.md` ED-02 | small | **Disclosed 2026-09-05 with its evidence.** `undo_ledger()` rows carry `{seq, subsystem, label, detail, at_ms, kind, reversible, reason, steps}` and nothing about saving; `save_project()` stamps no row. The boundary is therefore derived in the shell from the `EngineBridge.project_saved` signal (`_saved_seq`, `_saved_at_ms`), which is correct within a session and **blank for a project saved in an earlier one** — the panel says so rather than drawing a rule it cannot place. Fixing it needs a ledger field, i.e. Rust |
...
| ~~`DccWidgets.action` buttons are 39 px against a 44 px touch floor~~ — **CLOSED 2026-09-05, the row was a category error** | `DESIGN_HANDOFF.md` | small | Both halves were measured and they do not meet. **39 px is real** as a *drawn* height at POINTER density (`Refine detail` 118×39, `Bake ALL & finalize` 164×39, `Center landmasses` 149×39). **The 44 px floor is a TOUCH requirement**, and on touch `action()` sets `custom_minimum_size.y = DccTheme.role_px("btn_min_h")` = **44**, while on phone `DccShell.phone_fit()` floors every `BaseButton` regardless. So the pointer figure was being judged against a floor that does not apply to it, and the density where the floor does apply already meets it. `39` appears nowhere in `dcc_widgets.gd` — the pointer branch is a literal `26` plus padding and font. Two passes carried this row: one measuring the drawn height, one reading the constant, neither noticing they were describing different densities |
...
| Phone MORE — §6.6's twelve remaining sub-screens | `DESIGN_HANDOFF.md` | medium | **The five ruled screens shipped 2026-09-05** (Project · Civilization · Data · Simulation · Preferences), and **nothing stopped being reachable — measured, not asserted**: a probe dumped the ground truth from the live `MenuBar` (7 menus, every non-separator item, recursively), then rendered each screen and read the strings back; all 7 menus reachable, every top-level row of File/Data/Preferences drawn. Rows act on the real shell — each resolves to a real `PopupMenu` item by menu name + id, so a row added to `menus.gd` appears here with no edit. **What remains is §6.6's other twelve sub-screens** (assets, assets-grid, asset-slot, travel, travel-item, help, gestures, data-tiles, data-io, landmarks, lm-fam) — **not part of the ruling**; each root row currently drills the real popup or opens the real window instead |
| **14 no-design surfaces remain — the method changed on 2026-09-05: design first, then build** | `DESIGN_HANDOFF.md` | large | **Owner approved the first canvas 2026-09-05** (*"I like the layouts as proposed, implement those"*) **and all nine of its artboards are now built and verified**, across two batches: History (closing ED-02), Ecoregion, Faction ▸ Relations, CIVIL ▸ Religion and the modal pattern; then Settlement's three appended sections, Δ vertical, the Data-manager route pane and the status-bar states. **Two were patterns rather than screens** and that is where the leverage was: the modal shape took the shell's stock-dialog sites from 11 to 5 in three files, and the route pane now renders **12 of 14** Data-manager routes with 2 keeping bespoke bodies. **UM-03 closed as a side effect** — the 84 px plan thumbnail turned out to be producible from `urban_layout_draw.draw_layout()` at `detail = 0.0`, proved in pixels windowed (6 285 warm pixels in the thumbnail rect against 0 in the dock beside it). **Zero fabricated values across both batches** — every figure traces to a named engine source or is dashed. **A new failure mode appeared instead and is worth carrying forward: fabricated *reasons*** — three dashes whose stated cause was false, all three corrected before commit. **Ruled 2026-09-05: build all 37. Twenty-three have closed.** A second canvas is needed for the remaining 14 | Eight were never really undesigned — the five Data-manager and three Asset-library stock `FileDialog`s now route through `DccBrowseDialog`, the bespoke breadcrumb browser the DCC Shell canvas draws. Six more closed the same day by derivation (world data, performance, generation info, faction roster, place editor, city viewer), each recording in its own source which canvas vocabulary it came from. **Then the owner changed the method mid-batch:** *"For the missing designs, you can use your /design skills in conjunction with /ui-ux-pro-max and run it by me first. As with everything the latest designs should inform it and based on similar systems from for example azgaar or rts games where one has similar panels."* So the remaining 23 are **drawn before they are built**, not derived in source. First canvas published 2026-09-05 — nine artboards, measurements lifted from `05-right-dock-and-bars.md` and `04-left-dock.md` and checked against `dcc_theme.gd`, comparables named per surface (Azgaar's Religions editor for the religion table, RTS diplomacy panels for faction relations, the spec's own STAMP STACK for History). Working files in `design/proposed-2026-09-05/`. **Awaiting owner review — do not build from it yet** |
