...
rows mechanically, 2026-09-05 (thirty-fifth pass)**, after an earlier pass left four
...
| `vault_window.gd` still ticks a snapshot that is gone | `MARKDOWN_VAULT_SCOPE.md` §20 | small | **The engine half closed 2026-09-05** — `WorldGen::snapshot_on_disk` filters the Map checkbox and the note body, and three states stay apart (never generated / present-or-unknowable / deleted, the last flagged by a `missing` key that is **omitted** rather than `false`, because no value means unknown). **The panel does not read it yet**: `_build_snapshots()` draws its ✓ from `path` alone and labels the button *Regenerate*, so a deleted snapshot still reads as a tick. One branch on `d.has("missing")` |
...
| **Left dock — restructure to 12 mode-gated blocks** (`04-left-dock.md` §3) | `DCC_SHELL_SCOPE.md` | large | **Ruled 2026-09-05: restructure.** One body per rail node, gated by mode (WORLD·a = ten-stage pipeline + Finalize; WORLD·b = Sculpt; CIVIL = four category blocks…) replacing one accordion of every category per domain. **This deliberately hides controls reachable today** — the opposite call to the tablet ruling, made knowingly. Touches all three workspaces |
...
| **Three structural moves to the newest design** | `GUI_GAP_REGISTER.md` | medium | **Ruled 2026-09-05: move all three.** Journey planner → a CIVIL rail node (it is the only menu item that changes the viewport); Atlas *Refine detail for the current view* → the WORLD rail beside Bake & finalize; the Asset-pack submenu → the newest flat 9-row shape. **`Clear library… destructive` has no home in the flat shape — find it one and say where.** Dropping it would be a capability loss dressed as conformance |
| **37 no-design surfaces — derive from the DCC vocabulary and build** | `DESIGN_HANDOFF.md` | large | **Ruled 2026-09-05: build all 37.** Eight are derivable rather than undesigned — the DCC Shell canvas draws a breadcrumb browser that "replaces the stock OS tree picker" and `browse_dialog.gd` already exists, so those eight stock `FileDialog`s are wiring. **The other 29 have no drawing at all**, so each must record in its own source which canvas vocabulary it was derived from; the owner will first see them running rather than drawn |
