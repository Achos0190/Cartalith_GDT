...
**107 outstanding items** — 2026-09-21, unchanged in count through a 3-agent batch that filed and
...
| **`lod_cache_key` over-invalidates on `glacial_snowline` when ice is off** | `LOD_DETAIL_SCOPE.md` (LOD-D4/glacial-probe follow-up) | small | **Filed 2026-09-21**, found while closing the row above. `lod_cache_key` folds in `passes.glacial_snowline` unconditionally, but `render.rs`'s `glacier_on`/`cryo` locals only read the glacier field when `appearance.ice_strength > 0.0`. So with ice off, changing the snowline still forces a fresh `TileFields` build (up to ~480 ms per the cache-key doc comment's own cost figures) even though the rebuilt pixels are guaranteed identical to the stale ones. Gate that one cache-key term on `ice_strength > 0.0` to skip the wasted rebuild — never a wrong-pixel risk, purely a cost one. **In flight 2026-09-21.** |
