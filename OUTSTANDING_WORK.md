...
**107 outstanding items** — 2026-09-21, after a 3-agent batch closed four small LOD-D6-follow-up rows
...
|---|---|---|---|
| **The `UNWIRED_FUNCTIONS.md` backlog** — 4 open rows (1 small · 2 medium · 2 large minus 1 deferred-by-ruling = 1 actionable large), 0 open dangerous-class | `UNWIRED_FUNCTIONS.md` | large | **Corrected 2026-09-21 from the code, not from the document's own count** (the same defect class this row exists to catch): the previous entry here repeated `UNWIRED_FUNCTIONS.md`'s stale "already done" summary for statusMid, paint preview patches and Cut/Copy/Paste, none of which is actually true at the symbol. Re-verified directly this pass: **statusMid** is still ~90% built, `repaint NN ms` still blocked on Owner question 2 (unchanged). **Paint preview patches**: `build_paint_preview_patch` was built and measured 2026-09-04 (1.3x-19.0x less CPU, 15.5x-75.9x fewer bytes, byte-identical to a full re-upload) but has **no shell caller** — `world_workspace.gd::_paint_apply_dab` still calls the old full-texture `build_paint_preview_texture`, and `viewport.set_preview_texture()` takes no offset to receive a patch. The sculpt half remains honestly declined (SL-1: would need `render.rs`'s AO/wetness/sea passes reworked over a caller-supplied window). **Cut/Copy/Paste**: selection sets exist and Select-all works, but **the clipboard itself does not exist** — nothing serialises an icon or label into a buffer, holds it across a world change, or pastes it back; all three menu items are still `_todo`. **The 3D viewport** stays open by ruling (deferred, research first) — correctly parked, not actionable. **`label_glyph_layout`** stays open as a genuine design question (rerouting would silently change today's rendering — an arc+tracking contract gap), not a stale claim. **Dangerous class now 0 open, 9 closed** — re-verified at the symbol 2026-09-21, eight directly (`2c825ed`), the ninth (`performance_window.gd`, b-8) by the false string's total absence from the tree (the file itself no longer exists). Stale source comments: 0 open / 6 closed (unchanged from 2026-09-21's earlier pass, commit `e462ed9`). Large-tier header corrected 3→2 (Region ▸ New world from selection was already `CLOSED` in the table, only the header hadn't caught up). **Net actionable remainder: `label_glyph_layout` (small, design question), the paint-preview shell wiring (medium, mechanical), Cut/Copy/Paste (large, clipboard) — see the two new rows below for the latter two, now in flight.** |
| **Wire the already-built paint preview patch into its shell caller** | `UNWIRED_FUNCTIONS.md` (Medium) | medium | **In flight 2026-09-21.** `WorldGen::build_paint_preview_patch` (returns `{texture, x, y, w, h}` over `PassBuffer::preview_touched_into`) was built and measured 2026-09-04 — 1.3x/3.0x/7.8x/19.0x less CPU and 15.5x/33.1x/55.5x/75.9x fewer bytes across the FFI at 512/1024/2048/4096², byte-identical to a full re-upload inside the window — but nothing calls it. `world_workspace.gd::_paint_apply_dab` still calls the old full-texture `build_paint_preview_texture`, and `ViewportHost.set_preview_texture()` takes no offset to receive a patch. Wire the caller and extend `set_preview_texture()` (or add a sibling method) to composite a small patch at its own offset instead of the whole texture. |
| **Build the Cut/Copy/Paste clipboard (icons and labels)** | `UNWIRED_FUNCTIONS.md` (Large) | large | **In flight 2026-09-21.** Ruled: selection sets → clipboard → commands, in that order; step one (real per-entity-kind selection sets, Ctrl/Shift-click, Select-all) is done. Step two, the clipboard, does not exist: nothing can serialise an icon or a label into a buffer, hold it across a world change, or paste it back. `menus.gd:771/776/777` (Cut/Copy/Paste) are still `_todo`; `:808` (Select all) already works. Scoped to icons and labels only — the "scoped to the active layer" dispatch `DccApp.clear_selection()` already has a shape for is a stretch goal beyond the clipboard itself, not required to close this row. |
