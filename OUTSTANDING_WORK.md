...
| **Four probes still call `_set_panel_picker_open`, deleted under ruling 20** | `GUI_GAP_REGISTER.md` | small | **Found by the PhoneSheet lane and confirmed independently by the verifier, 2026-09-06.** `grep -rn "func _set_panel_picker_open" shell/ ./*.gd` returns **nothing**; `grep -rn "_set_panel_picker_open" ./*.gd` returns **five live call sites in four probes** — `_appbar20_probe.gd`, `_menuconf_probe.gd` (twice), `_phonesweep_probe.gd`, `_shot_phone.gd`. That is the error-then-hang failure mode `_phonechrome_probe.gd`'s own post-mortem documents: the call errors, the probe never reaches its assertions, and it stalls rather than failing. **One line each**, but they are dead instruments until fixed — a probe that cannot run is not coverage. Correctly reported rather than fixed outside the lane's grant |
