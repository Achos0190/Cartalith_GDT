...
| **The icon/landmark de-dup key truncates on one side and rounds on the other** | `GUI_GAP_REGISTER.md` | small | **Latent, not a defect today, and one integer away from becoming a silent one — found by the verifier 2026-09-06.** `map_overlay.gd::_draw_annotation_marks` keys `ringed` with `Vector2i(int(lm["x"]), int(lm["y"]))` (**truncation**) and looks it up with `Vector2i(roundi(float(ic["x"])), roundi(float(ic["y"])))` (**rounding**). It is correct only because `landmark_dict` writes `lm.x as i64` and `icon_candidates` writes that same integer as `l.x as f64`. **If either side ever carries a sub-cell coordinate the two disagree at `.5`, the de-dup stops firing, and landmarks quietly draw twice again with nothing failing.** Make both sides round the same way, or key on something that cannot drift |
| **30 phone controls have a minimum their parents ignore, on both axes** | `DESIGN_HANDOFF.md` | small | **Found while verifying the width-floor fix, 2026-09-06, and it is pre-existing rather than caused by it.** Of 4 315 controls censused at 1080×2340 `--force-touch` (`phone_scale` 2.621), **30 now carry a 115 px minimum that exceeds their drawn width and draw identically before and after** — their parents do not honour `custom_minimum_size` at all. So the tap floor is set and disregarded. Separately worth knowing from the same census: **no `VSlider`, `ProgressBar`, `TextureProgressBar` or `ScrollBar` exists anywhere in that tree**, which is why removing the width guard was safe — the one class where a width floor would be wrong is absent |
