...
| Item | Owns it | Size | Next step |
|---|---|---|---|
| Project archive remainder — project-layer panels, the `library/` and `drafts/` slots | `STATUS.md`, `SAVEFILE_COMPAT.md` §17 | medium | Nothing draws any of it. **Two claims this row made are STALE, corrected 2026-09-21, checked at the symbol rather than assumed:** (1) **foreign-entry preservation is not missing — it is already built and tested.** `cartalith-io::project.rs`'s `ProjectData::foreign`/`ProjectWrite::foreign` carry an unrecognised entry byte-for-byte, verbatim, "always wins" over a slot a later build registers (`ZipWriter` would otherwise refuse the duplicate name); `a_foreign_entry_survives_an_open_and_a_re_save` proves it end to end with a deliberately non-UTF8, non-JSON fixture. The Godot bridge threads it too: `project_bridge.rs:2276` (`self.carried_foreign = std::mem::take(&mut data.foreign)` on open) and `:2130` (`write.foreign = self.carried_foreign.clone()` on save) — also surfaced to GDScript as `foreign_entries` (`:2648`), so "reported" and "preserved" were never actually in tension, the row's own framing was wrong. (2) **the `preview.png` producer half is CLOSED 2026-09-21 (verified)** — `DccApp._capture_preview_png()` captures and PNG-encodes the map viewport at save time (256px on the longer edge), `project_save_with_documents` gained a caller-optional `PackedByteArray` parameter (backward compatible — every existing 2-arg call site keeps working unchanged) that writes `write.preview_png` when non-empty. Verified independently: full diff read, `cargo test --workspace --no-fail-fast` reproduced exactly (163/3488/0/34), both touched `.gd` files parse clean, the windowed capture probe's own review PNG viewed directly (a real rendered map frame, not blank). Commit `cf1eeec`. The remainder of this row (project-layer panels, the `library/` and `drafts/` slots — the only two genuinely unbuilt pieces left) stays open. |
...
| **Implement `Cartalith Settlement Editor.dc.html`** — new GUI: a faction/settlement editor for urban morphology | owner-supplied design canvas, project `067f80e7-dbb7-4492-8e69-96aaa8050a4d` | large | **UNBLOCKED 2026-09-21 — owner ran `/design-login` and explicitly instructed implementation.** Desktop variant. Canvas, `support.js`, and the design author's own audit (`docs/DESIGN_SETTLEMENT_EDITOR.md`) pulled via `DesignSync` and saved locally at `design/settlement-editor-2026-09-21/`. **The audit already resolves most of the scoping ambiguity this row originally flagged**: of 29 inventoried controls, ~24 are backed by real engine symbols (`NamedSettlement`, `PlaceExtras`, `CIV_TRAITS`, `civ_food_shed`, `CollapseSimRequest`/timeline, `civ_trade_flows`, `vault_bridge.rs`) and a handful are explicitly not (status/wealth-tier/founded-year pickers, a leadership/person type, authored timeline events, manual political periods) — the audit's own recommendation is to keep all five tabs and build the Economy tab first as "the cheapest of the five," since it needs no engine work, only a UI surface for an existing binding. 1g/1h (layout + generation-rules windows) are blocked on threading a rules argument through `urban_adapter::settlement_layout` — flagged by the audit as "the cheapest high-value change in the file," a signature change with byte-identical defaults. **Corrected in passing**: the audit itself found and disclosed three stale doc comments in this port's own code (`roster.rs`, `civ_roster_bridge.rs`) claiming settlement/wall edits and Government "reach nothing" — untrue, both bridges are live. Implementation plan to follow (large, multi-file, several architectural decisions the audit's own "Open decisions" section still leaves open — per this project's own working discipline, warrants a proper plan before code). |
| **Implement `Cartalith Settlement Editor Touch.dc.html`** — the touch/tablet variant of the same editor | owner-supplied design canvas, project `067f80e7-dbb7-4492-8e69-96aaa8050a4d` | large | **UNBLOCKED 2026-09-21 — owner ran `/design-login` and explicitly instructed implementation of both variants.** Companion to the row above — same project, same `support.js`, same audit (which covers touch in its own "Touch companion" section: four artboards — 1a tablet overview, 1b tablet layout, 2a phone drill-down stack, 2b phone layout/rules/roster). Every dashed panel and blocker from the desktop audit carries over unchanged; touch forces three real differences (phone drops tabs for a drill-down stack; tablet keeps the modal window; the four-column roster matrix survives on tablet but falls back to a profile layout on phone) and leaves one open question (whether the collapse simulator should be phone-reachable at all, given its overwrite confirmation is a three-line bottom sheet). Build after the desktop variant's data model and control set are established, so this is a real adaptation rather than a parallel guess, per the row's own original sequencing. |
