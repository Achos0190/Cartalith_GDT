...
**107 outstanding items** — 2026-09-21, after a 3-agent batch closed three small/medium rows and
...
| **`LithoSource` still deep-clones three fields for an LOD snapshot — same shape as the `WorldState` fix just closed** | `LOD_DETAIL_SCOPE.md` (WorldState-Arc follow-up) | medium | **Filed 2026-09-21**, flagged by the agent that closed the `WorldState` Arc row. `age_field`/`volcanic_field`/`crust_field`/`resistance_field` (the report names three of four — verify the exact set at the symbol before scheduling) are still plain `Vec` on `WorldState`, deep-cloned for the duration of one build — another ~43 MB transient peak, same mechanism the just-closed row fixed. Would take the same `Arc<Vec<f32>>` + `Arc::make_mut` treatment. **In flight 2026-09-21.** |
| **A loaded save still copies its three fields via `cartalith_io::SaveFields`, which owns plain `Vec`s** | `LOD_DETAIL_SCOPE.md` / `SAVEFILE_COMPAT.md` (WorldState-Arc follow-up) | small | **Filed 2026-09-21**, flagged by the same agent. Smaller than the `LithoSource` row above — a loaded-project path, not every generation — but the same shape: `SaveFields` owns plain `Vec`s that get copied rather than shared when an LOD snapshot needs them. **In flight 2026-09-21.** |
