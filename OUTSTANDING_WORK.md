...
| **The zoom readout still says `z1.1` where the canvas says `zoom 110%` — and the comment defending it is false** | `design/mcp-2026-09-07/Cartalith DCC Environment.dc.html` | small | **Found by the re-check 2026-09-07, and the irony is the point: this is the tree’s NEWEST false canvas citation, written by the very pass whose headline finding was false canvas citations.** Two of the three sub-defects were fixed (the `2D · ` prefix and the second line). The third was declined with a comment saying the port has no percentage referent because `_zoom` is *"a multiplier … not the prototype’s `view.s` against a fixed 4096² world"*. **That is wrong about the canvas.** Its own writer is `cartalith-dcc-parts.js:221` — **`'zoom '+Math.round(v.s*100)+'%'`** — and `ENV:1736` clamps `view.s` with `Math.min(4, Math.max(0.12, view.s*f))`. **It is a plain multiplier, the same kind of quantity as `_zoom`, and no world size enters the percent.** `z1.1` is `zoom 110%`. **Also unfixed:** the shipped line carries **four `·` segments where `ENV:914` has three** — `equirect · z1.1 · relief · Default` against `equirect · zoom 100% · relief`. **Delete the false comment with the fix, not after it** |
| **`_ds03fit_probe` is RED, and it is a live canvas-width conformance failure** | `design/mcp-2026-09-07/Cartalith DCC Environment.dc.html` | medium | **Pre-existing, and the re-check established what the fix lane deliberately declined to claim:** built a clean detached worktree at HEAD, imported it, and got the **identical two failures** — so it is not caused by this batch. `boot civilization/planner` and `world civilization/planner`: *"left dock is not forced open"*, **got=404 cap=372**. **372 is not an arbitrary cap — it is `ENV:25`’s `--ldW` and `DccTheme.W_LEFT_DOCK`.** So the left dock is **32 px wider than the canvas specifies**, in exactly the region the diff listed as "NOT reached". **A red probe that has been red for a while is the cheapest conformance finding available** and it was sitting in the tree unread |
