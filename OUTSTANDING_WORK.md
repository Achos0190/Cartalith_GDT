...
| **Dialog protocol: SEVEN of sixteen converted — including both BACKWARDS sites; nine remain** | `DCC_SHELL_SCOPE.md` | medium | **All done in the MAIN LOOP 2026-09-07, no lane — the economy the budget posture names first.** **Converted:** `right_dock::_confirm_revert`, `faction_roster_window::_confirm_remove`, `place_editor_window::confirm_delete`, `civilization_workspace::_confirm_destructive` (a SHARED helper, so every destructive action in that workspace came with it) and `::_tl_show_confirm`, plus `vault_window::_compare_dialog` and `::_preview_dialog`. **The vault pair were the worst class and needed the PROTOCOL, not `confirm()`** — a diff view and a preview `TextEdit` are not the text-only shape. They had calls two and three (`phone_fit` + `popup_centered`) **and not the first**, so the fit ran against a window never shaped for a phone. **That is worse than omitting it:** an absent fit leaves an obvious desktop dialog; a fit without the shaping produces a scaled-up desktop dialog that looks deliberate. **Two things the conversion had to get right, both recorded at the sites:** `phone_window()` sets `borderless`, so the title must be redrawn inside the content with `phone_head()` or the dialog opens nameless; and it also sets `ok_button_text = "Close"`, so `_preview_dialog`—whose caller supplies *"Write to Markdown"*—must set its own text AFTER the call, or the primary action is silently renamed on phones only. **`vault_window.gd` now has 3 `phone_window` / 3 `phone_present` and ZERO bare `phone_fit(dlg)`.** **NINE REMAIN:** `world_workspace::_confirm_discard`; the three prompt-shaped sites (`journey_planner_view::_save_journey`, `::_capture_preset`, `cartography_workspace::_prompt_label_name`) which take **`prompt()`, not `confirm()`** — do not convert them by pattern-matching the confirms; four in `menus.gd`; and `shortcuts_dialog.gd` |
