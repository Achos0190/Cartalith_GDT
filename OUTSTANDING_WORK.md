...
| **IN-13 — trade flows**: who trades with whom (bipartite match, network flow), prices, tariffs, caravans as entities — **moved here 2026-09-23, was mis-bucketed in §3.1 despite its own text saying it's unblocked** | `STATUS.md` | large | `TradeBalance` names *what* moves, never *who* holds it or in what unit — that was the blocker. **RULED 2026-09-21 — Ruling R (`LARGE_ITEM_RULINGS.md`, confirmed at the symbol): per-faction currencies, with an exchange rate between any two.** The currency-model question is settled; the subsystem itself (bipartite match, network flow, prices, tariffs, caravans as entities) is not yet scheduled. **Genuinely ready** — recommend scoping first given "large" plus four distinct sub-parts (matching how §20 and the urban-algorithm row were scoped rather than built blind this session), since a bipartite trade match plus network flow plus a new entity kind (caravans) is real architectural surface, not a parameter change. |
