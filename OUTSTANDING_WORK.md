...
| **Six phone scope chips sit at 43 px against a 115 px touch floor** | `DESIGN_HANDOFF.md` | small | **Found 2026-09-06 by the probe-hygiene lane in a file it did not own, and reported rather than fixed.** `_phonechrome_probe` at HEAD already reports `no tap-floor violations got=6`: the six chips `_fill_scope_chips` draws are **43 px against the 115 px floor** (44 dp × `phone_scale` 2.6214). These are the Find-on-map scope chips added the same day — `s / f / lb / r / . / l` — so the defect arrived with a feature that was otherwise verified. **A chip is a real finger target**, so this is not one of the shell's stated exemptions; either it floors like every other tappable row or the exemption has to be argued and written down |
