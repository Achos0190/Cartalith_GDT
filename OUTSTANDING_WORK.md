...
**124 outstanding items** — re-derived 2026-09-20 after closing the IME row (`f2b0e33`); previously 120 in one run of
...
| **Settlement river binding — real geometry, not a proxy (Ruling N, river half)** | `LARGE_ITEM_RULINGS.md` (Ruling N) | large | **Unblocked, filed 2026-09-20.** `cartalith-civ::build_settlement_suitability`’s river term becomes proximity to a real traced river polyline (`cartalith_hydrology::trace_river_polylines`’s existing whole-world output), not a raw flow/order sample — as a term inside the existing suitability ranking, never a separate gate, per the owner’s explicit constraint. `cartalith-urban::build_site`’s `riverPath`-truthiness bug (accepts an empty/one-point path) is fixed in the same pass, same family of defect. **Deliberate golden re-baseline**, disclosed per `DECISIONS.md`: re-baselines `build_settlement_suitability`’s golden tests and the `pathOfOne`/`pathEmpty` fixtures, and ripples into economy, urban layout and faction territory. See `ELEVATION_FIELD_ARCHITECTURE_RESEARCH.md` for the design and `LARGE_ITEM_RULINGS.md` Ruling N for the owner’s authorisation. |
| **Settlement coastal binding — real geometry, not a proxy (Ruling N, coastal half)** | `LARGE_ITEM_RULINGS.md` (Ruling N) | large | **Blocked on EF-6** (no coastline vector exists yet). Same shape as the river half: `civ_is_coastal`’s radius-proximity-to-any-ocean-cell proxy becomes proximity to a real traced coastline, as a suitability term. See `ELEVATION_FIELD_ARCHITECTURE_RESEARCH.md` for the design and `LARGE_ITEM_RULINGS.md` Ruling N for the owner’s authorisation. |
| **EF-3 · Erosion-consistent mountain detail** | `ELEVATION_FIELD_ARCHITECTURE_RESEARCH.md` | large | **Owner, 2026-09-20: erosion-consistent from the start**, not statistical-only — in scope, not deferred. Depends on EF-0 (landed, `2373c08`): a tile-bounded re-run of the engine’s own erosion kernels, boundary-seeded the same way EF-1 seeds hydrology. Iterative, not a single pass; per-tile cost needs measuring before committing further. See `ELEVATION_FIELD_ARCHITECTURE_RESEARCH.md` for the design and `LARGE_ITEM_RULINGS.md` Ruling N for the owner’s authorisation. |
| **EF-6 · Vector constraints beyond rivers (coastlines, faults, ridges)** | `ELEVATION_FIELD_ARCHITECTURE_RESEARCH.md` | medium | **Filed 2026-09-20, after EF-0/EF-1 landed and verified, as designed.** One new boundary/contour-tracing primitive covers coastlines (sea-level contour) and faults (`boundary_mask`/`boundary_type`, already computed); ridges need a second, TPI-based tracer. Unblocks the coastal half of Ruling N. Cheaper than EF-1 — no global-watershed boundary-condition problem. See `ELEVATION_FIELD_ARCHITECTURE_RESEARCH.md` for the design and `LARGE_ITEM_RULINGS.md` Ruling N for the owner’s authorisation. |
...
| **PLAN’s sheet subtitle goes stale when a new plan is computed** | `ANDROID_UI_SPEC.md` | small | **Found 2026-09-13 by the wf53 verifier.** With a real route and plan, the phone header reads `journey · <from> → <to>` once the planner sheet closes, but computing a new plan does not refresh it until the PLAN tab is picked again. Hook the header refresh to the planner’s plan-changed path. The detent probe only uses fake plan data |
...
| **A seed near 2 147 483 647 panics the noise crate and the failure is silent** | `GENERATION_PARAMETERS.md` | small | **Found 2026-09-13 by the wf48 SEED lane; the verifier found it plausible from the code but did not run it.** `cartalith-noise` `ridged()` and `ridged_oct()` compute `s + o * 131` on `i32`, which overflows in a debug build (*attempt to add with overflow*, `lib.rs`) — and the New World seed field allows up to `2147483647`. Generation fails without an error on screen and `get_seed()` keeps returning the previous world’s seed. **A release build wraps instead of panicking, so desktop debug and Android disagree.** **This is a parity question before it is a fix:** the reference adds in JS doubles, so neither wrapping nor clamping matches it above the overflow point — choose between clamping the field and matching JS arithmetic, then surface a failed generation either way |
