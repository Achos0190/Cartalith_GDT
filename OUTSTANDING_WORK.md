...
| Military manpower **finding 2** — **premise found false 2026-09-23, commit `6e91059`, not built** | `MILITARY_MANPOWER_SCOPE.md`, `LARGE_ITEM_RULINGS.md` Ruling AI | small remaining | Owner asked to fix the "era-fixed ratio" plus add industrialization scaling. A dispatched build found there is no era-fixed ratio, and the industrialization scaling already exists and is strong — measured at equal population/land/institutions, 1 090→24 617 standing army (11.09×) from subsistence to industrial, confirmed by one new mutation-tested test, no behaviour changed. Both documents corrected in place. Re-measured with today's date: the sparse 33-settlement world now reads 3 of 6 factions within band (was 1 of 6 in the stale text); most remaining below-band cases trace to a different, already-known map-scale effect, not era or industrialization. **Three real options for a fresh ruling, none chosen yet**: (a) accept the ag-tech rule as satisfied, close this row; (b) take on the map-scale normalization of land-capacity÷population separately; (c) rule on refitting `SOLDIER_UPKEEP` to vary with the ag-tech share, reproducing the era table's own non-monotone band shape — a real re-baseline of the owner's worked example. |
