...
**122 outstanding items** — re-derived 2026-09-20 after closing the IME row (`f2b0e33`); previously 120 in one run of
...
| **Settlement coastal binding — real geometry, not a proxy (Ruling N, coastal half)** | `LARGE_ITEM_RULINGS.md` (Ruling N) | large | Same shape as the river half: `civ_is_coastal`’s radius-proximity-to-any-ocean-cell proxy becomes proximity to a real traced coastline, as a suitability term. See `ELEVATION_FIELD_ARCHITECTURE_RESEARCH.md` for the design and `LARGE_ITEM_RULINGS.md` Ruling N for the owner’s authorisation. **UNBLOCKED 2026-09-20**: EF-6 landed (`630c0dd`), so a real traced coastline now exists to bind against. |
...
| Item | Owns it | Size | Next step |
|---|---|---|---|
| **PLAN’s header TITLE half (“PLAN · STAGE n”) goes stale on stage isolate** | `ANDROID_UI_SPEC.md` | small | **Found 2026-09-20 by the batch2 verifier, as a residual of the subtitle fix’s own scope.** `_on_stage_clicked()` (journey_planner_view.gd) rebuilds the inspector and matrix on isolate but never calls `_apply_result()`, so the header never advances to "PLAN · STAGE n". Measured directly: `_isolated_stage=1` leaves the title at plain "PLAN". One guarded call in `_on_stage_clicked()`, same shape as the subtitle fix. |
