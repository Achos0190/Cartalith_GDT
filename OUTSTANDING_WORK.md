...
| **Units: the two civ/infra workspaces now route; the journey planner still routes NONE** | `GUI_FEATURE_PARITY_SCOPE.md` | medium | **Measured by the menu audit 2026-09-07 and re-derived by the verifier over the whole shell tree** (the lane’s first count of 64 was 20% low because its sweep missed `shell/workspaces/`; **the method is recorded here so the number is not quoted onward without it** — non-comment lines carrying a quoted km token beside a `%` spec). **`DccUnits` is called from right_dock (25), world_workspace (4), viewport_host (3), menus (3), tool_bar (1).** Raw km: right_dock 24, **journey_planner_view 24 with `grep -c DccUnits` = ZERO**, civilization_workspace 8, infrastructure_workspace 5, engine_bridge 3, and nine more files with 1-2 each. **The scale bar is routed** (`_update_scale_bar` uses `DccUnits.format_adaptive`); **the planner is not** — its headline `"%s km"`, its `"%.1f km/d"`, the stage matrix column header and the status-bar write are all raw. **The measure tool converts 4 of its 7 modes** — distance, bearing, derived and vertical do; **area, radius and section do not**. So switching to miles today gives a user a partly-converted app | **NARROWED 2026-09-07.** `civilization_workspace.gd` (8 raw-km lines) and `infrastructure_workspace.gd` (5) now read `Preferences ▸ Units` through `DccUnits`, and the pass added **`DccUnits.format_area`** because **100 km² is 38.6 mi², not 62.1** — the linear factor squared, which a `format` call would have got wrong on every area readout. **Three deliberate non-conversions are documented at their symbols rather than left to look like misses:** `persons/km²` is a RATE and `DccUnits` has no helper that inverts a factor (hand-rolling `× 2.59` would be the second conversion path `dcc_units.gd`’s header exists to prevent); the four landmark class-radius SLIDERS are INPUTS whose `1.0 … 400.0` bounds and `1.0` step are km bounds and whose release writes `landmark_set_class_radius(km)`; and the two site thresholds are now named constants (`SITE_WATER_REACH_KM` 2.125, `SITE_RIVER_CONTEXT_KM` 25.0) because **each is printed twice** — readout and tooltip — and a threshold drifting from the distance measured against it is the defect. **Still open: `journey_planner_view.gd`’s 24 raw-km lines with `grep -c DccUnits` = ZERO**, `engine_bridge` (3), and the measure tool’s area/radius/section modes |
...
| **OWNER-REPORTED: the phone cannot set generation parameters at all — the tabs lift a one-row strip** | `ANDROID_BUILD_SCOPE.md` | large | **Owner, 2026-09-07, on glass: *"I don't have sliders or fields to change parameters on how the world is generated. Nothing from map, generate, plan or more really leads to a deeper menu — I can pull them up but there is one line that barely scrolls properly and a lot of white space."* **Mechanism identified in code, to be confirmed on glass:** `_pick_phone_tab()` does not open a panel. MAP and GENERATE call `_pick_bar_domain()` and **lift the TOOL-OPTIONS strip to a half detent**; PLAN calls `open_journey_planner()`; MORE toggles the overflow. **The generation parameters are in the LEFT DOCK**, which the bottom bar never opens — it is behind a separate sheet. So a tab promises a domain’s controls and delivers one horizontal strip in a half-height sheet. **A lane measured this exact shape on 2026-09-06 and deferred it:** the MAP tab’s half detent is **1 003 rows holding one horizontal strip — 933 blank with no world and 912 blank WITH one**. It declined because `PHONE_DETENT_HALF_FRAC = 0.46` is transcribed from the prototype and *"changing a transcribed detent needs an owner ruling"*. **That was the wrong conclusion from the right measurement:** the defect is not the fraction, it is that **the sheet has almost nothing in it**. A correct detent over empty content is still an empty screen. **This is the phone shell failing at its core purpose and it outranks every remaining GUI row** |
| **OWNER-REPORTED: New world on the phone has no km or size input** | `DESIGN_HANDOFF.md` §6.7 | medium | **Owner, 2026-09-07: *"even the initial or new map setup doesn't allow for a km/size input"*. Confirmed in code, and it is structural rather than broken.** `new_world_dialog.gd` builds a phone `card` and a `rest` container with **`rest.visible = false`**; `size_sec` — which holds **Map width**, **Width (km)**, **Grid columns** and **Aspect** — is parented to `rest`. Only *Resolution* is re-parented into the visible card (deliberately: the file argues it is *"the only creation-time value whose absence is a trap rather than a preference"*). So the phone card offers **seed, extent chips and resolution** and nothing else. **§6.7 drew it that way, so the canvas and the shell agree — and the owner has now said the result is wrong in use.** An owner decision is newer than any canvas. **The question to answer is not "is this a bug" but "which of `size_sec`’s four controls belong on the card"** — map width in km is a fundamental world property and a 200 km world and a 12 000 km world are different products |
...
| **Measure performance on a 2K map — on the OnePlus 6T and elsewhere** | `ANDROID_BUILD_SCOPE.md` | medium | **Owner request 2026-09-07: *"let's measure performance on a 2k map, among others on the OnePlus 6t"*.** **The device is identified, not assumed** — `ONEPLUS A6013` (that is the 6T), Android 15, **7 824 600 kB MemTotal (7.46 GiB)**, 8 cores, serial `9608b26b`. **Runs against the APK this batch is building**, so the numbers describe the current tree rather than a stale `.so`. **What is already known and must be reproduced rather than trusted:** the default **2048×1311** new world is recorded at **~878 MB peak** on the phone — that figure is the reason 16K export is impossible there, so it is load-bearing and deserves a fresh reading. **What to measure, per stage and not just in total:** wall time for each of the ten generation stages (`cartalith-engine::progress` already reports them), **peak resident memory polled from the HOST via adb** (a process cannot see its own Rust allocations — this is how the timeline lane got its numbers), time to first paint, and frame time at rest and while panning. **"Among others" means a comparison, so give the same run on desktop** — the same seed, the same grid, the same stages — or the phone numbers have nothing to sit against. **Two traps this project has already hit:** a debug `.so` is not a release one and the residue row records a 156 MB difference, so **state which build the numbers came from**; and Android may kill the app rather than return an allocation failure, so **a missing number is a finding, not a gap** |
