...
|---|---|---|---|
| **The design canvas for the 23 remaining no-design surfaces — with the owner, not started** | `DESIGN_HANDOFF.md` | medium | **Published 2026-09-05 and awaiting review; nothing may be built from it until the owner answers.** Nine artboards over three pages — History, Settlement's four appended sections, Ecoregion/wildlife, Faction relations, Δ vertical, CIVIL ▸ Religion, the Data-manager route pane, the modal pattern, the status-bar states. Every dimension is lifted from `05-right-dock-and-bars.md` / `04-left-dock.md` and cross-checked against `dcc_theme.gd`, so the mockups are the same program rather than a proposal beside it. Two of the nine are deliberately **patterns, not one-offs** — the modal closes the asset-library prompts, `app.gd`'s two `ConfirmationDialog`s and the import warnings together; the route pane closes eleven undrawn Data-manager routes. Working files `design/proposed-2026-09-05/` (artboards + `canvas.json` + `_shared.css` + `_mk.sh`); re-seed from those, never from the published `.html` |
...
| **The place editor and the right dock disagree about the same settlement** | `GUI_GAP_REGISTER.md` ED-03 | small | **Measured 2026-09-05 by `_pesibling_probe.tscn` against a live 40-settlement world, not inferred.** The dock draws a *snapshot* pushed by `on_settlement_selected(data, index)`; the editor writes the engine live; nothing re-pushes the snapshot when an edit lands. `PES ok A1` the engine took the rename — `PES FAIL A2` the dock still draws the old name — `PES FAIL A3` the new name is absent — `PES ok B1` a re-select shows it, which is what makes this a refresh gap and not a dock that cannot read the field — `PES FAIL C2` after a delete the dock keeps drawing a settlement that no longer exists, and every action row in it is keyed to a `_settlement_index` the delete has just renumbered. **C2 is the worse half.** The fix is not in `place_editor_window.gd`: `place_changed`/`place_deleted` are emitted correctly and `civilization_workspace.gd` owns both connections, but its `_on_civ_edited` refreshes the map overlay and its own categories and never re-pushes the dock. The dock already has the machinery — `right_dock.gd::_live_settlement(snapshot)` resolves by `tid` and returns `{}` for both "past the end" and "different town". **`_pesibling_probe.tscn` exits 1 by design and stays red until this closes** — a sweep that runs every `_*probe*.tscn` will trip on it; that is a finding, not a regression. Left red deliberately: inverting it would mean going green on the bug and red on the fix |
...
| 20 | **Does a diagnostics/Performance window exist in this design language at all, or is `Preferences ▸ Memory ▸ Working set` the whole surface?** | `DESIGN_HANDOFF.md` | Raised 2026-09-05 by the lane that restyled `performance_window.gd`, which brought the chrome into the DCC vocabulary and explicitly declined to answer the existence question. The spec designs a menu **row**, not a window: §2.5 Memory gives `Working set \| Read-only, "1.6 GB of 12 GB"` and Performance gives `GPU acceleration \| Toggle + backend readout`. §8 (Asset library) and §9 (Data manager) are the only two windows it draws, and §2.6's Window menu names exactly those two. Gates whether the window is designed onto the pending canvas or folded away |
