...
| **`build_parcels`'s frontage-width loop can hang — a real, reproducible defect, not the hypothetical the 2026-09-21 survey ruled out** | `cartalith-urban/src/blocks.rs`; `RC_ENGINE_CHANGES.md` §6r.5 | medium | **Found 2026-09-22 while building the market-town Rules preset**, and it corrects that survey's own correction (see §2.9's row above). The frontage loop (`while acc < e_len - 3.0`) draws a random width, subdivides it, and accepts what fits the remaining edge — but when a drawn width's first piece doesn't fit, `acc` doesn't advance and the loop redraws a fresh random width and tries again, unbounded. **Measured real hangs, not a theoretical risk**: a pop 7 000 town took ~35s (against ~90ms) at `frontage_width_variance = 0.15`; the existing "Planned Grid" chip in the shipped Generation rules window sets exactly that variance to `0.10` and a pop 4 000 town did not finish in 90 seconds. **This is shipped, reachable, user-facing behavior today** — anyone who picks "Planned Grid" can hang generation. **The fix, per the spec's own §6r.5 analysis of the JS original**: cap the retry loop with a bound, sized by measurement (the spec's own fix moved 46 of 8 939 parcels, 0.51%, at 10 of 12 towns). **This changes generation output and needs a golden re-baseline** — not done here, filed for scheduling. Until fixed, avoid the "Planned Grid" preset (or any custom `frontage_width_variance` low enough to reproduce the hang) in the shipped app. |
