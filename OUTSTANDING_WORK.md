...
**119 outstanding items** — 2026-09-21, after closing LOD-D1 (`f6d1bd5`; byte-identical golden parity
...
| **v2.73, half 1 · A village green as a distinct plaza kind** — **investigated and fully written, needs an owner ruling to land (golden re-baseline)** | `RC_ENGINE_CHANGES.md` §8.2 | medium | **2026-09-21: checked at the symbol, implemented, verified, then reverted rather than land unauthorised.** `PlazaKind::{Market, Green}` on `Plaza` (`plaza.rs`), defaulted `Market`, overwritten `Green` in `generate.rs` when `pop_target < 1500.0` (the exact chartered-town threshold `amenities.rs::build_civic`'s civic-hall gate already uses — the RC row's reuse claim checks out), the market-cross push in `hinterland.rs::build_details` gated on `kind == Market`. Compiled clean; **4 of 29 golden scenarios move by exactly −1 detail each** (`landlockedHamlet` 115→114, `popFloorClamp` 113→112, `hamletBoundary` 120→119, `venusTinyCanal` 49→48 — the withheld cross), every other case in the 268-test `cartalith-urban` suite unaffected. **This port cannot independently confirm those four new counts are the real v2.73 values** without the actual v2.73 JS or an owner-supplied recapture (`Cartalith_RC` not present on this machine). **Owner: rule (a) accept 114/112/119/48 as the re-baseline and apply the already-verified diff, or (b) get a real v2.73+ capture to confirm first.** Diff not committed; reverted via `git checkout --` on the 6 touched files. |
