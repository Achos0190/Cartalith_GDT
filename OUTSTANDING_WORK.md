...
| **`LithoSource` still deep-clones three fields for an LOD snapshot — same shape as the `WorldState` fix just closed** | `LOD_DETAIL_SCOPE.md` (WorldState-Arc follow-up) | medium | **Filed 2026-09-21**, flagged by the agent that closed the `WorldState` Arc row. `age_field`/`volcanic_field`/`crust_field`/`resistance_field` (the report names three of four — verify the exact set at the symbol before scheduling) are still plain `Vec` on `WorldState`, deep-cloned for the duration of one build — another ~43 MB transient peak, same mechanism the just-closed row fixed. Would take the same `Arc<Vec<f32>>` + `Arc::make_mut` treatment. |
