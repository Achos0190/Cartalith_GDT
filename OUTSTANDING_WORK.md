...
76 outstanding items — 2026-09-23, unchanged in count (three owner rulings recorded, unblocking two rows
...
| **A citadel enclosure straddling the town wall — siting ruled, genuinely ready to schedule** | `URBAN_MORPHOLOGY_SCOPE.md` | large | **Ruling I, 2026-09-12 — new engine work.** The plan's citadel sits astride the curtain on the north-east: its own walled enclosure with towers, one large building and an open court, reached from inside the town. **Nothing like it exists**: `fortify.rs` builds the curtain, gates, spurs and the star fort, and the only castle in the crate is the *"Castle keep"* civic **building** style in `amenities`. GUI-sequencing gate lifted 2026-09-22. **Siting ruled 2026-09-23 — Ruling AC (`LARGE_ITEM_RULINGS.md`): largest settlements only, by size tier, not gated on faction-seat status** — matches how the existing bastioned star fort is already gated by size/class. **One question remains, named in the ruling itself**: whether the citadel's area counts inside the wall circuit for growth purposes — a smaller, second-order question that shouldn't block starting the build (can be decided or defaulted when that code path is actually reached). It owns a golden once built, under Ruling H's re-baseline scope. Target: `design/owner-references-2026-09-12/urban-town-plan-walled-market-town.jpg`. |
...
| **Change the ported urban algorithm toward the owner's town plan (Ruling H) — candidate 1 ruled, genuinely ready; candidates 2/3 already delivered; candidate 4 still gated** | `URBAN_MORPHOLOGY_SCOPE.md` | large-in-name, small remaining | **Moved back here 2026-09-23**, after a scoping pass found 2 of 4 candidates already shipped and Ruling AA (`LARGE_ITEM_RULINGS.md`) settled the approach for candidate 1. (1) **Legible wedge blocks at size (radial arterials + curving cross-streets) — genuinely ready.** Owner ruled 2026-09-23: **extend the radial/`"radial"` branch** to carry the suburb-thinning and wall-gap machinery already built for the organic branch (`wallside.rs`, commits `5d780dc`/`21c6949`), rather than reshaping organic `grow`'s macro-structure — lower blast radius (4 of 29 golden cases are Venus/radial, not the 25 organic ones a `grow` reshape would risk). (2) **Suburbs along the roads outside the gates, thinning with distance — ALREADY BUILT**, verified at the symbol (`growth.rs`'s 90m/Clark-gradient gate) and against a real screenshot (`_wallside_town1.png`) — needs only a documentation note, not a build. (3) **A gap kept against the wall where there is no gate — ALREADY BUILT**, verified at the symbol (`cleanup::clear_fort_zone`'s clear band with wall-lots' faubourg as the only exemption) — same, doc note only. (4) **Perimeter lots with open courtyards at high density** — still gated: needs one structural definition (what "perimeter" and "high density" mean — outermost ring by market distance? a district flag? a rules threshold?) not yet asked. Golden re-baseline authorised under Ruling H; candidate 1's build should record what changed and which of the 4 Venus/radial cases moved, per `cartalith-porting-discipline`. |
