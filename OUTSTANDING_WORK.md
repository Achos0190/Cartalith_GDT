...
| **MAIN-LOOP JOB, fully specified: convert the 16 non-conforming dialog sites** | `DCC_SHELL_SCOPE.md` | medium | **The hard half is DONE. This is application, and it belongs in the main loop rather than a lane** — the budget posture in `SESSION_HANDOFF.md` names this exact case as the largest saving available. **What exists:** `DccWidgets.confirm()` and `prompt()` (the six identical text-only `ConfirmationDialog`s and three identical prompt-with-a-`LineEdit`s collapse into them), and **`DccWidgets.phone_protocol_grade(dlg, host)`** as the check. **The check encodes the RULE, not the slogan, and that distinction is load-bearing:** it measures three PROPERTIES — shaped (`not wrap_controls and borderless`), presented (position zero, size == screen minus the gesture inset, `csf == phone_scale`, min/max cleared), and every visible tappable ≥ 44 dp AND inside the client rect. **A check written against the call list would have failed `modal_present()`, the one path already correct**, because it deliberately skips `phone_fit()`. **It is proven to fail on real shipped code, not on synthetic specimens:** the verifier drove it over four unmodified sites — `right_dock_ctrl::_confirm_revert` grades `fails:shape,present,tap worst=29dp`, while three fixed sites grade `ok` at worst=44 dp, off=0. **Why the lane did not do it:** all 16 sites live in files no lane was granted, and three other lanes were live. **The conversions are specified at their symbols and are mechanical.** **Counts corrected by the verifier: 21 sites at HEAD, not 22; 14 `extends AcceptDialog` windows under `shell/`, not 15** (the 15th is `credits.gd` at the project root, and it is conformant — its dialog is built inline in `app.gd::open_credits()`) |
