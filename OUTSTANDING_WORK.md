...
**120 outstanding items** — 2026-09-21, after closing LOD-D0 (`11936cb`; baseline recorded over 6
worlds, and its findings change how D2/D3 should be graded — see its row). 121 immediately before
...
| **v2.71, half 2 · Woodland becomes an area** — **needs real v2.71 source or an owner ruling, not a same-session port** | `RC_ENGINE_CHANGES.md` §8.2 | medium | **Checked, not invented.** This port has **zero spatial representation for woodland anywhere** — grep-confirmed: the only hit is an abstract `woodland_ha: f64` hectare figure feeding trade/fuel economics in `cartalith-civ/src/trade.rs`, nothing placed. No `assart`/arable-wins-over-woodland rule exists because nothing exists for arable to win against. The RC spec states two *behavioural* rules (arable wins; no street-crossing guard, unlike farmland) but gives no siting algorithm, polygon shape or density constants to port faithfully — unlike most other RC_ENGINE_CHANGES.md rows, which name the exact function. `Cartalith_RC` (the live source) is not reachable from this session's filesystem to check the real `buildFarmland` v2.71 code. **Owner: either (a) this waits as its own milestone until the real source is available, comparable in scope to v2.68's farmland-fringe work, or (b) if wanted now as a non-ported placeholder, note this is a generation-time feature with real siting/conflict data dependencies, not a cosmetic overlay (unlike the port's one precedent for deliberately-new ink, `WALL_TOWER` curtain towers) — a bigger ask than it sounds.** |
