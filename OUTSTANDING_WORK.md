...
**122 outstanding items** — 2026-09-21, after filing six new §2.9 rows for the newly-pulled-in
RC_ENGINE_CHANGES.md content (v2.69, v2.70, v2.71, v2.72 [held, river rendering], v2.73, and the
orogeny/"Himalaya problem" spec contradiction that needs resolving at the source before it can
even be scheduled) — 116 immediately before. That was 2026-09-20, after closing the PLAN header
...
|---|---|---|---|
| **Draw what the town model already generates: trees, gardens, orchards and wells; towers on the wall; inside versus outside the wall** | `URBAN_MORPHOLOGY_SCOPE.md` | medium | **Ruling H, 2026-09-12 — renderer first, so no parity risk: the engine is untouched.** Target: `design/owner-references-2026-09-12/urban-town-plan-walled-market-town.jpg`. **(a)** `hinterland.rs::build_details` already generates garden trees in the block backlands, orchard rows on the agrarian fringe, wells and the market cross, and **`shell/urban_layout_draw.gd` draws none of them** — its draw functions are `draw_layout`, `_draw_farmland`, `_draw_wall` and `_draw_roofs`; the clutter is named only in its header. **(b)** Round towers at the curtain’s vertices: `_draw_wall` draws the ring, spurs and gate markers only. **(c)** Buildings inside the circuit tinted apart from those outside it, as the plan does; the `suburb` district already exists. **Check the layout dictionary carries the detail geometry before drawing it** — generated is not the same as delivered across the bridge. **Sequenced after the GUI rows, tablet first** (owner, 2026-09-12) **PARTIAL 2026-09-13 (verified run):** **round towers at every curtain-wall vertex are COMMITTED** (ink from the owner’s town plan; the reference draws no vertex ornament; 0 px changed elsewhere over 36 renders). **The intramural/extramural roof tint was REVERTED** — `building_district` is not a wall-containment test: `assign_districts` gives market and craftriver before the in-wall test, harbour after it, and the economy retags ignore the wall, so 906 of 2 097 buildings outside the wall were tinted as inside; a tint needs a real containment test (e.g. building centroid in `wall_ring`). **Trees, gardens, orchards and wells need Rust first:** `urban_adapter.rs::run_layout` filters `details` to field/pasture and `urban_bridge.rs::layout_dict` reads only farmland (54 keys over 19 walled layouts, none of them clutter) |
| **A settlement marked Fortified can generate a star fort that is drawn as a stone curtain around its gorge — the bridge never passes the fort trace** | `URBAN_MORPHOLOGY_SCOPE.md` | small | **Found 2026-09-12 while filing Ruling I, and it corrects the question that produced that ruling**, which told the owner star forts never appear because settlements carry no `fortified` trait. **That is false.** `civ_settlement_toggle_trait` — the Place Editor’s trait chips, through `civ_roster_bridge.rs::toggle_trait` — writes the trait; `urban_bridge.rs` reads it as `fortified_trait`; `generate.rs` grants the fort when the town is walled, at least `FORT_MIN` in population and on the `organic` gate scheme. **What is missing is the drawing**: `urban_layout_draw.gd::_draw_wall` branches on `curtain`, `palisade` and `ditch` only, and the comment above its wall constants says the reference’s `bastioned` branch was left out because *"this port’s settlements carry no traits. No town it generates can have one"* — a dashed reason that went stale when trait toggling shipped. **Confirm the bridge actually emits the bastioned trace and its style before drawing it**: mark a large walled town on the `organic` scheme Fortified, regenerate, and read its `wall_style` and `wall_ring`. Then draw it, and correct the comment. **Sequenced after the GUI rows, tablet first** (owner, 2026-09-12) **RE-OPENED AT ITS SYMBOLS 2026-09-13 (run wf52): the drawing cannot be built in GDScript — the bridge withholds the geometry.** Measured live: toggling *fortified* on an organic town above `FORT_MIN` (pop 2 400 stays curtain, 2 600 goes bastioned) flips `wall_style` curtain → bastioned and `wall_ring` from the hull to an 18-vertex gorge (9 throat pairs). But `urban_layouts()` returns the same 51 keys either way, none carrying the fort: `urban_bridge.rs::layout_dict` reads `l.wall` ring/style/gates/spurs/centroid and never `l.wall.fort`, while `Fort { trace, ravelins }` exists only in `cartalith-urban` `fortify.rs`. The reference’s `_umDrawLayout` bastioned branch (v2.11 ~23348-23350) strokes `fort.trace` then `fort.ravelins` and no gates. **So this row needs a Rust bridge change first** (emit trace and ravelins), then the draw branch. **And a live defect the row did not know:** a Fortified town today draws its 18-vertex gorge as a **stone curtain with 2 gate dots and 4 spurs** (493 px at z16), identical on HEAD. The stale *"settlements carry no traits"* comment is corrected. Sequencing note unchanged: after the GUI rows |
| **A third culture profile, shaped on the owner’s town plan** | `URBAN_MORPHOLOGY_SCOPE.md` | medium | **Ruling H, 2026-09-12 — additive, so no golden should move.** `rules.rs` resolves exactly two profiles by name, `"medieval"` and `"venus"`, and the golden fixtures exercise them by name; nothing found pins the count at two. **The knobs the plan needs already exist** on the profile and its street rules: `parallel_street_spacing`, `segment_length_median`, `exploration_start`/`decay`/`minimum`, `pierce_chance`, `subdivision_cap`, `frontage_width_variance`, `plot_depth_variance`, `wall_generation_extramural_share` and `max_wall_generations`. **Why it matters:** `grow` spends a population-derived street budget, so a town near 19 000 lays a lane mesh that shatters its blocks, while the small towns already build lots along block edges with open interiors — the plan’s own pattern. **Prove the additivity** with every existing golden byte-identical after the profile lands, and judge the result on a mid-size seed: the plan shows well under a thousand buildings. Target: `design/owner-references-2026-09-12/urban-town-plan-walled-market-town.jpg`. **Sequenced after the GUI rows, tablet first** (owner, 2026-09-12) |
...
| **The "under 11 px" font sweep measures against a threshold no canvas states — needs an owner LOOK, not a code change** | `design/mcp-2026-09-07/Cartalith Android.dc.html` | small | **Re-framed 2026-09-07 after checking where the 11 px came from, and the answer changes what this row is.** **The finding itself is sound:** a fixed `fs=10.0` label — *"no committed route selected"*, `journey_planner_view.gd:1509`/`:4467` — measures under 11 px on **15 of 23 screens at 1440×3168 and 19 of 23 at 1080×2400**. Both densities, because a fixed size scales with nothing. **But 11 px is not a design floor. It is a SURVEY threshold** from `GUI_GAP_REGISTER.md:7322`, a phone-composition audit counting small text (*"the data manager’s body labels are 39 of 69 under 11 px"*). Nothing in any canvas or ruling states it. **And the canvases draw smaller than that everywhere.** Measured today: the Android canvas uses **9.5 px 57 times, 10 px 47 times, 9 px 14 times and 8.5 px 3 times**; the PC canvas draws 8.5 px and 10 px too. **`FS_TINY := 10` is canvas-conformant**, and `phone_fit()` does scale explicit overrides by `unit`, so on a 1080-wide phone 10 canvas px lands near 26 physical px. **So raising these to 11 would be a DEVIATION FROM THE REFERENCE**, and parity with the canvases is the owner’s standing definition of done. **This is the same class as the invisible OFF switch track** (ruling C, 2026-09-07): where the shell faithfully draws what the canvas says and the result is questionable on glass, **the canvas is what changes, not the shell.** **What this row now needs is an owner LOOK on the device**, not an edit — and it is worth noting they have used the app all day and reported the planner, the drawer, Preferences, the file browser and the thumbnails, **but never text size.** Do not "fix" this by raising a font size |
...
| The vector river overlay | `FUNCTIONAL_CONTRACT.md` cap. 6 | small | The one leg of the old SDF row still genuinely unbuilt: `map_overlay.gd` has no `drawRiverWays` equivalent (`grep -n river map_overlay.gd` returns only settlement-badge prose and the new faith lines) **BUILT AND REVERTED 2026-09-13 (patch kept: `.claude/resume-2026-09-12/river_overlay_reverted_2026-09-13.patch`).** The engine side already exists (`get_rivers(min_order)`, `engine_bridge.gd::rivers()`), and the vector draw worked (1 079 runs, 18 740 px inked). **Refuted on three counts:** it drew over the debug and info views (19 007 px over *temp*) where the reference’s `drawRiverWays` runs only when `dbg==="off"` and `mode==="biome"` (v2.11 ~8667, ~11461); it had no off switch; and the raster river tint still drew underneath. **Next:** re-apply gated on biome mode with debug off, add a Layers switch, and suppress the raster tint where vectors draw |
| Re-derive which pack sections the live map actually composites | `FUNCTIONAL_CONTRACT.md` cap. 6 | small | **Owner ruling 2026-09-04: measure before ruling again.** For every section name the pack-import warning can emit, establish whether `pack.rs` composites it, and report the true unused set. Two premises about this warning have already failed on contact — first that biomes/terrains were unused (false), then that `trait` was the only remaining true clause (also false: `composite_map_icons`, `pack.rs:470`, draws settlement and poi). **Audit-only, closes nothing by itself**; the owner rules once it lands **RE-DERIVED 2026-09-13 (a lane’s "already done" was refuted):** the map composites only `pack.icons` art (`composite_map_icons`, `PACK_ICON_SLOTS`). **`structures.settlement`, `structures.poi`, `custom` and `seamarks` are undrawn too** (`pack.rs` `LoadedPack` doc: not decoded "because nothing draws them"), yet the unused-sections warning names only `trait` — `manifest.rs`’s comment above `unused.push("trait")` says widening the list is a behaviour decision the owner has not made. **Owner call:** name all four in the warning, or draw them |
...
| **LOD-D0 · Zoom-sweep harness** — measurement only, no pixel change | `LOD_DETAIL_SCOPE.md` | small | **Filed 2026-09-13 from the scope.** A repeatable continuous-zoom sweep over three seeds that records detail per screen pixel, pops, holes and seams. It is the baseline every later LOD milestone is graded on. It changes no pixels, so it is the one LOD piece a spare lane may run between GUI batches, provided it does not overlap a GUI verifier’s tree. See `LOD_DETAIL_SCOPE.md` for the design, the acceptance numbers and the owner questions with their defaults. |
| **LOD-D1 · Port `renderBiomeTileRGBA` as a pure engine function** (Ruling K, step 1a) | `LOD_DETAIL_SCOPE.md` | large | **Absorbs the engine half of the former §2.7 row *"DIAGNOSED: LOD tiles carry a SHADE RATIO, not colour"*.** That row measured that `shell/lod_tile.gdshader` multiplies a scalar tile ratio into `map_view.texture`, so a deeper level cannot be sharper: detail per screen pixel fell **0.0220 → 0.0017** across zooms 1→40, and hiding the LOD layer at zoom 16 moved mean \|dL\| by only **0.0083**. The level ladder itself is live (2/3/3/4/5/6/7 at zooms 1/2/2.5/4/8/16/40). `renderBiomeTileRGBA` (v2.11 lines 11668–11779) exists in this port only in comments; supersampling a shade ratio sharpens nothing. Waits on D0. See `LOD_DETAIL_SCOPE.md` for the design, the acceptance numbers and the owner questions with their defaults. |
| **LOD-D2 · Colour tiles on screen, and the sharpness bar** (Ruling K, step 1b) | `LOD_DETAIL_SCOPE.md` | medium | **Absorbs the rest of the shade-ratio row.** The bridge returns RGB tiles, the shader draws them instead of a ratio, and the saved pyramid’s size is recomputed. Acceptance is measured on the D0 harness. Waits on D1. See `LOD_DETAIL_SCOPE.md` for the design, the acceptance numbers and the owner questions with their defaults. |
| **LOD-D3 · Continuous transitions: parent fallback and a colour-space morph** | `LOD_DETAIL_SCOPE.md` | medium | **Filed 2026-09-13.** No holes while a child level loads, and a CDLOD-style morph in colour space so a level change does not pop. Acceptance: three seeds, two grid sizes, zero pops and zero holes on the D0 harness. Waits on D2. See `LOD_DETAIL_SCOPE.md` for the design, the acceptance numbers and the owner questions with their defaults. |
| **LOD-D5 · Scale-aware shading weights, and hydrology that resolves** | `LOD_DETAIL_SCOPE.md` | medium | **Filed 2026-09-13.** Shading weights chosen by physical scale rather than GUI zoom; check at the symbol whether `add_zoom_detail`’s frequency ignores cell size; rivers that resolve at deep zoom. Waits on D2 (the scope orders it after D4). See `LOD_DETAIL_SCOPE.md` for the design, the acceptance numbers and the owner questions with their defaults. |
| **LOD-D6 · Tile synthesis off the main thread, profiled per device** | `LOD_DETAIL_SCOPE.md` | medium | **Filed 2026-09-13.** Worker-thread synthesis with a tile budget, maximum level and cache size per quality tier. May start straight after D2 in its own lane if a phone regression appears. If the harness shows it, it takes over the device half of §2.7’s zoom-notch row (a 0.87 s frozen frame per notch on the 6T); that row stays open until then. See `LOD_DETAIL_SCOPE.md` for the design, the acceptance numbers and the owner questions with their defaults. |
...
| **`File ▸ Recent worlds` leaves show a filename where the canvas shows the world** | `DCC_SHELL_SPEC.md` §2.1 | small | **Found by the menu audit 2026-09-07.** Ten leaves, cap correct (`DccSettings.MAX_RECENT := 10`), each labelled with the **filename only** (`__diagreview_project__.zip`) and the full path on the tooltip. The 2026-08-31 canvas draws `VHAREN REACH — 129384 · 5 d ago` — world name, seed, age. **The secondary-text half is platform-forced** (a `PopupMenu` has none, so the path must be a tooltip); **the label is not** — it could carry the name and seed today. A buildable gap, not a limit **PARTIAL 2026-09-13 (run wf51).** Leaves now use the canvas’s separators and terse age (`cartalith-dcc-parts.js:136`, `VHAREN REACH — 129384 · 5 d ago`: a legacy fixture renders `mv_legacy_seed — 129384 · 5 d ago`), and a missing file says `<name> — file not found` with a true tooltip instead of *never opened*. **Two halves remain.** **Name:** `project.json` `world` carries grid_height, grid_width, map_width_km, origin, sea_level, seed and wrap_x — no name, so the label stays the filename until the format gains one (a `cartalith-io` change). **Seed:** every native save DOES carry `project.json` `world.seed`, but `open_project_dialog.gd::project_meta()` reads only the legacy `params.json` `state.tect.seed` — so no native save shows a seed here, on the welcome tiles or in the phone picker (*seed unread*). That function already parses `project.json`, so the fix costs no extra zip open. Popup build cost measured under 0.1 ms **SEED HALF FIXED 2026-09-13 (batch wf53):** `project_meta()` reads `project.json` `world.seed` first and falls back to the legacy `params.json` key — a real native save (seed 42424) now shows its seed in Recent worlds, welcome tiles, gallery tiles and phone picker cards (HEAD: *seed unread*); a real legacy save is unchanged. **Only the name half remains**, and it needs the save format to carry a world name |
...
| **Multi-ridge orogenic belts ("the Himalaya problem") — resolve a contradiction in the spec before scheduling** | `RC_ENGINE_CHANGES.md` §6b.2 vs §9 | large | **Two sections of the same file disagree about whether this ever shipped, and that has to be settled at the source, not guessed from prose.** §6b.2 describes `buildOrogenyField`'s collision branch, on the DCC line's v2.36, as becoming **"a stack of 4 sheets across `1.55 * halfBelt`"** with detailed measured figures (per-station crest counts, a resolution floor at 8 cells, a real-km width constant `orogenyWidthScaleK`, a named test harness `tests/perf/probe_orogeny.js`) — reads as shipped, tested code. §9 ("Deliberately NOT in the HTML — do not port these") calls the same multi-sheet thrust-belt profile **"prototyped and calibrated as a runtime override only — it is not in any shipped file and nothing was committed. Do not implement it from this note."** Both cannot be true of the same feature. **Neither this machine nor this port currently has `Cartalith_RC` available to check** (present 2026-09-17 per `CLAUDE.md`'s Constraints section, absent as of this row) — re-open at the actual source before writing any code, not from the spec alone. **What IS established, checked at the symbol:** this port already has `cartalith-terrain::build_orogeny_field`/`smooth_orogeny` (`OrogenyParams`, no multi-sheet fields), golden-tested against v2.11's own shape — reference HTML lines 2965-3071, "T2+T3: tectonic feature kernels", one Gaussian ridge plus satellites per boundary type (`golden_parity_orogeny.rs`). That part is real and already ported. **Both v2.35 (§6b.1, the boundary-tracer's junction predicate — raw 8-neighbour count wrongly used where the crossing number was needed) and v2.36 are gated behind `state.tect.tectonicGraph`, which defaults OFF in the HTML itself** and is not exposed in this port at all — so even the shipped reference app, at its own defaults, never draws whichever of these is real. **How this could fit "the new system" (EF-0/EF-1/EF-3, `ELEVATION_FIELD_ARCHITECTURE_RESEARCH.md`):** a richer coarse orogeny field is a change to the stage EF-0's tile refinement samples *from*, not something EF-0 itself would do — Everest-scale multi-ridge structure is macro, not a zoom primitive. But EF-3's erosion-consistent tile detail would visibly benefit from more structured coarse ridges to converge toward, and EF-6's boundary tracer (`cartalith-spatial::contour`, already built) may already solve §6b.1's crossing-number problem correctly as a side effect — worth checking directly before writing a second tracer. **Owner: resolve the §6b.2/§9 contradiction (get `Cartalith_RC` back on this machine and check the real DCC-line file, or rule that it stays unresolved and this is filed as "maybe never shipped") before any of it is scheduled** |
| **v2.69 · Tile refinement must not cross sea level** | `RC_ENGINE_CHANGES.md` §8.1 | medium | **Directly relevant to this port's own EF-0/EF-1 (commit `2373c08`) — check whether the same bug was inherited before assuming it wasn't.** The HTML found two defects in how a deep-zoom tile reconstructs the coarse field near a coast: (1) a settlement's water mask sampled the coarse field bilinearly while the tile renderer had moved to Catmull-Rom, a 17x land/sea disagreement; (2) `amplifyRegion`'s detail band tapers going down from the shelf toward sea level and does nothing going up, so a land pixel just above sea level could get pushed under — 7.6:1 asymmetric, worsening with zoom depth (0.40% of pixels at z=2, 8.56% at z=8). Fix was to clamp the *excursion* toward sea level at half the remaining headroom, not clamp the result. **Check `cartalith-engine/src/elevation.rs` (EF-0) and `cartalith-hydrology/src/tile.rs` (EF-1) for the same asymmetric-taper shape** — both do coarse-to-fine reconstruction near arbitrary elevations including sea level |
| **v2.70 · A flat limited-palette "Village map" style** | `RC_ENGINE_CHANGES.md` §8.2 | small | Pure rendering, opt-in, moves no generated value (`hash_gen1.js` identical). The HTML's shape is worth copying rather than rediscovering: one function colours land, one colours water, each already ends in a chain of per-pixel style steps gated on their own flags, so a new style costs one flag + one step in each chain + one preset + one button, and every surface that draws a map (including export) picks it up for free. Quantise the *lit* colour, not the biome; turn hillshade off in the recipe (applied before the ramp, would put the gradient back) |
| **v2.71 · Nothing the settlement layer draws may sit on water; woodland becomes an area** | `RC_ENGINE_CHANGES.md` §8.2 | medium | Not simulation, `hash_gen1.js` identical. A 22 m water mask (sea + lakes + river band) has always existed on the settlement adapter but never reached the renderer — the model record handed to drawing code is deliberately function-free. Fix is a clip using the mask (run-length encoded), not a geometry fix: zero blocks/parcels/buildings/street centrelines cross water, only the stroked street *width* does. Release the clip for bridges/fords. Woodland changes from scattered points to a polygon with points as texture inside it; two rules — arable wins over woodland (assarting), and woodland gets no street-crossing guard (unlike farmland) |
| **v2.72 · Raster river geometry, antimeridian wrap + a detection-ease used as a display threshold** — **HELD, river rendering, owner confirmation requested 2026-09-21** | `RC_ENGINE_CHANGES.md` §7.13, §8.1 | large | Filed here rather than scheduled: this is river-rendering work and the owner asked to be checked with before any of that is tackled. Two defects: (1) a receiver-tree stem wrapping in X (world/toroidal mode) paints a river clean across the map — the HTML has a polyline splitter (`splitRiverPolylines`) applied at three of four sites since v1.29, and the raster path (the one that actually draws at the default) was the missed fourth; (2) the same 16x coarse-map detection ease that decides whether a stream exists at all was also spent on what the renderer *draws*, cubing the drawn channel density at world extent (13.02% of the map painted as channel at 40 000 km vs 4.27% at 800 km) — §7.13 generalises this as "a detection threshold is not a carve threshold is not a display threshold," the same shape as v2.57 (carve) and this row (display) sharing one constant. See also the still-open, pre-existing §6j/§6l/§6m/§6n river sections this spec already documented — porting status for all of them is unestablished (this section's own survey row) |
| **v2.73 · A village green as a distinct plaza kind; a footpath class whose source never ran on the default profile** | `RC_ENGINE_CHANGES.md` §8.2 | medium | Not simulation, model hash identical (covers graph/blocks/parcels/buildings, not plaza/details/paths, by construction). A green is a plaza with a different *purpose* (common land, no market right, no cross), not a smaller one — same three `addStreet` calls, a `kind` field and a branch on the chartered-town population threshold already used elsewhere. The footpath class (alley-privatisation, removes cart routing but keeps foot traffic) had a source that never fires on the shipped default (`deadEndBias = 0`); the fix adds an always-on source from features already placed (church, wells, pond) to nearest street, gated so a path never crosses a building footprint (11.4% did before the fix, 0.0% after) |
...
|---|---|---|---|
| **Refine detail: does the 2026-09-07 canvas reverse the 2026-09-05 ruling that put it on the WORLD rail?** | `LARGE_ITEM_RULINGS.md` (2026-09-05, item 4) | small | **The one citation disagreement that is real shell drift, and its precedence is ambiguous, so it is the owner’s.** Ruling item 4 (2026-09-05): *Atlas "Refine detail for the current view" moves to the WORLD rail beside Bake & finalize* — and the shell does that (`app.gd`, `DccWidgets.action(row, "Refine detail", …)`). **The newer canvas does not draw it:** "refine" appears in none of the three 2026-09-07 canvases, and the DCC Environment canvas’s GENERATE panel says *"NOT A GENERATION STAGE — … tiled LOD, atlas → Preferences ▸ Tiles & LOD"* — which routes atlas work back to exactly the place the ruling moved it out of. `CLAUDE.md`’s *"an owner decision is newer than any canvas"* was written for a decision that postdated its canvas; here the canvas postdates the ruling. **Ask: keep Refine detail on the WORLD rail, or move it back under Preferences ▸ Tiles & LOD?** **Where it lives today, for the owner’s left-rail re-sort:** not the left rail — the **WORLD tool-options bar**, built by `app.gd::_tool_options_generate()` beside *Bake ALL & finalize*. That bar is `tool_options_row`, which the tablet COMPOSITION lane dispatched 2026-09-12 is reworking |
...
| **The in-session tile cache is not invalidated by a sculpt** | `GUI_GAP_REGISTER.md` | small | **Reported by the LodTiles lane as a symptom, with where it looked and no cause asserted.** `viewport_host.gd` clears `_lod_tiles` in `refresh()` and in `_set_lod_active(false)`, but `world_workspace.gd`'s sculpt-commit path **deliberately does not call `refresh()`** — it assigns `map_view.texture` directly and says why in its own comment. So a commit at a fixed zoom can leave **pre-sculpt relief on screen**. Separate from the archive-side staleness above and not fixed by it **PARTLY FIXED 2026-09-13 (commit on this branch):** `world_workspace.gd::_on_sculpt_commit` now calls the new `ViewportHost.invalidate_lod_tiles()` — windowed probe `_sculptlodcache_probe` PASS on the tree, FAIL on HEAD; WORLD dock census identical (944 = 944). **The row stays open for the other paths, measured stale by the verifier:** `right_dock.gd::_on_sculpt_stack_commit` (Commit to map) and `app.gd::undo_last` stay stale through +240 frames; by code also `tool_bar.gd::_on_sculpt_commit` (Commit chip), `app.gd::redo_last`, `menus.gd::_redo_last`, `right_dock.gd::_do_revert`, `dcc_shell.gd::_phone_revert_history`, `world_workspace.gd::_run_erode` and three paint commits — all write `map_view.texture` without invalidating. **Fix once where they converge** (or have each call `invalidate_lod_tiles()`), probe every path. Cost: a synchronous rebuild hitch of about +80 ms at 48 live tiles (verifier, under load), not the +40 ms the lane reported. Comment nits: both new comments cite `GUI_GAP_REGISTER.md` for this row (it lives here), and `invalidate_lod_tiles()`’s doc names `_run_erode()` as a caller it is not |
| **LOD tiles go stale after undo, redo, revert, erode, paint and the other two sculpt-commit buttons** | `OUTSTANDING_WORK.md` | small | **Found 2026-09-13 by the verifier of the tile-cache fix.** `invalidate_lod_tiles()` now runs on `world_workspace.gd::_on_sculpt_commit` only. Measured stale through +240 frames: `right_dock.gd::_on_sculpt_stack_commit` (Commit to map) and `app.gd::undo_last`. By code: `tool_bar.gd::_on_sculpt_commit`, `app.gd::redo_last`, `menus.gd::_redo_last`, `right_dock.gd::_do_revert`, `dcc_shell.gd::_phone_revert_history`, `world_workspace.gd::_run_erode`, and three paint commits. Every one writes `map_view.texture` directly. Fix at the point they converge (or call `invalidate_lod_tiles()` from each), with a frame-series probe per path; mind the ~+80 ms synchronous rebuild at 48 tiles |
...
| Item | Owns it | Size | Blocker |
|---|---|---|---|
| **LOD-D4 · Ice and snow from fields that already exist** (Ruling K, step 2) | `LOD_DETAIL_SCOPE.md` | medium | **Waits on LOD-D1 and D2 (Ruling K, 2026-09-12: sharpness first, then ice). Absorbs the former row *"Draw ice from fields that already exist, toward the Aletsch zoom target"*.** Target, read top-down with 3D still parked: `design/owner-references-2026-09-12/lod-zoom-target-aletsch-sentinel2.jpg`. **Inputs that already exist:** `glacial_kernel`’s carved U-shaped troughs, the `passes.glacial_snowline` parameter, and the snow and rock material fractions. **The glacial pass is off by default** (`WorldParams::defaults` sets `passes: ErosionPassParams::off()`), so the troughs are absent on a default world. **Not chosen:** a glaciation model (ice extent, flow, moraine stripes). Snow follows temperature, precipitation, slope, aspect and curvature, never an elevation cutoff (`docs/research/lod extra info.md` §16-17). Can run beside D3 if the lanes do not share `viewport_host.gd`. See `LOD_DETAIL_SCOPE.md` for the design, the acceptance numbers and the owner questions with their defaults. |
