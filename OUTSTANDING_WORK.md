...
| **Dialog protocol: TEN of sixteen converted; six remain, all in two files** | `DCC_SHELL_SCOPE.md` | medium | **All in the MAIN LOOP 2026-09-07, no lane.** `right_dock::_confirm_revert`, `faction_roster_window::_confirm_remove`, `place_editor_window::confirm_delete`, `civilization_workspace::_confirm_destructive` (**a shared helper — every destructive action in that workspace came with it**) and `::_tl_show_confirm`, `vault_window::_compare_dialog` and `::_preview_dialog` (**both had the protocol BACKWARDS**), `journey_planner_view::_save_journey` and `::_capture_preset`, `cartography_workspace::_prompt_label_name`. `journey_planner_view.gd` now holds **zero** hand-built dialogs. **Three shapes, and the third is the finding worth keeping.** `confirm()` takes the text-only sites; `prompt()` takes the ones with a field; and `_prompt_label_name` takes **neither** — it was converted by hand because `prompt()` calls back only on a non-empty stripped string while that site passes `edit.text` straight to `label_create()` with **no empty guard**, so an unnamed label is creatable today. **Whether that should stay possible is a question about labels, not about dialogs**, and converting would have answered it silently. It also wants a `placeholder_text` where `prompt()` offers an `initial` value — a different affordance. **Left as an open question at the site rather than decided in passing.** **The ordering trap, hit twice and recorded both times:** `phone_window()` sets `ok_button_text = "Close"`, so any site with its own verb (*"Write to Markdown"*, *"Create"*) must set it **after** the call or the primary action is silently renamed **on phones only**. `_phoneproto_probe` re-run after every conversion: `fail=0`. **SIX REMAIN, in two files:** four in `menus.gd` (`_open_pack_metadata`, the atlas-clear near :4221, `_prompt_save_layout`, `_prompt_forget_layout`) and `shortcuts_dialog.gd` (which has `phone_window` at :118 but bare `popup_centered()` at :160 and :170, and no `phone_fit` anywhere in the file) |
