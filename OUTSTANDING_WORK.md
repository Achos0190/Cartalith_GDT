...
**86 outstanding items** — re-derived mechanically 2026-09-06, **after
...
| **The phone's year scrub draws 96 × 84 px against a 115 px floor** | `DESIGN_HANDOFF.md` | small | **Found 2026-09-06 by widening a walk, and reported rather than fixed — the file was not that lane's.** `_phone_sim_slider` in `dcc_shell.gd::_build_phone_sim_strip()` measures **36.6 × 32.0 dp** against the 115 px floor its five siblings all meet. It is `SIZE_EXPAND_FILL` but **starved**: play plus three speed pills plus close are 115 px each in one `HBox`, leaving it the remainder. **Found only because the walk was widened** to `_widthfloor_probe`'s own tappable list — a `BaseButton`-only filter reported 5 targets where there are 6 |
...
| **The 14 undesigned surfaces: ALL DESIGNED, 12 BUILT — one to build, one waiting on the owner** | `DESIGN_HANDOFF.md` | small | **Corrected 2026-09-06; this row had been the standing GUI gate and was stale.** The owner supplied **three Claude Design canvases** covering exactly the 14 — batch 1 (ED-05, DM-10, HE-02, WI-01, SH-01), batch 2 (CA-07, CA-08, CA-09, JP-05, WW-03), batch 3 (PR-07, PR-10, PR-15, HE-01/HE-03) — each with a per-screen control inventory. **Twelve are built and verified:** Find on map · Data-manager Checks · Colour (the warning, after the canvas's "sRGB and nothing else" premise was found false) · Tiles &amp; LOD (ruling 29 shrank it; `Size cap` was already live) · Keyboard shortcuts (already built; a data-loss bug found and fixed) · Save layout (seeded) · Docs &amp; Report an issue (the diagnostic dump per the ruling) · Calculation trace · Sculpt falloffs · Style presets · Layer search (**no screen needed** — both footer tabs were dead names) · Rail expansion. **Remaining: CA-07 label font role, designed and not yet built** — and **PR-15 units, which needs an owner decision, not work.** **Five of the canvases' "Exists today" claims failed against the code**, twice turning a build row into "already shipped", so open the symbol before building the last one |
