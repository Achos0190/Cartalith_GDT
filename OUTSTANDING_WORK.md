...
| **A 3-line-wrapped pipeline-stage-row state label draws visually above its own row's number/name columns** — found in passing while verifying the row above, pre-existing, not this fix's fault | `cartalith-native/godot-project/shell/workspaces/world_workspace.gd` | small | The stage-9/10 state label wraps to 3 lines in the dock's width (an `HBoxContainer` cross-axis alignment behaviour when a wrapping `SIZE_EXPAND_FILL` sibling is taller than its fixed siblings), and its first wrapped line renders above the row's own number/name text rather than beside it — visible in this session's own verification screenshot. Not introduced by the wording fix above (the new string is shorter than the old one, wraps to the same or fewer lines) and out of that fix's scope. Whoever next touches this row's layout should fix the cross-axis alignment. |
