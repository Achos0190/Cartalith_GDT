...
| **The switch is the wrong colour ON THE OWNER’S OWN PALETTE — diagnosed to the pixel, fix is two lines, NOT YET APPLIED** | `design/mcp-2026-09-07/Cartalith DCC Environment.dc.html` | small | **Fully diagnosed 2026-09-07 and independently reproduced by the verifier on a real Vulkan context. The lane changed NOTHING, deliberately and correctly** — the fix belongs in `dcc_widgets.gd`, which another live run owned that batch. **This is a ready-to-apply row, not an investigation.** **THE FIX, verified before being declined:** two lines in `dcc_widgets.gd::_paint_switch(cb)` — `cb.add_theme_color_override("checkbox_checked_color", Color(1,1,1))` and the same for `checkbox_unchecked_color`. **Godot 4.7 modulates all four icon slots from just those two**, so two lines cover four; measured, the disabled knob returns to exactly `#5f6468` on `#191c1e`. Per-instance is REQUIRED — the seven bare `CheckBox.new()` sites (`journey_planner_view.gd:2444/2482/2487`, `travel_library_window.gd:844/868/900/931`) need the tint kept — and `_palette_watch` already re-asserts overrides on every flip. White is a literal, not a token, so nothing goes stale. **WHAT IT LOOKS LIKE TODAY on `light`, the palette this machine boots:** the OFF switch renders **`#666963` mid-grey where the canvas draws a `#f4f2ee` near-white pill**; ON renders a `#0a0701` knob on a `#692801` track where the canvas draws `#111210` on `#a4650f`. Dark is milder and still wrong (`#cc9644` knob, `#c56815` track). The arithmetic closes end to end: `#f4f2ee × #6b6f6a = #666963`. **Why it could not be fixed at theme level, proven three ways rather than asserted:** (a) the same two items must stay `--acc`/`--dim` for the seven bare checkboxes whose stock glyph they are FOR, and **one multiply cannot be both identity and a tint**; (b) every entry point `_paint_switch()` uses is a scalar-in/value-out static with no node reference, so a `dcc_theme.gd` helper would be dead code; (c) a theme-level white is destroyed by the palette flip — measured. **The 8 enumerated, and item 7 is the instructive one:** it is supplied by **`Button`, not `CheckBox`** — theme lookup walks the `ClassDB` chain, so the resource’s Button block leaks into CheckBox. Items 3–6 carry the canvas tokens verbatim and are correct. **Scoping caveat the verifier added: `dark_theme.tres` holds 47 colour entries across 15 classes; the 8 is 8-within-the-four-input-widget-classes** |
...
| **Preferences: the DESKTOP half is done, the PHONE half is one line and was never built** | `docs/DCC_SHELL_SPEC.md` §2.5 | small | **Owner, 2026-09-07: *"in the preferences menu we should show what options have been selected. For the preferences I’d say make the text of the selected option bold in the overview."*** **DESKTOP — DONE and proven at the pixels.** 10 of 15 value groups now stamp their current value onto the parent row (`menus.gd:2604`, separator pinned to the file’s own pre-existing three-space vocabulary; mutants to two spaces and to `" | "` both killed). Measured independently by the verifier: HEAD gave **0 of 15**, so the tracking the earlier version of this row claimed already existed was checkmarks INSIDE each submenu — invisible without opening the submenu, which is exactly what the owner asked for. Five groups still blank: `GpuDevices`, `LightingRig`, `ReliefExag`, `AtlasCache`, `LodDebug`. **BOLD is genuinely impossible on the desktop `PopupMenu`** — measured, not reasoned: 25 `set_item_*` methods, **zero** naming font, weight or style; the only font levers are whole-popup theme overrides that bold every row. Literal bold there needs `PopupMenu` replaced by a custom-drawn list. **PHONE — NOT BUILT, and this is the half the owner was actually looking at.** The lane answered "not achievable" and I passed that to the owner; **the verifier refuted it.** The phone Preferences screen is **not a `PopupMenu`**: `phone_menu.gd::_fill_prefs` expands each group through `_chips()` into `_chip()`, **a `Button` that already carries `add_theme_font_override("font", DccTheme.mono(0))`**, and `DccTheme.mono(spacing, medium)` already has a medium weight (`FONT_MONO_MED`). **Bolding the selected option there is one line.** **The lesson, and it is the expensive one: the lane measured the widget the BRIEF named rather than the widget the OWNER was touching, then inverted the brief on that basis.** The owner said on the device; nothing should have been declared impossible from a desktop class |
...
| **OWNER-REQUESTED: the picker tiles should show a small version of the MAP, not a colour gradient** | `design/mcp-2026-09-07/Cartalith DCC Environment.dc.html` | medium | **Owner, 2026-09-07, on the load/create world screen: *"Currently the tiles are with a color gradient, I’d like that to be a smaller version of the map."*** **The code already anticipated this exactly.** `open_project_dialog.gd`’s header says *"Thumbnails are generated, not stored … each tile takes a radial gradient hued from a hash of its own path — stable per world, distinct between worlds, and honest about being an identicon rather than a render. When the port grows a thumbnail on save, **this is the one function to replace**."* That function is `OpenProjectDialog.identicon(path)` (`:1259`), a `GradientTexture2D` at 96×72, and it is the **single** call site for both surfaces — the desktop gallery and `phone_project_picker.gd:258`. **THE PREMISE OF THAT COMMENT IS WRONG, AND THAT IS THE FINDING: no save-format change is needed.** The comment says *"a `.zip` save carries `params.json` plus raw fields and no preview image, so there is nothing to show."* But `SAVEFILE_COMPAT.md` §8.1 makes **`rasters/heightmap.f32` a MUST**, and §7 makes **`world.sea_level`, `world.grid_width` and `world.grid_height` MUST** — all four refuse-if-absent. **Every conforming archive already contains everything needed to draw its own coastline.** So this needs no new zip entry, no `format_version` bump, and no owner ruling on the format. **Do NOT reach for `build_color_texture()`.** It is the real map renderer and it needs the whole engine state loaded — a full project load per tile, for a gallery of them, is not acceptable. The cheap path is the one the LOD tiler already uses: **`render_height_tile_rgba`’s hypsometric ramp**, cited in `viewport_host.gd:379` as *"the reference’s"* ramp. Read the heightmap, stride-sample it down to tile size, run it through that ramp against `sea_level`. **Land/sea and the ramp are what make it read as this world**; the material palettes are not. **Cache it.** Generating on every gallery open re-reads and re-inflates a full-resolution f32 raster per tile — for a 2048×1311 world that is ~10 MB inflated to draw 96×72 pixels. Key a `user://` cache on path plus mtime; the tile already reads mtime for its *"edited 4 min ago"* caption, so the invalidation key costs nothing. **Keep `identicon()` as the fallback and say so at the symbol** — an archive can be damaged (§6.4a’s damage ladder) or mid-write, and a picker that throws instead of drawing a tile is worse than one drawing a gradient. **Measure the gallery open with a real set of saves before and after**, and report it |
| **Selected text in every text field is unreadable — 1.26:1 measured** | `design/mcp-2026-09-07/Cartalith DCC Environment.dc.html` | small | **Found by the COLOURS lane 2026-09-07 as a byproduct of enumerating item 8, and measured rather than eyeballed.** `font_selected_color` is **`#ffffff` straight from Godot’s DEFAULT theme**, which nothing in this shell remaps. Under the owner’s `light` palette the selection band draws `#d3bb99` and the brightest pixel over it is `#f6f1ea`: **contrast 1.26:1.** Highlight a word in any field and it disappears. Reaches `well()`, the `number()` SpinBox `LineEdit` path and the search-field path. **Item 8’s own value (`selection_color`, `--acc` at a=0.35) is fine — its unremapped PARTNER is not**, which is why an enumeration that only checked whether values matched tokens would have passed it. **NEEDS A DECISION, so do not guess:** the canvas draws no selection anywhere, and the obvious candidate is directional — `--ink` clears **9.8:1 on light but only ~2.6:1 on dark**. **Raising the band’s alpha may be the better half of the fix.** Bring both numbers to the owner rather than picking one |
| **`DccTheme.remap()` turns opaque white into black, and opaque black into white** | `DCC_SHELL_SCOPE.md` | medium | **Latent, found 2026-09-07 while proving why the switch fix cannot live in the theme, and deliberately NOT touched — the blast radius is every palette flip shell-wide.** Measured: **`DccTheme.remap(Color(1,1,1,1), DARK)` returns `#000000`.** **Mechanism:** `remap()`’s RGB-only fallback matches on RGB while ignoring alpha, and DARK `line` is `Color(1,1,1,0.10)` — so an opaque white matches the `line` token and comes back as LIGHT `line`’s RGB at alpha 1. Mirrored under light for opaque black. **Candidate guard, not yet a decision:** skip the alpha-derivative pass when the token’s alpha < 1 and the value’s is 1. **This needs its own pass with a before/after palette-flip census**, because every colour that survives a flip today does so through this function |
...
| **Both dock headers are built to a `34px` that the canvas does not contain — and the false comment is duplicated** | `design/mcp-2026-09-07/Cartalith DCC Environment.dc.html` | small | **Found by DOCK-CONFORM 2026-09-07, reported at the symbol rather than fixed because `dcc_shell.gd` was owned by a live run, and confirmed by the verifier as *understated*.** **The canvas contains ZERO occurrences of `height:34px`.** ENV:948 gives the right dock header `gap:8px;padding:8px var(--pad) 6px` = **38**; ENV:305 gives the left `padding:8px var(--pad) 0` = **32**; the chevron is `--ctl × --ctl` = 24×24 (ENV:949) and the title is `flex:1;text-align:right` (ENV:950). **There is no border-bottom in the block.** **The comment asserting *"the canvas gives both dock headers `height:34px`"* appears TWICE** — `dcc_shell.gd:3766` **and** `:3839`. **So the two headers are not even the same height in the canvas, and a single wrong figure was pasted onto both.** Fix the geometry and both comments in one pass |
| **REGRESSION SHIPPED IN `e830112`: every numeric field draws as a tiny glyph in an oversized empty box** | `design/mcp-2026-09-07/Cartalith Android.dc.html` | medium | **Flagged by BOTH build lanes independently and confirmed on glass by the verifier 2026-09-07, and it is on the very screen the owner complained about.** On the planner party form, Group size `4`, Hours/day `8.0`, Trade cargo `0` and Supplies `7` all render as a minuscule glyph in the **bottom-right corner** of an oversized empty box; the New World dialog’s **Seed and Width lost their input frames entirely**. **Bisected by comparing build `d33a2546988c` (installed 07:18, correct) against the current tree.** **It shipped in `e830112` "Inputs draw the canvas now" — the last commit to touch `dcc_widgets.gd`, and the one whose message says the inputs were proven against a hostile theme-overwrite test.** **That test proved the styleboxes were independent of the stale resource. It did not prove they were RIGHT** — zero styleboxes moving under a hostile rewrite says only that nothing inherits; it says nothing about content alignment, vertical centring or minimum size. **This is the highest-priority GUI row: it is a regression, it is visible, and it is ours** |
| **`CREATE WORLD` is clipped below the dialog edge on first open — a first-time user cannot reach a world** | `design/mcp-2026-09-07/Cartalith Android.dc.html` | medium | **Found by the planner lane while navigating from a cold launch, reproduced by the verifier 2026-09-07.** On the phone New World dialog the `CREATE WORLD` button sits **below the dialog’s visible edge**; a tap on the visible orange sliver at (714, 2100) left the screen **pixel-identical**. The dialog must be scrolled first, and nothing indicates that. **Every first-time user hits this before reaching a world**, which makes it a first-run blocker rather than a cosmetic clip. **It was found only because the lane was forbidden from calling a function to navigate** — a probe that opens the dialog and presses the button by name cannot see it |
...
| **On phone, chip groups draw their value twice** | `ANDROID_UI_SPEC.md` | small | **Found and photographed by the verifier 2026-09-07, created by the Preferences readout work.** `phone_menu.gd::_rest_of` renders the stamped parent-row text and `_expand_sub` separately lights the selected pill, so a group reads **`Tiled LOD   Manual`** above an orange **`Manual`** chip. **Theme and Units escape it** because `_expand_sub` passes an explicit label — which is also the shape of the fix. **Do this in the same pass as the phone bold**, since both are in `_fill_prefs`’s chip path |
