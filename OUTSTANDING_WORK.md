...
**110 outstanding items** — re-derived mechanically 2026-09-12, by one run of
the counting script.
...
| **DIAGNOSED: LOD tiles carry a SHADE RATIO, not colour — sharpening needs `renderBiomeTileRGBA` ported** | `LOD_TILING_BASE_SCOPE.md` | large | **Candidate (b), measured and verified at the shader source rather than inferred. Candidate (a) is ELIMINATED: the level ladder is live** — 2/3/3/4/5/6/7 at zooms 1/2/2.5/4/8/16/40, tiles from 2.5. **`shell/lod_tile.gdshader` computes `COLOR = vec4(base.rgb * ratio, base.a)` where `base = texture(base_tex, …)` and `base_tex` IS `map_view.texture`** — the grid-resolution raster from `build_color_texture`. **The tile supplies only a scalar** (`SHADE_RATIO_MID`/`GAIN`), and `lod_bridge.rs:84`’s *"What a tile actually contains"* says exactly that. **So a deeper level cannot be sharper: all colour comes from one grid-resolution texture at every zoom.** Detail per screen pixel collapses **0.0220 → 0.0017** across zooms 1→40, and hiding the whole LOD layer at zoom 16 moves mean \|dL\| by **0.0083** — under 1%. **`renderBiomeTileRGBA` exists in this port only inside COMMENTS.** That is the unported piece, and it is a **milestone, not a fix**. **My earlier `LOD_SUPERSAMPLE` lead was the wrong half of the answer**: the reference renders at 2× capped 2560 AND its tiles carry real colour. Supersampling a shade ratio would sharpen nothing **DIRECTION ARRIVED 2026-09-12: `docs/research/lod extra info.md`**, owner-supplied, on scale-dependent terrain detail — and it describes exactly the absence this row measured: *"the renderer needs to reveal new spatial frequencies of the same terrain."* It also agrees with this row’s verdict on supersampling: *"Simply increasing contrast does not solve this. Neither does sharpening."* **Map it before scoping from it**: its crate names (`cartalith-grid`, `-compute`, `-worldgen`, `-cartograph`, `-render`, `-logistics`, `-project`, `-types`) are **not this workspace’s** — rendering here lives mostly in `cartalith-godot`, pyramid addressing in `cartalith-spatial::pyramid`; **several of its phases are already built** (the pyramid, the live level ladder above, `TERRAIN_APPEARANCE_SCOPE.md` §16’s macro/meso/micro weights, hillshade, multi-sun, AO, crest); and it assumes glacial-simulation fields while `glacier` appears in exactly **one** file in the workspace. **`ROADMAP.md`’s "revisit when a concrete need appears" (§6.8) now has its concrete need** **PRIORITY RULED 2026-09-12 (Ruling K):** this port is the first step toward the owner’s zoom target, `design/owner-references-2026-09-12/lod-zoom-target-aletsch-sentinel2.jpg`, read top-down with 3D still parked — and drawing ice waits on it (§3.2) |
...
| Item | Owns it | Size | Blocker |
|---|---|---|---|
| **Draw ice from fields that already exist, toward the Aletsch zoom target** | `TERRAIN_APPEARANCE_SCOPE.md` | medium | **Waits on the LOD colour milestone — §2.7’s *"LOD tiles carry a SHADE RATIO, not colour"*, with `renderBiomeTileRGBA` unported. Ruling K, 2026-09-12, fixed that order: sharpness first, then ice.** Target, read top-down with 3D still parked: `design/owner-references-2026-09-12/lod-zoom-target-aletsch-sentinel2.jpg`. **Inputs that already exist:** `glacial_kernel`’s carved U-shaped troughs, the `passes.glacial_snowline` parameter, and the snow and rock material fractions. **Not chosen:** a glaciation model — ice extent, flow, moraine stripes — which nothing models today (`glacier` appears in one comment in the workspace). Design input: `docs/research/lod extra info.md` §16-17, which derives snow from temperature, precipitation, slope, aspect and curvature, never from an elevation cutoff |
