...
| **The `user://` probe enumeration is wrong in membership — 26 readers, not 28** | `ANDROID_UI_SPEC.md` | small | **Filed 2026-09-07 with the correction already made, because the list is the deliverable and a wrong list is worse than none.** A lane enumerated the probes that read shared mutable state as **78 naming `user://` or `DccSettings` in code / 28 reading shared state**. The verifier re-derived it independently over `git ls-files *.gd` with comments stripped: **79 / 26**, and accounted for every difference. **Three over-inclusions:** `_ds13cluster_probe` (reads `DccSettings.RELIEF_EXAG_CHOICES`, a `const Array[float]` no sibling can change), `_datapane_probe` (reads back only the file it wrote at `_gis_dest`), `_layerstack_probe` (reads back only its own PNGs in its own directory). **One omission, and it is the pointed one: `_nwsize_probe.gd` itself**, which the same change made a reader via `DccSettings.recent_projects()`. **The remaining 26 are the real isolation surface** and the row to work from; the 50 that only write their own artefacts are not a hazard |
| **Two mutation survivors in `PgField`: a dead default and an unasserted caret** | `ANDROID_UI_SPEC.md` | small | **Found by the verifier 2026-09-07 running twelve mutations where the lane ran eleven — both survive, so both are coverage the tests do not give.** **M8:** `PgField`’s `var slop := 8.0` changed to `400.0` leaves `fail=0`, because `touch_focus_field()` always overwrites it — the declared default is never reached. **`PgSlider` carries the identical dead default at `dcc_widgets.gd:1424`, so this is pre-existing house style rather than new** — fix both or neither, and say which. **M9:** `caret_column = text.length()` changed to `0` leaves `fail=0`, so **nothing pins the caret placement the lane itself names as a user-visible cost** (the caret lands at end-of-text rather than under the finger, since 4.7.1 exposes no pixel-to-column call). **A cost that is documented but unasserted drifts silently**, which is the whole reason it was worth writing down |
