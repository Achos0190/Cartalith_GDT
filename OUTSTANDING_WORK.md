...
| **Roof polygons fail triangulation at deep zoom, and the buildings silently do not draw** | `URBAN_MORPHOLOGY_SCOPE.md` | medium | **Found incidentally 2026-09-08 by the settlement-pixel probe, then reproduced by the verifier and by the main loop.** A deep-zoom draw of a revealed capital emits *"Invalid polygon data, triangulation failed"* **repeatedly** — nine or more per frame in the main loop’s own run. **The citation was corrected before filing**, which is why it is worth trusting: the lane attributed it to `urban_layout_draw.gd:235`, and the verifier’s backtrace reads `_draw_roofs` (**`:524`**) called from `draw_layout` (**`:281`**); `:235` is a different `draw_colored_polygon` site the backtrace never names. **The failure is SILENT to the user** — Godot logs and skips, so the roof simply is not there, and the layout renders incomplete with no indication. It did not suppress the pixel measurement (other draw calls still painted 34/441), which is exactly why it went unnoticed. **Not chased, deliberately** — filed rather than folded into a cheap batch. Likely degenerate or self-intersecting polygons at small scale; **the first question is whether the geometry is bad or only bad once quantised to screen space**, and those have different fixes |
