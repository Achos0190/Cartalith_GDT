...
| **A vertical swipe starting on a dropdown opens it instead of scrolling** | `ANDROID_UI_SPEC.md` | medium | **Found by the main loop on the handset 2026-09-07, while confirming the batch that fixed the same shape for sliders.** A swipe from `(540,1600)` to `(540,1100)` on the New World card — a plainly vertical gesture — landed on the **Archetype `OptionButton`** and **opened its six-item popup**; the card did not scroll. The same swipe started from the label column at `x=200` scrolls normally, which is the control. **`PgSlider` does not cover this: an `OptionButton` is not a `Range`.** **It is milder than the slider defect and not harmless** — the user sees the popup and can dismiss it, so nothing changes silently, but **this card carries three dropdowns and two SpinBoxes, so a large share of its surface cannot be used to scroll it** — on a card that must be scrolled to reach CREATE WORLD. **The fix is the same arbitration one class up:** withhold the press until the gesture resolves, which is what `DccWidgets.touch_slider()` already does for `Range`. **Inventory it the way the slider hazard was inventoried** — by census, not by grep — before fixing |
...
| **The Android APK still ships a `.so` older than the shell, and logcat says so on every boot** | `ANDROID_BUILD_SCOPE.md` | medium | **Observed by the main loop on the handset 2026-09-07, from the app’s own warnings rather than by inference.** A cold launch of a fresh `--export-debug` build logs a run of *"the loaded GDExtension has no `WorldGen.<fn>()` … This almost always means the native library is older than the shell (a stale `libcartalith_godot.so`)"* — seen for `icon_placement_families()`, `label_class_table()` and `labels_generate()`, each with a full GDScript backtrace from `app.gd::_register_workspaces`. **The Android export carries the `android-dev` `.so` (2026-09-02), not the release one**, so **every on-glass check in this session and the last three ran against a library missing shell-visible functions**. **That does not invalidate the gesture and layout work** — those are shell-side and the degradation is a documented safe default — **but it does mean no on-glass claim about a native-backed feature is safe** until the export picks up a current library. **The warning is doing its job; nobody was reading it** |
