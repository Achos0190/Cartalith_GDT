...
| **The settlements layer owns ZERO pixels at deep zoom when `urban_layouts` is on** | `URBAN_MORPHOLOGY_SCOPE.md` | small | **Raised by the lane 2026-09-08 and deliberately NOT chased — filed rather than folded into the label work.** `_urban_revealed` (`map_overlay.gd:3544`, populated `:3698`) gates the settlement pin at `:1984`, so at deep zoom with urban layouts on **the pin draws nothing**. **The label-blur probe disables `urban_layouts` before measuring for exactly this reason**, which is why the finding surfaced at all. **Whether that is correct is a separate question**: a revealed urban layout arguably replaces its own pin. **But nothing states it**, and a reader measuring the settlements layer at deep zoom will find an empty layer and no explanation |
