...
| **The Android app boots ONLY because its `.so` is stale — a current one spins forever at startup** | `ANDROID_BUILD_SCOPE.md` | large | **Filed 2026-09-07 as a stale-library row; rebuilding the library turned it into something much worse, and the measurement is the row.** **The stale half is real and precise:** the `.gdextension` maps `android.debug.arm64` — what every `--export-debug` build uses, which is every on-glass pass this project has run — to `target/aarch64-linux-android/android-dev/`, and that library was **2026-09-02 with 138 `.rs` newer**. Every cold boot logged *"the loaded GDExtension has no `WorldGen.<fn>()` … the native library is older than the shell"* for `icon_placement_families()`, `label_class_table()` and `labels_generate()`, each with a full backtrace from `app.gd::_register_workspaces`. **Then it was rebuilt** — `cargo ndk -t arm64-v8a build --profile android-dev -p cartalith-godot`, exit 0 in 39.6 s, 186 783 824 B, **0 `.rs` newer** — exported, installed, hash-matched on disk and on device (`73cf095e…`), and cold-launched. **The app never reaches its own shell.** It prints five log lines, stops after `Cartalith shell build d33a2546988c`, and then holds **~50% of a core with a flat 538 MB RSS for over four minutes** (CPU time 1:12 → 1:37 across one polling round) with the screen still on the Godot splash at 94.2% dark. **Alive, not crashed** — `pidof` resolves, state `S`, 34 threads, no FATAL, no ANR, no tombstone. **Correction to my own reading of that: the CPU figure is NOT evidence of work.** I took ~50% of a core as "working, not hung"; the app sits at **54.8% at the painted picker**, which is the already-recorded finding that it draws a full 60 fps continuously with no idle mode. **The only signal that discriminates is the godot log line count: 5 = stuck after the shell build line, 50+ = progressing.** Use that, not CPU and not RSS. **Discriminating in BOTH directions, which is what makes it a finding rather than a bad build:** reinstalling the previous APK (`4ff2e257…`, identical shell code, stale library) brings back **50+ log lines, the five stale-function warnings, and a working picker — pixel-identical to the earlier capture, `ImageChops.difference` bbox `None`**. The only variable is the library. **The first hypothesis was mine and it is REFUTED — recorded so the next pass does not re-derive it.** I proposed that the three missing functions were being *"degraded to a safe default"* (which is what the warning says) and that one of them, present, does not terminate — with `labels_generate()` as the candidate, since `cartography_workspace.gd::_regenerate_labels` is `call_deferred` at the end of CARTO’s `_build()` and therefore runs at boot with no world. **Opened at the symbol: it cannot be the spin.** `label_bridge/generate.rs:236` opens with `if self.labels.is_none()` and returns a complete `ok:false` dictionary immediately, before touching anything. **And the absence of warnings in the spinning run is NOT evidence about how far it got** — with a current library those functions exist, so there is nothing to warn about; the quiet log is expected either way. **What is actually known is narrow:** the shell prints its build line, then something between there and a painted picker consumes ~50% of a core indefinitely. **Do not start from the three named functions** — they are the ones that happened to be *reported*, not a list of what changed across 138 files. **One narrowing IS established, and it is the useful one: the same source does NOT spin on desktop.** `target/debug/cartalith_godot.dll` (2026-09-06 22:44) and the fresh `android-dev` `.so` are built from the same tree — `find crates -name '*.rs' -newer <dll>` returns exactly **1**, and that one edit is entirely inside `mod tests`, so it cannot reach a non-test build (`cargo build -p cartalith-godot` confirms by relinking nothing). Against that library a full shell boot completes: `_rangeswipe_probe --census-only` walks eight named states, reports 247 `Range` nodes and **exits 0 in 7.0 s wall**. **So this is arm64-, NDK-, or `android-dev`-profile-specific, not a logic bug in the shared source** — which is where to look, and it is a much smaller place than 138 files. **That experiment was run and it did NOT come out cleanly, which is why it is written down rather than left for someone to repeat.** Installing `Cartalith-0907-signed.apk` — the release-profile APK carrying the current release `.so`, **the very build the owner used and the perf pass generated 2K worlds on earlier the same day** — now spins too: 5 log lines, flat 472 MB. **But it is confounded**: it carries a different shell (`8fd916035577` against today’s `d33a2546988c`), so it is not a one-variable comparison and cannot be used to blame the profile. **The clean pair is the one to trust:** `Cartalith-0907c.apk` and `Cartalith-0907d.apk` differ **only** in which `android-dev` library they carry — same shell, same export, same signing. `c` (stale library) reached the painted picker **three times out of three**, twice confirmed `ImageChops.difference` bbox `None` against a reference capture. `d` (current library) never got past 5 log lines. **Left on `c`, the build that boots.** **Two consequences, and the second is the expensive one:** the export cannot be fixed by rebuilding, so the warning stays until the spin is found; **and every on-glass claim about a native-backed feature made in the last four batches was made against a library missing three shell-visible functions** — the shell-side gesture and layout work is unaffected and independently proven, but nothing native is safe. **The device was left on the build that boots** |
| **A swipe on a SpinBox or text field blocks the scroll and raises the keyboard — and the census could not see them** | `ANDROID_UI_SPEC.md` | medium | **Two findings in one row because the second is why the first was missed.** **(1) The walk that produced every gesture census used `get_children()`, which omits INTERNAL children** — 4 401 nodes against **5 512** at 1080×2340. What it hid was not padding: **12 `SpinBoxLineEdit`** (5 in `new_world_dialog.gd`, 5 in the slicer `AcceptDialog`, 2 in `asset_library_window.gd`) plus a `TabBar` in `world_data_window.gd` — all `LineEdit`-derived, all acting on press, all live under live vertical scrollers, and **absent from both the before and the after numbers**. The class the probe exists to count was undercounted by the walk that counts it. **Fixed 2026-09-07** (`get_children(true)`), and the corrected census reports **34 unarbitrated touch-DOWN controls inside a live vertical scroller at a world-less boot, against the 22 the undercounting walk saw** — `SpinBoxLineEdit ×5` and `LineEdit ×18` among them, where the report said 4. **The dropdown fix is unaffected and all four phone probes stay green**: the residue is a different mechanism, not a regression. **(2) The hazard is real and is NOT the one SpinBox was cleared of.** The previous pass ruled SpinBox out by showing a swipe does not STEP the value — true, and the wrong question. Measured on the New World card: a jittered vertical swipe on the **Seed** field gives **scroll `0 → 0` with its internal `SpinBoxLineEdit` focused**, while the label column at the same `y` scrolls `0 → 62`. **On Android taking focus raises the soft keyboard over the sheet.** So the card cannot be scrolled from its own input fields, and the user gets a keyboard they did not ask for. **`action_mode` is not the switch here** — a `LineEdit` is not a `BaseButton` — so this needs its own answer, and the two prior fixes are the shape to follow, not the code to reuse |
...
| **OWNER-REPORTED ON GLASS: the journey planner cannot be used on the phone** | `JOURNEY_PLANNER_SCOPE.md` | medium | **Owner, 2026-09-07: *"am not able to use the journey planner"*.** **Routes exist in code:** a `PLAN` tab in `PHONE_TABS` and an `Open journey planner` row in the phone MORE screen (`_go_journey_planner`). `_jpinsw_probe` reports 0 rows wider than a 1080 px screen and 0 of 23 tappables under the phone floor — **all measured with synthesised input on a desktop window.** **So "it fits" and "it can be operated with a finger" are different claims and only the first was ever tested.** Establish which failure it is: the route is not findable, the tab does not respond to touch, or the planner opens and its controls do not take input. **Do not reason from the desktop probes — they would pass in all three cases** |
