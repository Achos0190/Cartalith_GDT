...
| **OWNER-REPORTED on BOTH platforms: Data management and Storage locations take a typed path with no way to browse to it** | `DCC_SHELL_SPEC.md` §2.1 | medium | **Owner, 2026-09-07: *"on android and pc … for datamanagement and storage location it just accepts a path, it doesn’t open a file explorer or browser to manually navigate and appoint."*** **START FROM THIS, because it changes the whole shape of the work: the browser EXISTS and both surfaces already call it.** `DccBrowseDialog` (`browse_dialog.gd`) is a full 760×640 breadcrumb picker with three modes (`FOLDERS` / `FILES` / `SAVE`), built from the *"Select folder dialog 1920"* mockup. Storage locations gives **every root its own `Browse…` button** (`app.gd:3470` → `choose_folder`), and Data Management uses it at `data_manager_window.gd:2417`, `:2422`, `:2430`, `:2480`. **So this is NOT a missing feature. It is a reachability or a capability defect, and the two platforms almost certainly have DIFFERENT causes. Establish which on each before changing anything.** **ANDROID — mechanism identified, and the file already knew:** `DccBrowseDialog.home_dir()` (`browse_dialog.gd:425`) tries `USERPROFILE` then `HOME`, and **its own comment at `:424` says *"Android in particular has neither variable"*** — so it falls back to `OS.get_user_data_dir()`, **the app-scoped sandbox**. Every listing goes through `DirAccess` (`:507`, `:522`, `:650`), which under Android scoped storage cannot reach shared storage at all. **The browser opens onto a sandbox the user cannot navigate out of** — which from the outside is indistinguishable from *"it doesn’t open a browser"*. **The Android fix needs a platform decision, so bring it back rather than picking one:** `DisplayServer.file_dialog_show` (Godot’s native/SAF path) versus seeding from `OS.get_system_dir(SYSTEM_DIR_DOCUMENTS)`, and whether a storage permission is needed — **`export_presets.cfg` is a GUARDED file, so a permission change is an owner decision, not a lane’s.** **PC — NOT diagnosed, and do not assume it is the same cause.** `USERPROFILE` exists there, so `home_dir()` resolves properly and the browser should work. **Candidate to test first, not a conclusion:** the `Browse…` button is not on screen. `open_storage_locations()` builds a **raw `AcceptDialog.new()`** by hand — not through any shell dialog path, so no fit pass touches it — sets `d.size = Vector2i(680, 340)`, and then hangs **hard minimums of 560 px and 620 px** on its two note labels (`app.gd`, the `cache_note` and `footnote` blocks). Each row is `[label 140][readout EXPAND_FILL][Browse…]`, so a dialog forced wider than its frame **clips `Browse…` off the right edge.** **And that is exactly the class of defect this project just proved its checks cannot see:** `get_global_rect()` reports a child’s **UNCLIPPED** rect, which is why every existing check passed the `CREATE WORLD` clip. **Assert the button’s drawn rect lies inside the dialog’s VISIBLE rect** — and drive it by tapping what is on screen, never by calling the handler |
