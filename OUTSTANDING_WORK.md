...
**76 outstanding items** — 2026-09-23 (latest), **net −1**: **`ComputeTier` classifier, CLOSED**
...
| Item | Owns it | Size | Next step |
|---|---|---|---|
| **`_exportraster_probe.gd`'s own byte-count check (§13) fails on HEAD, not just after any recent change** | `godot-project/_exportraster_probe.gd` | small | **Found in passing 2026-09-23 by the Ruling AN batch, disclosed rather than silently ignored or fixed out of scope.** 489 of 4 169 796 bytes differ against a tolerance of ~417 bytes, worst single-byte difference 1 — confirmed to reproduce identically on an unmodified HEAD build, so it predates that batch and is not caused by it. Needs its own investigation: is the tolerance too tight for real encoder variance, or is there a genuine one-off encoding nondeterminism worth chasing. Not investigated further here. |
