...
| **EVERY BUTTON AND INPUT IS AN OUTLINE WHERE THE CANVAS DRAWS A FILL — owner-reported** | `design/mcp-2026-09-07/Cartalith DCC Environment.dc.html` | large | **Owner, 2026-09-07, looking at the shipped PC screen: *"all buttons and inputs are visually not the same."* Measured at the symbol, and they are right — this is systematic, not per-screen.** **The canvas fills its buttons.** Primary: `background:var(--acc); color:var(--accInk); border-radius:8px; padding:4px 12px; font:500 var(--m1)`. Secondary: `background:var(--ins); color:var(--sec); border-radius:8px; min-height:var(--btnH)`. **The shipped button is an outline with no fill and no radius.** `DccWidgets.action()` styles every button with `DccTheme.outline(edge)` where `edge` is `accent` (primary) or `border`; `outline()` sets **`bg_color = Color(0,0,0,0)`** when no background token is passed — which is the case for the `normal` and `disabled` boxes — a 1 px border, and **never calls `set_corner_radius_*` at all, so the radius is 0**. So on every button: **fill transparent vs filled, radius 0 vs 8, and amber-on-transparent text vs dark ink on an amber block.** **The root cause is a superseded design rule, which is why it is uniform:** `dcc_theme.gd` cites a **§11 radius-0 rule**, and both current canvases have moved off it — a lane counted **81 `border-radius:999px` pills and 76 `8px` corners** in the PC canvas alone. The shell is faithfully implementing a rule the design abandoned. **Why the conformance passes missed it:** they ranked palette tokens (20/20 exact) and dock widths, and **a widget’s stylebox is neither** — the colours are right, the *shape* is not. **The next audit must walk every `DccWidgets` factory** (`action`, `choice`, `slider`, `number`, `toggle`, `field`…) against its canvas counterpart, not just the tokens they draw with. **Expect this to move perceived parity more than everything else on this list combined**, and expect it to touch every screen at once — which is the argument for doing it as one change with one probe, not per surface |
