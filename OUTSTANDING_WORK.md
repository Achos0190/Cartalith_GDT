...
| **INPUTS are still unstyled and inherit a theme resource from 2026-08-17 — buttons are DONE** | `project.godot` §gui | medium | **Owner, 2026-09-07: *"all buttons and inputs are visually not the same."* The BUTTON half is fixed and verified (441 checks against canvas hex literals, 0 fails, zero layout movement). The INPUT half has a different and larger cause.** **`choice()`, `toggle()` and `number()` apply NO stylebox at all** — they call `OptionButton.new()` / `CheckBox.new()` / `SpinBox.new()` with only a font-size override and **zero `add_theme_stylebox_override`** — so every dropdown, checkbox and spin box in the shell takes its whole appearance from the project theme. **And that theme is stale: `project.godot:75` sets `theme/custom = res://theme/dark_theme.tres`, whose own header is dated 2026-08-17 and says it was *"authored against the mockup"*.** So this is the second instance of the mechanism the owner named — an outdated document holding the old style in place — and it is the one that covers every input in the product. **`project.godot` is a GUARDED file**, so pointing it at a new resource, or replacing the resource, is an owner decision rather than a lane’s. **The measurement is done; the ruling is not** |
