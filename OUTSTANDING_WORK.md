...
| **Dialog protocol: FIVE of sixteen sites converted, eleven remain** | `DCC_SHELL_SCOPE.md` | medium | **Done in the MAIN LOOP 2026-09-07, not a lane — the budget posture in `SESSION_HANDOFF.md` names this case as the largest saving available, and this row is the evidence it works.** **Converted:** `right_dock::_confirm_revert`, `faction_roster_window::_confirm_remove`, `place_editor_window::confirm_delete`, `civilization_workspace::_confirm_destructive` and `::_tl_show_confirm`. All four files now hold **zero** hand-built `ConfirmationDialog`s. **`_confirm_destructive` is the one that mattered: it is a SHARED helper**, so every destructive action in that workspace reached the phone correctly from one edit. **Proven, not assumed:** `right_dock::_confirm_revert` was the verifier’s known-bad specimen — unmodified it graded `fails:shape,present,tap worst=29dp(‘Revert’)` at 638×136 with `content_scale_factor` 1.0000. `DccWidgets.confirm()` grades **`ok`, worst=44dp, off=0** on `_phoneproto_probe` (windowed, `--force-touch`, fail=0). **One deliberate text change, recorded at the site:** `_confirm_revert`’s question moves out of the body and becomes the title, because `_phone_dialog_body()` draws the title as the header — leaving it inside the prose renders it twice on the phone and namelessly on the desktop. **ELEVEN REMAIN**, and the two worst are `vault_window::_compare_dialog` and `::_preview_dialog`, which have the protocol **BACKWARDS** — `app.phone_fit(dlg, 1.0)` with no `phone_window()`, so the fit they perform is **wrong rather than absent**. Also: `world_workspace::_confirm_discard`, the three prompt-shaped sites (`journey_planner_view::_save_journey`, `::_capture_preset`, `cartography_workspace::_prompt_label_name` — these take `prompt()`, not `confirm()`), four in `menus.gd`, and `shortcuts_dialog.gd` |
