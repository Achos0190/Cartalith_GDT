...
| **Input STYLEBOXES are now canvas-drawn and theme-independent; eight COLOUR items still are not** | `project.godot` §gui | medium | **The stylebox half is done and proven by the strongest test used on this project yet.** The verifier’s `_vfy_indep_probe` **overwrites the 60 theme items `dark_theme.tres` defines for `CheckBox`/`OptionButton`/`LineEdit`/`SpinBox` inside the loaded `Theme` instance itself** — `load()` returns the cached resource `ThemeDB` holds, so the rewrite is hostile and live — then re-measures. **0 of the styleboxes moved.** The 2026-08-17 `#1a1b1d` bordered slab is genuinely retired: dropdowns, checkboxes, spin boxes and text fields now draw the canvas’s `--ins` ground, no border, radius 8/12. **What is NOT done: eight COLOUR items still resolve from `dark_theme.tres`, and two are load-bearing.** The sharpest is `CheckBox`: **Godot 4.7 modulates the checked/unchecked ICONS with `checkbox_checked_color` / `checkbox_unchecked_color`**, which the resource sets to `#e0a34a` / `#8d9396` and the lane did not override — so the switch it drew is **tinted by the stale theme at the screen**, which is why the lane’s own probe passed while the pixels did not. **A stylebox-shaped test cannot see a modulate.** **The remaining job is small and specified:** override those eight colour items at the node, the same way the styleboxes now are, and extend the probe to assert them under the same hostile-theme swap |
...
| **OWNER-REPORTED: dragging the drawer up in the SCULPT menu does not work** | `ANDROID_UI_SPEC.md` | medium | **Owner, 2026-09-07, on glass: *"it seems an issue with dragging the drawer up in the sculpt menu."*** **A CANDIDATE CAUSE, flagged as a hypothesis and not a claim, because it would be a regression from this week’s own work:** the phone sheet is raised by dragging its grab handle (`_on_phone_sheet_grab_input`, `_snap_phone_sheet`, `PHONE_DETENT_ANIM`), and the SCULPT sheet shares its `ScrollContainer` with GENERATE, which this week gained three layers of gesture arbitration — `PgSlider` (8 dp slop, axis verdict), `touch_release_button()` (`ACTION_MODE_BUTTON_RELEASE` + `PASS`) and `PgField` (`focus_mode` parked). **Each was proven not to break scrolling; none was tested against the SHEET-RAISE drag, which starts on the handle and travels vertically — the same gesture shape the arbitration is tuned to hand to the scroller.** **Do not assume that is it.** Reproduce on the device first — `adb exec-out screencap` plus `adb shell input swipe` from the handle — and bisect against a pre-arbitration build if it reproduces. **The three fixes are all in `dcc_widgets.gd`/`dcc_shell.gd` and each is individually revertible**, which makes this cheap to isolate |
| **OWNER-REPORTED: Preferences does not show which option is selected — bold the selected row** | `docs/DCC_SHELL_SPEC.md` §2.5 | small | **Owner, 2026-09-07: *"in the preferences menu we should show what options have been selected. For the preferences I'd say make the text of the selected option bold in the overview."*** **This is a specification from the owner, so it outranks the canvas and the spec** — record it at the call site as an owner decision or the next conformance pass will revert it toward whatever §2.5 draws. **What exists today:** `menus.gd` already uses `add_radio_check_item()` and `set_item_checked()` for the value groups (autosave interval, units, theme, multi-GPU mode, fallback, render quality…), so the SELECTION is tracked and drawn as a radio mark inside each submenu. **What the owner is asking for is the OVERVIEW** — the selected option should read **bold**, so the current value is legible at a glance rather than by finding a small check glyph. **Two things to settle before building:** whether "overview" means the parent Preferences menu should also carry the current value beside each group, or only that the selected row within each submenu is bolded — **ask rather than guess if the code does not make it obvious**; and how bold is achieved on a `PopupMenu` item, since Godot styles items by theme rather than per-item font — `set_item_*` has no weight, so this may need a bold `Font` override on the popup plus per-item selection, or a different mechanism. **Measure that before promising it** |
...
| **OWNER-REPORTED TWICE: the journey planner still does not function** | `JOURNEY_PLANNER_SCOPE.md` | medium | **Owner, 2026-09-07: *"am not able to use the journey planner"*.** **Routes exist in code:** a `PLAN` tab in `PHONE_TABS` and an `Open journey planner` row in the phone MORE screen (`_go_journey_planner`). `_jpinsw_probe` reports 0 rows wider than a 1080 px screen and 0 of 23 tappables under the phone floor — **all measured with synthesised input on a desktop window.** **So "it fits" and "it can be operated with a finger" are different claims and only the first was ever tested.** Establish which failure it is: the route is not findable, the tab does not respond to touch, or the planner opens and its controls do not take input. **Do not reason from the desktop probes — they would pass in all three cases** | **RE-REPORTED 2026-09-07 after a day of GUI work: *"the planner doesn’t seem to be functioning."* Nothing in this row has been actioned — every phone batch since has gone to gestures, the GENERATE sheet, the New World card and widget styling, and the planner was never opened.** **It is now one of the three things the owner names as remaining in the app**, alongside the touch drawer and the Preferences selected-state, so it stops being a background row. **Start by establishing WHICH failure it is**, because the three need different fixes and the existing evidence cannot tell them apart: the route is not findable, the PLAN tab does not respond to touch, or the planner opens and its controls do not take input. **`journey_planner_view.gd` is 4 757 lines and `TABLET_UI_SPEC.md` §4.1 measured it a SUPERSET of the canvas’s `scrPlanner` including all four result groups by name** — so this is far more likely to be a reachability or input defect than a missing feature, which is exactly the distinction desktop probes cannot make |
