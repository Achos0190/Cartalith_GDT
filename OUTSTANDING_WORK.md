...
**85 outstanding items** — re-derived mechanically 2026-09-06, **after
...
| Item | Owns it | Size | What was found |
|---|---|---|---|
| ~~**The 14 undesigned surfaces**~~ — **ALL DESIGNED, 13 BUILT; one owner decision left** | `DESIGN_HANDOFF.md` | small | **CA-07 label roles landed 2026-09-06 — the last buildable screen of the fourteen.** The owner supplied three Claude Design canvases covering exactly the 14. **Thirteen are built and verified.** **Remaining: PR-15 units, which needs an owner decision, not work** — there is no km/mi toggle anywhere in `shell/*.gd`, so the contradiction is between a canvas and a kilometre-only engine, and Reading 3 (remove it from the canvas) costs nothing to build. **Seven of the canvases' "Exists today" claims failed against the code** across the whole round — twice turning a build row into "already shipped", and twice telling a lane to dash a field that was fully live |
