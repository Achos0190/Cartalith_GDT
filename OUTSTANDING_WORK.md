...
| **Ruling 13 (landmarks as a vault `EntityKind`) is BLOCKED on ruling 10, and the dependency was invisible** | `MARKDOWN_VAULT_SCOPE.md` | medium | **Declined 2026-09-06 by the lane sent to build it, correctly, and the verifier reproduced all three premises.** A vault link needs a stable identity and landmarks have none: `cartalith-civ/src/landmark.rs:3596` assigns `lm.id = k as u64 + 1` — **a position in the output vector** — and the `seed` field's own doc says to derive from it *"never the id"*. `landmark_seed(world_seed, feature, class, version)` takes no `kind`, so same-class kinds at one cell collide. **And the persistence ruling 10 authorises is not built**: `LandmarksDoc` has one field, `settings: LandmarkSettingsDto`, and no results — so nothing survives a reload to link to. Finally `vault_bridge::entity_values`/`entity_cell` end at `EntityKind::Culture` with **no catch-all**, so adding the variant would break the build in a file that lane did not own. **Do ruling 10 first**; this row is startable the moment landmarks have a persisted, stable id |
