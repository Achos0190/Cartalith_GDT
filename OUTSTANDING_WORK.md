...
|---|---|---|---|
| **The design fix pass — 7 of 13 round-2 boards carry factual defects** | `DESIGN_HANDOFF.md` | medium | **Found 2026-09-05 by the round-2 design critic; queued, not applied.** Clean: `WorldDockA`, `BrowseModes`, `PhoneOverflow`, `PhoneFailureStates`. **Blocking — `DataManager`:** `geojson.rs:309` emits a **sixth** `properties.layer`, `poi`, unconditionally (round-trip test at `:449` asserts it), so a chip is missing and *LAYERS 5 of 7* is wrong; it also draws live `CPU 38`/`GPU 71` against **no utilisation binding anywhere** (zero hits for `cpu_percent`/`gpu_percent`), reverts `dcc_theme.gd:476`'s recorded 34→36 px menu-bar correction, and draws `Dry run` live with no dry-run path. **`LayersPopover`:** fjord swatch drawn `#3f6f8c`, `FJORD_HI` is `#3cc8f0` — the constant exists precisely so a swatch cannot drift from its raster. **`AssetFamilies`:** `ANCHOR bottom` for Sea marks where `slots.rs:356` is `Anchor::Center`, with an eight-line comment explaining why. **`PhoneViewportControls`:** anchors at `right:12 bottom:104`, which `06-phone.md` §6.2 already assigns to the FAB column — either the cluster replaces it (and the 3D/2D toggle is homeless and unmentioned) or they collide; discs also drop 48→44. **`BiomePaint`**, **`MenuOrphans`**: one claim error each. Plus the owner's washed-pill ruling applied across `Timeline`/`WorldDockB`/`BiomePaint` |
