...
| **THE SHIPPED TABLET DOES NOT FIT ITS OWN FRAME IN PORTRAIT — content is off screen** | `TABLET_UI_SPEC.md` | large | **Measured by the inventory lane and reproduced exactly by the verifier at 800×1280 `--force-touch`. This is a shipped DEFECT, not a resemblance gap, and it was not previously recorded.** The dock row sums to **rail 47 + left_dock 400 + viewport 237 + right_dock 400 = 1084 px**, and the right dock’s right edge lands at **x = 1085** against a frame of **800** — **overflow +285 px**, and **+185 px** at 900×1440. **Both docks report `get_combined_minimum_size().x = 400`.** Landscape is fine: 1280×800 sums 1279, overflow 0. **The canvas fits by design:** `52 + 232 + 232 = 516`, leaving 284 px of map. The canvas specifies **232 dp portrait / 320 dp landscape** (`Cartalith Tablet.dc.html:897`) against the shipped flat 400. **Last batch’s own 800×1280 capture confirms it visually** — SAMPLE values sliced at the frame edge, menu bar cut mid-word, no map visible at all. **And no portrait/landscape split exists anywhere in the GDScript to hang the fix on:** `grep -rn "tablet_portrait|TABLET_PORTRAIT" --include=*.gd` returns nothing, and all 24 `_landscape` consumers in `dcc_shell.gd` live in exactly seven `_phone_*` functions. **This is the one adoption item that buys a defect fix rather than a resemblance** |
