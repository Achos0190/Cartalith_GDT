...
| **Fold `performance_window.gd` away into Preferences rows** (ruling 19) | `DESIGN_HANDOFF.md` | small | **Owner ruling 2026-09-06: no diagnostics window exists in this design language.** The spec draws exactly two windows and §2.6's Window menu names those two; §2.5 gives `Working set` as a read row and `GPU acceleration` as a toggle plus readout. **Its DCC restyle hours earlier is superseded, not wasted — the rows move.** **Inventory what the window shows and give each item a menu-row home or drop it deliberately**, or this ruling quietly loses capability |
