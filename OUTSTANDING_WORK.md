...
| **Investigated 2026-09-21, not independently closed: LOD-tile zoom "sawtooth" reported on the same device** — **substantially improved by two fixes already landed this session, not uniformly under LOD-D2's own bar** | `cartalith-hydrology/src/lib.rs`, `cartalith-godot/src/render.rs` | small | The two river-rendering fixes landed earlier in this same session (`f8d8bcd` diagonal-disc connectivity, `42754bc` `channel_tint` unit mismatch) were re-measured by a dedicated investigation using the project's own `_lodsweep_probe.gd` seam-ratio harness: median seam ratio **1.8527 → 1.6381 → 1.4533** across the two fixes (same seed/grid, `8e71a01` → `9889076` → HEAD), under LOD-D2's own `≤1.5` bar at that measurement. **Independently re-run by the coordinating session** (not just trusted from the agent's own report): confirms substantial improvement but a MIXED picture — of 18 sweep-run medians collected across seeds/configs at current HEAD, most are comfortably under 1.5 (1.05–1.49) but several still exceed it (1.65, 1.73, 1.60, 2.27) — so this is not a uniform closure of LOD-D2's seam-ratio bar for every configuration, even though it very likely resolves the specific visual complaint (both now-fixed defects — broken river discs and red-brown tile-edge color bleed — were real, high-contrast sources of a sawtooth-reading artifact). No new code written in this pass; both underlying fixes are already committed and counted at their own rows. Left open as its own row rather than folded into either fix's closure, since the seam-ratio bar itself remains partially unmet — a future pass should determine whether the remaining above-bar configs need their own fix or are within this milestone's already-accepted tolerance. |
