...
| **The phone dialog path is a THREE-CALL protocol, and 16 of 22 sites do not complete it** | `DCC_SHELL_SCOPE.md` | medium | **The enumeration this batch was sent to produce, and it is worth more than the instance that prompted it.** The Storage locations modal (desktop scale on a 1080 px phone, ~7 px text) is **fixed**; the pattern behind it is not. **The protocol, spread across build time and open time:** `DccWidgets.phone_window(dlg, host)` at build, `DccShell.phone_fit(dlg, 1.0)` after the body, `DccWidgets.phone_present(dlg, host)` **instead of** `popup_centered()`. **22 hand-built dialog sites in `shell/` were graded against it.** **The structural finding: a caller can get it PARTLY right, and two got it BACKWARDS** — they call `phone_fit()` without `phone_window()`, so the fit they perform is wrong rather than merely absent. **That is worse than omitting it**, and no check catches it. **The remaining 16 are not 16 bespoke fixes:** six are the identical text-only `ConfirmationDialog` and three the identical prompt-with-a-`LineEdit`. **The honest shape is two helpers in `dcc_widgets.gd` (~60 lines) — a `confirm()` and a `prompt()` — plus 15 call-site edits that mostly DELETE code.** **`DccWidgets.modal_present()` already collapses two of the three calls**, which is the shape to extend rather than a new mechanism. **One genuine design question inside it:** `_lm_open_funnel`’s anchored 342 px popover has no phone form at all, and `phone_present_popup()` only takes a `PopupMenu`. **That one needs the owner** |
| **RULED — the selected Preferences chip keeps Medium and GAINS the accent ink** | `ANDROID_UI_SPEC.md` | small | **Owner ruling B, 2026-09-07 (`LARGE_ITEM_RULINGS.md`). Startable now, and small.** Weight stays `DccTheme.mono(0, on)` — **Medium’s advance is identical to Regular at 93.00, so nothing re-flows** — and the selected chip additionally takes the accent ink. **The reason is layout and it is the point of the ruling:** a Bold (700) face has a wider advance and would re-flow **every chip row**, and phone chip rows are exactly where this project’s clipping defects have appeared (`CREATE WORLD` at 7 of 46 dp painted; the SAVE foot’s primary button off-screen). **Colour costs no layout at all.** **This supersedes the literal reading of the owner’s own earlier wording** (*"make the text of the selected option bold"*) — by the owner, with the trade-off in front of them. **Record it at the call site**, or a later conformance pass reads the accent as drift and removes it. **Prove the colour as DRAWN, not as a property set** — that failure class has cost two batches |
...
| **Handset: `export_world`’s three footer chips are 477 px against 464 px of pane** | `ANDROID_UI_SPEC.md` | small | **Partial, 2026-09-07: the footer fix cut the handset demand from 1119 to 514 at 500 dp and 1080×2340 is `fail=0`, but 14 px of chip width remains at the narrowest.** **`FOOT_NOTE_MIN_W` is deliberately pointer/tablet-only, and the reason is measured:** applying its 160 px floor on the handset put the route’s own picker at **`shown=0.000`** — the floor that fixes the desktop breaks the phone. **The remedy is to let the footer chips WRAP**, which is a phone-composition change; the lane that found it did not own the handset |
| **`_panemin_probe` completes its measurements then HANGS at exit** | `MISTAKES.md` | small | **Found in the main loop 2026-09-07 while re-verifying the batch, and it will mislead the next session.** The probe prints every route, every assertion and its result, **then hangs in Godot’s shutdown** — leaked `NavMeshGeometryParser2D`/`3D` RIDs, an unjoined `Thread`, and `Unreferenced static string` spam. A `timeout` therefore kills it and returns **124**, which reads as a failed probe when every assertion passed. **This is the mirror image of the vacuous-pass problem just fixed in `_nwclip_probe`:** there a probe reported success it had not earned; here one reports failure it did not commit. **Both are the exit code disagreeing with the output**, which is why this project already grades Godot probes on stderr content rather than exit status. **Make it call `quit()` cleanly, or document the hang in its own header** |
| **RULED — route Android file-picking to SAF, and audit every caller** | `ANDROID_BUILD_SCOPE.md` | medium | **Owner ruling A, 2026-09-07 (`LARGE_ITEM_RULINGS.md`). Startable now.** `choose_file()` — Open project, import asset pack, choose sprite sheet — goes through `DisplayServer.file_dialog_show`; **folder-picking KEEPS the in-shell browser**, which is what closed the owner’s original complaint. Two mechanisms split by mode, each on the side it works. **The measured facts this rests on:** `FEATURE_NATIVE_DIALOG_FILE = true`; `OPEN_FILE` returns a `content://` document URI that **`FileAccess` and `ZIPReader` both accept (err=0, exact byte length)**; `OPEN_DIR` returns a tree URI `DirAccess` cannot open (err 31), which is why folders stay where they are. **THE WORK IS THE CALLER AUDIT, not the dialog call.** `app.gd`’s `_on_open_zip()`/`_on_import_pack()` and `asset_library_window.gd` treat the return as a filesystem path. **Grep every consumer for `get_base_dir()`, `path_join()` and `dir_exists` on that value** — each needs a branch or it will fail silently on a URI. **`MANAGE_EXTERNAL_STORAGE` was offered and DECLINED** — do not re-propose it as a shortcut |
...
| Item | Owns it | Size | Next step |
|---|---|---|---|
| **ADVERSARIAL VERIFICATION IS OWED on the pane-minimum / dialog batch** | `MISTAKES.md` | small | **2026-09-07. The batch’s verifier did not run — it errored with *"You’ve hit your session limit, resets 9pm"*, roughly two hours out. Both build lanes reported; neither was independently checked.** **What the main loop DID establish, and it is not nothing:** the cargo floor exact at **157 result lines / 3 253 passed / 0 failed** (lines counted before the total was read); `--check-only` clean on all four touched files; guarded files unmoved; **`_panemin_probe` re-run independently — `export_world` 1372 → 926, `export_maps` 1348 → 995, `export_gis` 1149 → 943, every route now under the declared 1024, 0 FAIL**; and `_nwclip_probe`’s three legs confirmed (no touch → abort, touch+landscape → abort, touch+phone → `fail=0`), reporting **`fail=abort`** so it can never read as a pass. **What is NOT established, and this is the point of the row:** nobody adversarially attacked the lanes’ reasoning, re-derived the pre-fix state independently, or mutated their constants. **The main loop checking a lane’s own probe is not independent verification** — it runs the same assertions against the same code. **Discharge it by folding this batch into the NEXT batch’s verifier brief**, which has a quiet tree containing both. **Do not dispatch a separate verifier while another batch is live** — that mistake is already filed twice above |
