...
| The phone MORE tab is a full-rect overlay, not §5's detented sheet | `DESIGN_HANDOFF.md` | small | **Disclosed 2026-09-05 by the lane that built the five screens, and outside its file.** §5.1/§5.2 give the sheet peek 66 / half `round(fh*0.46)` / full `fh-96` with §5.3's drag handle; the MORE tab is a full-rect overlay whose insets come from `DccShell::apply_insets()`. Every drill inside the stack now pushes one level so presentation no longer changes twice on the way down, but the detent conversion needs `dcc_shell.gd` |
