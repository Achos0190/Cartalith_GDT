...
| **The new `Browse…` chip is untappable below ~1372 px, and the window’s own declared minimum is 1024×640** | `GUI_GAP_REGISTER.md` | small | **Disclosed by the PC lane as "worth a row" and SHARPENED by the verifier 2026-09-07 — the correct statement is that the fix does not work at the window’s own declared minimum.** Re-measured at **1152×648**: `export_world` contents minimum **1372** against a size of 1152, the new `Browse…` at **shown=0.000**, and tapping where it is drawn opens **nothing**. The window declares `min_size = 1024×640`. **The cause is pre-existing and was not made worse** — `export_maps`’ `Choose…` fails identically at 1348 — **but a control that only works above 1372 px in a window that permits 1024 is a defect in the pane’s minimum, not in the chip.** The data-manager route panes need 1372 px; either they reflow or the window’s minimum is wrong. **Fix the pane, not the button** |
| **The Storage locations modal renders at DESKTOP scale on the phone** | `ANDROID_UI_SPEC.md` | small | **Photographed by the ANDROID lane 2026-09-07 and reported rather than touched — it lives in `app.gd`, outside that lane’s grant.** ~7 px text and near-untappable `Browse…` buttons on a 1080 px screen. **The mechanism is already known and is the same one that produced two other defects this week:** `open_storage_locations()` builds a **raw `AcceptDialog.new()` by hand**, so **no fit pass ever touches it** — not `phone_fit()`, not `dock_fit()`. **Also worth recording: on the phone this modal is not where the brief said it was.** It sits behind **MORE ▸ Project ▸ "Change locations…"**, not the desktop’s File/Preferences entry points. **Look for other hand-built dialogs while fixing this one** — the raw-`AcceptDialog` pattern is the shared cause and enumerating it is worth more than fixing this instance |
| **`_nwclip_probe` reports `fail=0` on the desktop while measuring nothing** | `MISTAKES.md` | small | **Found by the verifier 2026-09-07 when the brief asked it to re-run the probe.** At **both** 1680×1010 and 1080×2340 it prints `phone=false … fail=0 (not a phone: 6.7’s card is not built here)`. **`DccTheme.is_phone()` needs touch, which no desktop run supplies**, so the probe cannot answer on PC — and its `fail=0` pins nothing while reading as a pass. **Pre-existing and caused by neither lane**, but it means the `CREATE WORLD` clip fix has NO standing desktop regression guard — it is only checked when someone remembers `--force-touch`. **The fix is to make it exit non-zero when it cannot run**, the way `_nwsize` already refuses headless rather than passing vacuously. **A probe that cannot answer must not report a pass** |
| **OWNER DECISION: the bolded Preferences chip is Medium (500), not Bold (700)** | `ANDROID_UI_SPEC.md` | small | **Built and drawn 2026-09-07, and the lane disclosed the gap rather than letting it read as done.** The owner asked to *"make the text of the selected option bold"*; what ships is **IBM Plex Mono Medium** — `DccTheme.mono(0, on)` selects `FONT_MONO_MED`, the weight the shell already loads. **Measured, not estimated: +3.3% ink area** (3018.5 mass over 3074 px versus 2922.2 over 2986), advance identical at 93.00 on both faces so **nothing reflows**. Real and repeatable — and subtle. **The question for the owner: is 500 enough, or should this be a true 700?** Going to Bold means loading a fourth face, and **the zero-reflow property is what makes the current fix safe** — a heavier face with a different advance would re-flow every chip row and put the phone Preferences screen back into the clipping class this batch just cleared. **Ask before changing it** |
...
| **On the light palette an OFF switch is a lone dark knob — 1.03:1, and the CANVAS says so** | `design/mcp-2026-09-07/Cartalith DCC Environment.dc.html` | small | **Observed and pinned rather than fixed 2026-09-07, because the canvas draws it this way.** The OFF track is `--sur` (`#f4f2ee`) against a `#fbfaf7` dock: **1.03:1**, so the track vanishes and only the knob reads. Visible on the planner’s *"Re-pack per stage"* and *"Auto-promote Walking"* rows. **`ENV:1354` is the authority and it is unambiguous**, so `_cklight_probe` pins the current behaviour — changing it is a **deviation from the canvas and needs an owner ruling**, not a lane’s judgement. **Raise it with the switch-weight question**, since both are "the canvas says X, is X what you want on this palette" |
| **RULING NEEDED: Android FILES mode now lands in real storage and lists ZERO files** | `ANDROID_BUILD_SCOPE.md` | medium | **The owner’s folder-browsing complaint is FIXED on both platforms (closed below). This is the half that is not, and the verifier escalated it because the lane’s stated reason for declining it describes the state its own shipped change created.** **Measured with ZERO permissions, and the two candidate paths are exactly complementary:** **SAF** (`DisplayServer.file_dialog_show`, `FEATURE_NATIVE_DIALOG_FILE=true`) browses the whole device and `OPEN_FILE` returns a `content://` URI that **`FileAccess` and `ZIPReader` both accept** (err=0, exact length) — but `OPEN_DIR` returns a tree URI that `DirAccess` cannot open (err 31), **so SAF works for FILES and cannot back a storage root.** The **in-shell browser** is the mirror image: folders work fully (`/storage/emulated/0` lists 16 dirs, mkdir + write + readback succeed in Documents and Download) but **files written by other uids are invisible** — the app saw `files=0` where SAF saw two real `.zip`s. **The live defect:** `home_dir()` is **mode-agnostic**, and `_on_open_zip()` passes `storage_root("projects")` (`…/files/Worlds`) which does not exist, so `navigate()` falls back to the new home. **On glass, Open project now lands in real Documents where `Werk.zip` (3.5 MB) sits and lists nothing.** Before the fix it landed in the sandbox and listed nothing; it is not worse, but it now looks like a working browser that is empty. **Three options, and this is the owner’s call:** (a) route `choose_file()` to SAF — correct, but the callers are in `app.gd` and may treat the return as a path, so they need auditing; (b) land FILES mode back in App storage, honest and useless; (c) ship an explicit empty state saying files here are not visible to this app. **`MANAGE_EXTERNAL_STORAGE` would fix (b) but is NOT available:** it is absent from the manifest, `request_permission` returns false, **and Android’s "All files access" toggle is greyed out so the user cannot grant it by hand either.** `export_presets.cfg` is guarded, so adding it is an owner decision |
