...
|---|---|---|---|
| **`EngineBridge` has no wrapper for `civ_place_smelting`, and `trade_store.gd` calls it unguarded** | `UNWIRED_FUNCTIONS.md` | small | **Found 2026-09-05 by the batch-34 verifier, which also corrected the lane's diagnosis of it.** A probe aborted with `Nonexistent function 'civ_place_smelting' in base 'Node (EngineBridge)'` and the lane attributed it to a stale `target/debug/cartalith_godot.dll`. **That cause is wrong and the wrong cause is the expensive part** — it routes the fix to a rebuild that cannot help. `grep -a civ_place_smelting target/debug/cartalith_godot.dll` finds it PRESENT, and the dll postdates the source (landed in `cff1edc`). `EngineBridge` is the **GDScript** wrapper (`engine_bridge.gd`, `extends Node`), and `grep -n civ_place_smelting engine_bridge.gd` returns nothing: the `#[func]` lives on `WorldGen` and no wrapper forwards it. Two edits — add the wrapper, and guard the call site at `trade_store.gd:127`. Pre-existing; no batch caused it |
...
| History's steps block draws a `§ STEPS` header above a row that already says `N STEPS` | small | **Layout drift caught by the batch-34 verifier.** The approved artboard (`design/proposed-2026-09-05/Main.dc.html`) has **no** section header there — its only heading is the dock title `HISTORY`. `DccWidgets.section()` always draws one, so this needs either a headerless variant or the block built without the factory. Left rather than hacked: the next batch touching `right_dock.gd` should do it properly. The artboard's `✕ discard the N undone steps` button is also undrawn, and that one is blocked rather than cosmetic — `clear_undo()` is the only control over these buffers and it clears the undo stack too, which is more than the button offers |
