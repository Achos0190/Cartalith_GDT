...
| **The New World modal’s action row is an undisclosed departure from the canvas** | `ANDROID_UI_SPEC.md` | small | **Found by the verifier 2026-09-07; pre-existing rather than a regression, so the defect is DISCLOSURE.** The canvas draws **CANCEL (flex 1) on the left and CREATE WORLD (flex 1.4) on the right**, both `min-height:46px` `border-radius:23px`, spanning the card. **The shipped dialog draws `Create` at x96, 59×44 dp, then `Cancel` at x255, 60×44 dp** — **order inverted, about a third of the width, 44 dp not 46**. `ANDROID_UI_SPEC.md` §6.1 quotes the canvas’s numbers and **no row records the difference**, in the section created to record exactly this. **Two honest closes and they are not equivalent:** build the row to the canvas (this is the *"100% faithfully"* ask, and a full-width primary is also the better phone target), or record the departure with its reason. **Note the button order is Godot’s `AcceptDialog` convention, so building it may mean not using that convention** — open it before choosing |
| **The 8 dp touch slop is not pinned from below** | `ANDROID_UI_SPEC.md` | small | **Mutation-found by the verifier 2026-09-07, and it is the "a floor is not a pin" shape again.** `slider.slop = float(_pg_px(8))` mutated **upward** to `_pg_px(400)` kills five probe checks; mutated **down to `0.0` the probe stays green**. **A zero slop makes the first pixel of motion choose the axis, so a one-pixel sideways wobble during a scroll writes the parameter** — the precise hazard the constant exists to prevent, and the one the whole `PgSlider` class was written for. **The shipped value is right; the test does not hold it.** `ANDROID_UI_SPEC.md` §1.14 said the probe *"pins it"* and has been corrected to say what it covers. **The close is a check that swipes vertically with a small deliberate cross-axis jitter** and asserts the value is still byte-identical |
...
| **`phone_fit()`’s comment describes the behaviour `PgSlider` replaced** | `ANDROID_UI_SPEC.md` | small | **Reported by the lane 2026-09-07 from outside its own files — the right call, and the ×48 "hunt the prose" rule.** `dcc_shell.gd::phone_fit()` says *"An HSlider is deliberately NOT included: a drag that starts on a slider means ‘move this slider’, on every touch platform there is."* **That was the reasoning the scroll-vs-write defect refuted.** **And the scope is the real finding:** `PgSlider` fixes the GENERATE sheet only, while **the same vertical-swipe hazard exists on every other phone surface `phone_fit()` touches** — any dialog carrying a slider inside a scroller. **So this is not a comment fix: it is the inventory of where else the defect lives**, and the comment is the pointer to it |
