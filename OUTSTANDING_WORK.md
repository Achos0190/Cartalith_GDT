...
| **TABLET PORTRAIT: the left dock is sized by workspace panels that are NOT ON SCREEN** | `TABLET_UI_SPEC.md` | medium | **Measured 2026-09-07 with a driver walk added to `_tabspec_probe` (the `_panemin_probe` pattern: walk DOWN, print every descendant whose combined minimum meets the container’s).** **The right dock is canvas-exact** — `min=232.0 want=232.0 over=0.0`, *"within its role, nothing to blame"*. The roles resolve correctly, so `TABLET_PORTRAIT` and `set_portrait()` are working. **The left dock is `min=331.0 want=232.0 over=99.0`, and EVERY driver it names is `vis=false`.** The largest is **`CivilizationWorkspace` at 361 px**, then hidden `VBoxContainer` chains at 357, 346, 337 and 335, down to one at exactly 331. **So the dock is 99 px over because of panels a user cannot see.** Whatever workspace is displayed, the dock reserves room for the widest one ever built — which is why this does not move with the domain and why it looks like "the PC layout" at any tablet size. **One thing is NOT yet explained and must not be glossed: `CivilizationWorkspace` reports 361 while the dock reports 331.** The dock is therefore **not** simply taking the max of its children — something clamps between them. **Find that before changing anything**, because a fix aimed at the wrong link either does nothing or removes the clamp and makes the dock 361. **This is the same FAMILY as the PC left dock at 404-against-372** — a container reserving width for content it is not showing — but **not the same instance**: that one was `fit_to_longest_item` on a visible dropdown, and these are whole hidden workspaces. **99 of the 285 px portrait overflow is this**, so the long-running overflow row should be re-checked against it rather than pursued separately |
