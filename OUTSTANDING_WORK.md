...
**110 outstanding items** — 2026-09-21, after closing LOD-D6 (`c74a150`; off-main-thread tile
synthesis, desktop bars met — 199.50ms → 0.98ms at LOD entry, memory bounded by assertion — phone
bars honestly unmeasured, no device reachable) and filing two small follow-ups it flagged: `Arc`-ing
`WorldState`'s four `Vec<f32>` fields (the real further memory lever, deliberately not taken inside
D6) and `_loddbg_probe.gd`'s stale row-count assertion (pre-existing, unrelated to D6). Net: 109 − 1
(D6 closed) + 2 (follow-ups filed) = 110. 109 immediately before, after closing LOD-D5 (`c685930`;
...
| **`lod_cache_key`'s `glacial_snowline` term has no test coverage — its own mutant survives** | `LOD_DETAIL_SCOPE.md` (LOD-D4 follow-up) | small | **Filed 2026-09-21, from LOD-D4's own mutation pass (commit `02f6d51`).** Mutating `glacial_snowline.to_bits() → 0u64` inside `lod_cache_key` survives — the line is structurally right (without it, changing the snowline with the glacial pass off would serve a stale cached tile), but it's untestable from a unit test: `lod_cache_key` is a private method on `WorldGen`, a cdylib `GodotClass` no unit test can construct (`MISTAKES.md` row 32). Needs a `.tscn` probe: change `glacial_snowline` with the glacial pass off, re-render a tile, assert the ice moved. |
...
| **`_loddbg_probe.gd` asserts a stale row count (3, real is 5)** | `LOD_DETAIL_SCOPE.md` (LOD-D6 follow-up) | small | **Found 2026-09-21 as a pre-existing, unrelated failure surfaced while verifying LOD-D6** (confirmed via `git diff --name-only` showing no menu file touched by that work). `_build_lod_debug_submenu` now adds Grid, Colors, Labels, a separator and "Show tile borders on the map" — 5 items — while the probe still asserts 3. An earlier, undated menu change added rows without updating this probe's assertion. |
