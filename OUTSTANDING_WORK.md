...
| Story planning **SP-3** — the settlement timeline strip (simulated history + authored vault events + journey passes) — **stale blocker, corrected 2026-09-23** | `STORY_PLANNING_SCOPE.md` | large, real scope narrower than stated | **This row's own claim ("no per-settlement history accessor in `timeline.rs`") is false, confirmed at the symbol.** `cartalith_civ::timeline::civ_settlement_ownership_periods` is a real, tested per-settlement ownership-history accessor (`timeline.rs` ~1936, with its own test suite ~3371-3517), bridged to Godot (`lib.rs` ~1216-1222) and already consumed by `place_editor_window.gd` as the Settlement Editor's "Political history" tab — built as part of that editor's own closed plan (2026-09-21). `civilization_workspace.gd:1633`'s world-level strip citation may also be stale (not re-verified). **What SP-3 actually still needs, not yet scoped**: whether the existing Political History tab's simulated-history display already satisfies SP-3's "simulated history" leg, and if so, the real remaining work is narrower — merging in authored vault events and journey passes alongside it, not building a history accessor from scratch. Needs a scoping pass before building, not a re-derivation of this row's own stale premise. |
