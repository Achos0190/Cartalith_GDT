...
| **TABLET PORTRAIT: the VISIBLE World workspace needs 316 px and a disabled-axis scroller folds it onto a 232 px dock** | `TABLET_UI_SPEC.md` | medium | **Complete diagnosis, 2026-09-07, after two wrong ones I filed and retracted the same hour — both retractions kept below, because each was believed before it was checked.** **THE CHAIN, measured by a driver walk that descends only through children whose OWN `visible` is true:** `left_dock 331` ← `HBoxContainer 330` ← `VBoxContainer 324` ← **`ScrollContainer 324` with `h_scroll=DISABLED`** ← `VBoxContainer 316` ← **`WorldWorkspace 316`** ← … ← **five `HBoxContainer`s at 290 px each**. **So it is the trap `MISTAKES.md` already names, in the shape it names it:** *"a `ScrollContainer` with an axis DISABLED folds its child’s minimum into its own on that axis, so the overflow propagates to every ancestor with no scrollbar to reveal it."* `_scroll()` (`dcc_shell.gd:3963`) sets `horizontal_scroll_mode = SCROLL_MODE_DISABLED`, and the dock’s 232 role cannot win against a folded child minimum. **It is the VISIBLE workspace, not a hidden one.** `WorldWorkspace` is what is on screen, and its rows are authored for a wider dock than the tablet canvas draws. **So DS-03’s *"keep everything; reflow only"* is exactly the right instruction and simply has not been carried out for these rows.** The target is concrete: **five sibling `HBoxContainer`s at 290 px** inside `WorldWorkspace`, against a 232 px dock. **Do NOT "fix" it by enabling horizontal scrolling** — that stops the fold and hides the problem behind a scrollbar the canvas does not draw. **RETRACTED #1: "the role is not applied."** It is — `_left_width=232.0`, `role_left=232`, `ld_cms=232.0`. **RETRACTED #2: "hidden workspace panels size it."** They cannot; `CivilizationWorkspace` is `own_vis=false`. That one came from a probe printing `is_visible_in_tree()` where a container minimum uses the child’s **own** `visible` — **the mirror of the planner defect** |
