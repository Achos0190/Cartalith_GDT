...
2. **§2.9's three rows hide a survey, not an estimate.** `RC_ENGINE_CHANGES.md`
   specifies **43 distinct engine items** across v2.11–v2.68: 27 through v2.52,
   ...
   v2.62's navigable-river routing/flow-direction pass (§6n) and v2.67's ward-driven plot grain plus the corner-only parcel water test it exposed (§6s), and v2.68's never-rendered farmland fringe (§8.2 — render-only, but a port inherits the invisible-detail-kind defect) — the last of
