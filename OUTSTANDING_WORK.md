...
| **Hydrology reclassification's world-wrap judgment call** — **RULED 2026-09-21 — Ruling T: special-case `world=true`** | `LARGE_ITEM_RULINGS.md` Ruling T | small | Ruling Q's boundary-touching-is-ocean rule was applied uniformly to `world=true` (toroidal) maps, where X has no real edge — a judgment call the agent flagged as not dictated by `HYDROLOGY_CLASSIFICATION_RESEARCH.md`, which doesn't address wrapped-world topology at all. Confirmed to change real output: every golden file's `case_1_world_wrap` fixture moved under this rule. **Owner ruling: a wrapped map keeps the old size-primary rule; only bounded maps use Ruling Q's boundary-touching rule.** In flight — `build_water_bodies` branches on `world`, and every wrapped-world golden fixture across the 13 re-baselined files needs re-deriving toward its pre-Ruling-Q values for the wrapped case specifically. |
...
|---|---|---|---|
| **Refine detail placement** — **RULED 2026-09-21 — Ruling U: move to Preferences ▸ Tiles & LOD** | `LARGE_ITEM_RULINGS.md` Ruling U | small | **The one citation disagreement that was real shell drift.** Ruling item 4 (2026-09-05): *Atlas "Refine detail for the current view" moves to the WORLD rail beside Bake & finalize* — and the shell did that (`app.gd`, `DccWidgets.action(row, "Refine detail", …)`). The 2026-09-07 canvas doesn't draw it there at all; its GENERATE panel routes atlas/LOD work to Preferences ▸ Tiles & LOD instead, and the canvas postdates the ruling. **Owner ruling: follow the canvas.** In flight — move the control from the WORLD tool-options bar (`app.gd::_tool_options_generate()`, part of `tool_options_row`) to Preferences ▸ Tiles & LOD. |
