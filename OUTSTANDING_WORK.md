...
| **Hydrology reclassification's world-wrap judgment call — needs its own owner ruling if it matters** | `LARGE_ITEM_RULINGS.md` Ruling Q | small | Ruling Q's boundary-touching-is-ocean rule was applied uniformly to `world=true` (toroidal) maps, where X has no real edge — a judgment call the agent flagged as not dictated by `HYDROLOGY_CLASSIFICATION_RESEARCH.md`, which doesn't address wrapped-world topology at all. Confirmed to change real output: every golden file's `case_1_world_wrap` fixture moved under this rule. **Open question for the owner**: should a `world=true` map keep the old size-primary rule instead (since "touches the X edge" carries no marine signal when X wraps), or is uniform treatment fine? No code change implied either way unless the owner picks the special-case option. |
