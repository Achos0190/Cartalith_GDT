...
| **The vector river overlay** — **queued next, now the direct route to the owner's "flowing, smooth" bar** | `FUNCTIONAL_CONTRACT.md` cap. 6, `.claude/resume-2026-09-12/river_overlay_reverted_2026-09-13.patch` | small | The one leg of the old SDF row still genuinely unbuilt: `map_overlay.gd` has no `drawRiverWays` equivalent. **BUILT AND REVERTED 2026-09-13** (patch kept). The engine side already exists (`get_rivers(min_order)`, `engine_bridge.gd::rivers()`), and the vector draw worked (1 079 runs, 18 740 px inked). **Refuted on three counts:** it drew over the debug and info views where the reference's `drawRiverWays` runs only when `dbg==="off"` and `mode==="biome"`; it had no off switch; and the raster river tint still drew underneath. **Next:** re-apply gated on biome mode with debug off, add a Layers switch, and suppress the raster tint where vectors draw. **2026-09-21: now the direct route to the owner's explicit quality bar** stated when authorizing v2.72 — *"make sure we dont get the pixilated look. Rivers are flowing, smooth"*. v2.72's own fix (commit `fcaafea`) makes the raster network sparser but does not smooth any single river's stair-stepped disc edge; this row is what actually addresses that. `cartalith-hydrology`/`render.rs` are free again now that v2.72 landed. See also `channel_disc`'s linear falloff, flagged by the v2.72 agent as a cheap smoothing candidate but a deliberate deviation from the reference's own literal ink formula — needs a ruling if pursued, not built silently. |
