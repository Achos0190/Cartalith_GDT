...
| **OWNER-REPORTED: ALL overlay text is blurry — four draw sites, one cause: the overlay is a child of the SCALED camera** | `TERRAIN_APPEARANCE_SCOPE.md` | medium | **Owner, 2026-09-08: *"Settlement names are blurry, they don’t seem to live on their own layer so that they stay visually sharp and don’t get influenced by a zoom."*** **The diagnosis is the owner’s and it is CORRECT — confirmed at two symbols before this row was written.** `viewport_host.gd:802` and `:922` set **`_camera.scale = Vector2(_zoom, _zoom)`**, and `:447` does **`_camera.add_child(overlay)`**. So the overlay is *inside* the scaled node. **`map_overlay.gd` draws label text with `draw_string`/`draw_string_outline`** (`:2105-2106`) at a `label_px` its own header describes as *"in THIS control’s own local pixels"* (`:905`). **Vector text rasterised in local space and then magnified by the parent’s scale is a bitmap being stretched** — which is exactly what "blurry, influenced by a zoom" describes. **The fix is the owner’s words: labels need their own UNSCALED layer**, positioned by transforming world coordinates to screen rather than by living under the zoom. `viewport_host.gd:356` already notes that `_camera`’s logical `size` always equals the viewport’s, with *"`scale`/`position` handling zoom/pan on top"* — so the transform to do it with already exists. **Do not just raise the font size or the texture filter.** Both make it a different kind of wrong: the text would still rasterise once and scale, and `label_font_size` is a ported reference figure (`labels.rs`, `grid_w / 512` × `civ_zoom_k`) that other things read. **Check what else is under `_camera` and would want the same treatment**: pins, trait badges and the measure readouts all draw through the same `_draw()` | **WIDENED 2026-09-08 after the owner added *"Also blurry are map labels."* — and the widening changes the SCOPE of the fix, so it matters more than a second report.** **`map_overlay.gd` has FOUR text draw sites and every one is inside the same scaled control:** trait badges (`:1409`), glyphs (`:2063`), **settlement names (`:2105-2106`)** and **map labels (`:2480-2481`)**. The owner has now reported two of the four. **So this is not "settlement labels are blurry" — it is that every string this overlay draws is, for one reason.** The unscaled-layer fix covers all four; **doing it for settlement names alone would leave three behind and would look like a partial fix to the person who reported both.** **The two label systems are otherwise unrelated**, which is what makes the shared cause worth stating: settlement names come from the civ layer, map labels are the CARTO `MapLabel` entities a user places by hand. **They have different data, different sizing (`_label_font_px` vs `labels.rs::label_font_size`) and different lifecycles — and they blur identically, because the blur is not theirs.** **Check the fourth site before assuming symmetry:** `:2471` notes *"untracked upright text keeps the single `draw_string` it always"* had, so at least one path is deliberately different and may not move the same way |
| **OWNER-REPORTED on PC: the LOD/Atlas tile DEBUG layer does not render properly** | `LOD_TILING_BASE_SCOPE.md` | small | **Owner, 2026-09-08: *"the debug layer for the LOD/Atlas tiles doesn’t seem to render properly on pc."*** **The machinery is all present, so this is a behaviour question rather than a missing feature.** `_lod_debug_layer` is built at `viewport_host.gd:410`, its `draw` signal is connected to `_draw_lod_debug` at `:415`, it is parented into `_lod_layer` at `:416`, `_draw_lod_debug` really does draw (`:3107`, `draw_rect`), and `Help ▸ LOD debug` drives three flags through `menus.gd::_build_lod_debug_submenu()`. **Two candidates, both cheap to separate and neither confirmed:** **(a) it is not redrawn when the camera moves.** `:2977-2978` sets `visible` from the three toggles and then calls `queue_redraw()` — and the comment right above it (`:2975-2976`) says *"hidden when every toggle is off means `_draw_lod_debug` is not even queued on a camera move"*, which describes the OFF case and leaves the ON case unstated. **If nothing queues a redraw on pan/zoom, the overlay is a stale picture of where the tiles used to be** — which reads exactly as "does not render properly". **(b) it draws in the wrong space.** `_lod_layer` is a child of `_camera` (`:400`), whose scale is the zoom, so anything drawn in local pixels is magnified — **the same mechanism confirmed for the blurry settlement labels**, one row up. **Do (a) first: it is one `queue_redraw()` and a signal, and it would explain the report without touching the draw code at all.** Check on PC with a toggle ON and the camera moving, and say which candidate the evidence picks |
| **OWNER-REPORTED: zooming and changing LOD does not make the view sharper** | `LOD_TILING_BASE_SCOPE.md` | medium | **Owner, 2026-09-08: *"Zooming and Shifting to another LOD time doesn’t seem to make the view sharper."*** **Not yet diagnosed, and the three candidates need separating before anything is changed** — they have different fixes and the existing evidence cannot tell them apart: **(a) the LOD never switches** — `_update_lod()` exists (`viewport_host.gd:212` area) with `native_px_per_cell` and `LOD_AUTO_SCALE = 2.2`, so the arithmetic is there; whether the switch fires on zoom is a separate question. **(b) it switches and the tiles carry no more detail** — `lod_synthesize_tile(z, col, row)` (`lib.rs:11733`) SYNTHESISES a tile; if a deeper `z` is upsampled from the same source grid rather than resampled from finer data, a sharper level cannot exist. **This is the one that would make the report permanently true**, and it is a question about the engine, not the shell. **(c) it switches, the tiles are finer, and a filter blurs them** — `map_view.texture_filter` and the sprite scaling at `viewport_host.gd:2745`. **Establish WHICH on glass first**, and note that the owner reports it while also reporting blurry labels: **those are separate mechanisms and one is already confirmed**, so do not let the label finding stand in for this one | **THE REFERENCE ANSWERS THIS, and the owner said to look there** (*"maybe check the system in the html to see how it works there"*). Read out of `reference/Cartalith Gen1 v2.11.html`: **`_lodRenderW()` (`:10706`) returns `max(GW, min(LOD_MAX_RENDER_PX, round(GW * LOD_SUPERSAMPLE)))`, with `LOD_SUPERSAMPLE = 2` and `LOD_MAX_RENDER_PX = 2560`.** So the reference renders its LOD view at **twice the grid width, capped at 2560 px** — never below `GW`. **`grep` for `SUPERSAMPLE`, `MAX_RENDER`, `render_w` or `2560` across `lod_bridge.rs`, `lib.rs` and `viewport_host.gd` returns NOTHING.** The port appears to have ported the detail half and not the resolution half. **The detail half IS ported and golden-tested** — `lod_bridge.rs:28` cites `cartalith_terrain::amplify::amplify_region`, *"bilinear upsample of the coarse height field plus procedural sub-cell detail"*, and `:138` quotes the reference’s own `addZoomDetail` header. **So this is not "the tiles carry no detail"; it is that they may be rendered at 1× where the reference uses 2×.** **And the reference records this as an OWNER REQUEST, in the same words:** `:11203` heads a v0.93 fix with *"owner request: optimize + 'no pixelated views or blockyness when zoomin in on terrain'"*, and names a **512 px cap fix** that *"already eliminated the blocky/faceted quantization for the steady-state image"*. **The same complaint, answered once already, in the system this one is a port of.** **Treat this as a LEAD, not the answer.** It is a strong one and it was found by following the owner’s suggestion rather than by measuring the port — whether the port compensates elsewhere (more tiles at deeper `z` rather than more pixels per tile) is **not established**, and candidate (a) has not been excluded |
| **OWNER-REPORTED: settlements that appear on zoom are not connected to roads** | `PHASE2_SCOPE.md` | medium | **Owner, 2026-09-08: *"Not all settlements (the ones that appear on zoom) are not connected to roads."*** **Read as: the settlements which only become visible at higher zoom have no roads reaching them.** **Where the two systems meet, and why this is plausible rather than surprising:** `map_overlay.gd`’s `CIV_LOD_PLACE` (its header cites reference line 15373) gates a settlement’s DRAWING on a minimum camera zoom — *"a capital or city (threshold 0) is always full-size, town needs a little zoom-in"*. **So zoom controls visibility, by rank.** If road generation also works by rank, the low-rank settlements would have been excluded from the network rather than merely hidden. **The distinction that decides the fix, and it must be measured not assumed:** are the roads MISSING from the data (a generation question, `cartalith-civ`), or present and not DRAWN at that zoom (an overlay question, the same `CIV_LOD_PLACE` gate applied to ways)? **A screenshot cannot tell those apart** — query the route/way data for a settlement the owner can see and check whether an edge reaches it. **If the roads are genuinely absent, that is correct-by-design and the row becomes a design question**: a hamlet may not warrant a road. **Ask before building a connector** |
