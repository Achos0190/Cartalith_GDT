...
| **Change the ported urban algorithm toward the owner's town plan (Ruling H) — 3 of 4 candidates now done; candidate 4 ruled and ready to build** | `URBAN_MORPHOLOGY_SCOPE.md` | small remaining | (1) **Legible wedge blocks at size — CLOSED 2026-09-23 (verified), commit `ff8c525`.** Radial/Venus towns now get the wall lots and the faubourg the organic branch already had (Ruling AA: extend the radial branch, not a `grow` reshape). `generate.rs`'s `profile.planning != "radial"` gate around `build_wall_lots` removed; `wallside.rs` needed no code change (graph-agnostic); `districts.rs::build_buildings` gained two `WallBacking::No` guards so the Venus grammar can't reach what wallside placed. Golden re-baseline: 3 of 29 cases move (`venusRadial`, `venusSmall`, `venusTinyCanal`), disclosed old→new; 26 unmoved (23 organic control + 3 Venus cases structurally unreachable — 2 bastioned, 1 with no wall circuit). `cargo test --workspace --no-fail-fast` reproduced 3559/0/37. Ribbon suburbs from `grow` deliberately NOT ported (not separable from `grow`'s organic-specific loop — would have been the macro-structure reshape Ruling AA rejected); most radial towns have no land gate at all, so the faubourg falls back to a wall starting point rather than an approach road, disclosed in the golden header. (2) **Suburbs thinning with distance — ALREADY BUILT** (doc note only, folded into (1)'s closure). (3) **A wall gap where there is no gate — ALREADY BUILT** (same). (4) **Perimeter lots with open courtyards at high density — ruled 2026-09-23, Ruling AD: the outermost ring by distance from market**, matching how other density gradients in this engine already key on market distance. Not yet built — genuinely ready now, courtyard buildings today exist only as per-parcel typology on strip lots (`districts.rs::build_buildings`), not the ring-around-a-shared-courtyard block subdivision the owner's image describes. |
...
| **IN-13 — trade flows: 3 of 4 sub-questions ruled, genuinely ready to schedule** | `STATUS.md`, `GUI_GAP_REGISTER.md` §42/§43, `LARGE_ITEM_RULINGS.md` Ruling R | large | **Moved back here 2026-09-23 after a scoping pass (no code changed) found this row's own framing was stale in the same way as two other large rows this session** — it read as if all four named pieces (bipartite match, network flow, prices/tariffs, caravans) were open; **the first two closed 2026-08-25 under `GUI_GAP_REGISTER.md` §42** and this row was never updated to say so. `cartalith_civ::trade::trade_flows()` (`trade.rs`) is a real bipartite match generalizing the reference's food-only `_civFoodShed`/`_civFoodConnected`/`_civGoodReach`/`_civRoadComponents` to all 15 `CIV_RESOURCE_KEYS`, with a `WayRouter` doing real Dijkstra network flow over the way graph (not a straight-line assumption) — wired end to end (`civ_trade_bridge.rs`, `trade_store.gd`'s whole-network-then-filter cache, three live UI surfaces), verified by a committed, runnable probe (`_in13_probe.gd`: 624 flows over 7/15 goods in 1ms, 0.18MB transient, 0 resident, determinism and independently-restated-constant checks all pass). **What's actually left, per §42's own 2026-08-25 closing note**: prices, tariffs, caravans as entities, and trade that changes over time. Ruling R (2026-09-21, quoted exactly: *"each faction has its own currency, with an exchange rate between any two [...] Cross-faction trade needs a conversion step at the point of exchange, not a single universal unit of account"*) answers only the currency-model question, not the rest. **Grepped, not assumed**: zero real hits for currency/exchange-rate/tariff anywhere in the engine (two doc-comment hits are both negative references — "never a currency"); "caravan" exists only as an already-ported journey-planner party-size classifier and travel-library flavor text, not an entity. **The reference has neither a price/tariff system nor a caravan entity** — this is genuinely new engine work with no JS function to port. **Why there's no safe small slice**: `TradeFlow::volume` is a physical demand quantity, not a price — there is no price anywhere in the engine today, and `trade_flows()` is faction-blind (never reads which faction owns which settlement), so a tariff cannot apply "at the point of exchange" without first making the already-shipped, probe-verified match faction-aware, which is a change to its existing `TradeInput`/`TradeFlow` shape, not additive work beside it. Currency with no price and no consumer is exactly the dead-code trap `RC_ENGINE_CHANGES.md` §6p names ("a field with no consumer is dead code"). Caravans have a concrete architectural template (`cartalith_civ::travel_library::Journey` — route + start_year + a DTO persisted per `SAVEFILE_COMPAT.md` §9.6) but undecided semantics (does one caravan represent one `TradeFlow` row, an aggregate per way, or a manual shipment; how does it move against the Timeline's year cursor, given `trade_flows()` is deliberately stateless — "allocates, answers, drops... nothing here is stored"). **Three of four sub-questions now ruled 2026-09-23.** **Ruling AB**: price derives from `TradeBalance`'s existing surplus/deficit (scarcity-based), no new authored table — settles only the price-basis question, not the derivation curve's exact shape. **Ruling AE**: a tariff rate gets its own new relationship field, not a reuse of `civ_faction_relations` (whose own module doc is explicit it deliberately isn't diplomacy). **Ruling AF**: one caravan is one aggregate shipment per way — a periodic bundle of all matched trade volume currently routed over one way, not one entity per flow and not a manual player action; closest in shape to `cartalith_civ::travel_library::Journey` (route + start_year + a persisted DTO). **Genuinely ready to schedule now** — this is real architectural surface (making the already-shipped, probe-verified `trade_flows()` match faction-aware; a new price-derivation curve; a new relationship field; a new persisted entity type following `Journey`'s template), recommend scoping/sequencing first given the size. **One question remains, not yet asked**: confirm the faction-aware match must not move the existing single-faction/no-tariff probe output — a verification constraint to design against, not an owner decision, so it doesn't block starting. A future build's acceptance bar (no golden is possible — no reference exists): a probe against a real multi-faction world checking cross-faction conversion round-trips at a stated tolerance, a tariff strictly reducing realized volume/value versus the untariffed case (direction-of-effect, not just "a number changed"), a caravan surviving save/load byte-identically like `Journey`'s DTO, and the existing determinism/zero-retention bar `_in13_probe.gd` already sets. |
