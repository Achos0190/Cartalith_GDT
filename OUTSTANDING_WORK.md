...
**74 outstanding items** — 2026-09-23 (latest), **net 0**: **§2.6's GPU memory pooling row,
...
| **Landmark M9** — cultural interpretation and temporal state | `LANDMARK_GENERATION_SCOPE.md` | large | Open questions 1-2, plus the wiring itself. **SP-4 no longer blocks it** (built 2026-09-23): `cartalith_civ::conflict` exists with year ranges and `tid` anchors, but `LandmarkInputs` does not take conflicts, so `battlefield`, `battlefield_historic` and `destroyed_fortress` stay `buildable: false` — their `not_built` reasons now say exactly that instead of "there is no conflict entity". |
