...
|---|---|---|---|
| **The `UNWIRED_FUNCTIONS.md` backlog** — 3 open rows (1 small · 0 medium · 2 large minus 1 deferred-by-ruling = 1 actionable large), 0 open dangerous-class | `UNWIRED_FUNCTIONS.md` | large | **Corrected 2026-09-21 from the code, not from the document's own count** (the same defect class this row exists to catch, twice over this pass — see below). Re-verified directly: **statusMid** is still ~90% built, `repaint NN ms` still blocked on Owner question 2 (unchanged). **Paint preview patches: CLOSED**, and this row's own first draft got it wrong too — it repeated `UNWIRED_FUNCTIONS.md`'s stale "no shell caller" claim and filed a new "in flight" row for it; an agent dispatched to build that found the wiring already existed (commit `45df3019`, 2026-09-04, same day as the patch API), independently re-verified (`_paintpatch_probe.gd` re-run headless by the coordinating session, `PASS 0 failures`). The sculpt half remains honestly declined (SL-1: would need `render.rs`'s AO/wetness/sea passes reworked over a caller-supplied window). **Cut/Copy/Paste**: selection sets exist and Select-all works, but **the clipboard itself does not exist** — nothing serialises an icon or label into a buffer, holds it across a world change, or pastes it back; all three menu items are still `_todo`. Still genuinely in flight this batch — see below. **The 3D viewport** stays open by ruling (deferred, research first) — correctly parked, not actionable. **`label_glyph_layout`** stays open as a genuine design question (rerouting would silently change today's rendering — an arc+tracking contract gap), not a stale claim. **Dangerous class: 0 open, 9 closed** — re-verified at the symbol 2026-09-21, eight directly (`2c825ed`), the ninth (`performance_window.gd`, b-8) by the false string's total absence from the tree (the file itself no longer exists). Stale source comments: 0 open / 6 closed (commit `e462ed9`). Large-tier header corrected 3→2 (Region ▸ New world from selection already `CLOSED` in the table). **Net actionable remainder: `label_glyph_layout` (small, design question — not dispatched, needs care) and Cut/Copy/Paste (large, clipboard — in flight, see below).** |
