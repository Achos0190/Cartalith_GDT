...
| Story planning **SP-2** — journey progression over the cursor — **both open questions RULED 2026-09-23, not built** | `STORY_PLANNING_SCOPE.md`, `LARGE_ITEM_RULINGS.md` Ruling AO | large | **No longer blocked on a decision.** Owner ruling: regenerate re-snaps a journey's route onto the new terrain (must handle a stop point that no longer exists and a route no longer geometrically valid — name the edge-case behaviour explicitly). Grain is a real calendar date, reusing the year/month/day Chronos (Ruling AM) already writes rather than a second date system — `chronos::Event`'s month/day are parsed but currently dropped by the reader; a specific 12-month/365-day calendar still needs picking by whoever builds this. Ready to build. |
