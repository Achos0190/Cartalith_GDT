...
| **A scroll gesture that starts on a slider silently rewrites the parameter** | `ANDROID_UI_SPEC.md` | medium | **Found by the verifier on glass, on its first swipe, 2026-09-07 — and it is the kind this project treats as worst: a silent wrong value with no on-screen trace.** `_pg_open_gestures()` gives `Range` `MOUSE_FILTER_STOP`, which is documented and correct for a horizontal drag. **A vertical swipe intended as a scroll is then consumed as a value write:** Ocean depth went **0.60 → 0.14 in one gesture** and marked stage 03 stale. **The canvas has the same shape** (`input[type=range]{height:22px}`), **but this batch’s own 44 dp touch floor widens the hazard band from 32 dp to 44 dp** — so a change made here makes an existing hazard measurably worse, and neither the code comment nor `ANDROID_UI_SPEC.md` §1.11 names the consequence. **The fix is gesture arbitration, not a smaller target:** a drag whose motion is predominantly vertical past a threshold belongs to the scroller. **Do not "fix" it by shrinking the row back under the tap floor** |
| **The phone Archetype row routes the user to a control the phone does not have** | `DESIGN_HANDOFF.md` §6.7 | small | **Found by the verifier 2026-09-07 and confirmed on glass; five of the six dashed reasons held, this one did not.** The GENERATE sheet dashes Archetype with *"Pick it in File ▸ New world"* — **and on a phone that dialog has no Archetype control.** `new_world_dialog.gd::_build()` sets `rest.visible = false` when `_phone`, and `archetype_input` is built into `struct_sec`, which is parented to `rest`. The phone card shows SEED, EXTENT and RESOLUTION and nothing else. **So the reason is true about the desktop and false about the device it is printed on, which is worse than no reason at all** — a dash’s reason is itself a claim. **Same root as the km/size row below**, and both are answered by the same question: which of the hidden `rest` controls belong on the phone card |
