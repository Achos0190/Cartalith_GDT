...
**108 outstanding items** — 2026-09-21, after closing `LARGE_ITEM_RULINGS.md`'s Ruling L in full
...
| **`_railfold_probe.gd`'s Labels/Icons placement-rule controls are live, not inert — the probe still asserts inert** | CARTO / `icon_bridge`, no owning scope document | small | **Found 2026-09-21 while closing Ruling L**, confirmed pre-existing and unrelated to the rail re-sort (identical assertion, identical failure, on the unmodified `HEAD` copy). `_railfold_probe.gd` §6 asserts the Labels category's three per-class dials and the Icons category's scale/min-spacing sliders are drawn-but-inert stub controls, and that all three Icons placement rules (`avoid label boxes`, `enforce min spacing`, `snap sea marks to coast`) are disabled. All are now live/enabled with no reason text. Something wired real functionality behind these controls without the probe (or any backlog row) being updated — re-open at the symbol (`icon_bridge`'s placement-rule code, `cartography_workspace.gd`'s Labels per-class dials) before assuming this is a regression rather than a shipped feature the probe never caught up to. **In flight 2026-09-21.** |
| **`_v3menu_probe.gd`'s Data-menu-popup lookup is stale — keys off "Journey planner" text, which is no longer in the Data menu** | `LARGE_ITEM_RULINGS.md` (Ruling L, CIVIL half follow-up) | small | **Found 2026-09-21 while closing Ruling L's WORLD/CARTO halves**, confirmed pre-existing (unchanged from `HEAD`, so caused by the *earlier* CIVIL half of Ruling L, commit `5f839d7`/`7fbf7b9`, not by this session's WORLD/CARTO work). `_v3menu_probe.gd::_data_popup()` finds "the Data menu" by searching every `PopupMenu` in the tree for one containing item text matching "Journey planner" — a detection method that broke when Journey planner became a CIVIL rail node instead of a Data-menu row. Needs a detection method keyed on something that still lives in the Data menu, or a direct node reference. **In flight 2026-09-21.** |
