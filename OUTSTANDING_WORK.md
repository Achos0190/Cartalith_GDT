...
| **`ASSET_GRID_COLS` is held by the design spec alone — no assertion covers it** | `DESIGN_HANDOFF.md` | small | **A coverage claim that would not have gone red, caught by the verifier 2026-09-06.** The constant's comment said the touch floor pinned it and *"a fifth column would take it under on a narrow handset"*. Setting it to 5 and re-running at 540x1200 and 1080x2400 gives **`0 failure(s)` both times** — five columns at 412 dp is about **71 dp square**, still well clear of `PHONE_TAP_MIN`; `fits` misses it too (`body_min_w` 819 → 1008 against a 1080 screen). **The value 4 is correct** — §6.6 draws `repeat(4,1fr)` — but nothing tests it. Comment corrected to say so plainly. **Either write the assertion that pins the design geometry, or leave it stated as spec-held; do not restore the floor claim** |
...
| **Timeline stores mutations, not per-year snapshots** (ruling 27) | `SAVEFILE_COMPAT.md` §10.2 | large | **Owner ruling 2026-09-06, and the memory cost is the headline, not the archive.** `TimelineSnapshot` is `{ year, territory: Vec<i32>, settlements, ways }` — every recorded year keeps a **full territory raster and a complete copy of every settlement and way** — and `CivData.timeline` is a live `Vec`, so one snapshot is **10.74 MB of territory in RAM** at 2048×1311 against a phone that already peaks near 878 MB with none. **On disk the redundancy is real but milder:** deflate compresses each year independently, so 100 unchanged years cost 100 × ~40 KB rather than ~40 KB. **Reconstruction must be exact and that is the test** — replay to year N against a full snapshot taken the old way. **Keyframes are probably still wanted** (the year cursor is dragged; pure deltas replay from the start). Ends §10.2's on-disk shape, so it needs a `format_version` bump and §18.4's **fail-loud** marker discipline — a reader ignoring it must not read plausible-looking noise |
| **LOD tiles stored in the save, optionally** (ruling 28) | `SAVEFILE_COMPAT.md` | medium | **Owner ruling 2026-09-06, and smaller than it looks: the archive already carries them.** `cartography/tiles/**` round-trips today as **foreign entries**, with a test asserting a tile survives open-and-resave byte-identically — so this is promotion to a first-class optional slot, not new plumbing. **Two things it does need:** a **producer** (tiles are synthesised on demand by `lod_bridge::synthesize_tile_rgba`; the atlas cache is deferred at M3, so nothing holds a set to write), and **invalidation** — a stored set is a cache that goes stale against a regenerated or sculpted heightmap, and `StageGraph`/`Staleness` is the existing mechanism. **Silently drawing a stale tile is the failure to design against; prefer dropping them.** Default recommended **off with the size shown at save time** — a pyramid is ~4/3 of its base and is the one slot that can outgrow the three float grids. **Measure a real pyramid before writing a default into the UI** |
