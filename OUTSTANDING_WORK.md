...
| `DccBrowseDialog` opens narrower than its own contents at deep paths | `UNWIRED_FUNCTIONS.md` | small | **Measured 2026-09-05 by a verifier at three named densities**, refuting the lane's own claim that the size set in `setup()` is discarded: the window is **(760, 640)** in all three, but `contents_min` reaches **790 px** at a 9-segment path and **800 px** at 13 — so a deep breadcrumb overflows its own window by 30-40 px. Newly reachable, because eight pickers now open this dialog where they previously opened a stock `FileDialog` |
