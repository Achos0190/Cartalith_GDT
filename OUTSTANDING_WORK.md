...
| **TABLET PORTRAIT: the RIGHT dock is already canvas-exact; the LEFT dock is 99 px over its own role** | `TABLET_UI_SPEC.md` | medium | **Measured 2026-09-07 in the main loop, and it narrows the tablet problem sharply. `_tabspec_probe` at `--force-touch --vp 800x1280`:** `w_left_dock=232  w_right_dock=232` — **the roles resolve to the canvas figure**, so `TABLET_PORTRAIT` and `DccShell`’s `set_portrait(not _landscape)` (`dcc_shell.gd:841`) are **working**. **`right_dock w=232.0 canvas=232.0`** — exact. **`left_dock w=331.0 canvas=232.0`, and `left_dock min.x=331.0`** — so it is not the role being ignored, **a CHILD is forcing the dock 99 px wider than the role it was given.** **This corrects `TABLET_UI_SPEC.md` §2.4/§2.6, which record both docks at "400 px, every frame."** They are not: at portrait the right is 232 and the left is 331. Neither is 400. **And it accounts for most of the overflow that has cost two batches:** rail 48 + left 331 + right 232 + viewport = the 1085 shell floor against an 800 px frame. **99 of the 285 px is this one dock.** **HYPOTHESIS, flagged as one because three of mine were wrong this week:** this is the same shape as the PC left dock at 404-against-372 — `fit_to_longest_item` reserving *"Established Caravan Route"* (185 px) while displaying *"Auto"* (59 px). That was fixed dock-scoped and PC went 10/10 conformant; **whether the fix reaches tablet density, or whether a different child now dominates, is NOT established.** **`_dockfit_probe` cannot answer it** — it reads `-- --vp WxH` and nothing else, and treats `--force-touch` as fatal, so it is pointer-only. **Finding the driver needs a walk down from `left_dock` printing each child’s `get_combined_minimum_size().x`, the way `_panemin_probe` does for the data manager** — that probe is the pattern to copy |
