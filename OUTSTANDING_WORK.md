...
| **`landmark.rs`'s own doc comments cite stale buildable/blocked counts** — found in passing while landing border marker, 2026-09-21 | `cartalith-civ/src/landmark.rs` | small | The module's top-of-file comment (`//! [`kinds`] declares all 49 landmark types... Twenty are...`) and the `not_built` field's own doc comment (citing 36, then 34, then 29 blocked at various past points) both predate M7 and the border-marker fix — real count as of `92cd8d0` is 23 blocked / 26 buildable. A one-line fix, deliberately left alone by the agent that found it (out of that task's scope) rather than touched in passing. |
