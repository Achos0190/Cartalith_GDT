...
**107 outstanding items** — 2026-09-21, unchanged in count through a batch that filed two rows and
...
| **Landmark M8 residual** — 29 of 49 declared kinds still ship `buildable:false` (denominator and blocked count both corrected 2026-09-21 — see below) | `LANDMARK_GENERATION_SCOPE.md` | large | **Twenty** generate today (`landmark.rs::kinds()`, each unbuilt kind carrying a `not_built:` reason). **Counting artifact found and corrected 2026-09-21**: `grep -c "KindSpec {"` and `grep -c "buildable: false"` each over-count by one — the former also matches `pub struct LandmarkKindSpec {` (line 210), the latter also matches the field's own doc comment, *"Why a `buildable: false` kind is not generated"* (line 231). Real counts, confirmed by direct read of both lines: **49** declared kinds, **29** blocked (20 buildable = 49 − 29, unchanged). **All 29 blocked kinds independently re-audited 2026-09-21, at their cited symbol, for the exact defect this session found twice elsewhere the same day (`UNWIRED_FUNCTIONS.md`'s paint-preview and clipboard rows — a reason naming a missing capability that had since shipped): zero found stale.** Two genuine near-misses investigated and refuted, not just dismissed: `cartalith-erosion::route_sediment`'s real delta-building behavior (gated behind the optional interactive Erode op, not persistent post-generation, no queryable per-cell record) for `delta`; and `build_landform_field`'s cirque classifier (a real morphometric test, but a deliberate 2026-09-01 judgment that it's "a shape test, not a history of ice," re-confirmed still true) for `glacial_feature`. `ruin`/`abandoned_settlement`'s claimed mechanism spot-checked directly by the coordinating session: `civ_collapse_step` (`timeline.rs:1557`) genuinely drops an abandoned settlement's coordinates via `continue` before `out_places.push(p)`. `cargo test --workspace --no-fail-fast`: 3417 passed, 0 failed, unchanged (no code touched — a negative finding, not a build). **2026-09-02: the five way-graph kinds landed** — `market_site`, `trade_depot`, `caravan_station`, `bridge_site`, `road_junction` — taking buildable 15 → 20, each verified placing on a real `generate_terrain` world rather than by flipping its flag. `resource_extraction_site` went buildable 2026-09-01. Six still need M7's viewshed (confirmed still open, `STATUS.md` LM-7); several need §13's route load or `STORY_PLANNING_SCOPE.md` SP-4 (both confirmed still open); the military family is downstream of Fort. |
...
| **CV-23** — historical territorial occupation over time | `STATUS.md` | large | **In flight 2026-09-21.** Timeline work, not territory work — `GUI_GAP_REGISTER.md`'s own CV-23 entry (§41, narrowed 2026-08-25) closed the border/claims/influence half and left exactly this: no per-year ownership record beyond the timeline's settlement snapshots. Confirmed unbuilt before dispatch: `cartalith-io`'s `HISTORY_TERRITORY_PREFIX` (`"history/territory/"`, `project.rs`, citing `SAVEFILE_COMPAT.md` §5.1) is a reserved save-format slot with **zero readers or writers** anywhere in the workspace — re-exported once, used nowhere, the same "declared and never wired" shape this session found twice already today. Scoped to the engine-side historical record only (extend the existing per-year `TimelineSnapshot`/`civ_year_diff` mechanism to also capture territory ownership, and use the reserved save prefix for real) — no new UI required to close this row. |
...
| **`lod_cache_key` over-invalidates on `glacial_snowline` when ice is off** | `LOD_DETAIL_SCOPE.md` (LOD-D4/glacial-probe follow-up) | small | **Filed 2026-09-21**, found while closing the row above. `lod_cache_key` folds in `passes.glacial_snowline` unconditionally, but `render.rs`'s `glacier_on`/`cryo` locals only read the glacier field when `appearance.ice_strength > 0.0`. So with ice off, changing the snowline still forces a fresh `TileFields` build (up to ~480 ms per the cache-key doc comment's own cost figures) even though the rebuilt pixels are guaranteed identical to the stale ones. Gate that one cache-key term on `ice_strength > 0.0` to skip the wasted rebuild — never a wrong-pixel risk, purely a cost one. |
...
| **`WorldState`'s four `Vec<f32>` fields should be `Arc`, not cloned, for LOD-D6's snapshot** | `LOD_DETAIL_SCOPE.md` (LOD-D6 follow-up) | medium | **Filed 2026-09-21, from LOD-D6's own memory measurement (commit `c74a150`).** The `LodSnapshot`'s retained clone is 51.21 MB at 2048×1311 — 81% of the milestone's own 60 MiB memory budget on its own, so per-tier cache size is barely a lever at this grid; the real cost is cloning the four world fields (plus river ink) because a background thread cannot borrow from `WorldGen`. Making `WorldState`'s fields `Arc<Vec<f32>>` at the source would let the snapshot clone the `Arc`, not the data — a cross-crate type change (six crates read `WorldState`) deliberately not taken inside LOD-D6 itself. |
...
| **Landmark M7 — viewshed / line-of-sight** | `LANDMARK_GENERATION_SCOPE.md` | large | **In flight 2026-09-21.** Open question 5: the accuracy/cost budget (observer count, radius cap, grid resolution). §5 states the complexity honestly and deliberately does not choose a number. **Gates six of the 49 landmark kinds**, and `needs_viewshed` already ships as a declared flag with no implementation behind it. 8192² is 67 M cells, so it is not a naive all-pairs proposition. Confirmed unbuilt before dispatch: no `fn viewshed`/`line_of_sight`/`los_` symbol anywhere in the workspace. Dispatched with instructions to pick and clearly disclose conservative, bounded default parameters rather than wait on the owner's number, following this session's established pattern (LOD_DETAIL_SCOPE.md's "owner questions with defaults", the paste-offset/hitch-budget defaults chosen autonomously and disclosed elsewhere this session). |
