...
|---|---|---|---|
| **27 of `WorldGen`'s 459 `#[func]`s have no `engine_bridge.gd` forwarder; 12 are unreachable from GDScript entirely** | `UNWIRED_FUNCTIONS.md` | medium | **Measured 2026-09-05 by the lane that fixed two of them, and re-derived independently by the verifier.** `EngineBridge` is the GDScript wrapper, so a `#[func]` with no forwarder is invisible to the shell no matter how healthy the `.dll` is — the failure reads as `Nonexistent function 'X' in base 'Node (EngineBridge)'` and is routinely misdiagnosed as a stale build (it was, twice, this week). Of the 27, **15 are reached directly** by callers that skip the wrapper and **12 are unreached entirely**. `get_rivers` is on that list and is the reason the Data-manager's `rivers` chip is dashed — the count exists, the forwarder does not. Also measured: **1 861 `bridge.*` call sites, 4 naming an undefined method before this batch and 2 after**, both survivors already guarded. Do not fix all 27 blind — the 12 unreached ones need a caller before a forwarder means anything |
| **`cartalith-civ`'s `UrbanLayout` adapter drops `bridges`/`ford`, so the dock cannot count real crossings** | `URBAN_MORPHOLOGY_SCOPE.md` | small | **Found 2026-09-05 by a verifier correcting a lane's diagnosis, and the wrong diagnosis is the expensive part.** The shell dashed the settlement's BRIDGES row with *"detectRiverCrossings is unported"* — **in a user-visible tooltip**. It is ported: `cartalith-urban/src/water.rs::detect_river_crossings` runs at `generate.rs:683` on the final graph, writes `site.bridges`/`site.ford`, and carries dedicated ordering tests. The gap is one layer nearer — the adapter does not carry the field through, which is exactly what `urban_bridge.rs`'s own doc means by *"still 'one field away' and unsurfaced"*. **The fix is an adapter field plus a `d.set`, not a port**; the false reason would have scheduled a subsystem that already exists |
| **Drawing the status bar's autosave slot exposes a duplicate only `app.gd` can remove** | `DESIGN_HANDOFF.md` | small | **Confirmed 2026-09-05 by the verifier, as a real bug rather than a lane error.** The autosave slot was registered and never drawn on desktop before this batch; drawing it surfaces a second copy of the same state that the status bar cannot reach, because `app.gd` owns it. Out of the lane's file, so left rather than half-fixed |
...
| Nine Asset-pack commands lost their search terms to the flat submenu | `STATUS.md` | small | **Disclosed 2026-09-05 and confirmed: no capability lost, only discoverability.** The ruled 4-bands → 9-row flattening dropped fifteen item rows; `collect into set`, `slot transform` and `duplicate` now return 0 title matches. Every action remains reachable in the Asset library window itself. Either add `command_index.gd` `EXTRAS` rows for the ones worth finding by name, or record that the library window is their only home |
| `DccWidgets.action` buttons are 39 px against a 44 px touch floor | `DESIGN_HANDOFF.md` | small | **Measured 2026-09-05, and shell-wide rather than new.** `Refine detail` is 118×39; its two neighbours on the same bar, `Bake ALL & finalize` (164×39) and `Center landmasses` (149×39), predate this batch. 39 px is `DccWidgets.action`'s height everywhere. A standing issue against the 44×44 minimum, not a regression of the move that surfaced it |
...
| **Δ vertical — re-present as a horizontal profile** (X = distance, Y = height) | `DESIGN_HANDOFF.md` | medium | **Ruled 2026-09-05.** It stays live in 2D and the canvas's "3D relief only" is superseded (no 3D viewport exists, so gating it would make the mode unreachable). **New work beyond conformance:** the owner asked for the readout rotated to a horizontal **cross-section profile chart** — distance on X, height on Y — over the same measurements the numeric list already computes. **A drawn design for it already existed and nobody had noticed** (found 2026-09-05): `05-right-dock-and-bars.md` §5.7's cross-section strip is exactly this chart — same axes, `viewBox="0 0 1000 130"`, gridlines at `y=43`/`y=86`, `secFillCol`/`secLineCol` per theme, x-labels `A · 0` / mid / `B · len`. So this is a **re-siting at dock width, not a new design**, and it inherits §5.7's two recorded defects: the header claims `×4 exaggeration` the code never applies, and says *120 samples* where the loop yields **121**. Drawn on the 2026-09-05 canvas with the true figures |
