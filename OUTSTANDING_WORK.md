...
| **`_datapane_probe.gd:398` is red and nobody owns it** | `GUI_GAP_REGISTER.md` | small | **Found 2026-09-06 and reproduced by the verifier: 75 ok / 1 fail.** The assertion is *"rivers is dashed before a run"* and it is simply **stale** — `EngineBridge.rivers(min_order)` exists and reports `{"key":"rivers","carried":true}`, so the chip is legitimately no longer dashed. **Genuinely pre-existing:** the whole batch diff contains zero lines mentioning `river`. The lane reported rather than rewrote it, which was right — **its correct form is seed-dependent**, so fixing it needs a decision about what the assertion should say, not a patch. **The batch nonetheless ends with a standing red probe**, which is the part worth not forgetting |
