...
| **`DccShell.phone_fit()` never floors the WIDTH of a control that declares none** | `DESIGN_HANDOFF.md` | small | **Root cause found 2026-09-06 while fixing one 35x27 button; the button was fixed in its own file and THIS was not.** `phone_fit()` does `min_size.y = maxf(min_size.y, tap)` **unconditionally** but nests `min_size.x = maxf(min_size.x, tap)` under `if min_size.x > 0.0` — so **an icon-only button that declares no width is never floored horizontally**, however many times `phone_fit()` runs over it. Measured: the offending control came out `35 x 115` — **13.4 dp wide against a 44 dp floor** — *after* `phone_fit()` had run. That was on the only route to the route-map backdrop menu. **The class is the row, not the instance:** every icon-only button in the phone tree is exposed to it, and `_phonechrome_probe`'s walk cannot see them once fitted, because `phone_fit()` flips a fitted button to `MOUSE_FILTER_PASS` and the walk counts only `STOP` |
| **`_phonechrome_probe`'s tap-floor walk stops seeing a control once it is fitted** | `DESIGN_HANDOFF.md` | small | **Disclosed by the lane and confirmed by the verifier, 2026-09-06.** Removing the fix entirely still gives `no tap-floor violations got=0 want=0`, because `phone_fit()` sets the fitted button to `MOUSE_FILTER_PASS` and the walk counts only `MOUSE_FILTER_STOP` (26 checked → 25). Coverage now rests **entirely on four `_ok` assertions** — they do hold it, and that mutant produced 2 FAILURES — but delete those four lines and nothing turns red. **Net coverage still beats HEAD**: the walk only ever saw the *unfitted* 35x27, the assertions see the shipped fitted 35x115. Recorded as a fragile seam rather than a defect |
