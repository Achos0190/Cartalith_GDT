...
| **`_nwsize_probe` passes or fails depending on persisted user settings, not on the code** | `ANDROID_UI_SPEC.md` | medium | **Found 2026-09-07 by two sessions disagreeing about the same tree, which is the only way this kind of defect surfaces.** A verifier ran `_nwsize_probe` at HEAD `351b614` with the documented arguments and got **fail=1** — *"the project picker draws a readable `+ NEW WORLD` action"* — **deterministically, twice**. The main loop ran the same probe, same tree, same `target/debug/cartalith_godot.dll`, and got **fail=0 twice**, with and without `--tag`. **Neither is flaky; they are reading different persisted state.** **The state is shared and mutable:** `%APPDATA%/Godot/app_userdata/Cartalith Terrain Generator/` holds `cartalith_settings.cfg` — whose mtime lands inside the verifier’s run window, so probe runs WRITE it — carrying a `[recent] paths` list of ten `.zip` projects and a `projects=…/Worlds` root (empty here). **Whether the picker is presented, and what it draws, depends on that.** The verifier also noted the probe reports `phone=true` while seeing main-shell captions (PIPELINE / SCULPT / GENERATE WORLD), i.e. the picker was not presented in its harness at all, while on the handset it draws `+ NEW WORLD` correctly. **This is worse than a false negative: a probe that reads state its own siblings write can go green for reasons unrelated to the code, and every "fail=0" this session quoted from it inherits the doubt.** **The fix is isolation** — a probe must set up the state it asserts on, or declare and clear it, not inherit whatever the last run left in `user://` |
