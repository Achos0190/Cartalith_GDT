...
| **Every export path skips `render::apply_color_space`** | `TERRAIN_APPEARANCE_SCOPE.md` | small | **Escalated by the verifier 2026-09-06 and latent only by luck.** `grep -n 'apply_color_space' export_raster.rs` returns nothing; the sole production call is inside `build_color_texture`. It does not bite today because `WorldGen` initialises `color_space: render::ColorSpace::Srgb` and the function early-returns on `Srgb` — **so the exported PNG matches the screen exactly as long as nobody changes that default.** The moment a non-sRGB space is selectable, every export silently ships the wrong colours while the viewport shows the right ones. Cheap to fix now, invisible and expensive later |
| **`_exportraster_probe` section 13 has been red at HEAD, and nobody noticed** | `GUI_GAP_REGISTER.md` | small | **Proved pre-existing by the verifier 2026-09-06 with a control build** — HEAD's `export_raster.rs`, `lib.rs` and `undo.rs` restored, rebuilt, re-run: **199 909 of 8 060 928 bytes differ, worst delta 73**, byte-identical to the working-tree run. So the tiled and single-file paths do **not** produce the same pixels, against that section's own assertion. `apply_color_space` is ruled out as the cause (it early-returns at the default). **The probe was red before this batch and stayed red**, which is its own finding: a probe nobody runs is not coverage |
