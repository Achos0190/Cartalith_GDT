...
same day filed one unrowed defect, then the seven rows owner Rulings H-K opened and two from that evening’s batch. The parity audit and the first on-glass pass opened
...
|---|---|---|---|
| **Refine detail: does the 2026-09-07 canvas reverse the 2026-09-05 ruling that put it on the WORLD rail?** | `LARGE_ITEM_RULINGS.md` (2026-09-05, item 4) | small | **The one citation disagreement that is real shell drift, and its precedence is ambiguous, so it is the owner’s.** Ruling item 4 (2026-09-05): *Atlas "Refine detail for the current view" moves to the WORLD rail beside Bake & finalize* — and the shell does that (`app.gd`, `DccWidgets.action(row, "Refine detail", …)`). **The newer canvas does not draw it:** "refine" appears in none of the three 2026-09-07 canvases, and the DCC Environment canvas’s GENERATE panel says *"NOT A GENERATION STAGE — … tiled LOD, atlas → Preferences ▸ Tiles & LOD"* — which routes atlas work back to exactly the place the ruling moved it out of. `CLAUDE.md`’s *"an owner decision is newer than any canvas"* was written for a decision that postdated its canvas; here the canvas postdates the ruling. **Ask: keep Refine detail on the WORLD rail, or move it back under Preferences ▸ Tiles & LOD?** |
