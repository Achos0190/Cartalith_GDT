...
| **Landmark M8 residual** — 24 of 49 declared kinds still ship `buildable:false` (was 29, before M7's viewshed unblocked 5 more — see M7's own row) | `LANDMARK_GENERATION_SCOPE.md` | large | **Twenty-five** generate today (`landmark.rs::kinds()`, each unbuilt kind carrying a `not_built:` reason). **2026-09-21: M7's viewshed unblocked 5** — `fort`, `watchtower`, `fortified_pass`, `fortified_crossing`, `volcanic_feature` — taking buildable 20 → 25 (blocked 29 → 24). Two of the six `needs_viewshed` kinds stay blocked on real remaining gaps, not on the viewshed primitive itself: `border_marker` (no faction/territory field) and `sacred_mountain` (no cultural-meaning input, `LANDMARK_GENERATION_SCOPE.md` §26 forbids hardcoding one). **Counting artifact found and corrected 2026-09-21** (before M7 landed): `grep -c "KindSpec {"` and `grep -c "buildable: false"` each over-count by one — the former also matches `pub struct LandmarkKindSpec {`, the latter also matches the field's own doc comment. Real denominator confirmed by direct read: **49** declared kinds. **All 29 then-blocked kinds independently re-audited 2026-09-21, at their cited symbol, for the exact defect this session found repeatedly the same day (a reason naming a missing capability that had since shipped): zero found stale beyond what M7 itself closed.** Two genuine near-misses investigated and refuted: `route_sediment`'s delta-building (gated behind the optional interactive Erode op, no queryable per-cell record) for `delta`; the cirque classifier (a deliberate 2026-09-01 judgment, re-confirmed still true) for `glacial_feature`. **2026-09-02: the five way-graph kinds landed** — `market_site`, `trade_depot`, `caravan_station`, `bridge_site`, `road_junction` — taking buildable 15 → 20. `resource_extraction_site` went buildable 2026-09-01. Several still need §13's route load or `STORY_PLANNING_SCOPE.md` SP-4 (both confirmed still open 2026-09-21); the remainder of the military family is downstream of Fort, now built. |
| **Zoom-dependent label density** — owner report, 2026-09-21: "labels and names are creating a mess", asked for Google-Maps-style progressive reveal by zoom/importance | `map_overlay.gd`, `cartalith-civ/src/labels.rs` | medium | **In flight.** Confirmed at the symbol: settlement PINS already implement exactly this (`SETTLEMENT_LOD`, `map_overlay.gd` ~line 91, citing OSM Carto's z9/z12/z14 bands and the MapLibre `minzoom`/`maxzoom` spec as precedent), but the generic label layer (`_labels`, drawn unconditionally by `_draw_labels()`) has no live zoom gating at all — its only culling is a one-time collision-avoidance pass at generation time (`labels.rs::labels_generate`). `label_candidates()` (`labels.rs` ~line 1287) already computes a per-label `weight` across all 5 classes (continent/region/settlement/water/landmark), sorted descending for that one-time pass — the same importance signal the owner asked for ("the bigger the settlement/importance, the less zoom you need") is already there, just not threaded to the live renderer. Agent dispatched to extend `SETTLEMENT_LOD`'s own pattern to `_draw_labels()`, keyed on `class` and `weight`. Verification owed: before/after screenshots at several zoom levels, hand-placed labels confirmed unaffected, workspace test count. |
...
| **River ink paints onto lake and ocean water, unmasked** — found from a PC screenshot review, 2026-09-21 | `cartalith-hydrology/src/lib.rs`, `cartalith-godot/src/render.rs` | medium | **Owner report, confirmed at the symbol.** `stamp_river_intensity` (`cartalith-hydrology/src/lib.rs:1143`) stamps a parabolic width disc around every channel cell (`chan[i] != 0`) with no land/water gate — the disc radius reaches up to `9·width_k` cells, and at small world extents that is wide enough to reach a receiving lake or the coastline. `channel_tint` (`cartalith-godot/src/render.rs`, the function `build_color_texture` and `bake_rect` both call) then tints **any** cell by that ink value with no check of `water_bodies` classification at all — so wherever a channelised cell's disc, or the channel trace itself, lands on a lake or ocean cell, the river's dark ink paints directly over the water fill. Visible in `shot_03_zoom8p00_lod5.png` (this session's screenshot sweep): dark branching cracks running across the full width of "Lake Valarcca", not just at its inflow. **Not yet root-caused whether `chan[]` itself carries cells that are later classified as lake/ocean** (e.g. a pre-lake-fill drainage trace never re-masked once the depression floods) or whether this is purely disc bleed at the shoreline/river-mouth — needs a fresh read of `build_channels`' ordering relative to lake-fill before fixing. **Queued behind CA-19** (§2.5's `render.rs`/`paint_bridge.rs` build, in flight) — same file, cannot land concurrently. The safest, most local fix is gating `channel_tint`'s tint application (or `stamp_river_intensity`'s own disc write) on the cell's `water_bodies` classification being land, not lake/ocean; check the v2.11 reference's own `drawRiverWays`/channel-tint equivalent first per `MISTAKES.md`'s "the reference's errors are part of the contract" rule — if the reference itself gates on water, this is a straight port gap; if it doesn't, this is a deliberate improvement needing the same disclosure a golden re-baseline gets. |
| **Pipeline stage rows 09/10 read as "didn't run" when they did** — found from a PC screenshot review, 2026-09-21 | `cartalith-native/godot-project/shell/workspaces/world_workspace.gd`, `cartalith-engine/src/progress.rs` | small | **Confirmed at the symbol as a wording bug, not an engine gap.** `world_workspace.gd::_paint_stage_rows()` (~line 2249) hardcodes the state text `"done  %.2fs  (no engine work this run -- see gap note above)"` for stage indices 8 and 9 (Ecology & biomes, Resources & soils) on every run. `cartalith-engine/src/progress.rs`'s own doc comment (~line 100) says plainly what that note elides: biome classification and soil/resource potentials **genuinely run on every `generate()`**, in `cartalith-godot::compute_civilisation` (`build_biome_raster`, `build_soil_fertility`, `build_resource_potentials`), called right after `generate_terrain_inner` returns — just outside the ten-stage timed domain this readout represents, and with no exposed per-stage dials (which is what "no engine work" is actually trying to say). The owner read the UI text exactly as a careless reading of it warns against ("Pipeline 9 and 10 dont seem to run"). **Fix is a wording change only** — e.g. "no dials of their own -- computed automatically after the pipeline" in place of "no engine work this run" — touching only the gap-note string in `_paint_stage_rows()` and, if any probe asserts the old string verbatim, that probe's fixture text too. Does not touch generation behaviour or any golden. Safe to build concurrently with everything else in this document (file untouched elsewhere). |
...
| **Implement `Cartalith Settlement Editor.dc.html`** — new GUI: a faction/settlement editor for urban morphology | owner-supplied design canvas, project `067f80e7-dbb7-4492-8e69-96aaa8050a4d` | large | **Filed 2026-09-21, owner instruction.** Desktop variant. Import via the design MCP (`claude.ai/design/p/067f80e7-dbb7-4492-8e69-96aaa8050a4d?file=Cartalith+Settlement+Editor.dc.html`), reading the whole project including the imported `support.js` dependency. **Blocked on `/design-login`** — the design MCP needs design-system authorization the coordinating session does not carry by default; requested from the owner, not yet run as of filing. Supporting research already in the repo: an owner-uploaded `Urban_Morphology_v0.1.html` proof-of-concept (culture profiles, street-graph growth, parcels, buildings, walls, an inspector/metrics/evidence-ledger side panel) — read as reference for what the editor's data model likely needs to expose, not itself the design to implement. Likely intersects the existing Culture Profiles window (`culture_profiles_window.gd`, built earlier this session) and the Faction Roster window's existing religion/ag-tech/government editors — a design-refinement prompt on that exact overlap was already handed to the owner earlier this session, unresolved; this new canvas may be the owner's own answer to it. Scope and implementation plan to follow once the canvas is actually read. |
| **Implement `Cartalith Settlement Editor Touch.dc.html`** — the touch/tablet variant of the same editor | owner-supplied design canvas, project `067f80e7-dbb7-4492-8e69-96aaa8050a4d` | large | **Filed 2026-09-21, owner instruction.** Companion to the row above — same project, same `support.js` dependency, touch-sized variant (`claude.ai/design/p/067f80e7-dbb7-4492-8e69-96aaa8050a4d?file=Cartalith+Settlement+Editor+Touch.dc.html`). **Blocked on `/design-login`**, same as the row above. Build after the desktop variant, once its data model and control set are established, so the touch build is a real adaptation rather than a parallel guess. |
...
| **CA-19** — a writable biome colour table | `STATUS.md`, `PARITY_AUDIT.md` | medium | Buildable today, but **costs a golden re-baseline** that `DECISIONS.md` §7a protects. **AUTHORIZED 2026-09-21 — Ruling P (`LARGE_ITEM_RULINGS.md`).** Queued, next batch. |
