...
| **OWNER-PROPOSED: rename the project-save extension from `.zip` to `.ctl`** | `SAVEFILE_COMPAT.md` | small | **Owner, 2026-09-07: *"to prevent the application from opening zips that it shouldn’t maybe we should rename the extension to ctl instead of zip (Cartalith). Then we would only need to import that to the html version and opening an old zip file from the html would only need a rename of the extension type (not the file type or storage or anything else)."*** **The motivation is traceable to a defect found the same evening:** the picker offered `Werk.zip` — a 2024 archive that is not a Cartalith save — and the reader then refused it with *"missing zip entry: params.json"*. A distinct extension stops that being offered at all. **NO FORMAT CHANGE IS NEEDED, and this was checked rather than assumed: `SAVEFILE_COMPAT.md` §3 never mandates a file extension.** It constrains the container (standard PKZIP), the entry names, compression methods and zip64 — and the only extensions it discusses are *entry* extensions (`.f32`, `.u8`, `.i32`, `.bin`). The archive’s own filename is outside the spec, so `.ctl` is conformant today. **The spec should still RECORD the convention**, or a future implementer guesses. **Rust: zero sites.** The readers open by path and read the container; no `.zip` extension logic exists. (`#[cfg(feature = "zip")]` is a cargo feature and the `.zip(` hits are iterator zips.) **GDScript: 10 sites, and they are TWO POPULATIONS — getting this wrong is the whole risk.** **Rename (project saves):** `app.gd:2995` Save-as, `:2991` the suggested `world_%d.zip`, `:2792` `.autosave.zip`; `open_project_dialog.gd:1128`; `phone_project_picker.gd:329`. **LEAVE AS `.zip` — different artefacts, not saves:** asset packs (`app.gd:3331`, `asset_library_window.gd:1282/3213/3226`), the atlas cache (`menus.gd:4174/4201`), tile export (`data_manager_window.gd:3152`) and the World Data import badge (`:369`). **THE ESSENTIAL REQUIREMENT: the reader must accept BOTH.** `open_project_dialog.gd:804` and `:1138` filter listings with `get_extension().to_lower() == "zip"` — **that comparison is what makes a save visible at all**, so it must become a membership test over `["ctl", "zip"]` or every existing world disappears from the picker on upgrade. **Android caveat, already mitigated:** `.ctl` has no registered MIME type, so SAF’s typed filter (Godot turns each filter into `EXTRA_MIME_TYPES`) could hide it — but `browse_dialog.gd` already appends `"*.* ; All files"` as a SECOND filter for exactly this hazard, added 2026-09-07. **Verify on glass anyway**, because a provider reporting an unexpected type is what that mitigation exists for |
| **TABLET PORTRAIT: the left dock’s 99 px comes from its CONTENT, not its floor — and my first two diagnoses were both wrong** | `TABLET_UI_SPEC.md` | medium | **Measured 2026-09-07, then twice corrected in the same sitting. The retractions are recorded because each was filed before it was checked.** **ESTABLISHED, and this part has held through every re-measure:** at `--force-touch --vp 800x1280` the **right dock is canvas-exact** (`min=232 want=232 over=0`), and the **left dock is `min=331 want=232 over=99`**. 99 of the 285 px portrait overflow is this dock. **RETRACTED #1 — "the role is not being applied."** It is. `[floor] _left_width=232.0 role_left=232 ld_cms=232.0`: the cached width, the live role and the dock’s own `custom_minimum_size.x` **all read 232**. `TABLET_PORTRAIT` and `set_portrait()` work. **RETRACTED #2 — "hidden workspace panels are sizing it."** They are not. My first driver walk printed `is_visible_in_tree()` and I read it as "invisible". **Those are different facts:** a container’s minimum counts a child by the child’s **own** `visible`, while `is_visible_in_tree()` is false the moment ANY ancestor is hidden. Re-run printing both: `CivilizationWorkspace` is `own_vis=false`, so it **cannot** feed its parent. **This is the exact mirror of the planner defect** — there `.visible` was true while `is_visible_in_tree()` was false and a probe believed the first; here a probe believed the second. The probe now prints **both**, permanently. **WHERE IT ACTUALLY IS:** `get_combined_minimum_size()` is `max(custom_minimum_size, get_minimum_size())`. The floor is 232, so **the 331 is `get_minimum_size()` — the `PanelContainer`’s CONTENT plus its stylebox margins.** Something inside genuinely needs 331. **NEXT STEP, and do not skip it:** the driver walk filters at `min >= dock_min`, and **no `own_vis=true` chain reaching the dock appeared at that threshold** — so the filter is hiding the answer, or the contributor sits behind the `SCROLL_MODE_DISABLED` scroller `_scroll()` builds (`dcc_shell.gd:3963`), which folds a child minimum outward and is the trap `MISTAKES.md` already names. **Lower the threshold and walk only `own_vis=true` ancestry.** Three guesses have now cost more than one careful walk would have |
| **OWNER-REPORTED on PC: the sculpt tools also show under WORLD ▸ Geology — they must appear ONLY when the sculpt menu is accessed** | `DCC_SHELL_SCOPE.md` | small | **Owner, 2026-09-07: *"On the pc version the sculpt tools are also under the geology tab in world … All those tools should only and strictly be presented when the sculpt menu is accessed."*** **Mechanism found, and the file states it in its own comment** (`workspaces/world_workspace.gd:427-429`): *"`_sculpt_body`/`_paint_body` are unaffected and are still parented into their categories rather than into a mode panel — **the mode hides the category, it does not re-home the body**."* **So this is a deliberate design, and the owner has now overruled it.** An owner instruction outranks the scope document; record it at the symbol or the next conformance pass restores the old behaviour by citing that same comment. **The fix has a precedent IN THE SAME FUNCTION, twenty lines down: `_paint_body.visible = false` (`:441`).** Biome paint is hidden by default and shown only while its tool is armed — *"the same 'arming a tool never changes the workspace' independence §4.5 establishes"*. **`_sculpt_body` gets no such line**, so it is visible from birth and appears whenever its category is open. **Do not just add `visible = false`.** `_build_sculpt(_sculpt_body)` is called from **four** sites (`:452`, `:498`, `:517`, `:1939`) — a rebuild path among them — and a body hidden at construction can be re-shown by any of them. **Follow how `_paint_body` is toggled on arm/disarm and mirror it**, then check the disarm path: `MISTAKES.md` records that *"the disarm path is the obvious one and the arm-another-tool path is the one that gets missed"*. **Verify by opening WORLD ▸ Geology with no tool armed and asserting the sculpt controls are not in the tree** — not by reading the flag |
