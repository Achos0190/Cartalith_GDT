...
| AS-17 | Slicer: canvas interaction | `asset_library_window.gd` slicer modal | the reference's pan/zoom, draggable grid lines and click-to-select-cells are unported; the modal slices the whole uniform grid rather than a hand-picked selection | new row, 2026-08-20 (opened by the AS-09 pass) | §8 | (B) small — pure presentation. `slicer::compute_cells` is deliberately written against line *fractions* rather than `cols`/`rows` so a draggable-line UI can supply its own without touching the golden-verified arithmetic |
