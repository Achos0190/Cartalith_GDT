...
| SH-14 | **The reference's whole mobile navigation cluster — the `#zoomOverlay` zoom pad, the ✋ pan toggle, `zoomReset`, and `#sculptNavpad`** | not built; `viewport_host.gd`'s `reset_view()` exists but **no control anywhere calls it** | never disclosed — no register row, and `GUI_GAP_REGISTER.md` had no "zoom pad"/"reset view" entry before this one | yes | none in this port's own design language; the reference has a complete one | **(D)** — **owner design decision, raised rather than improvised.** Owner-reported in the same question as SH-13. The reference ships a mobile-only floating cluster (`#zoomOverlay`, HTML 233 + 749-754, shown by `isMobile` at 13435) of four buttons: **`zoomIn`/`zoomOut`** (13464-13465, `×1.35` about the view centre, LOD- and 3D-aware), **`panBtn` ✋** (13963), a *hold-to-pan toggle* that makes single-finger drag pan instead of paint — the missing third pan route SH-13 does not restore — and **`zoomReset` ⟳** (13466), which is the owner's *"snapping the view back to 100%"*. Two things about that button matter and neither is guessable: it also **clears `panMode`** and un-highlights ✋, and since v1.13 it deliberately does **not** call `resetView()` (13390, `scale=1, pan=0`) but `_viewFill()` (13354) — **"100%" in this app means the COVER scale, the zoom at which the map fills the display, not scale 1**. That distinction is itself a second, larger gap: the reference's default and reset view is *cover* (`_viewCoverScale`, v1.01, from the owner's own *"big unused areas above and below"*) with a *fit*-scale zoom floor (`_viewFitScale`, v1.13) and a pan clamp that can never expose the letterbox (`_viewClampFill`); **this port's `reset_view()` is plain fit/letterbox**, which is visibly the state v1.01 was raised to fix — a phone screenshot from this pass shows the map as a band with large black margins above and below. Also unbuilt: `#sculptNavpad` (HTML 238 + 759-760, `_sculptNav*` 9146-9200), the velocity joystick that pans while a single finger is captured by the Sculpt tool (`MAX_OFFSET 24`, `DEAD_ZONE 4`, `MAX_SPEED 12`, knob offset **negated** so the camera travels the way you push — v1.22 fixed exactly that sign) — already registered as **SH-03** and left standing. What is *not* wanted here is four floating buttons dropped onto the DCC canvas: the reference's cluster is a mobile-web idiom, this shell has its own §13 phone language, and translating one into the other is real design work the owner is best placed to direct — which is why this is (D) and not (B) |
